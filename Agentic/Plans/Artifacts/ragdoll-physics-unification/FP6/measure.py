...
def main():
    ...
    output = root / "TestOutput/ragdoll-physics-unification/FP6/measurements"
    env = {key.upper(): value for key, value in os.environ.items()}
    revisions = (("fp5", "c8a4332cf31bd043bfd928125855ec21f476e676"), ("candidate", None))
    for name, revision in revisions:
        ...
        fixture = Path(__file__).with_name("loaded_chain_probe.cpp").read_text(encoding="utf-8")
        if revision is None:
            fixture = fixture.replace("joint.stiffness = 0.22f", "joint.frequencyHz = 40.0f")
            fixture = fixture.replace("joint.damping = 0.35f", "joint.dampingRatio = 1.0f")
            fixture = fixture.replace("*4.0", "*8.0")
        (target / "main.cpp").write_text(fixture, encoding="utf-8", newline="\n")
...
''')
    ...
    # Alternate runs with the same fixture, compiler and support library. FP5
    # uses four sweeps; the candidate uses eight, reported per joint iteration.
