...
| **Witness** | `TestReplaySolverHashWitness.h`, `SessionState.md` | A regression test comparing checksums | **Determinism Test / Golden Checksum** | *Indirect:* `"The witness verifies production state"`<br>*Direct:* `"Check replay physics solver checksums match"` |
...
| **File Comment Template** | `comment-style-guide.md` | File-level comment header | **File Docstring / Header Comment** | *Needlessly formal:* `"Add every template heading before editing"`<br>*Direct:* `"Add a concise comment only when the file needs orientation"` |
...
- [ ] **Plain wording:** Retired audit vocabulary is absent from comments and variable names.
