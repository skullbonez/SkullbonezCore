...
runtime-command replay is not implemented; saved v2 file restore failures now
emit queryable SkullScope rows with failure reasons.
...
| Phase 3: Event stream | Partial | `ReplayEventRecorder` now records bounded timeline-start, runtime-command, hash-verified branch-restore, generated-scene-config, world-override, editor-place, editor-transform, launcher-config, and launcher-fire rows. V2 saves emit/query `EVNT` event chunks plus `ECUR` checkpoint event-cursor chunks; `replay_query` decodes the typed generated-scene/world/editor-placement/editor-transform/launcher payloads; loaded solver checkpoints receive their cursor metadata in C++; the saved target probe replays typed world/editor-placement/editor-transform/launcher events deterministically, verifies generated-scene config metadata when encountered, and can rebuild deterministic generated topology from the saved config event before applying a mismatched checkpoint. Missing: broader runtime command replay. |
...
| Phase 5: Branch and resume | Partial | `Enter` or the scrubber `BRANCH` button on the retained solver row makes the selected retained sample live, assigns a child branch id with parent/source frame/hash metadata, records a branch-restore event, and resets the replay timeline so old future samples do not mix into the new branch. The same action on a loaded `V2 FILE` row uses the v2 checkpoint/event/hash tracks to make the selected saved frame live after hash verification. Missing: broader deterministic event-forward replay coverage for runtime mutations. |
| Phase 6: Saved authoritative replay artifacts | Partial | Presentation saves now produce chunked binary v2 artifacts for smooth scrub playback/querying, with a body dictionary, 32-byte pose records, seek index, branch provenance rows, typed event rows, checkpoint event-cursor rows, per-tick solver hash rows, sparse solver checkpoint chunks, C++ presentation/checkpoint/event/hash loaders, file-backed runtime presentation scrub source, in-app v2 save and `.skreplay` load controls, saved checkpoint-frame restore verification, saved non-checkpoint target checkpoint-plus-event restore verification, generated topology rebuild from saved config events before mismatched checkpoint restore, live branch-from-file verification, saved-file restore failure diagnostics, `tools\replay_query` bridge, and `tools\validate_replay_v2_artifact.bat` runtime artifact/load/hash/branch/event/event-cursor/checkpoint/restore proof. Missing: broader runtime mutation coverage. |
...
| `TestOutput\validation\replay_v2\replay_save_probe.skreplay` | v2 binary artifact from `replay_v2_solver_one.scene.json`, 24 frames, 2 bodies, 7 events, 1 checkpoint | 12,850 bytes |
| `TestOutput\validation\replay_v2\replay_generated_topology_probe.skreplay` | v2 binary artifact from `replay_v2_generated_topology.scene.json`, 24 frames, 6 bodies after the save-probe editor placement, 7 events, 1 checkpoint | 17,022 bytes |
