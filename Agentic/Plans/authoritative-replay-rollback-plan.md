...
implemented; arbitrary target checkpoint/event rollback, deterministic event
replay, and branch-from-file are not implemented.
...
| Saving/loading | The presentation save path writes `replays\replay_v2_####.skreplay` binary v2 presentation artifacts with `MANI`, `BODY`, `PRES`, `BRAN`, `EVNT`, `ECUR`, `HASH`, `SCHK`, and `INDX` chunks. `--replay-load <path>` loads one of those artifacts as the runtime presentation scrub source after scene initialization. Solver row save still writes `replays\solver_replay_####.skreplay` v1 JSON with compact authoritative snapshot summaries plus branch/event-cursor fields. V2 files now contain branch provenance, bounded timeline/runtime event rows, checkpoint event cursors, and sparse solver checkpoint payloads, and `--replay-restore-file-probe <path>` can load and hash-verify a checkpoint frame in a fresh Debug runtime. They are not complete branchable authoritative replay artifacts yet because arbitrary event replay, arbitrary target restore, and branch-from-file are still missing. |
...
| Determinism evidence | `tools\validate_replay_scrub.bat` uses SkullScope to prove a selected visual replay sample maps to queried body state and that retained solver restore matches the original solver hash before branching. `tools\validate_replay_v2_artifact.bat` writes a real runtime v2 presentation artifact with branch rows, event rows, checkpoint event-cursor rows, solver hash rows, and sparse solver checkpoint rows, reloads it through the C++ v2 reader, proves an older loaded pose can be applied/restored, proves `--replay-load-probe` can drive the runtime scrub source from that file, proves `--replay-restore-file-probe` can load and hash-verify a saved checkpoint frame with a nonzero event cursor in a fresh Debug runtime, queries presentation frames/bodies/branches/events/event-cursors/hashes/checkpoints, exports a bounded SkullScope slice, and imports that slice through `physics_query`. There is no saved arbitrary-target checkpoint-plus-event restore test yet. |
...
| Phase 3: Event stream | Partial | `ReplayEventRecorder` now records bounded timeline-start, runtime-command, and hash-verified branch-restore rows. V2 saves emit/query `EVNT` event chunks plus `ECUR` checkpoint event-cursor chunks, and loaded solver checkpoints receive their cursor metadata in C++. Missing: event replay and full coverage for launcher fire, world edits, editor commits, and generated-scene rebuild payloads. |
