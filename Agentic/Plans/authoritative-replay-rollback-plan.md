...
| Saving/loading | The presentation save path writes `replays\replay_v2_####.skreplay` binary v2 presentation artifacts with `MANI`, `BODY`, `PRES`, `BRAN`, `HASH`, `SCHK`, and `INDX` chunks. `--replay-load <path>` loads one of those artifacts as the runtime presentation scrub source after scene initialization. Solver row save still writes `replays\solver_replay_####.skreplay` v1 JSON with compact authoritative snapshot summaries plus branch fields. V2 files now contain branch provenance and sparse solver checkpoint payloads, and `--replay-restore-file-probe <path>` can load and hash-verify a checkpoint frame in a fresh Debug runtime. They are not complete branchable authoritative replay artifacts yet because event chunks, arbitrary target restore, and branch-from-file are still missing. |
...
| Determinism evidence | `tools\validate_replay_scrub.bat` uses SkullScope to prove a selected visual replay sample maps to queried body state and that retained solver restore matches the original solver hash before branching. `tools\validate_replay_v2_artifact.bat` writes a real runtime v2 presentation artifact with branch rows, solver hash rows, and sparse solver checkpoint rows, reloads it through the C++ v2 reader, proves an older loaded pose can be applied/restored, proves `--replay-load-probe` can drive the runtime scrub source from that file, proves `--replay-restore-file-probe` can load and hash-verify a saved checkpoint frame in a fresh Debug runtime, queries presentation frames/bodies/branches/hashes/checkpoints, exports a bounded SkullScope slice, and imports that slice through `physics_query`. There is no saved arbitrary-target checkpoint-plus-event restore test yet. |
...
| Phase 5: Branch and resume | Partial | `Enter` on the solver row makes the selected retained sample live, assigns a child branch id with parent/source frame/hash metadata, and resets the replay timeline so old future samples do not mix into the new branch. Missing: explicit branch UI affordance beyond `Enter`, event cursor metadata, and branch-from-file. |
| Phase 6: Saved authoritative replay artifacts | Partial | Presentation saves now produce chunked binary v2 artifacts for smooth scrub playback/querying, with a body dictionary, 32-byte pose records, seek index, branch provenance rows, per-tick solver hash rows, sparse solver checkpoint chunks, C++ presentation/checkpoint loaders, file-backed runtime presentation scrub source, saved checkpoint-frame restore verification, `tools\replay_query` bridge, and `tools\validate_replay_v2_artifact.bat` runtime artifact/load/hash/branch/checkpoint/restore proof. Missing: event stream chunks, in-app file picker/load command, arbitrary target restore verifier, and branch-from-file. |
