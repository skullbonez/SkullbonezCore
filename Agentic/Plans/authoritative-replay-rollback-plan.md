...
unsupported inside a checkpoint-to-target replay window; saved v2 file restore
failures now emit queryable SkullScope rows with failure reasons.
...
| Phase 2: In-memory solver checkpoint ring | Partial shortcut | Dense per-frame `ReplaySolverFrameSample` storage exists. `m_checkpoints` stores `ReplayCheckpointSummary` only, but v2 save now emits sparse `SCHK` checkpoint payloads from checkpoint-boundary retained solver samples. |
| Phase 3: Event stream | Partial | `ReplayEventRecorder` now records bounded timeline-start, runtime-command, hash-verified branch-restore, generated-scene-config, world-override, editor-place, editor-transform, launcher-config, and launcher-fire rows. V2 saves emit/query `EVNT` event chunks plus `ECUR` checkpoint event-cursor chunks; `replay_query` decodes runtime-command and typed generated-scene/world/editor-placement/editor-transform/launcher payloads; loaded solver checkpoints receive their cursor metadata in C++; the saved target probe replays typed world/editor-placement/editor-transform/launcher events deterministically, verifies generated-scene config metadata when encountered, can rebuild deterministic generated topology from the saved config event before applying a mismatched checkpoint, and validates a real queued `ResetCurrentScene` command as a post-reset checkpoint precondition. Missing: external scene-load/create/advance runtime command semantics are still query-only/unsupported across checkpoint-to-target replay windows. |
| Phase 4: Restore to tick without branch resume | Partial | Current interactive restore applies the selected retained in-memory sample directly, recomputes the solver hash from the restored live state, emits `replay_restore`, and resets the live replay branch only on a hash match. The Debug saved-file probe can load one sparse checkpoint frame from v2 and hash-verify that exact checkpoint after applying it. The Debug target probe can restore the nearest saved sparse checkpoint and replay typed world/editor-placement/editor-transform/launcher events plus fixed ticks to the saved non-checkpoint target hash selected by the probe. Saved-file target restore success and failure paths now emit `replay_restore` SkullScope rows with source, checkpoint, hash, fallback, and failure-reason fields. |
| Phase 5: Branch and resume | Partial | `Enter` or the scrubber `BRANCH` button on the retained solver row makes the selected retained sample live, assigns a child branch id with parent/source frame/hash metadata, records a branch-restore event, and resets the replay timeline so old future samples do not mix into the new branch. The same action on a loaded `V2 FILE` row uses the v2 checkpoint/event/hash tracks to make the selected saved frame live after hash verification. Remaining limitation: external scene-load/create/advance runtime commands are not branch-replayed across checkpoint windows. |
| Phase 6: Saved authoritative replay artifacts | Partial | Presentation saves now produce chunked binary v2 artifacts for smooth scrub playback/querying, with a body dictionary, 32-byte pose records, seek index, branch provenance rows, typed event rows, checkpoint event-cursor rows, per-tick solver hash rows, sparse solver checkpoint chunks, C++ presentation/checkpoint/event/hash loaders, file-backed runtime presentation scrub source, in-app v2 save and `.skreplay` load controls, saved checkpoint-frame restore verification, saved non-checkpoint target checkpoint-plus-event restore verification, generated topology rebuild from saved config events before mismatched checkpoint restore, reset-command event capture/query/checkpoint-precondition validation, live branch-from-file verification, saved-file restore failure diagnostics, `tools\replay_query` bridge, and `tools\validate_replay_v2_artifact.bat` runtime artifact/load/hash/branch/event/event-cursor/checkpoint/restore proof. Remaining limitation: whole-timeline scene-load/create/advance commands are query-only rather than replayed inside a checkpoint-to-target window. |
