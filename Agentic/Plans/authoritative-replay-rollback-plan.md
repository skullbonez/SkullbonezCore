...
launcher configuration changes, and launcher fire payloads; broad editor event
coverage and broad generated-scene restore coverage are not implemented; saved
v2 file restore failures now emit queryable SkullScope rows with failure
reasons.
...
typed generated-scene config/world/launcher events and fixed physics ticks
forward, and compare every stepped target hash through the saved non-checkpoint
target selected by the probe. A loaded v2 file can now branch live through the same helper when the
...
| Saving/loading | The scrubber save icon and presentation save path write `replays\replay_v2_####.skreplay` binary v2 presentation artifacts with `MANI`, `BODY`, `PRES`, `BRAN`, `EVNT`, `ECUR`, `HASH`, `SCHK`, and `INDX` chunks. `--replay-load <path>` loads one of those artifacts as the runtime presentation scrub source after scene initialization, and the scrubber `LOAD` button opens a `.skreplay` picker for the same file-backed `V2 FILE` row. The legacy solver JSON exporter still exists for debug inspection of `replays\solver_replay_####.skreplay` with compact authoritative snapshot summaries plus branch/event-cursor fields. V2 files now contain branch provenance, bounded timeline/runtime event rows, typed generated-scene-config/world-override/launcher-config/launcher-fire event payloads, checkpoint event cursors, and sparse solver checkpoint payloads. `--replay-restore-file-probe <path>` can load and hash-verify a checkpoint frame in a fresh Debug runtime; `--replay-restore-target-file-probe <path>` can restore the nearest checkpoint and replay typed events/fixed ticks to a saved non-checkpoint target; `--replay-restore-branch-file-probe <path>` proves the same target can become a live branch; `--replay-restore-failure-file-probe <path>` proves expected saved-file restore failures emit bounded SkullScope rows. They are not complete branchable authoritative replay artifacts yet because editor event coverage and broader generated-scene restore coverage are still missing. |
...
| `TestOutput\validation\replay_v2\replay_save_probe.skreplay` | v2 binary artifact from `replay_v2_solver_one.scene.json`, 24 frames, 1 body, 5 events, 1 checkpoint | 11,734 bytes |
...
as the normal saved scrub format. The tiny v2 generated-scene proof is not a
scale comparison, but it confirms the binary/query path remains inspectable and
cheap for small deterministic restore scenarios.
