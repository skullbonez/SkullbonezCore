...
editor placement commits, committed editor translate/rotate gizmo transforms,
launcher configuration changes, and launcher fire payloads; editor
scale/shape-changing gizmo event coverage and broad generated-scene restore
coverage are not implemented; saved v2 file restore
...
| Phase 3: Event stream | Partial | `ReplayEventRecorder` now records bounded timeline-start, runtime-command, hash-verified branch-restore, generated-scene-config, world-override, editor-place, editor-transform, launcher-config, and launcher-fire rows. V2 saves emit/query `EVNT` event chunks plus `ECUR` checkpoint event-cursor chunks; `replay_query` decodes the typed generated-scene/world/editor-placement/editor-transform/launcher payloads; loaded solver checkpoints receive their cursor metadata in C++; the saved target probe replays typed world/editor-placement/editor-transform/launcher events deterministically and verifies generated-scene config metadata when encountered. Missing: scale/shape-changing editor gizmo commits, broad generated-scene topology replay beyond the small v2 proof scene, and broader runtime command replay. |
...
| User physics actions | Fire launcher, spawn projectile, place editor object (implemented for placement commits), move/rotate object when committed through editor gizmos (implemented for translate/rotate), scale object when it commits to physics (still open because it mutates collision-shape payloads). |
