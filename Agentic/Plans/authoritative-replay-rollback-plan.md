...
while inspecting it, resume from the live edge, save the retained presentation
buffer, and load a v2 presentation file for smooth backwards file scrubbing.
Authoritative rollback means the user can choose a replay time, restore the
actual solver state for that tick, and continue simulation from there as a new
live branch.
...
visual scrub data only. Those artifacts can now be loaded as a file-backed
presentation scrub source through `--replay-load`; the far-right scrub position
is the file end, not a live simulation edge. V2 now also carries a compact
per-tick solver hash track for divergence checks plus sparse solver checkpoint
payloads at checkpoint boundaries and a compact branch provenance chunk naming
the saved timeline's branch id, parent branch id, retained frame window, and
source restore frame/hash. V2 also serializes typed event rows and checkpoint
event cursors. A Debug runtime probe can load a saved v2 checkpoint frame from a
fresh process, apply it through the live solver restore path, and require the
recomputed solver hash to match before passing. A second Debug probe can restore
the nearest sparse checkpoint, clear consumed one-shot impulse state, replay
typed generated-scene config/world/editor-placement/editor-transform/launcher events and fixed
physics ticks forward, and compare every stepped target hash through the saved
non-checkpoint target selected by the probe. A loaded v2 file can now branch live through the same helper when the
scrubber is paused on `V2 FILE` and `Enter` or `BRANCH` is pressed. Current retained
interactive branch restore still works from the in-memory
`ReplaySolverFrameSample` chain, while saved-file branch restore uses the v2
checkpoint/event/hash tracks. Saved-file restore failures now emit
`replay_restore` SkullScope rows with `restore_source`, checkpoint frame, and
failure reason fields so failed restore attempts are queryable too.

Audit note, 2026-06-23: solver hashes now include launcher control state needed
for future deterministic inputs, but exclude transient ray/laser visual age and
ring cursor state so visual feedback does not falsely fail physics rollback.
Full launcher visual state still stays in checkpoint payloads for scrub/render
restore.
...
The immediate user priority is smooth backwards scrubbing, with authoritative
restore layered behind it rather than run during drag. That changes the useful
slice order:

1. Preserve smooth visual reverse scrub as a dense presentation track.
2. Do not promise broad saved authoritative restore until `.skreplay` v2 has
   enough event coverage for the scene/editor actions being replayed.
...
## V2 Benefit And Legacy Cleanup Direction

The real benefit of v2 is not that it makes physics run backward. It separates
the replay into queryable, seekable tracks:

| Benefit | Why it matters |
|---------|----------------|
| Smooth scrub track | Dense binary presentation frames can be read backward/forward cheaply without touching live physics. |
| Smaller saved files | Body metadata is deduplicated and per-frame pose rows are fixed-size binary instead of repeated JSON text. |
| Restore proof track | Sparse checkpoints, typed events, and per-tick hashes let a fresh process prove checkpoint-plus-event restore. |
| Debuggable binary | `tools\replay_query.bat` and `export-skullscope` keep binary artifacts inspectable without loading raw files into GPT. |
| Format evolution | Chunks allow new tracks without rewriting the whole artifact or breaking old preview-only loads. |

Cleanup should wait until v2 file playback, branch-from-file, and event coverage
are stable. The
candidate removals are:

| Old path | Removal condition |
|----------|-------------------|
| v1 JSON presentation save (`replay_####.skreplay`) | Remove after v2 presentation save/load/query is the only supported smooth-scrub save path. |
| Routine solver JSON save (`solver_replay_####.skreplay`) | Keep as a Debug inspection/export path until v2 branch-from-file, broader event coverage, and checkpoint queries cover the same evidence. |
| Duplicate presentation loading/export helpers | Remove after all UI, CLI probes, and docs point at v2 loaders and `replay_query`. |
| Hash-only CSV/debug exports that duplicate v2 `HASH` queries | Remove after validation gates and handoff docs consume the v2 query bridge. |
| Legacy assumptions that `.skreplay` means JSON | Remove once v1 files fail gracefully as preview/debug-only and v2 is the default runtime artifact. |
...
| Branch restore | Press `Enter` or click `BRANCH` while paused on the solver row to restore the selected retained in-memory solver frame as the new live branch. Use the same action while paused on a loaded `V2 FILE` row to restore the selected saved frame by applying the nearest sparse checkpoint, replaying typed events/fixed ticks forward, comparing the target solver hash, assigning a child branch id, and resetting the live replay timeline. Retained restore and file restore both record branch provenance only after hash match. |
...
| Saving/loading | The scrubber save icon and presentation save path write `replays\replay_v2_####.skreplay` binary v2 presentation artifacts with `MANI`, `BODY`, `PRES`, `BRAN`, `EVNT`, `ECUR`, `HASH`, `SCHK`, and `INDX` chunks. `--replay-load <path>` loads one of those artifacts as the runtime presentation scrub source after scene initialization, and the scrubber `LOAD` button opens a `.skreplay` picker for the same file-backed `V2 FILE` row. The legacy solver JSON exporter still exists for debug inspection of `replays\solver_replay_####.skreplay` with compact authoritative snapshot summaries plus branch/event-cursor fields. V2 files now contain branch provenance, bounded timeline/runtime event rows, typed generated-scene-config/world-override/editor-place/editor-transform/launcher-config/launcher-fire event payloads, checkpoint event cursors, and sparse solver checkpoint payloads. `--replay-restore-file-probe <path>` can load and hash-verify a checkpoint frame in a fresh Debug runtime; `--replay-restore-target-file-probe <path>` can restore the nearest checkpoint and replay typed events/fixed ticks to a saved non-checkpoint target; `--replay-restore-branch-file-probe <path>` proves the same target can become a live branch; `--replay-restore-failure-file-probe <path>` proves expected saved-file restore failures emit bounded SkullScope rows. Saved target restore now uses the saved generated-scene config event to rebuild deterministic generated topology when the live process topology does not match the saved checkpoint. V2 artifact validation also performs a real queued `ResetCurrentScene`, saves the restarted timeline with the decoded `runtimeCommand` row, treats that reset as a checkpoint precondition, and proves post-reset restore still matches the target hash. Scene-load/create/advance runtime commands are checkpoint barriers because they are whole-timeline transitions, not fixed-tick events; the v2 artifact checker now mutates a copied artifact to place `CreateScene` inside the checkpoint-to-target replay window and requires the runtime to reject it with `unsupported runtime timeline mutation event`. |
...
| Determinism evidence | `tools\validate_replay_scrub.bat` uses SkullScope to prove a selected visual replay sample maps to queried body state and that retained solver restore matches the original solver hash before branching. `tools\validate_replay_v2_artifact.bat` writes a real runtime v2 presentation artifact from `replay_v2_solver_one.scene.json` with branch rows, typed event rows, checkpoint event-cursor rows, solver hash rows, and sparse solver checkpoint rows, reloads it through the C++ v2 reader, proves an older loaded pose can be applied/restored, proves `--replay-load-probe` can drive the runtime scrub source from that file, proves `--replay-restore-file-probe` can load and hash-verify a saved checkpoint frame with a nonzero event cursor in a fresh Debug runtime, proves `--replay-restore-target-file-probe` can replay typed events/fixed ticks from the checkpoint to a saved target hash, proves `--replay-restore-branch-file-probe` can make that saved target a live branch, proves expected saved-file restore failures emit a `replay_restore` SkullScope row and query result, mutates a copied artifact to prove scene-transition commands are rejected inside the checkpoint-to-target replay window, writes a second six-body generated-topology v2 artifact from `replay_v2_generated_topology.scene.json`, restores it from the mismatched one-body scene with `generated_topology_rebuilt=1`, queries presentation frames/bodies/branches/events/event-cursors/hashes/checkpoints, requires decoded runtime-command/generated-scene-config/world-override/editor-place/editor-transform/launcher-config/launcher-fire payloads, exports a bounded SkullScope slice, and imports that slice through `physics_query`. |

## Implementation Status Audit, 2026-06-23

| Plan phase | Status | Current evidence and gap |
|------------|--------|--------------------------|
| Phase 1: State inventory and hash contract | Done for current v2 scope | Solver hashes include body fields and `ReplaySolverWorldSnapshot`, and retained restore gates live-branch reset on a recomputed hash match. `Agentic/Reports/2026-06-23/replay-v2-authoritative-state-inventory.md` now lists the current serialize/rebuild/query-boundary decisions for runtime, body, physics world, contact, sleep, broadphase, event, launcher, and scene-transition state. |
| Phase 2: In-memory solver checkpoint ring | Done for current v2 scope | Dense per-frame `ReplaySolverFrameSample` retention remains the in-memory source for smooth scrub/branch workflows, and v2 save emits sparse `SCHK` checkpoint payloads from checkpoint-boundary retained solver samples. `m_checkpoints` still stores summaries for runtime stats; the full sparse checkpoint payload is materialized at v2 save time rather than kept in a second live ring. |
| Phase 3: Event stream | Done for current v2 scope | `ReplayEventRecorder` now records bounded timeline-start, runtime-command, hash-verified branch-restore, generated-scene-config, world-override, editor-place, editor-transform, launcher-config, and launcher-fire rows. V2 saves emit/query `EVNT` event chunks plus `ECUR` checkpoint event-cursor chunks; `replay_query` decodes runtime-command and typed generated-scene/world/editor-placement/editor-transform/launcher payloads; loaded solver checkpoints receive their cursor metadata in C++; the saved target probe replays typed world/editor-placement/editor-transform/launcher events deterministically, verifies generated-scene config metadata when encountered, can rebuild deterministic generated topology from the saved config event before applying a mismatched checkpoint, and validates a real queued `ResetCurrentScene` command as a post-reset checkpoint precondition. External scene-load/create/advance commands are accepted as checkpoint barriers: queryable when captured, restorable from checkpoints after the transition, and explicitly rejected if mutated into the checkpoint-to-target replay window. |
| Phase 4: Restore to tick without branch resume | Done for current v2 scope | Current interactive restore applies the selected retained in-memory sample directly, recomputes the solver hash from the restored live state, emits `replay_restore`, and resets the live replay branch only on a hash match. The Debug saved-file probe can load one sparse checkpoint frame from v2 and hash-verify that exact checkpoint after applying it. The Debug target probe can restore the nearest saved sparse checkpoint and replay typed world/editor-placement/editor-transform/launcher events plus fixed ticks to the saved non-checkpoint target hash selected by the probe. Saved-file target restore success and failure paths now emit `replay_restore` SkullScope rows with source, checkpoint, hash, fallback, and failure-reason fields. |
| Phase 5: Branch and resume | Done for current v2 scope | `Enter` or the scrubber `BRANCH` button on the retained solver row makes the selected retained sample live, assigns a child branch id with parent/source frame/hash metadata, records a branch-restore event, and resets the replay timeline so old future samples do not mix into the new branch. The same action on a loaded `V2 FILE` row uses the v2 checkpoint/event/hash tracks to make the selected saved frame live after hash verification. External scene-load/create/advance runtime commands are not branch-replayed across checkpoint windows; they are whole-timeline checkpoint barriers. |
| Phase 6: Saved authoritative replay artifacts | Done for current v2 scope | Presentation saves now produce chunked binary v2 artifacts for smooth scrub playback/querying, with a body dictionary, 32-byte pose records, seek index, branch provenance rows, typed event rows, checkpoint event-cursor rows, per-tick solver hash rows, sparse solver checkpoint chunks, C++ presentation/checkpoint/event/hash loaders, file-backed runtime presentation scrub source, in-app v2 save and `.skreplay` load controls, saved checkpoint-frame restore verification, saved non-checkpoint target checkpoint-plus-event restore verification, generated topology rebuild from saved config events before mismatched checkpoint restore, reset-command event capture/query/checkpoint-precondition validation, live branch-from-file verification, saved-file restore failure diagnostics, explicit timeline-mutation rejection proof, `tools\replay_query` bridge, and `tools\validate_replay_v2_artifact.bat` runtime artifact/load/hash/branch/event/event-cursor/checkpoint/restore proof. Whole-timeline scene-load/create/advance commands are query-only inside a checkpoint-to-target window by design, but checkpoints after those transitions remain restorable. |
...
| `TestOutput\validation\replay_v2\replay_save_probe.skreplay` | v2 binary artifact from `replay_v2_solver_one.scene.json`, 24 post-reset frames, 2 bodies, 8 events including decoded `ResetCurrentScene`, 1 checkpoint | 13,050 bytes |
| `TestOutput\validation\replay_v2\replay_generated_topology_probe.skreplay` | v2 binary artifact from `replay_v2_generated_topology.scene.json`, 24 post-reset frames, 6 bodies after the save-probe editor placement, 8 events including decoded `ResetCurrentScene`, 1 checkpoint | 17,222 bytes |
...
as the normal saved scrub format. The tiny v2 generated-scene/editor-placement/editor-transform
proof is not a scale comparison, but it confirms the binary/query path remains
inspectable and cheap for small deterministic restore scenarios.
...
| User physics actions | Fire launcher, spawn projectile, place editor object (implemented for placement commits), move/rotate/scale object when committed through editor gizmos (implemented; scale stores axis and factor against the checkpoint shape). |
...
| Commit restore | Use the scrubber `BRANCH` button while paused on a retained solver or loaded `V2 FILE` target. |
| Commit restore, keyboard | Press `Enter` while paused on the solver or `V2 FILE` row. |
...
tools\replay_query.bat replays\sample.skreplay branches
tools\replay_query.bat replays\sample.skreplay events --frames 1200:1260
tools\replay_query.bat replays\sample.skreplay event-cursors --frames 1200:1260
tools\replay_query.bat replays\sample.skreplay hashes --frames 1200:1260
tools\replay_query.bat replays\sample.skreplay checkpoints --frames 1200:1260
