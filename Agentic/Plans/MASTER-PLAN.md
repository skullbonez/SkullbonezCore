...
|------|--------|---|----------------|
| [03 Governance apparatus removal](../../engine-cleanup-plans/03-governance-apparatus-reduction.md) | In Progress | 45% | Steps 0.2 and 1.1 complete: DX12-only runtime renderer exclusivity now lives in a typed launch option table plus a doctest. Next step 1.2 deletes the regex boundary checker + all `MAX_*` ratchets and invocations; then strip regex-enforced gates from `AGENTS.md`; relax comment gate. **Prerequisite:** step 2.1 waits on `TODO/behavioral-test-depth.md` P1+P4 (tests are the stated enforcement replacement). |
