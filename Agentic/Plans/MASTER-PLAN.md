...
|------|--------|---|----------------|
| [03 Governance apparatus removal](../../engine-cleanup-plans/03-governance-apparatus-reduction.md) | In Progress | 30% | Step 0.2 inventory complete: only DX12-only runtime renderer exclusivity survives as a real boolean check; next step 1.1 adds that check. Then delete the regex boundary checker + all `MAX_*` ratchets and invocations; strip regex-enforced gates from `AGENTS.md`; relax comment gate. **Prerequisite:** step 2.1 waits on `TODO/behavioral-test-depth.md` P1+P4 (tests are the stated enforcement replacement). |
