...
| [replay-visuals-prediction-and-memory](TODO/replay-visuals-prediction-and-memory.md) | In progress | 20% | Mega plan (2026-07-09): supersedes `replay-prediction-and-memory.md`, absorbing the trajectory-visuals repair (flicker/ribbon/lock-step investigation complete; 12 confirmed failure modes recorded in-plan). Interleaved stages: deterministic drawing → TrajectoryStore → lock-step fixes → prediction worker job (fable-03 P3) → ribbon renderer rewrite → memory-quality tuning (re-derive data model first) → replay code-size right-sizing. |
