...
| [replay-visuals-prediction-and-memory](TODO/replay-visuals-prediction-and-memory.md) | In progress | 42% | Mega plan (2026-07-09): supersedes `replay-prediction-and-memory.md`, absorbing the trajectory-visuals repair. Stage 0 instrumentation/repro, Stage 1 deterministic drawing, and Stage 2 rebuild/reveal churn plus visibility/contact-completeness controls are complete; next is Stage 3.1 TrajectoryStore records/versioning/published prefixes before lock-step fixes, prediction worker job (fable-03 P3), ribbon renderer rewrite, memory-quality tuning, and replay code-size right-sizing. |
