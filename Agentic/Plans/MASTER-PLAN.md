...
| [replay-visuals-prediction-and-memory](TODO/replay-visuals-prediction-and-memory.md) | In progress | 46% | Mega plan (2026-07-09): supersedes `replay-prediction-and-memory.md`, absorbing the trajectory-visuals repair. Stage 0 instrumentation/repro, Stage 1 deterministic drawing, Stage 2 rebuild/reveal churn plus visibility/contact-completeness controls, and Stage 3.1 TrajectoryStore records/versioning/published-prefix shell are complete; next is Stage 3.2 build pass fed by prediction publish events and solver-ring appends before lock-step fixes, prediction worker job (fable-03 P3), ribbon renderer rewrite, memory-quality tuning, and replay code-size right-sizing. |
