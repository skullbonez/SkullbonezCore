...
## Scene Browser Index Helper Slice

This shared scene-browser cleanup slice moved current-scene browser selection
lookup from `Run` into `SceneRuntimeLoad`. Input navigation and the render-host
callback now call `CurrentSceneBrowserIndex(m_sceneController, m_sceneBrowser)`
directly, reusing the scene-runtime path normalization and browser index lookup
logic instead of bouncing through a private `Run` wrapper.
...
- `Run::CurrentSceneBrowserIndex`

New owner surface:

- `CurrentSceneBrowserIndex(const SceneController&, const RunSceneBrowserState&)` in
  `Runtime/Scene/SceneRuntimeLoad.*`

Updated call sites:

- `Run::BuildRuntimeRenderHostCallbacks`
- `Run::TakeInput`

Boundary/tooling guard:

- `tools/check_runtime_boundaries.py` lowers the `Run.h` private-method
  ratchet from 157 to 156.
- New header and source guardrails reject the removed scene browser index
  wrapper declaration and `Run::` definition from returning.
- Synthetic self-tests cover the removed header and source surfaces.
...
- Touched source-bearing files inspected:
  ...
  `SkullbonezSource/Runtime/RunInput.cpp`, and
  `tools/check_runtime_boundaries.py`.
- No comments were added. Existing browser-index behavior is preserved while
  the path-normalization lookup now lives with the scene load helpers.
- No subsystem-wide checklist was required; this was a touched-file audit, not
  a comment remediation pass.
...
- Targeted Profile build: `tools\validate_build.bat Profile`, logged at
  `TestOutput\validation\agent_logs\scene_browser_index_profile_build.log`;
  passed with 0 warnings and 0 errors in 40.25s.
- Fast gate: `tools\validate_fast.bat`, logged at
  `TestOutput\validation\agent_logs\scene_browser_index_validate_fast.log`;
  passed formatting, project filters, runtime boundaries, and Profile/Debug
  builds in 49.77s.
- Runtime boundary gate: `python tools\check_runtime_boundaries.py --repo .`,
  logged at
  `TestOutput\validation\agent_logs\scene_browser_index_runtime_boundaries.log`;
  passed with 0 errors in 2.75s.
- Broad gate: `tools\validate_full.bat`, logged at
  `TestOutput\validation\agent_logs\scene_browser_index_validate_full.log`;
  passed project filters, runtime boundaries, Profile/Debug builds, DX12
  validation with 0 errors and matching screenshots, and byte-exact
  `physics_regression_solver.csv` in 28.09s.

Rubber-duck review was intentionally deferred by explicit user instruction until
the end of the remaining plan work. Do not move this plan to `Done/` until the
final rubber-duck pass is satisfied.

Residual architecture risk: scene browser refresh, cinematic scene apply, scene
...
## Diagnostics Memory-Dump Wrapper Slice

Status: Implemented and validated on `nightrunner-26th-July`.

This diagnostics slice removed the private `Run` shutdown memory-dump wrapper.
Shutdown now calls `DiagnosticsRuntime::WriteMainMemoryDump(...)` directly with
the existing replay, model collection, scene state, checkpoint, and simulation
time inputs. The behavior is unchanged, but memory-dump policy is no longer
hidden behind a `Run.h` declaration.
...
- `DiagnosticsRuntime::WriteMainMemoryDump(...)` remains the owner of
  CLI-requested main-memory JSON output.
...
- `tools/check_runtime_boundaries.py` lowers the `Run.h` private-method ratchet
  from 152 to 151.
- New header and source guardrails reject `WriteMainMemoryDump` wrapper
  declarations or `Run::WriteMainMemoryDump` definitions from returning.

Validation:

- Targeted Profile build: `tools\validate_build.bat Profile`, logged at
  `TestOutput\validation\agent_logs\diagnostics_memory_dump_profile_build.log`;
  passed with 0 warnings and 0 errors in 39.8s.
- Fast gate: `tools\validate_fast.bat`, logged at
  `TestOutput\validation\agent_logs\diagnostics_memory_dump_validate_fast.log`;
  passed formatting, project filters, runtime boundaries, and Profile/Debug
  builds in 49.3s.
- Runtime boundary gate: `python tools\check_runtime_boundaries.py --repo .`,
  logged at
  `TestOutput\validation\agent_logs\diagnostics_memory_dump_runtime_boundaries.log`;
  passed with 0 errors in 2.9s.
- Broad gate: `tools\validate_full.bat`, logged at
  `TestOutput\validation\agent_logs\diagnostics_memory_dump_validate_full.log`;
  passed project filters, runtime boundaries, Profile/Debug builds, DX12
  validation with 0 errors and matching screenshots, and byte-exact
  `physics_regression_solver.csv` in 27.9s.
...
## Scene Creation Ownership Slice

Status: Implemented and validated on `nightrunner-26th-July`.

This scene-runtime slice removed the Create Scene command wrapper from `Run.h`.
`SceneRuntimeCreate` now owns starter scene filename sanitization, unique path
selection, starter scene JSON writing, scene browser refresh, queue append, and
the scene-control action returned to `RunInput`. `RunInput` still executes the
returned action through the existing scene-control transition path.
...
- `tools/check_runtime_boundaries.py` lowers the `Run.h` private-method ratchet
  from 146 to 145.
- New header and source guardrails reject `CreateSceneFromUI` declarations or
  `Run::CreateSceneFromUI` definitions from returning.
- `tools/validate_project_filters.py` now recognizes `SceneRuntimeCreate` as a
  runtime scene module prefix.

Validation:

- Direct project-filter gate: `python tools\validate_project_filters.py`, logged
  at `TestOutput\validation\agent_logs\scene_create_project_filters.log`;
  passed with 0 errors in 0.8s.
- Runtime boundary gate: `python tools\check_runtime_boundaries.py`, logged at
  `TestOutput\validation\agent_logs\scene_create_runtime_boundaries.log`;
  passed with 0 errors in 3.1s.
- Targeted Profile build: `tools\validate_build.bat Profile`, logged at
  `TestOutput\validation\agent_logs\scene_create_profile_build.log`; passed
  with 0 warnings and 0 errors in 41.9s.
- Fast gate: `tools\validate_fast.bat`, logged at
  `TestOutput\validation\agent_logs\scene_create_validate_fast.log`; passed
  formatting, project filters, runtime boundaries, and Profile/Debug builds in
  52.4s.
- Broad gate: `tools\validate_full.bat`, logged at
  `TestOutput\validation\agent_logs\scene_create_validate_full.log`; passed
  project filters, runtime boundaries, Profile/Debug builds, DX12 validation
  with 0 errors and matching screenshots, and byte-exact
  `physics_regression_solver.csv` in 27.7s.
