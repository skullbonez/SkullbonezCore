...
| Config and CLI parsing | Config settings are table-driven, CLI value/flag handling has directive-table footholds, renderer selection uses a renderer option table, per-component physics debug flags now use a directive table, and suite-file scene lists use scoped file IO. Remaining parser pressure is mostly resolved-config reporting and shrinking the specialized physics/scene argument helpers. |
