...
| Render backend capabilities | A compatibility-preserving `RenderCapabilities` query now centralizes GPU timer, DXR, debug line, instancing, capture, and dynamic-VB support checks; capture-dependent callers now honor the capture/dynamic-VB capability boundary. |
...
| Render backend capability split | Partly done through `RenderCapabilities`; capture callers now use the capability boundary, while deeper interface split remains | `tools\validate_renderers.bat` plus DX12 validation log check. |
