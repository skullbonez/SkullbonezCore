...
| File-handle and shader-blob RAII | Config loading, GL shader source loading, terrain raw loading, scene parsing, text atlas IO, backbuffer capture, DX11/DX12 shader compile blobs, and DX12 root-signature/generate-mips temporary blobs now use scoped ownership rather than manual close/release paths. |
...
| RAII cleanup | In progress; file handles/source buffers plus selected DX11/DX12 shader and root-signature blobs are improved, broader backend COM resources remain | Renderer-specific validation for COM/resource changes. |
