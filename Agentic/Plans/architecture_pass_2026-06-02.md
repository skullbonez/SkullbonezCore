...
| File-handle and shader-blob RAII | Config loading, GL shader source loading, terrain raw loading, scene parsing, text atlas IO, backbuffer capture, and DX11/DX12 shader compile blobs now use scoped ownership rather than manual close/release paths. |
...
| RAII cleanup | In progress; file handles/source buffers and DX11/DX12 shader compile blobs are improved, broader backend COM resources remain | Renderer-specific validation for COM/resource changes. |
