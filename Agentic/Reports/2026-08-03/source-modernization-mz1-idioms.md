...
uses the same direct Debug/non-Debug build predicate at its two preprocessor
sites, preserving the existing Debug scalar and Profile/Release SSE selection.
...
- Exact scoped scans find zero `NULL` or `SKULLBONEZ_INTRINSICS` tokens and zero
  remaining MZ0 target-type cast spellings in the 17 touched files.
