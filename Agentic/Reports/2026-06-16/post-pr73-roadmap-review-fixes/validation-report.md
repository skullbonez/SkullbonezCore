...
cmd.exe /c tools\validate_scene_parser_tests.bat 2>&1 | Tee-Object -FilePath TestOutput\post_pr73_final_scene_parser_tests.log
cmd.exe /c tools\validate_full.bat 2>&1 | Tee-Object -FilePath TestOutput\post_pr73_final_validate_full.log
```

The `TestOutput\post_pr73_review_*.log` and `TestOutput\post_pr73_final_*.log` files are local validation artifacts and are intentionally not committed.
...
After the final audit cleanup, `tools\validate_scene_parser_tests.bat` passed again with exit code `0` in `3.7s`.
...
PASS: Style material authoring contract
PASS: Scene material authoring sample loads
PASS: Material authoring rejects malformed options
PASS: all scene parser unit tests passed.
PASS: scene parser unit tests passed.
VALIDATE_SCENE_PARSER_TESTS_EXIT_CODE=0
VALIDATE_SCENE_PARSER_TESTS_ELAPSED_SECONDS=3.7
```

After the final audit cleanup, `tools\validate_full.bat` passed again with exit code `0` in `107.2s`.
...
VALIDATE_FULL: ALL PHASES PASSED
VALIDATE_FULL_EXIT_CODE=0
VALIDATE_FULL_ELAPSED_SECONDS=107.2
```
