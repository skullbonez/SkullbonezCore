...
## Non-Negotiable Orchestration Path

Always use the repository orchestrator path for implementation work. Start,
resume, transition, verify, validate, finalize, and report roadmap items through
`tools\orchestrator.bat` commands so the state machine, run artifacts,
rubber-duck telemetry, queue updates, and mandatory ledger are generated by the
repo-owned tooling.

Do not bypass the orchestrator with an ad hoc Codex/current-agent
implementation path, even for explicit plan paths or small work. If an explicit
plan path is not already represented in `queue.json`, create or request a
proper queue entry first, then run it through the orchestrator.

If `queue.json`, policy, machine state, dependencies, legal transitions, active
item accounting, or required queue fields are malformed or contradictory, stop
and ask the user what to do before proceeding. Do not repair, infer, skip, or
manually work around malformed queue state unless the user explicitly directs
that fix.
...
the file names only when queue state is valid. If the queue is malformed, ask
the user how to proceed before creating or running any entry.
...
- Stop instead of guessing when malformed queue state, a merge conflict,
  missing credential, failing validation, or policy mismatch would make the next
  action unsafe.
...
5. Spawn exactly one worker through the orchestrator. If the orchestrator cannot
   run or record the worker path, stop and ask the user how to proceed instead
   of performing an ad hoc current-agent implementation.
