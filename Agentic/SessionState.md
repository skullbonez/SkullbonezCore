...
| Last documentation milestone | Fable-02 capture-facade evidence records the deleted `GfxCapture()` fallback/facade, checker ratchet drop, blocker wording update, and validation gates. |
| Last source/data milestone | Fable-02 capture-facade cleanup makes `Run::SaveScreenshot` use the startup-bound capture backend capability only, deletes `GfxCapture()`, and guards the deleted facade in the checker. Current blocker ledger: 26 blocked rows remain. |
...
| Validation | Latest source gates passed on 2026-07-07 for fable-02 capture-facade cleanup: `python tools\check_runtime_boundaries.py --self-test` (0.320s), `python tools\check_runtime_boundaries.py` (15.693s, 0 errors), `tools\validate_fast.bat` (38.833s; formatting/project filters/staged-size/runtime-boundaries/Profile+Debug builds passed with 0 warnings/errors; unit tests passed: 44 doctest cases, 574 assertions), and `tools\validate_dx12_renderer.bat` (19.420s; DX12 InfoQueue 0 validation errors; screenshots matched baselines). Logs: `Agentic\Reports\2026-07-07\logs\fable-02-capture-facade-boundary-self-test.log`, `Agentic\Reports\2026-07-07\logs\fable-02-capture-facade-boundary-scan.log`, `Agentic\Reports\2026-07-07\logs\fable-02-capture-facade-validate-fast.log`, `Agentic\Reports\2026-07-07\logs\fable-02-capture-facade-validate-dx12-renderer.log`. |
