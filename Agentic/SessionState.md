...
| Active objective | Complete all fable plans. Fable-04 L3/L4 and C1/C2 plus fable-05 P3 SpatialGrid/PhysicsWorld plus GameModelCollection classification/pure topology and legacy camera pose-read conversions are complete; fable-03, remaining fable-04, remaining fable-05, remaining fable-06, and fable-07 still have open work. |
| Last documentation milestone | `fable_plans/HANDOFF-2026-07-07.md` and `fable_plans/04-build-layering-and-repo-hygiene-progress.md` now document fable-04 C2 scene capacity extraction and updated counts. |
| Last source/data milestone | Fable-04 C2 scene capacity extraction is implemented: `GameObjects/SceneCapacity.h` owns `TOTAL_CAMERA_COUNT`, `TOTAL_TEXTURE_COUNT`, `DEFAULT_GAME_MODEL_CAPACITY`, `MAX_GAME_MODELS`, and `DEFAULT_GAME_MODELS`; `Common.h` includes it during the aliasing period, and `tools/validate_project_filters.py` recognizes the new GameObjects header. PHYS-012 capacity policy did not move. |
| Pending work | Not all fable plans are complete. Open work remains in fable-03, remaining fable-04, remaining fable-05, remaining fable-06, and fable-07 per `fable_plans/HANDOFF-2026-07-07.md`; fable-01, fable-02, fable-08, and fable-09 are checklist complete. Next small slices: fable-04 C3 asset key/hash extraction or fable-05 GameModelCollection Lane R append cleanup at `GameModelCollection.cpp:288`, `:410`, and `:417`. |
...
| Validation | Latest source gate passed on 2026-07-08 for fable-04 C2 scene capacity extraction: touched-file comment audit inspected `Common.h`, `SceneCapacity.h`, and `tools\validate_project_filters.py` with 0 deferred; `python tools\validate_project_filters.py --repo .` passed in 00:00:00.9114220 with 0 errors; `tools\validate_fast.bat` passed in 00:01:04.6129410; `tools\validate_full.bat` passed in 00:00:45.5548641 with runtime boundaries at 0 errors, Profile/Debug builds at 0 warnings/errors, DX12 validation errors 0, screenshots matching committed baselines, and `physics_regression_solver.csv` byte-exact. Logs live under `Agentic\Reports\2026-07-08\logs\fable-04-c2-scene-capacity-*`. |
