...
| Current Plan 14 inventory | 2026-07-10 Plan 14 Step 0.1 is complete as a documentation-only slice. `PhysicsApi.h`/`PhysicsEngine.h` contain no `GameModel` matches, but `PhysicsEngine.h` still exposes raw `modelIndex`/`modelCount` authority and store/debug containers through public signatures. `PhysicsApi.h` has `PhysicsReplaySolverSnapshotView::modelCount` plus public-header implementation exposure to concrete store/debug types. No repository validation was required. |
...
| Public physics API boundary | Active plan (20%) | `engine-cleanup-plans/14-public-physics-api-boundary.md`; Step 0.1 inventory is complete. Next source slices replace raw dense row authority and public solver-container signatures in `PhysicsApi.h`/`PhysicsEngine.h`. |
