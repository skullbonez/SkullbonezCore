...
| Last source milestone | K004 store-force slice is implemented, K003 pose integration now advances `PhysicsBodyRecord` state inside `PhysicsBodyStore`, `GameModel::UpdatePosition` is deleted, tornado field no longer borrows `modelAccess.Models()`, PersistentContactSolver no longer receives `PhysicsModelAccess` or opens raw model ranges, object/object CCD no longer uses `GameModel::SweepGameModel`, RunSolverPhysics and wake-time compatibility writebacks now go through named `PhysicsModelAccess` commands, terrain CCD/manifold generation now lives in `Physics/TerrainContactManifold` instead of the `GameModel` response mailbox, and K005 allocator-owned body/collider handles are implemented through `PhysicsBodyStore` and `ColliderStore`. |
| Pending work | K003 still has PhysicsScene bridge ranges. K005 is done in the tracker; no `MakeCompatibility*Handle`, compatibility generation, `SetCompatibilityBodies`, or no-store point-joint index helper hits remain. |
...
| Validation | Commit `536b2fb0` touched-source comment audit inspected `GameModel.cpp`, `GameModel.h`, `PhysicsWorld.cpp`, `PhysicsWorld.h`, `TerrainContactManifold.cpp`, and `TerrainContactManifold.h` with 0 deferred files; it passed `tools\validate_build.bat Profile` with log `TestOutput\agent_build_profile_terrain_contact_view.log`, `python tools\check_runtime_boundaries.py --repo .` with log `TestOutput\agent_runtime_boundaries_terrain_contact_view.log`, and `tools\validate_physics.bat` with log `TestOutput\agent_validate_physics_terrain_contact_view.log`; physics gate built Debug/Profile at 0 warnings and 0 errors and produced a byte-exact `physics_regression_solver.csv`. Commit `b806fce9` touched-source comment audit inspected `PhysicsWorld.cpp` with 0 deferred files; it passed `tools\validate_build.bat Profile` with log `TestOutput\agent_build_profile_wake_writeback_command.log`, `python tools\check_runtime_boundaries.py --repo .` with log `TestOutput\agent_runtime_boundaries_wake_writeback_command.log`, and `tools\validate_physics.bat` with log `TestOutput\agent_validate_physics_wake_writeback_command.log`; physics gate built Debug/Profile at 0 warnings and 0 errors, matched standalone/runtime handle mirror smoke, and produced a byte-exact `physics_regression_solver.csv`. Commit `ad41c98c` touched-source comment audit inspected `PhysicsWorld.cpp` with 0 deferred files; it passed `tools\validate_build.bat Profile` with log `TestOutput\agent_build_profile_writeback_commands.log`, `python tools\check_runtime_boundaries.py --repo .` with log `TestOutput\agent_runtime_boundaries_writeback_commands.log`, and `tools\validate_physics.bat` with log `TestOutput\agent_validate_physics_writeback_commands.log`; physics gate built Debug/Profile at 0 warnings and 0 errors, matched standalone/runtime handle mirror smoke, and produced a byte-exact `physics_regression_solver.csv`. Commit `4366ad7c` touched-source comment audit inspected `GameModel.cpp`, `GameModel.h`, `ObjectContactManifold.cpp`, `ObjectContactManifold.h`, and `PhysicsWorld.cpp` with 0 deferred files; it passed `tools\validate_build.bat Profile` with log `TestOutput\agent_build_profile_object_sweep_view.log`, `python tools\check_runtime_boundaries.py --repo .` with log `TestOutput\agent_runtime_boundaries_object_sweep_view.log`, and `tools\validate_physics.bat` with log `TestOutput\agent_validate_physics_object_sweep_view.log`; physics gate built Debug/Profile at 0 warnings and 0 errors, matched standalone/runtime handle mirror smoke, and produced a byte-exact `physics_regression_solver.csv`. Commit `87df4b77` touched-source comment audit inspected `ObjectContactManifold.cpp`, `ObjectContactManifold.h`, `PersistentContactSolver.cpp`, `PersistentContactSolver.h`, `PhysicsWorld.cpp`, and `PhysicsWorld.h` with 0 deferred files; it passed `tools\validate_build.bat Profile` with log `TestOutput\agent_build_profile_persistent_solver_view.log`, `python tools\check_runtime_boundaries.py --repo .` with log `TestOutput\agent_runtime_boundaries_persistent_solver_view.log`, and `tools\validate_physics.bat` with log `TestOutput\agent_validate_physics_persistent_solver_view.log`; physics gate built Debug/Profile at 0 warnings and 0 errors, matched standalone/runtime handle mirror smoke, and produced a byte-exact `physics_regression_solver.csv`. Commit `78533f94` touched-source comment audit inspected `GameModelCollection.cpp`, `GameModelCollection.h`, `PhysicsModelAccess.h`, and `PhysicsWorld.cpp` with 0 deferred files; it passed `tools\validate_build.bat Profile` with log `TestOutput\agent_build_profile_tornado_sync.log`, `python tools\check_runtime_boundaries.py --repo .` with log `TestOutput\agent_runtime_boundaries_tornado_sync.log` and 0 errors, and `tools\validate_physics.bat` with log `TestOutput\agent_validate_physics_tornado_sync.log`; physics gate built Debug/Profile at 0 warnings and 0 errors, matched standalone/runtime handle mirror smoke, and produced a byte-exact `physics_regression_solver.csv`. Commit `a20329cf` touched-source comment audit inspected `GameModel.cpp`, `GameModel.h`, `PhysicsBodyStore.cpp`, `PhysicsBodyStore.h`, and `PhysicsWorld.cpp` with 0 deferred files; it passed `tools\validate_build.bat Profile`, runtime-boundary checks, and `tools\validate_physics.bat`. Commit `abe3f4bf` passed build, runtime-boundary, physics, and full gates. Earlier store-force and wake/ragdoll slices passed their targeted build, runtime-boundary, and physics gates. Deep physics known-issue signature mismatch is pre-existing on this branch. |
