...
| Validation | 2026-07-05 Phase 10 render snapshot authority: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_fast.bat` passed; intermittent `tools\validate_physics.bat` passed with byte-exact physics regression; `tools\validate_dx12_renderer.bat` passed. `tools\validate_perf.bat` still fails only `PHYSICS_BENCH` `Frame/Render.avg` against the old baseline after memory, p50, DX12 perf, and absolute budgets pass; detached clean-HEAD no-audio isolation also fails worse, so this is tracked as pre-existing perf-baseline/restart-frame drift. Logs: `TestOutput\agent_validate_fast_render_instance_store_shape_kind_fixed.log`, `TestOutput\agent_validate_physics_render_instance_store_shape_kind.log`, `TestOutput\agent_validate_dx12_render_instance_store_shape_kind.log`, `TestOutput\agent_validate_perf_render_instance_store_shape_kind.log`, `TestOutput\agent_validate_perf_render_instance_store_shape_kind_rerun.log`. Prior Phase 10 logs remain in `TestOutput\agent_validate_fast_*` and `TestOutput\agent_validate_physics_*`. |
