...
| Last source/data milestone | Moved replay editor-transform event payload recording off `GameModel` pose/orientation mirrors. `ReplayRuntime::RecordEditorTransformEvent` now takes replay id plus body position/orientation values, and editor/probe callers read committed `PhysicsBodyStore` rows after editor mutation. |
...
| Validation | 2026-07-05 replay editor-transform event store-authority slice: `git diff --check` passed; focused Debug build passed with 0 warnings/errors; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; touched-file comment audit passed for all touched source/tool files; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors. |
