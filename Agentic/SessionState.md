...
| Last source/data milestone | Moved editor selection outline and gizmo overlay frame drawing off `GameModel` pose/shape mirrors. `BuildEditorToolOverlayTrace` still uses `GameModel` only for cold ragdoll group identity, then traces selection presentation from bounded `PhysicsBodyStore`/`ColliderStore` pointer scratch. |
...
| Validation | 2026-07-05 selection overlay store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; focused Debug build passed with 0 warnings/errors; touched-file comment audit inspected `RunEditorTools.cpp`, `RunEditorOverlayTools.inl`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`. |
