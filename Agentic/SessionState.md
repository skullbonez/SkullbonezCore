...
| Last documentation milestone | Updated `Agentic/Plans/To_Eval/contact-audio-perceptual-model-plan.md` with 200-brick SkullScope baseline counts and the non-cinematic/no-water launch used for contact-audio evidence. |
| Last source milestone | Removed the illegal `Frame/SoA` profiler scope from `GameModelSoACache::RefreshBodyData`; scene-load sleep seeding can refresh this cache before the profiler frame opens. |
| Pending work | Finish the remaining contact-audio plan phases: explicit heavy-landing/support reporting, optional raw-fact/merge aggregate SkullScope rows, material-layer polish, acceptance-scene proof, and the unrelated DX12 cinematic/water transition break seen in `aaa_ragdoll_sunset_showcase`. The physics/GameModel authority bridge still needs later store-authority and render-projection cleanup, but the clear physics inheritance violation is fixed. |
...
| Validation | 2026-07-03 200-brick/profiler fix slice: `tools\validate_build.bat Debug` passed with 0 warnings and 0 errors (`TestOutput\agent_build_debug_soa_profiler_fix.log`). A 60-frame `aaa_ragdoll_sunset_showcase` probe with `--cinematic off --no-water` completed all 60 frames. The 1800-frame SkullScope run completed frames 0..1799, wrote `Debug\contact_audio_ragdoll_wall_200.physicsdiag.ndjson`, and produced 205,287 contact-audio verdict rows with 191 submitted voices; `dx12_validation.txt` ended at 0 errors. `git diff --check` passed. `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors. `tools\validate_fast.bat` passed with log `TestOutput\agent_validate_fast_soa_profiler_200brick.log`. |
