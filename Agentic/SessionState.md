...
| Last source/data milestone | Deleted the launcher fixed-tree per-release `GameModel` writeback. `PhysicsScene` now releases/wakes fixed-tree rows inside the stores; `GameModelCollection::WriteBackPhysicsBody`, `PhysicsBodyStore::WriteBackToModelAt`, and the released-row output vector are gone. |
| Pending work | Continue narrowing the remaining explicit bulk fixed-step compatibility writeback, moving runtime/replay/editor identity paths toward durable handles, and lowering allowlists as future deletion commits remove borrowers. |
...
| Validation | 2026-07-05 fixed-tree release writeback deletion slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings/errors (`TestOutput\agent_build_debug_fixed_tree_release_writeback.log`); touched-file comment audit passed for `GameModelCollection.cpp`, `GameModelCollection.h`, `PhysicsBodyStore.cpp`, `PhysicsBodyStore.h`, `PhysicsEngine.cpp`, `PhysicsEngine.h`, `PhysicsScene.cpp`, `PhysicsScene.h`, and `tools/check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors (`TestOutput\agent_validate_fast_fixed_tree_release_writeback_rerun.log`); intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_fixed_tree_release_writeback.log`). |
...
| Physics/GameModel authority | Active follow-up | `Agentic/Plans/To_Eval/physics-game-model-authority-plan.md`; persistent solver and launcher fixed-tree per-release writeback boundaries are narrowed, but strict store authority and the final bulk compatibility step boundary remain future work. |
