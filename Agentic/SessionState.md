...
| Last source/data milestone | Replay path/query/prediction target identity now resolves `ReplayBodyId` through `PhysicsBodyStore` rows and `PhysicsBodyStore::HandleForReplayBodyId()` instead of reading `GameModel` replay ids in the replay target UI paths. |
| Pending work | Continue durable editor identity cleanup, then run a final completion audit for remaining `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 replay path/query identity slice passed: residue scan found no `GetReplayBodyId()` or `TryGetModel()` target lookup in the touched replay target files outside checker self-tests; `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings/errors in 4.4s; touched-file comment audit inspected `RunReplayTools.cpp`, `RunReplayQueryTools.inl`, `RunReplayPredictionVisualizer.inl`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed in 29.8s with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed in 13.6s with standalone/runtime handle smoke and byte-exact 20,001-line `physics_regression_solver.csv`. |
