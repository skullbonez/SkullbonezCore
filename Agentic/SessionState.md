...
| Last source/data milestone | Moved launcher repro picking and snapshot physics payloads off `GameModel` body/shape mirrors. Launcher repro now resolves live position, velocity, angular velocity, inertia, orientation, mass, restitution, shape, radius, area, and drag from `PhysicsBodyStore`/`ColliderStore` records while keeping `GameModel` only for cold display-name metadata. |
...
| Validation | 2026-07-05 launcher repro store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; focused Debug build passed with 0 warnings/errors; touched-file comment audit inspected `LauncherTools.cpp` and `check_runtime_boundaries.py`; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`; after one single-file clang-format correction, `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors. |
