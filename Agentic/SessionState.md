...
| Last documentation milestone | Marked contact-audio Phase 4 complete after adding `contact_audio_frame` aggregate rows, updating the SkullScope reference, and refreshing `physics_query_varied.json` from the final Debug executable. |
| Last source milestone | Contact audio now reports reducer-facing counters: raw facts, reduced patch candidates, merged patch facts, queue overflow, burst-window skips, budget rejections, quiet rejections, and submitted voices. The Sound tab displays those reducer terms instead of old threshold-only counters. |
| Pending work | Finish the remaining contact-audio plan phases: explicit heavy-landing/support reporting, material-layer polish, acceptance-scene proof, possible cluster budgeting if evidence shows one local cluster can still monopolize a collapse, and the unrelated DX12 cinematic/water transition break seen in `aaa_ragdoll_sunset_showcase`. The physics/GameModel authority bridge still needs later store-authority and render-projection cleanup, but the clear physics inheritance violation is fixed. |
...
| Blockers | Default cinematic/water launch of `aaa_ragdoll_sunset_showcase.scene.json` trips a DX12 debug-layer transition break in `EmitDx12RenderGraphTransitionBarrier`; contact-audio evidence uses `--cinematic off --no-water` until that render path is fixed. |
| Validation | 2026-07-03 contact-audio aggregate slice: `tools\validate_build.bat Debug` passed with 0 warnings and 0 errors (`TestOutput\agent_build_debug_contact_audio_aggregate.log`); `Debug\SKULLBONEZ_CORE.exe --contact-audio-smoke` submitted one voice (`TestOutput\agent_contact_audio_smoke_aggregate.log`); `python -m py_compile tools\physics_query.py tools\check_physics_query_regression.py` passed; `python tools\check_physics_query_regression.py --update` refreshed `TestOutput/baselines/physics_query_varied.json`; `python tools\check_physics_query_regression.py` passed exact baseline match (`TestOutput\agent_physics_query_regression_contact_audio_aggregate_check.log`); `tools\validate_format.bat`, `tools\validate_fast.bat`, and `tools\validate_physics_deep.bat` passed with deep query baseline exact match (`TestOutput\agent_validate_physics_deep_contact_audio_aggregate.log`). Previous 200-brick/profiler fix slice also passed `validate_fast` and the 1800-frame non-cinematic/no-water trace with DX12 InfoQueue 0 errors. |
