...
| Last source/data milestone | `GameModel` no longer stores or exposes replay body ids. `GameModelCollection` owns dense scene-order replay-id metadata, passes ids directly into body-record creation/import, and `PhysicsBodyStore` no longer reads replay identity from `GameModel`. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, finish direct collider registration so refresh no longer needs a `GameModel` input, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body-state readers. |
...
| Validation | 2026-07-05 GameModel replay-id mirror deletion slice passed: focused Profile build at 0 warnings/errors; `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; `tools\validate_fast.bat`; and `tools\validate_full.bat` with DX12 InfoQueue 0 errors, screenshot baselines passing, standalone/runtime physics smoke passing, and byte-exact 20,001-line `physics_regression_solver.csv`. |
