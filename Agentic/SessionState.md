...
| Active objective | Owner redirected active work to `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md`. Stage 0 is complete: trajectory counters, per-category replay memory byte accounting, and manual H1-H5 repro documentation are recorded. Next actionable work is Stage 1.1 deterministic drawing: remove wall-clock budget checks from draw loops while keeping build/tree pass budgets. The overall engine cleanup goal remains active and incomplete. |
...
| Replay visuals/prediction/memory/size | Active plan (~27%) | `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md` - Stage 0.1 trajectory counters/reporting, Stage 0.2 byte categories, and Stage 0.3 H1-H5 manual repro documentation are complete; next is Stage 1.1 deterministic drawing. Do not run its stages concurrently in separate sessions. |
