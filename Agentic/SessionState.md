...
| Portfolio progress | 323 / 329 tasks = 98% rounded overall |
...
| Validation for current edits | M3 passed formatting, 679/679 project/filter parity, allocation-policy checks, zero-warning Profile, the unchanged one-generation 2,401-tick mega gate with 187 grounded sleepers, the full repository gate including byte-exact physics, and the no-engine scrub-alias propagation probe; no baseline changed. |
...
2. `replay-monolith-decomposition` is active at 4/9. M0 certified the exact
   ...
   diagnostic owner while leaving mixed prediction/draw statics for the M6/M8
...
`Plans/TODO/replay-monolith-decomposition.md` M4. Extract timeline first, then
scrubber and its restore transaction boundary. The permanent one-presentation
200-box gate remains the unchanged per-task oracle.
