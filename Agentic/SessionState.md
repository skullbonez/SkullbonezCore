...
| Last source/data milestone | Object/object manifold APIs no longer expose `GameModel` overloads; required scene-contact gates now build exact manifolds from `PhysicsBodyStore` pose and `ColliderStore` shape snapshots instead of the post-step `GameModel` mirror. |
...
| Validation | 2026-07-05 object-contact store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings/errors (`TestOutput\agent_build_debug_object_contact_store.log`); touched-file comment audit passed for `ObjectContactManifold.cpp`, `ObjectContactManifold.h`, `RunScene.cpp`, and `tools/check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors (`TestOutput\agent_validate_fast_object_contact_store.log`); intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_object_contact_store.log`). |
