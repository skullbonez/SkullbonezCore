...
| Last committed milestone | Runtime run composition-root shrink replay event frame cursor wrapper slice is validated for commit: `NextReplayEventFrameIndex` is removed from `Run`, and replay event cursor reads now go directly through `ReplayRuntime::NextEventFrameIndex()`. |
...
| Validation | Replay event frame cursor wrapper validation: targeted Profile build (`TestOutput\validation\agent_logs\replay_event_frame_cursor_profile_build.log`) passed in 39.61s, `tools\validate_fast.bat` (`TestOutput\validation\agent_logs\replay_event_frame_cursor_validate_fast.log`) passed in 49.29s, direct `check_runtime_boundaries.py` (`TestOutput\validation\agent_logs\replay_event_frame_cursor_runtime_boundaries.log`) passed in 2.15s, and `tools\validate_full.bat` (`TestOutput\validation\agent_logs\replay_event_frame_cursor_validate_full.log`) passed in 27.47s. Evidence includes formatting clean, project filters clean, runtime-boundary 0 errors, Profile/Debug 0-warning builds, DX12 validation errors 0 with screenshots matching baselines, and byte-exact `physics_regression_solver.csv`. |
