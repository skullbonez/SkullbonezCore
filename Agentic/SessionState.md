...
| Last source/data milestone | Moved replay future marker and scrub query pick radii off replay-local `EditorModelRadius(GameModel)`/`GameModel::GetCollisionShape()` reads. Retained and predicted replay marker radii now resolve `ColliderStore` rows by model-index handle maps. |
| Pending work | Continue moving runtime/replay/editor identity paths toward durable handles, narrow remaining replay metadata borrowers, and then return to the normal runtime explicit bulk fixed-step compatibility writeback. |
...
| Validation | 2026-07-05 replay marker radius store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; focused Debug build passed with 0 warnings/errors; touched-file comment audit inspected `RunReplayTools.cpp`, `RunReplayPredictionHelpers.inl`, `RunReplayPredictionVisualizer.inl`, `RunReplayQueryTools.inl`, and `check_runtime_boundaries.py`; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` in 17.2s; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors in 22.7s. |
