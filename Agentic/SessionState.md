...
| Active objective | Owner redirected active work to `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md`. Stage 2 is complete: same-target prediction refreshes preserve committed reveal state until the rebuild prefix catches up, the replay scrubber has an explicit `pastPathVisible` toggle, retained past-path drawing requires both a selected target and the toggle, and prediction contact reserve failures surface as `contactsIncomplete` in UI/automation instead of silently implying a complete cause tree. Next actionable work is Stage 3.1: add TrajectoryStore records, versioning, and published prefixes under `replay_prediction_working_set`. The overall engine cleanup goal remains active and incomplete. |
...
| Current replay visuals validation | 2026-07-10 Stage 2 is complete. Stage 2.2/2.3 touched-source comment audit inspected 9 source-bearing files with 0 deferred. Focused `tools\validate_build.bat Profile` passed in 9.78s with 0 warnings/errors (`Agentic/Reports/validate_build_profile_replay_stage2_2_3_20260710.log`). Required Stage 2 gate passed: `tools\validate_full.bat` in 40.05s (`Agentic/Reports/validate_full_replay_visuals_stage2_2_3_20260710.log`) with project filters clean, Profile/Debug builds clean, DX12 validation errors 0, DX12 screenshots matching baselines, and `physics_regression_solver.csv` byte-exact; `tools\validate_replay_scrub.bat` in 10.89s (`Agentic/Reports/validate_replay_scrub_replay_visuals_stage2_2_3_20260710.log`) with scrub/restore SkullScope probes passed, trace bytes 54,932 + 54,912, SQLite bytes 225,280 + 225,280, and model-read query output bytes 1,512 + 967. |
...
| Replay visuals/prediction/memory/size | Active plan (~42%) | `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md` - Stage 0 instrumentation/repro, Stage 1 deterministic drawing, and Stage 2 rebuild/reveal churn plus visibility/contact-completeness controls are complete. Next is Stage 3.1 TrajectoryStore records/versioning/published prefixes. Do not run its stages concurrently in separate sessions. |
