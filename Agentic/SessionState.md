...
| Active objective | Owner redirected active work to `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md`. Stage 7 is complete: replay trajectory age/depth palettes, shader feather tuning, depth-hint underlay, and entry-box emphasis are green. Next actionable work is Stage 8: replay memory data-model tuning. The overall engine cleanup goal remains active and incomplete. |
...
| Current replay visuals validation | 2026-07-10 Stage 7 is complete. Touched-source comment audit inspected 8 source-bearing files with 0 deferred. Focused checks passed: `tools\validate_format.bat` in 9.96s after the comment audit, `tools\validate_build.bat Profile` in 15.81s with 0 warnings/errors, `python .\tools\validate_shaders.py` in 0.15s (0 errors, 11 pre-existing warnings), and `git diff --check`. Final gates passed: `tools\validate_replay_scrub.bat` in 50.55s; `tools\validate_dx12_renderer.bat` x3 in 23.61s, 22.62s, 22.74s with DX12 validation errors 0 and screenshots matching baselines. No baseline files were updated. SkullScope proof accounting: scrub trace 54,932 bytes + SQLite 225,280 bytes + query output 1,512 bytes; restore trace 54,912 bytes + SQLite 225,280 bytes + query output 967 bytes; total GPT-read query output 2,479 bytes. Prediction reports were 4,614 bytes each; bounded stdout excerpts were 60,294 bytes each; fingerprint `0x0165312C5422A5F1` matched across two runs (402 records, 73,021 points, 361 active frames). Logs live under `Agentic/Reports/*stage7*20260710.log`. |
...
| Replay visuals/prediction/memory/size | Active plan (~82%) | `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md` - Stage 0 instrumentation/repro, Stage 1 deterministic drawing, Stage 2 rebuild/reveal churn plus visibility/contact-completeness controls, Stage 3.1 TrajectoryStore records/versioning/published prefixes, Stage 3.2 build writers fed by solver-ring appends plus prediction publish events, Stage 3.3 store-only draw reads plus per-frame `targetVisualizer` rebuild/`futureNodes` copy deletion, Stage 3.4 default-off legacy draw fallback, Stage 4 lock-step hierarchy correctness, Stage 5 prediction worker job, Stage 6 DX12 trajectory-ribbon renderer, and Stage 7 visual polish are complete. Next is Stage 8 memory data-model tuning. Do not run its stages concurrently in separate sessions. |
