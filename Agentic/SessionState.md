...
| Last committed milestone | Runtime run decomposition is committed through Phase 4B on `nightrunner-24th-june-refactor`: generated/authored scene setup lives in scene helpers, public physics handle/API scaffolding is in place, and `PhysicsEngine` now owns the existing `PhysicsScene` behind a compatibility facade. |
