...
| Active objective | Continue the 2026-07-07 takeover cleanup after fable-02 phase 4 L2 Window demotion; all five authoritative CSVs were drained earlier, and follow-up blocker-plan work remains. |
| Last documentation milestone | Fable-02 phase 4 L2 Window demotion recorded: WorkerPool and Window are startup-owned, the singleton census is down to 8 source files, and the global-service checker budget is 143. |
| Last source/data milestone | Fable-02 phase 4 L2 Window demotion deleted `Window::Instance()`, `Window::Destroy()`, and `Window::pInstance`; `Runtime\Init.cpp` now owns both local `WorkerPool` and `Window` startup objects. Current authoritative blocker-ledger totals remain 131 done and 29 blocked. |
...
| Validation | Latest source gates passed on 2026-07-07 for fable-02 Window demotion: `python tools\check_runtime_boundaries.py --self-test`, `python tools\check_runtime_boundaries.py` (0 errors), `tools\validate_fast.bat` (48.314s, 0 warnings/errors), and `tools\validate_full.bat` (42.849s, DX12 validation errors 0, screenshots matched, byte-exact `physics_regression_solver.csv`). |
