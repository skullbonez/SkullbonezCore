...
| Last source/data milestone | Added constraint-only standalone island evidence: the smoke now proves two colliderless bodies merge through a live point joint and split after `DestroyConstraint()`, while `PhysicsStandaloneWorld::Islands()` remains store-owned and deterministic. |
| Pending work | Physics standalone still needs legacy scene/ragdoll `PointJointConstraint` caller migration, later shape-pair contact coverage if required, full support propagation, remaining update-desc/state cleanup, compatibility assertion/guardrail shrink decisions, and runtime adapter migration. Contact-audio one-shot impact work remains complete; separate roll/slide loops are future work. |
...
| Validation | 2026-07-04 constraint-island slice: focused Debug build passed with 0 warnings / 0 errors in 3.9s (`TestOutput\agent_build_debug_constraint_islands.log`); standalone smoke passed in 1.7s with `lifecycle_checks=pass`, `contacts=2`, `contact_hash=0x5DBDF5257E90EA9B`, runtime mirror pass, and deterministic hash `0xB3B6CEEFB6CCAFFA` (`TestOutput\agent_physics_standalone_smoke_constraint_islands_report.txt`); `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors in 7.0s (`TestOutput\agent_runtime_boundaries_constraint_islands.log`); `tools\validate_physics.bat` passed in 16.1s with Debug/Profile builds at 0 warnings / 0 errors and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_constraint_islands.log`). |
