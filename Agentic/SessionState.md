...
| Last source/data milestone | Scene-object grouping now lives in a dense `GameModelCollection` sidecar; `GameModel` no longer stores runtime collection kind/root/part fields or exposes `SetRuntimeCollection`/`GetRuntimeCollection*`. |
| Pending work | Continue scene/entity metadata cleanup so names/asset identity leave `GameModelCollection`, clear remaining model-index fallbacks that are not pure UI/presentation hints, and run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 scene-object grouping sidecar passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; `tools\validate_format.bat`; focused Profile build at 0 warnings/errors; touched-source comment audit of 19 files; and `tools\validate_full.bat` with project filters/runtime boundaries, Profile/Debug builds at 0 warnings/errors, DX12 InfoQueue 0 errors, screenshots matching baselines, and byte-exact `physics_regression_solver.csv`. |
