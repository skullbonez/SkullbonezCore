...
| Last source/data milestone | Moved replay scrub/save/load validation probes and restore-target diagnostics off `GameModel` body mirrors. They now resolve live `PhysicsBodyStore` rows by model-index handle maps before proving replay presentation overrides leave simulation state untouched. |
| Pending work | Continue moving runtime/replay/editor identity paths toward durable handles, then remove the final normal runtime explicit bulk fixed-step compatibility writeback once the remaining mirror borrowers are gone. |
...
| Validation | 2026-07-05 replay probe body-store authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; focused Debug build passed with 0 warnings/errors in 5.1s; touched-file comment audit inspected `RunFrame.cpp` and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors in 27.0s; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` in about 13.9s by log timestamps. |
