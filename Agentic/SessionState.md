...
| Active objective | Complete handoff for the GameModel compatibility endgame/checker consolidation slice and the strict runtime static-allocation enforcement slice. |
...
| Last source/data milestone | `GameModel` is now presentation metadata only; physics creation/repair/edit paths use `PhysicsBodyCreateDesc`, `PhysicsColliderCreateDesc`, stores, and descriptor sidecars; deleted `PhysicsModelAccess.h`, `RigidBody.*`, `LoadFromModels`, `RefreshBodyFromModel`, and `MakeBodyRecordFromAuthoredModel`; moved authored contact-material tokens into collider descriptors/records; added strict gameplay allocation-guard failure, `RuntimeReserveAllocator` owner accounting, WorkerPool/physics/render/replay preallocation, and clean ordinary/replay-capture guard proof. The temporary physics CSV drift was fixed in code by restoring sphere descriptor metric parity; no intentional baseline update remains. |
| Pending work | Plan 1 is complete and moved to `Agentic/Plans/Done/game-model-compat-endgame-and-fence-consolidation-plan.md`. Plan 2 strict ordinary/replay-capture enforcement is landed, but the full allocation-policy plan intentionally remains open: comparable Phase 0 perf evidence, backing arenas/budgets, full runtime owner conversion, policy comments everywhere, registered replay reserve-bump positive proof, and complete pool registration/diagnostics are still tracked in `Agentic/Plans/In_Progress/runtime-static-allocation-policy-plan.md`. |
...
| Validation | 2026-07-06 `nightrunner-5th-july`: `python -m py_compile tools\check_runtime_boundaries.py tools\check_allocation_policy.py tools\validate_project_filters.py`, runtime-boundary self-test, and allocation-policy self-test passed. Final strict-slice gates passed: `tools\validate_fast.bat` (26.39s), `tools\validate_perf.bat` (39.74s, clean `perf_1000` guard: `gameplay_violations=0`, `policy_violations=0`), and `tools\validate_full.bat` (36.95s, DX12 InfoQueue 0, screenshots matched baselines, physics CSV byte-exact). Final replay probes: no-interaction capture exited 0 with `gameplay_violations=0`, and the expected-fail interaction/report harness produced `gameplay_violations=3166`, `policy_violations=3166`, `VIOLATION`, `FAIL`, and `interaction_exit=9`. |
...
| Runtime static allocation policy | Strict enforcement slice landed; full plan still open | `Agentic/Plans/In_Progress/runtime-static-allocation-policy-plan.md`; ordinary `perf_1000` and no-interaction replay capture are clean under `--allocation-guard gameplay`, the broader replay interaction/report harness now fails strict mode as expected, and remaining registered-growth/pool/diagnostics work is tracked in the plan. |
