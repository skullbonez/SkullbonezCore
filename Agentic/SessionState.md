...
| Last committed milestone | Runtime run composition-root shrink scene generated-control ownership slice is validated for commit: `ApplyUIModelCountOverride` and `ApplyUISolverObjectCounts` are removed from `Run`; `SceneRuntimeGeneratedControls` owns live generated-object rebuilds and returns replay/profiler follow-up actions; and the `Run.h` private-method ratchet is 142. |
...
| Pending work | Continue run-shrink work with remaining scene load teardown/generated/authored setup phases, replay timeline reset ownership, remaining replay tool/helper ownership, render-host splitting, and shared cine/path helper cleanup. Per current user instruction, defer rubber-duck review until the end and move plans to `Done/` only after that final review is satisfied. |
| Blockers | None known. |
...
| Validation | Scene generated-control ownership validation: direct `validate_project_filters.py` (`TestOutput\validation\agent_logs\generated_controls_project_filters.log`) passed with 0 errors in 1.1s; direct `check_runtime_boundaries.py` (`TestOutput\validation\agent_logs\generated_controls_runtime_boundaries.log`) passed with 0 errors in 3.5s; targeted Profile build (`TestOutput\validation\agent_logs\generated_controls_profile_build.log`) passed in 40.6s with 0 warnings and 0 errors; `tools\validate_fast.bat` (`TestOutput\validation\agent_logs\generated_controls_validate_fast.log`) passed in 51.3s; and `tools\validate_full.bat` (`TestOutput\validation\agent_logs\generated_controls_validate_full.log`) passed in 28.4s. Evidence includes formatting clean, project filters clean, runtime-boundary 0 errors, Profile/Debug 0-warning builds, DX12 validation errors 0 with screenshots matching baselines, and byte-exact `physics_regression_solver.csv`. |
