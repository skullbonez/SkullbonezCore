...
| Validation | 2026-07-04 store wake cache-edge slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_format.bat` passed; focused Debug build passed with 0 warnings/errors after removing a dead `modelAccess` parameter; `tools\validate_physics.bat` passed in 23.8s with byte-exact `physics_regression_solver.csv`; `tools\validate_fast.bat` passed in 22.1s; `tools\validate_perf.bat` first failed PHYSICS_BENCH frame and memory thresholds, then rerun passed in 21.5s with no DX12 or PHYSICS_BENCH regressions. |
