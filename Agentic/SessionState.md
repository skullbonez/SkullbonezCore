...
| Last source/data milestone | Phase 10 `PHY-1027` is complete in this branch: authored/generated scene construction no longer resolves just-created bodies through `GameModelCollectionPhysicsAdapter`; `GameModelCollection::AddGameModel` appends one matching `PhysicsBodyStore` record and returns the `PhysicsBodyHandle` used for initial impulses and sleep seeding. Guardrails block scene setup adapter body lookups from returning. |
...
| Validation | 2026-07-05 `PHY-1027`: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings/errors; touched-file comment audit passed; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors; `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`; source residue scan found no scene setup `GameModelCollectionPhysicsAdapter` or `BodyHandleForModelIndex` use. |
