...
| Last source milestone | K004 store-force slice is implemented, K003 pose integration now advances `PhysicsBodyRecord` state inside `PhysicsBodyStore`, `GameModel::UpdatePosition` is deleted, tornado field no longer borrows `modelAccess.Models()`, PersistentContactSolver no longer receives `PhysicsModelAccess` or opens raw model ranges, object/object CCD no longer uses `GameModel::SweepGameModel`, RunSolverPhysics compatibility writebacks now go through named `PhysicsModelAccess` commands, and K005 allocator-owned body/collider handles are implemented through `PhysicsBodyStore` and `ColliderStore`. |
