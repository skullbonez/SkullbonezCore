...
| Last source/data milestone | Launcher ray-hit broad picking now scans `PhysicsBodyStore` positions and `ColliderStore` bounding radii instead of `GameModel` position/shape mirrors. `FireLauncherLaser()` and `FireLauncherProjectile()` repair count drift once at the tool edge, then call store-owned `RuntimeTools::TryRayCastTestHit()`. |
...
| Validation | 2026-07-05 launcher store-raycast slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; touched-file comment audit passed for 3 source-bearing files; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors (`TestOutput\agent_validate_fast_launcher_store_raycast.log`); intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_launcher_store_raycast.log`). |
