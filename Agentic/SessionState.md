...
| Last source/data milestone | Runtime scene setup pending impulses and sleep seeds now resolve `PhysicsBodyHandle` at construction and call handle-keyed `PhysicsEngine` commands. A runtime-boundary guardrail blocks those model-index physics wrappers from returning in authored/generated scene setup. |
| Pending work | Phase 7 still needs editor, mouse pickup, launcher, replay, ragdoll, and diagnostics caller groups migrated from model-index physics commands, then proven-dead `GameModelCollection` wrappers can be deleted. Later strict authority work still needs runtime stepping and final writeback pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 scene-setup handle-command slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings / 0 errors (`TestOutput\agent_build_debug_scene_setup_handles.log`); `tools\validate_format.bat` passed (`TestOutput\agent_validate_format_scene_setup_handles.log`); `tools\validate_fast.bat` passed (`TestOutput\agent_validate_fast_scene_setup_handles.log`); `tools\validate_full.bat` passed with DX12 InfoQueue 0 errors, DX12 screenshots matching baselines, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_full_scene_setup_handles.log`). |
