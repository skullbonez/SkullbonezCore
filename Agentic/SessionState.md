...
| Last source/data milestone | Moved attached-camera target identity to `PhysicsBodyHandle`/`PhysicsColliderHandle`; model index is now a UI/recovery hint and follow/orbit/ragdoll-eye sampling resolves live pose/velocity/radius from physics stores. |
| Pending work | Continue durable editor/replay identity cleanup and run a final completion audit for remaining `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 attached-camera handle identity slice passed: residue scan found no old `TryResolveAttachedCameraPhysicsTarget(..., modelIndex, ...)` source calls; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; `git diff --check` passed; focused Debug build passed with 0 warnings/errors in 4.9s; touched-file comment audit inspected `RunInput.cpp`, `RunState.h`, `Run.h`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed in 36.1s with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed in about 13.8s with standalone/runtime handle smoke and byte-exact `physics_regression_solver.csv`. |
...
| Physics/GameModel authority | Active follow-up | `Agentic/Plans/To_Eval/physics-game-model-authority-plan.md`; persistent solver, launcher fixed-tree per-release writeback, replay prediction writeback, replay render-pose override boundaries, final normal runtime bulk body mirror, and attached-camera live target identity are narrowed/deleted or handle-owned; durable editor/replay identity cleanup and final `GameModel` body-reader audit remain future work. |
