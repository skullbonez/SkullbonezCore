...
| Active objective | Owner redirected active work to `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md`. Stage 3.1 is complete: replay now has a TrajectoryStore publication shell with lane-keyed records, versioning, published prefixes, reserve helpers under `replay_prediction_working_set`, memory accounting, and non-replay perf launches no longer reserve the large replay-only cause-row/ghost-request buffers. Next actionable work is Stage 3.2: build pass fed by prediction publish events and solver-ring appends, with incremental cursors preserved. The overall engine cleanup goal remains active and incomplete. |
...
| Current replay visuals validation | 2026-07-10 Stage 3.1 is complete. Touched-source comment audit inspected 8 source-bearing files with 0 deferred. Focused checks passed: allocation policy self-test, allocation scan (`allowlist_errors=0`), `tools\validate_project_filters.bat` in 1.28s, and `tools\validate_build.bat Profile` in 5.44s with 0 warnings/errors. Required Stage 3.1 gates passed: `tools\validate_fast.bat` in 19.38s (`Agentic/Reports/validate_fast_replay_stage3_1_20260710.log`); `tools\validate_full.bat` in 31.89s (`Agentic/Reports/validate_full_replay_visuals_stage3_1_20260710.log`) with DX12 validation errors 0, screenshots matching baselines, and `physics_regression_solver.csv` byte-exact; `tools\validate_replay_scrub.bat` in 11.09s (`Agentic/Reports/validate_replay_scrub_replay_visuals_stage3_1_20260710.log`) with scrub/restore probes passed, trace bytes 54,932 + 54,912, SQLite bytes 225,280 + 225,280, and model-read query output bytes 1,512 + 967; `tools\validate_perf.bat` in 30.79s (`Agentic/Reports/validate_perf_replay_stage3_1_20260710.log`) with allocation guard PASS and 0 gameplay violations after refreshing stale DX12/physics-bench perf baselines from current artifacts. |
...
| Replay visuals/prediction/memory/size | Active plan (~46%) | `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md` - Stage 0 instrumentation/repro, Stage 1 deterministic drawing, Stage 2 rebuild/reveal churn plus visibility/contact-completeness controls, and Stage 3.1 TrajectoryStore records/versioning/published prefixes are complete. Next is Stage 3.2 build pass fed by prediction publish events and solver-ring appends. Do not run its stages concurrently in separate sessions. |
