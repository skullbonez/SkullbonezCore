...
| Validation for current edits | M3-M7 remain reopened by M8 review. The fourth remediation checkpoint removed velocity-authoring mutation from `ReplayInteractionController`; its final source passed a zero-warning Profile build and the exact one-process 200-box oracle (2,401 ticks, 200 moved, 187 strict topples, 199 causal nodes, one generation, one presentation, all controls). No baseline changed. |
