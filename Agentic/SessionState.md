...
| Current objective | Architecture Follow-Up Campaign Round 5. `extraction-scar-remediation` closed ES0-ES2, so `scene-sized-store-capacity` SC4 is binding. All three blocking owner decisions are ruled; no plan is waiting on the owner. |
| Active/future progress | 4/50 (8%). Round 5 is the live queue; completed governance and extraction-scar plans are excluded under rule 4. |
...
NOW. Architecture Follow-Up Campaign Round 5 is live at 4/50, registered
...
rule 4. Plan 2 SC4 is now binding. Evidence:
...
Continue `scene-sized-store-capacity` at SC4. Extraction-scar remediation is
closed, so the solver and sleep files are clear for scene-sized store capacity
binding work.
