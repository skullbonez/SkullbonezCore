...
| Last source/data milestone | Fixed-tree release metadata import no longer reads `GameModel` runtime collection fields inside `PhysicsBodyStore`; `GameModelCollection` passes an explicit cold `fixedTreeReleaseRootIndex` scalar at append/topology repair. |
| Pending work | Continue scene/entity metadata cleanup so grouping/names/asset identity leave `GameModelCollection`, clear remaining model-index fallbacks that are not pure UI/presentation hints, and run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 fixed-tree metadata import passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; touched-source comment audit; `tools\validate_fast.bat` with format/project filters/runtime boundaries plus Profile/Debug builds at 0 warnings/errors; and `tools\validate_physics.bat` with standalone/runtime handle smoke plus byte-exact `physics_regression_solver.csv`. |
