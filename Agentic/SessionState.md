...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, and launcher laser/projectile wake commands now resolve `PhysicsBodyHandle` at their command boundaries and call handle-keyed `PhysicsEngine` commands. Runtime-boundary guardrails block those model-index physics wrappers from returning in the migrated files. |
| Pending work | Phase 7 still needs replay, ragdoll, and diagnostics caller groups migrated from model-index physics commands, then proven-dead `GameModelCollection` wrappers can be deleted. Later strict authority work still needs runtime stepping and final writeback pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 launcher handle-command slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings / 0 errors (`TestOutput\agent_build_debug_launcher_handles.log`); `tools\validate_format.bat` passed (`TestOutput\agent_validate_format_launcher_handles.log`); `tools\validate_fast.bat` passed (`TestOutput\agent_validate_fast_launcher_handles.log`); `tools\validate_full.bat` passed with DX12 InfoQueue 0 errors, screenshots matched committed baselines, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_full_launcher_handles.log`). |
