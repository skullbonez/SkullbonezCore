...
| Validation for current edits | M3-M7 remain reopened by M8 review. The fifteenth remediation checkpoint deleted mutable `ReplayPrediction::State()` access: scheduling, preparation, archives, and trajectory mutation now use owner commands, while bounded value effects replace runtime reach-back. The exact one-process oracle passed in 469.7 seconds (2,401 ticks, 200 moved, 187 strict topples, 199 causal nodes, one generation, one presentation, durable reconstruction, zero reserve growth, all controls), and the 111.7-second full CPU/Profile/Debug/DX12/physics gate passed with zero warnings/DX12 errors and byte-exact physics. No baseline changed. |
...
`Plans/TODO/replay-monolith-decomposition.md` reopened M3-M7. Presentation
drawing is now read-only and prediction state is private; next physically
separate prediction scheduling from presentation drawing, then remove mutable
presentation/root accessors. Remove the replay root from external frame views
and make probes consume published views/owner commands. Reclose each task only
after its unchanged one-presentation 200-box oracle, then repeat M8's
