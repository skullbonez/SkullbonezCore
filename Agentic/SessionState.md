...
| Last source/data milestone | Deleted the final `PhysicsBodyStore` model-index command overloads: `WakeBody`, `SeedBodyAsleep`, `SetPendingBodyImpulse`, and `ApplyBodyImpulse` are handle-only; runtime handle smoke seeds reorder state through the saved handle; `PhysicsWorld` dense-row wake propagation mutates the selected `PhysicsBodyRecord` directly. |
...
| Validation | 2026-07-05 body-store command deletion slice passed: residue scan found no deleted int command overloads in `PhysicsBodyStore.h/.cpp`; `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings/errors in 9.1s; touched-file comment audit inspected `Init.cpp`, `PhysicsBodyStore.h/.cpp`, `PhysicsWorld.cpp`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed in 30.9s with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed in 13.4s with standalone/runtime handle smoke and byte-exact `physics_regression_solver.csv`. |
...
| Physics/GameModel authority | Active follow-up | `Agentic/Plans/To_Eval/physics-game-model-authority-plan.md`; persistent solver, launcher fixed-tree per-release writeback, replay prediction writeback, replay render-pose override boundaries, final normal runtime bulk body mirror, attached-camera live target identity, replay restore physics API authority, and body-store model-index command overloads are narrowed/deleted or handle-owned; durable editor/replay identity cleanup and final `GameModel` body-reader audit remain future work. |
