...
| Last source/data milestone | Phase 10 `PHY-1022` is complete in this branch: manipulator mouse pickup now stores the picked `PhysicsBodyHandle`, reads pickup simulation state from `PhysicsBodyStore`, restores angular velocity and applies drag impulses through `PhysicsEngine` handle commands, and no longer uses the mouse-pickup model-index impulse helper. Guardrails block `GameModel` body reads and model-index pickup physics commands from returning. |
...
| Validation | 2026-07-05 `PHY-1022`: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; touched-file comment audit passed; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`. |
