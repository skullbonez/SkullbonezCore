...
| Field | Value |
|---|---|
| Branch | `nightrunner-29th-JUL-26` (clean before campaign bootstrap documentation). |
| Current baseline | Main tip `90e4d52f`; PR #137 merged the 28 July takeover branch. |
| Current objective | Execute the Fresh-Read Engine Review Campaign plans 1→8 in order. Plan 1 BG0 is complete; BG1 is next. The contact-identity regression is preserved at `origin/codex/contact-identity-regression-29th-jul-26` commit `27906417`; stale source/baseline hunks were not imported and the original stash was dropped. |
| Active/future progress | 1/35 (3%). Broadphase canonical-order guard BG0 binds both radix passes to `MAX_SCENE_OBJECTS`; BG1 remains. |
| Validation for broadphase guard BG0 | Profile instructions are unchanged apart from `.debug$S` metadata. A temporary 8193-body ceiling fails the named layout assertion. Focused SpatialGrid passes 15/15 cases and 8,507/8,507 assertions; focused determinism passes 3/3 cases and 30,897/30,897 assertions. `tools\validate_physics.bat` passes in 71.80 seconds with the lifecycle smoke and all 44,401 Physics rows byte-exact (`output runs=2`, `baseline runs=1`). No baseline was refreshed. |
