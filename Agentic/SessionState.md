...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, replay restore/prediction, replay/editor transform restore wake, ragdoll start-asleep commands, fixed-tree release wake, and tornado fixed-tree release now avoid `GameModelCollection` model-index physics wrappers at their migrated boundaries. The obsolete `GameModelCollection::WakeModel`, `SeedModelAsleep`, `ApplyBodyImpulse`, and `SetPendingBodyImpulse` wrappers were deleted. Contact-highlight ticking, fixed-contact highlight notification, persistent-contact fixed-tree release, tornado release writeback/reload, post-step model stream invalidation, Debug diagnostics emission, and bulk compatibility writeback moved out of `PhysicsWorld::RunPhysics`/side-effect hot paths toward `PhysicsBodyStore` plus the `PhysicsScene` compatibility edge. Runtime-boundary guardrails block those regressions from returning. |
| Pending work | Commit/push the tornado store-release slice, then continue reducing runtime stepping dependence on `PhysicsModelAccess` and `GameModelCollection`. Remaining strict authority work includes wake/body-stream compatibility paths and final diagnostics/SkullScope view cleanup. |
...
| Validation | 2026-07-04 tornado store-release slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_format.bat` passed; focused Debug build passed in 8.7s with 0 warnings/errors; `tools\validate_physics.bat` passed in 20.9s with byte-exact `physics_regression_solver.csv`; `tools\validate_fast.bat` passed in 22.0s; `tools\validate_perf.bat` first failed PHYSICS_BENCH memory by +0.34 MB over threshold, then rerun passed in 21.3s with no DX12 or PHYSICS_BENCH regressions. |
