...
| Last source/data milestone | Moved replay path and replay camera body-focus target marker drawing off `GameModel` pose/shape mirrors. Replay target markers still use `GameModel` only for cold replay-id identity lookup, then draw from `PhysicsBodyStore`/`ColliderStore` records. |
...
| Validation | 2026-07-05 replay target marker store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; focused Debug build passed with 0 warnings/errors; touched-file comment audit inspected `RunReplayTools.cpp`, `RunEditorTracer.inl`, `RuntimeTools.h`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`. |
