...
| Last source/data milestone | Editor selection frame, overlay, and gizmo selected-member reads now use the stored `PhysicsBodyHandle`/`PhysicsColliderHandle`; group siblings still use model-order grouping metadata until scene/entity grouping leaves `GameModel`. |
...
| Validation | 2026-07-05 editor selected-handle frame slice passed: `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed at 0 warnings/errors; `tools\validate_fast.bat` passed with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed with standalone/runtime handle smoke and byte-exact 20,001-line `physics_regression_solver.csv`. |
