...
| Last source/data milestone | Replay cause-tree rows and replay focus masks now resolve `ReplayBodyId` through `PhysicsBodyStore::HandleForReplayBodyId()` instead of scanning `GameModel` replay ids; `BuildCauseTreeRows()` and `BuildFocusModelMask()` take body-store authority explicitly, with solver samples used only as historical cause-tree fallback. |
| Pending work | Continue replay query/prediction metadata cleanup, durable editor identity cleanup, and run a final completion audit for remaining `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 replay cause/focus identity slice passed: residue scan found no live cause/focus GameModel replay-id lookup outside checker self-tests; `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --max-errors 20` passed with 0 errors; focused Debug build passed with 0 warnings/errors in 4.5s; touched-file comment audit inspected `ReplayRuntime.h/.cpp`, `ReplayOverlayRenderer.h/.cpp`, `RunReplayCauseTreeTools.inl`, `RuntimeRenderHost.h/.cpp`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed in 29.9s with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed in 13.5s with standalone/runtime handle smoke and byte-exact 20,001-line `physics_regression_solver.csv`. |
