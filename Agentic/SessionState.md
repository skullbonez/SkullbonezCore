...
| Validation | Replay sample comparison helper validation: targeted Profile build (`TestOutput\validation\agent_logs\replay_sample_compare_profile_build.log`) passed in 39.63s, `tools\validate_fast.bat` (`TestOutput\validation\agent_logs\replay_sample_compare_validate_fast.log`) passed in 47.99s, direct `check_runtime_boundaries.py` (`TestOutput\validation\agent_logs\replay_sample_compare_runtime_boundaries.log`) passed in 1.83s, and `tools\validate_full.bat` (`TestOutput\validation\agent_logs\replay_sample_compare_validate_full.log`) passed in 26.12s. Evidence includes formatting clean, project filters clean, runtime-boundary 0 errors, Profile/Debug 0-warning builds, DX12 validation errors 0 with screenshots matching baselines, and byte-exact `physics_regression_solver.csv`. |
