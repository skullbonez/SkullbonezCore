...
| Last source/data milestone | Replay velocity-edit target identity now resolves through `PhysicsBodyStore::HandleForReplayBodyId()` and `ReplayRuntime::ResolveVelocityEditBodyHandle()`; `RunReplayVelocityEdit.inl` no longer scans `GameModel` replay ids through `collection.Models()`, and drag mutation carries the resolved `PhysicsBodyHandle` directly. |
| Pending work | Continue remaining direct model-index restore/query hints, durable editor identity cleanup, and run a final completion audit for remaining `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 replay velocity target identity slice passed: residue scan found no deleted velocity target lookup in the velocity-edit path; `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --max-errors 20` passed with 0 errors; focused Debug build passed with 0 warnings/errors in 8.6s; touched-file comment audit inspected `PhysicsBodyStore.h/.cpp`, `ReplayRuntime.h/.cpp`, `RunReplayVelocityEdit.inl`, and `check_runtime_boundaries.py`; `tools\validate_fast.bat` passed with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed in 13.8s with standalone/runtime handle smoke and byte-exact 20,001-line `physics_regression_solver.csv`. |
