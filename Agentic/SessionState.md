...
| Active/future progress | 18 / 47 live tasks; 38%. |
...
NOW. Eight live plans, 47 tasks; 18 complete (38%). The remaining architecture-review campaign
...
`physics-facade-unification` F0 is complete. F1 absorb/delete is next with no
blocker; the inventory proves 49 single-target relays, 13 read projections,
three private helpers, and zero direct external `PhysicsScene` type users.
...
Continue `physics-facade-unification` F1 on `nightrunner-20th-july`.
Absorb the inventoried `PhysicsScene` implementation/state into PhysicsEngine,
delete both Scene files/project rows, and preserve the exact relay order and
read projections. The Profiler semantic
