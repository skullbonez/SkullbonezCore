...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, and mouse-pickup impulse commands now resolve `PhysicsBodyHandle` at their command boundaries and call handle-keyed `PhysicsEngine` commands. Runtime-boundary guardrails block those model-index physics wrappers from returning in the migrated files. |
| Pending work | Phase 7 still needs launcher, replay, ragdoll, and diagnostics caller groups migrated from model-index physics commands, then proven-dead `GameModelCollection` wrappers can be deleted. Later strict authority work still needs runtime stepping and final writeback pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 mouse-pickup handle-command slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings / 0 errors (`TestOutput\agent_build_debug_mouse_pickup_handles.log`); `tools\validate_format.bat` passed (`TestOutput\agent_validate_format_mouse_pickup_handles.log`); `tools\validate_fast.bat` passed (`TestOutput\agent_validate_fast_mouse_pickup_handles.log`); `tools\validate_physics.bat` passed with standalone/runtime handle smoke and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_mouse_pickup_handles.log`). |
