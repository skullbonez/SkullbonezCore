...
| Active objective | Owner redirected active work to `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md`. Stage 2.1 rebuild/reveal churn is complete: same-target prediction refreshes keep the committed future/reveal anchor until the rebuilding prefix reaches the captured reveal cursor, while empty replacements and target/physics/enable changes still restart cleanly. Next actionable work is Stage 2.2: add an explicit `pastPathVisible` toggle so the past lane renders only when both a target and the toggle are active. The overall engine cleanup goal remains active and incomplete. |
...
| Current replay visuals validation | 2026-07-10 Stage 2.1 same-target reveal preservation is complete. Touched-source comment audit inspected 2 source-bearing files (`ReplayRuntime.h`, `RunReplayTools.cpp`) with 0 deferred. Focused `tools\validate_build.bat Profile` passed in 9.96s with 0 warnings/errors (`Agentic/Reports/validate_build_profile_replay_stage2_1_20260710.log`). Required Stage 2 gate passed: `tools\validate_full.bat` in 45.89s (`Agentic/Reports/validate_full_replay_visuals_stage2_1_20260710.log`) with project filters/formatting clean, Profile/Debug builds clean, DX12 validation errors 0, DX12 screenshots matching baselines, and `physics_regression_solver.csv` byte-exact; `tools\validate_replay_scrub.bat` in 10.78s (`Agentic/Reports/validate_replay_scrub_replay_visuals_stage2_1_20260710.log`) with scrub/restore SkullScope probes passed and query output bytes 1,512 + 967. |
...
| Replay visuals/prediction/memory/size | Active plan (~38%) | `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md` - Stage 0.1 trajectory counters/reporting, Stage 0.2 byte categories, Stage 0.3 H1-H5 manual repro documentation, Stage 1 deterministic drawing, and Stage 2.1 rebuild/reveal churn are complete. Next is Stage 2.2 `pastPathVisible`. Do not run its stages concurrently in separate sessions. |
