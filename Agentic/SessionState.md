...
| Last documentation milestone | Fable-01 S4 complete: plan progress/report now record PhysicsBodyStore/ColliderStore handle coverage, Terrain link-stub rationale, static-fixture correction, comment-audit, and validation evidence. |
| Last source/data milestone | Fable-01 S4 complete: `SKULLBONEZ_TESTS` now compiles `PhysicsBodyStore.cpp` and `ColliderStore.cpp` and runs focused store tests for fresh handles, model-index inverse lookup, replay-id hint fallback, dense-row deletion moves, stale generation rejection, collider body lookup, and collider scene-object lookup. Current authoritative blocker-ledger totals remain 131 done and 29 blocked. |
...
| Validation | Latest gate passed on 2026-07-07 for fable-01 S4: `tools\validate_tests.bat` (4.011s, 27 doctest cases and 174 assertions passed, 0 warnings/errors). Fable-01 M4 phase gate `tools\validate_fast.bat` also passed (33.651s, 0 warnings/errors). |
