...
| Last source/data milestone | `PhysicsModelAccess` no longer exposes `RefreshRenderInstances`; `GameModelCollection` owns the one cold render projection fill after physics prepares body/collider rows. |
...
| Validation | 2026-07-05 render projection facade shrink passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; touched-source comment audit; and `tools\validate_full.bat` with Profile/Debug builds at 0 warnings/errors, DX12 InfoQueue 0 errors, screenshots matching baselines, and byte-exact 20,001-line `physics_regression_solver.csv`. |
