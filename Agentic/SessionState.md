...
| Last source/data milestone | Scene/editor/runtime creation now allocates `PhysicsSceneObjectId` values from `RunSceneState`; `GameModelCollection` no longer owns a replay-id cursor and only forwards caller-owned ids into body/collider rows. |
...
| Validation | 2026-07-05 scene-owned creation identity slice passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; workqueue CSV parse; focused Profile build at 0 warnings/errors; `tools\validate_format.bat`; touched-source comment audit; and `tools\validate_full.bat` with Project Filters/runtime boundaries clean, Profile/Debug builds at 0 warnings/errors, DX12 InfoQueue 0 errors, screenshots matching baselines, and byte-exact 20,001-line `physics_regression_solver.csv`. |
