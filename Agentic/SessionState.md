...
| Last source/data milestone | Runtime config collider material now updates dense `ColliderStore` rows directly instead of recapturing full collider descriptors from `GameModel`. `GameModelCollection::ApplyRuntimeConfig()` keeps compatibility scalar policy on models for editor/presentation, while physics touches only friction and sphere drag in existing collider rows. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, replace same-count editor shape/topology `CaptureAuthoredColliderDesc()` recapture with explicit collider update commands, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 runtime config collider material slice passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; focused Profile build at 0 warnings/errors; touched-source comment audit; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing, `collider_refresh=pass`, and byte-exact 20,001-line `physics_regression_solver.csv`. |
