...
| Active objective | Owner redirected active work to `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md`. Stage 3.2 is complete: replay now has TrajectoryStore publication plus active build writers fed by solver-ring appends and prediction publish events, with cursor rebuilds for retained past paths and branch-separated prediction root/child records. Next actionable work is Stage 3.3: draw passes read the store only and delete the per-frame `targetVisualizer` rebuild plus `futureNodes` copy. The overall engine cleanup goal remains active and incomplete. |
...
| Current replay visuals validation | 2026-07-10 Stage 3.2 is complete. Touched-source comment audit inspected 3 source-bearing files with 0 deferred. Focused checks passed: `tools\validate_build.bat Profile` in 9.20s with 0 warnings/errors (`Agentic/Reports/validate_build_profile_replay_stage3_2_20260710.log`), `tools\validate_format.bat` (`Agentic/Reports/validate_format_replay_stage3_2_20260710.log`), allocation policy self-test, and allocation scan (`scanned=296 direct_heap_findings=28 dynamic_stl_member_findings=0 allowlist_errors=0`). Required gates passed: `tools\validate_full.bat` in ~47s (`Agentic/Reports/validate_full_replay_visuals_stage3_2_20260710.log`) with DX12 validation errors 0, screenshots matching baselines, and `physics_regression_solver.csv` byte-exact; `tools\validate_replay_scrub.bat` in ~11s (`Agentic/Reports/validate_replay_scrub_replay_visuals_stage3_2_20260710.log`) with scrub/restore probes passed, trace bytes 54,932 + 54,912, SQLite bytes 225,280 + 225,280, and model-read query output bytes 1,512 + 967; `tools\validate_perf.bat` in 28.47s (`Agentic/Reports/validate_perf_replay_stage3_2_20260710.log`) with allocation guard PASS, `gameplay_violations=0`, runtime reserve `policy_violations=0`, absolute DX12/PHYSICS_BENCH budgets clean, and no perf regressions. |
...
| Replay visuals/prediction/memory/size | Active plan (~50%) | `Agentic/Plans/TODO/replay-visuals-prediction-and-memory.md` - Stage 0 instrumentation/repro, Stage 1 deterministic drawing, Stage 2 rebuild/reveal churn plus visibility/contact-completeness controls, Stage 3.1 TrajectoryStore records/versioning/published prefixes, and Stage 3.2 build writers fed by solver-ring appends plus prediction publish events are complete. Next is Stage 3.3 store-only draw reads and deletion of the per-frame `targetVisualizer` rebuild/`futureNodes` copy. Do not run its stages concurrently in separate sessions. |
