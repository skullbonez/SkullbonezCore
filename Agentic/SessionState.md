...
| Last source/data milestone | `GameModelCollection::AddGameModel()` now appends the new body row and paired collider row directly. `PhysicsBodyStore` owns the body handle/replay id row, `ColliderStore` owns the body-to-collider mapping immediately, and the checker blocks body-only append regressions. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, move remaining collider shape/material authoring from `GameModel` into explicit collider descriptors, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body-state readers. |
...
| Validation | 2026-07-05 append-time collider registration slice passed: focused Profile build at 0 warnings/errors; `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing and byte-exact 20,001-line `physics_regression_solver.csv`. |
