...
| Last source/data milestone | Launcher fixed-tree release now reads release policy, source fixed state, hit position, and mass from `PhysicsBodyStore` records. `PhysicsScene::ReleaseFixedBodyAndAttachedTreeParts()` releases/wakes source and same-tree bodies through the store, and `GameModelCollection` mirrors only touched compatibility rows. |
...
| Validation | 2026-07-05 launcher fixed-tree store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_format.bat` passed; touched-file comment audit passed for 8 source-bearing files; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors (`TestOutput\agent_validate_fast_fixed_tree_store.log`); intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_fixed_tree_store.log`). |
