...
| Last source/data milestone | Attached-camera follow now samples target pose, orientation, linear velocity, and broad radius from `PhysicsBodyStore`/`ColliderStore` instead of the post-step `GameModel` body mirror. `GameModel` remains only cold attach identity/name/replay/ragdoll metadata for this camera path. |
...
| Validation | 2026-07-05 attached-camera store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; touched-file comment audit passed for `RunInput.cpp`, `Run.h`, and `tools/check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors (`TestOutput\agent_validate_fast_attached_camera_store.log`); intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_attached_camera_store.log`). |
