...
| Last source/data milestone | `ColliderStore::Refresh()` now derives collider replay identity and body links from the matching `PhysicsBodyStore` row instead of re-reading `GameModel::GetReplayBodyId()`; `GameModel` remains only the temporary authoring source for collider shape/material fields not yet registered directly. |
| Pending work | Continue scene/entity metadata cleanup, move body creation/import replay-id ownership out of `GameModel`, finish direct collider registration so refresh no longer needs a `GameModel` input, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body-state readers. |
