...
| Last source/data milestone | `PHY-1032` is complete in this branch: replay velocity edit no longer constructs `GameModelCollectionPhysicsAdapter`; it resolves the selected live body directly through `PhysicsBodyStore::HandleForModelIndex()` before calling the handle-keyed `PhysicsEngine::SetBodyVelocity()` command. Guardrails block adapter/model-index resolver names from returning to `RunReplayVelocityEdit.inl`. |
...
| Validation | 2026-07-05 `PHY-1032`: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; touched-file comment audit passed for `RunReplayTools.cpp`, `RunReplayVelocityEdit.inl`, and `tools/check_runtime_boundaries.py`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors (`TestOutput\agent_validate_fast_phy1032.log`); intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke (`store_handles=pass`, `render_mirror=pass`, `collider_refresh=pass`), and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_phy1032.log`). |
