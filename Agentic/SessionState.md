...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, replay restore/prediction, replay/editor transform restore wake, ragdoll start-asleep commands, fixed-tree release wake, tornado fixed-tree release, and store-owned sleep seeding now avoid `GameModelCollection` model-index physics wrappers or model-stream rebuilds at their migrated boundaries. The obsolete `GameModelCollection::WakeModel`, `SeedModelAsleep`, `ApplyBodyImpulse`, and `SetPendingBodyImpulse` wrappers were deleted. Contact-highlight ticking, fixed-contact highlight notification, persistent-contact fixed-tree release, tornado release writeback/reload, store sleep-seed invalidation, post-step model stream invalidation, Debug diagnostics emission, and bulk compatibility writeback moved out of `PhysicsWorld::RunPhysics`/side-effect hot paths toward `PhysicsBodyStore` plus the `PhysicsScene` compatibility edge. Runtime-boundary guardrails block those regressions from returning. |
| Pending work | Commit/push the store seed-asleep slice, then continue reducing runtime stepping dependence on `PhysicsModelAccess` and `GameModelCollection`. Remaining strict authority work includes wake/body-stream compatibility paths and final diagnostics/SkullScope view cleanup. |
...
| Validation | 2026-07-04 store seed-asleep slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors after tightening the guardrail scope; `tools\validate_format.bat` passed; focused Debug build passed in 7.8s with 0 warnings/errors; `tools\validate_physics.bat` passed in 20.7s with byte-exact `physics_regression_solver.csv`; `tools\validate_fast.bat` passed in 21.9s; `tools\validate_perf.bat` first failed DX12 memory by +0.02 MB over threshold, then rerun passed in 21.3s with no DX12 or PHYSICS_BENCH regressions. |
