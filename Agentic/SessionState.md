...
| Last source/data milestone | Authored scene releasable-tree grouping now imports into parsed `SceneObjectGroupMetadata`, editable scene snapshots write explicit `objectGroup` JSON, and `SceneAuthoredSetup` converts that metadata into `SceneObjectGroupCreateDesc` at append. `GameModelCollection` no longer recovers simple-ragdoll, editor-tree, or authored-tree groups from display names. |
| Pending work | The tracked workqueue is closed after `PHY-1076` plus final handoff. Continue only if the user wants the next audit pass: remaining non-presentation model-index fallbacks, name/asset identity cleanup outside grouping, and a final `GameModel` body/collider reader audit. |
...
| Validation | 2026-07-05 authored tree grouping metadata passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; workqueue CSV parse with 165 rows/13 columns; focused Profile build at 0 warnings/errors; touched-source comment audit; `tools\validate_fast.bat` passed; and `tools\validate_physics.bat` passed in about 14s with standalone/runtime smoke and byte-exact `physics_regression_solver.csv`. |
...
| Physics/GameModel authority | Active follow-up | `Agentic/Plans/To_Eval/physics-game-model-authority-plan.md`; persistent solver, launcher fixed-tree per-release writeback, replay prediction writeback, replay render-pose override boundaries, replay render apply/ghost identity, replay restore identity, runtime replay-id preflight, final normal runtime bulk body mirror, attached-camera live target identity, replay restore physics API authority, body-store model-index command overloads, replay velocity target identity, replay cause/focus identity, replay path/query/prediction target identity, replay probe body-state reads, authored scene orientation conversion, editor reset wake fixed-state, fixed-contact highlight fixed-state, replay prediction ghost render shape/material reads, convex-hull render shape reads, editor/replay shape-scale reads, editor selection frame math, editor selection body/collider identity, editor selected-member frame/overlay handle reads, editor ragdoll transform grouping, ColliderStore refresh replay/body identity, direct append-time collider registration, same-count collider row ownership, descriptor-owned collider import, scene primitive/hull descriptor ownership, append collider descriptor ownership, runtime config collider material ownership, deleted GameModel replay-id mirror, deleted collection replay-id sidecar, deleted collection replay-id allocator, deleted PhysicsModelAccess count query, deleted PhysicsModelAccess render refresh facade, fixed-tree release metadata import, deleted GameModel runtime collection metadata, explicit simple-ragdoll grouping, explicit editor-tree grouping, and authored tree grouping metadata are narrowed/deleted or store-owned; next work is remaining non-presentation model-index fallback audit, name/asset identity cleanup outside grouping, and final `GameModel` body/collider reader audit. |
