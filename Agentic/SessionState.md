...
| Last committed milestone | Runtime run decomposition is committed through Phase 3C on `nightrunner-24th-june-refactor`: generated setup lives in `SceneGeneratedSetup`, and authored camera/model/gate setup lives in `SceneAuthoredSetup`, with `Run` reduced to context-building wrappers and live gate updates. |
