...
| Last source/data milestone | Editor/replay transform-scale restore now reads base shapes and replay identity from `ColliderStore`/`PhysicsBodyStore` rows instead of `GameModel::GetCollisionShape()` or `GameModel::GetReplayBodyId()` in `RunFrame`. |
| Pending work | Continue scene/entity metadata cleanup, clear remaining replay identity/model-index presentation fallbacks, then run a final completion audit for `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 editor/replay transform-scale collider-authority slice passed: focused Profile build at 0 warnings/errors, `tools\validate_format.bat`, `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` with 0 errors; `tools\validate_full.bat` passed with DX12 InfoQueue errors 0, screenshots matching committed baselines, standalone/runtime physics smoke passing, and byte-exact 20,001-line `physics_regression_solver.csv` match. |
