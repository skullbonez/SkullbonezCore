...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, replay restore/prediction, and ragdoll start-asleep commands now avoid model-index/model-state physics authority at their migrated boundaries. Replay prediction body restore uses `PhysicsEngine::RestoreReplayBodyState`, render-pose overlays no longer recapture presentation-only poses into `PhysicsBodyStore`, and ragdoll construction seeds sleep through `PhysicsEngine::SeedBodyAsleep` with body handles. Runtime-boundary guardrails block those regressions from returning in the migrated files. |
| Pending work | Phase 7 still has one known collection-wrapper borrower before wrapper deletion: `RunFrame.cpp` fixed-contact release wake still calls `GameModelCollection::WakeModel`. Migrate that event path through handle-keyed physics first, then delete proven-dead `GameModelCollection` wrappers. Later strict authority work still needs runtime stepping and final writeback pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 ragdoll handle seed slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings / 0 errors (`TestOutput\agent_build_debug_ragdoll_handles.log`); `tools\validate_format.bat` passed (`TestOutput\agent_validate_format_ragdoll_handles.log`); `tools\validate_fast.bat` passed in 21.4s with runtime boundaries at 0 errors and Profile/Debug builds at 0 warnings / 0 errors (`TestOutput\agent_validate_fast_ragdoll_handles.log`); `tools\validate_physics.bat` passed with byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_ragdoll_handles.log`). |
