...
| Last source/data milestone | Convex-hull normal/shadow rendering now borrows prepared `ColliderStore` shape rows lazily from `GameModelRenderer` instead of reopening `GameModel` collision shapes or invoking topology repair during render submission. |
| Pending work | Move the remaining editor/replay transform-scale shape reads off `GameModel::GetCollisionShape()`, continue scene/entity metadata cleanup, then run a final completion audit for `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 convex-hull render collider-snapshot slice passed: focused Profile build at 0 warnings/errors, `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` with 0 errors; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; `tools\validate_dx12_renderer.bat` passed with DX12 InfoQueue errors 0 and screenshots matching committed baselines; `tools\validate_perf.bat` passed after the renderer stopped touching collider storage for sphere/box/pine-only passes. |
