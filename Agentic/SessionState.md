...
| Last source/data milestone | Replay restore and prediction-restore state now validate `ReplayBodyId` through `PhysicsBodyStore` records before projecting the restored values back to the temporary `GameModel` mirror. |
| Pending work | Continue scene/entity metadata cleanup, clear remaining replay identity/model-index fallbacks outside the now-store-owned render apply, ghost, probe, path, focus, velocity, and restore paths, then run a final completion audit for `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 replay restore identity store-authority slice passed: focused Profile build at 0 warnings/errors; `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; `tools\validate_fast.bat`; and intermittent `tools\validate_physics.bat` with standalone/runtime handle smoke passing and byte-exact 20,001-line `physics_regression_solver.csv`. |
