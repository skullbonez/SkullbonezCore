...
| Active objective | Complete all fable plans. Fable-04 L1-L4 and C1-C3 plus fable-05 P3 SpatialGrid/PhysicsWorld plus GameModelCollection classification/pure topology and legacy camera pose-read conversions are complete; fable-03, remaining fable-04, remaining fable-05, remaining fable-06, and fable-07 still have open work. |
| Last documentation milestone | `fable_plans/HANDOFF-2026-07-07.md` and `fable_plans/04-build-layering-and-repo-hygiene-progress.md` now document fable-04 C3 asset key/hash extraction and updated counts. |
| Last source/data milestone | Fable-04 C3 asset key/hash extraction is implemented: `Assets/AssetKeys.h` owns `HashStr`, built-in texture hash constants, and camera hash constants; `Common.h` includes it during the aliasing period, and `tools/validate_project_filters.py` recognizes the new Assets header. Window/data-root constants remain for C4. |
| Pending work | Not all fable plans are complete. Open work remains in fable-03, remaining fable-04, remaining fable-05, remaining fable-06, and fable-07 per `fable_plans/HANDOFF-2026-07-07.md`; fable-01, fable-02, fable-08, and fable-09 are checklist complete. Next small slices: fable-04 C4 window/data-root constants extraction, fable-05 GameModelCollection Lane R append cleanup at `GameModelCollection.cpp:288`, `:410`, and `:417`, or fable-06 `ModelRowHint`/body-store resolver. |
...
| Validation | Latest source gate passed on 2026-07-08 for fable-04 C3 asset key/hash extraction: touched-file comment audit inspected `AssetKeys.h`, `Common.h`, and `tools\validate_project_filters.py` with 0 deferred; `python tools\validate_project_filters.py --repo .` passed in 00:00:00.9300368 with 0 errors; `tools\validate_fast.bat` passed in 00:01:04.6590509; `tools\validate_full.bat` passed in 00:00:45.9861865 with runtime boundaries at 0 errors, Profile/Debug builds at 0 warnings/errors, DX12 validation errors 0, screenshots matching committed baselines, and `physics_regression_solver.csv` byte-exact. Logs live under `Agentic\Reports\2026-07-08\logs\fable-04-c3-asset-keys-*`. |
