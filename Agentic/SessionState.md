...
| Last source/data milestone | Persistent replay identity now lives in `PhysicsBodyStore` rows instead of a `GameModelCollection` scene-order sidecar; collection code keeps only id allocation and cold reload scratch. |
| Pending work | Continue scene/entity metadata cleanup so creation/id allocation can leave `GameModelCollection`, clear remaining model-index fallbacks that are not pure UI/presentation hints, and run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 replay-identity store-authority slice passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; workqueue CSV parse; focused Profile build at 0 warnings/errors; touched-source comment audit; `tools\validate_format.bat`; `tools\validate_fast.bat` in 38.4s; and `tools\validate_physics.bat` in 13.9s with standalone/runtime physics smoke passing, `collider_refresh=pass`, and byte-exact 20,001-line `physics_regression_solver.csv`. |
