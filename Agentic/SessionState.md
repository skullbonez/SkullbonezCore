...
| Last source/data milestone | Editor ragdoll transform grouping now uses integer collection metadata instead of per-frame name/suffix parsing; legacy saved-scene names are converted to `SimpleRagdoll` metadata at the cold `GameModelCollection::AddGameModel()` import boundary, with late-torso loads repairing earlier limbs to the torso root. |
...
| Validation | 2026-07-05 editor transform-group metadata slice passed: `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed at 0 warnings/errors; `tools\validate_fast.bat` passed with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed with standalone/runtime smoke and byte-exact 20,001-line `physics_regression_solver.csv`. |
