...
| Current Plan 14 progress | 2026-07-10 Plan 14 Steps 0.1 and 1.1 are complete. Inventory confirmed no `GameModel` matches in `PhysicsApi.h`/`PhysicsEngine.h`, while `PhysicsEngine.h` still exposes raw `modelIndex`/`modelCount` authority and store/debug containers through public signatures. `PhysicsHandles.h` now defines `PhysicsBodyCount`, `PhysicsColliderCount`, `PhysicsAuthoredBodyCount`, and `ModelRowHint::IsValid()` for the next source slices. Touched-source comment audit inspected 1 file with 0 deferred. `tools\validate_physics.bat` passed in 00:00:44.2320777 with Debug/Profile builds at 0 warnings/errors and `physics_regression_solver.csv` byte-exact at 20001 lines. Ignored log: `Agentic/Reports/validate_physics_plan14_identity_types_20260710.log`. |
...
| Public physics API boundary | Active plan (40%) | `engine-cleanup-plans/14-public-physics-api-boundary.md`; Steps 0.1 and 1.1 are complete. Next source slices replace raw dense row authority and public solver-container signatures in `PhysicsApi.h`/`PhysicsEngine.h`. |
