...
| Last source/data milestone | Every `GameModelCollection::AddGameModel()` append caller now passes `PhysicsColliderCreateDesc`; the public bare append overload and append-time `GameModel` collider recapture fallback are gone. Ragdoll, editor placement, runtime smoke, launcher projectile, authored scenes, and generated scenes all submit descriptors from construction-time shape facts. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, replace same-count editor/config/topology `CaptureAuthoredColliderDesc()` recapture with explicit collider update commands, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 append collider descriptor ownership slice passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; focused Profile build at 0 warnings/errors after one namespace typo fix; touched-source comment audit; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing, `collider_refresh=pass`, and byte-exact 20,001-line `physics_regression_solver.csv`. |
