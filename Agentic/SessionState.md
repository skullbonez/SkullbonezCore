...
| Last source/data milestone | Phase 10 `PHY-1025` is complete in this branch: replay editor-transform restore wake now resolves wake-ready `PhysicsBodyHandle` values at the replay event boundary and calls `PhysicsEngine::WakeBody` directly instead of using `GameModelCollectionPhysicsAdapter` model-index command wrappers. Guardrails block RunFrame replay adapter wake wrappers from returning. |
...
| Validation | 2026-07-05 `PHY-1025`: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; touched-file comment audit passed; `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`; initial `tools\validate_full.bat` stopped on formatting only; narrow `RunFrame.cpp` clang-format fixed it; rerun `tools\validate_full.bat` passed DX12 InfoQueue 0 errors, screenshots matching baselines, and byte-exact `physics_regression_solver.csv`. |
