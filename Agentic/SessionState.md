...
| Last source/data milestone | `PhysicsModelAccess` no longer exposes `ModelCount`; `GameModelCollection` passes expected model counts into body/render refresh calls so the facade stays refresh-only. |
...
| Validation | 2026-07-05 PhysicsModelAccess count-query deletion slice passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; focused Profile build at 0 warnings/errors; touched-source comment audit; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing and byte-exact 20,001-line `physics_regression_solver.csv`. |
