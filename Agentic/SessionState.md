...
| Last source/data milestone | Authored/generated scene balls, boxes, and convex hulls now pass `PhysicsColliderCreateDesc` directly into `GameModelCollection::AddGameModel()`. `GameModelCollection` fills body identity and collection material policy before registering the collider, and the checker blocks bare scene `AddGameModel(std::move(...))` calls from returning. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, make ragdoll/editor/config creation write collider descriptors directly so `CaptureAuthoredColliderDesc()` can disappear, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 scene collider descriptor ownership slice passed: focused Profile build at 0 warnings/errors; `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; targeted header format plus `tools\validate_format.bat`; touched-source comment audit; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing, `collider_refresh=pass`, and byte-exact 20,001-line `physics_regression_solver.csv`. |
