...
| Last source/data milestone | Authored collider registration/update now crosses the collection-to-physics boundary as `PhysicsColliderCreateDesc`. `PhysicsScene` derives live `ColliderRecord` rows and shape kind, `UpdateAuthoredCollider()` is handle-keyed, and the checker blocks `BuildColliderRecordFromModel` returning to collection code. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, make scene/entity creation write collider descriptors directly so `CaptureAuthoredColliderDesc()` can disappear, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body/collider readers. |
