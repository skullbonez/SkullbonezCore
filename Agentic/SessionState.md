...
| Last source/data milestone | Simple-ragdoll grouping now passes explicit `SceneObjectGroupCreateDesc` root/part metadata from `Ragdoll::AddSimpleHumanoid`; `GameModelCollection` no longer parses ragdoll display-name suffixes or scans siblings to infer ragdoll groups. |
| Pending work | Continue scene/entity metadata cleanup so fixed-tree grouping and names/asset identity leave `GameModelCollection`, clear remaining model-index fallbacks that are not pure UI/presentation hints, and run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 simple-ragdoll grouping descriptor passed: source residue scan found no deleted ragdoll inference tokens under `SkullbonezSource`; `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; workqueue CSV parse; focused Profile build at 0 warnings/errors; touched-source comment audit of 4 files; `tools\validate_fast.bat` passed; and `tools\validate_physics.bat` passed with standalone/runtime handle smoke and byte-exact `physics_regression_solver.csv`. |
