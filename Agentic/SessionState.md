...
| Last source/data milestone | Same-count collider refresh is store-owned. `ColliderStore` keeps the dense live `ColliderRecord` rows, body-binding refresh only rebases body handle/replay identity, editor/config changes replace one row in place, and the checker blocks the deleted collection-side collider authoring sidecar plus `ColliderStore` `GameModel` dependencies. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, move the remaining cold `GameModel` collider authoring import into explicit collider descriptors, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 collider store row ownership slice passed: targeted residue scan found no deleted collider-authoring sidecar or `colliderStore.Refresh(` spellings under `SkullbonezSource`; focused Profile build at 0 warnings/errors; `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; touched-source comment audit; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing, `collider_refresh=pass`, and byte-exact 20,001-line `physics_regression_solver.csv`. |
