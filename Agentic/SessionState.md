...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, and replay restore/prediction commands now avoid model-index/model-state physics authority at their migrated boundaries. Replay prediction body restore uses `PhysicsEngine::RestoreReplayBodyState`, and render-pose overlays no longer recapture presentation-only poses into `PhysicsBodyStore`. Runtime-boundary guardrails block those regressions from returning in the migrated files. |
| Pending work | Phase 7 still needs ragdoll and diagnostics caller groups migrated from model-index physics commands, then proven-dead `GameModelCollection` wrappers can be deleted. Later strict authority work still needs runtime stepping and final writeback pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 replay prediction restore slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed with 0 warnings / 0 errors (`TestOutput\agent_build_debug_replay_prediction_restore.log`); `tools\validate_format.bat` passed (`TestOutput\agent_validate_format_replay_prediction_restore.log`); `tools\validate_fast.bat` passed (`TestOutput\agent_validate_fast_replay_prediction_restore.log`); `tools\validate_full.bat` passed with DX12 InfoQueue 0 errors, screenshots matched committed baselines, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_full_replay_prediction_restore.log`). |
