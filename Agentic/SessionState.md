...
| Last documentation milestone | Fable-02 Window resize lifecycle evidence records the `IRenderBackend*` to `IRenderDeviceLifecycle*` narrowing, checker ratchet drop, blocker wording update, and validation gates. |
| Last source/data milestone | Fable-02 Window resize lifecycle cleanup makes Win32 resize callbacks borrow only `IRenderDeviceLifecycle*` through `Window::SetResizeRenderLifecycle`, wired from `RuntimeRenderBackendView::deviceLifecycle`. Current blocker ledger: 26 blocked rows remain. |
| Pending work | Continue fable-02 G3 / SVC-001/SVC-002 Gfx accessor endgame. Next direct `Gfx()` reduction is `CollisionVisualizer.cpp`; read-only subagent Socrates mapped resource calls to `IRenderResourceFactory` and draw/state calls to `IRenderCommandContext`. Fable-08 and fable-09 are intentionally left alone on this branch. |
...
| Validation | Latest source gates passed on 2026-07-07 for fable-02 Window resize lifecycle cleanup: `python tools\check_runtime_boundaries.py --self-test` (0.316s), `python tools\check_runtime_boundaries.py` (15.780s, 0 errors), `tools\validate_fast.bat` (51.173s; formatting/project filters/staged-size/runtime-boundaries/Profile+Debug builds passed with 0 warnings/errors; unit tests passed: 44 doctest cases, 574 assertions), and `tools\validate_full.bat` (42.335s; DX12 errors 0, screenshots matched, `physics_regression_solver.csv` byte-exact). Logs: `Agentic\Reports\2026-07-07\logs\fable-02-window-lifecycle-boundary-self-test.log`, `Agentic\Reports\2026-07-07\logs\fable-02-window-lifecycle-boundary-scan.log`, `Agentic\Reports\2026-07-07\logs\fable-02-window-lifecycle-validate-fast.log`, `Agentic\Reports\2026-07-07\logs\fable-02-window-lifecycle-validate-full.log`. |
