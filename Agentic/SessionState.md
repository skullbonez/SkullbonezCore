...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, replay restore/prediction, replay/editor transform restore wake, ragdoll start-asleep commands, fixed-tree release wake, tornado fixed-tree release, store-owned sleep seeding, store-owned wake propagation, dead `PhysicsWorld` model-stream wake/seed deletion, regression CSV diagnostics row building, and SkullScope frame emission now avoid `GameModelCollection` model-index physics wrappers or model-stream rebuilds at their migrated boundaries. The obsolete `GameModelCollection::WakeModel`, `SeedModelAsleep`, `ApplyBodyImpulse`, `SetPendingBodyImpulse`, full diagnostics-record bridge, and diagnostics-view bridge were deleted. Contact-highlight ticking, fixed-contact highlight notification, persistent-contact fixed-tree release, tornado release writeback/reload, store wake/seed invalidation, post-step model stream invalidation, Debug diagnostics emission, and bulk compatibility writeback moved out of `PhysicsWorld::RunPhysics`/side-effect hot paths toward `PhysicsBodyStore` plus the `PhysicsScene` compatibility edge. `PhysicsBodyStore` now owns fixed-tree release-group metadata and attached fixed-tree release application for solver/tornado step paths; `PhysicsWorld::RunPhysics`, `RunSolverPhysics`, and `ApplyTornadoField` no longer take `PhysicsModelAccess`. `PhysicsScene::SetPendingBodyImpulse` now deletes the pending-only `GameModel` writeback/model-stream invalidation after the authoritative store mutation. Replay prediction body backup now samples simulation state from `PhysicsBodyStore` records instead of `GameModel` physics getters, leaving `GameModel` only for fixed-contact presentation highlight seconds. `PhysicsScene::SetBodyVelocity` now leaves replay velocity edits in `PhysicsBodyStore` plus optional wake state without a per-command `GameModel` writeback or stream invalidation. `SeedBodyAsleep(PhysicsModelAccess&, PhysicsBodyHandle)` was deleted; ragdoll/editor/adapter sleep seeding now targets `PhysicsBodyStore` and `PhysicsWorld` by handle without per-command `GameModel` projection. `SetPendingBodyImpulse(PhysicsModelAccess&, PhysicsBodyHandle, ...)` was deleted; adapter and `ApplyBodyImpulse` pending enqueue now use `SetPendingBodyImpulse(body, impulse, point)`. `PhysicsScene::WakeBody` now stops after store/world wake propagation and no longer performs a one-body `GameModel` writeback or model-stream invalidation. `WakeBody(PhysicsModelAccess&, ...)` and `ApplyBodyImpulse(PhysicsModelAccess&, ...)` were deleted; runtime/editor/replay callers enter handle-owned wake/apply commands, with count-gated body/collider topology refresh centralized in `GameModelCollectionPhysicsAdapter` for legacy model-index callers. `SetBodyVelocity(PhysicsModelAccess&, ...)` was deleted; replay velocity edits now enter `SetBodyVelocity(body, linearVelocity, angularVelocity, wakeIfMoving)` after adapter-owned count-gated topology refresh. `PhysicsWorld::EmitStepDiagnostics` and collision-time diagnostics no longer borrow `PhysicsModelAccess`; `PhysicsScene` owns the Debug-only diagnostics name overlay and passes a cold pointer/count view into world diagnostics when logs are enabled. Runtime-boundary guardrails block those regressions from returning. |
| Pending work | Continue reducing runtime stepping dependence on `PhysicsModelAccess` and `GameModelCollection`. Phase 8 diagnostics/SkullScope rows, Phase 9 public/standalone guardrail rows, and Phase 10 pending-impulse mirror/overload, prediction-capture, velocity-command, sleep-seed mirror, wake-command mirror, wake/apply signature, velocity signature, step-diagnostics names, collision-time names, and fixed-tree store release rows are closed; remaining strict authority work includes retiring the broader compatibility step boundary, reducing step-time refresh boundaries, and lowering allowlists or adding exact guardrails as future deletion commits remove borrowers. |
...
| Validation | 2026-07-05 Phase 10 fixed-tree store release: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_format.bat` passed after scoped `PhysicsBodyStore.h` formatting; focused Debug build passed with 0 warnings/0 errors; `tools\validate_fast.bat` passed; intermittent `tools\validate_physics.bat` passed with byte-exact `physics_regression_solver.csv` (20,001 lines). Logs: `TestOutput\agent_validate_fast_fixed_tree_store_release.log`, `TestOutput\agent_validate_physics_fixed_tree_store_release.log`. Prior Phase 10 logs: `TestOutput\agent_validate_fast_collision_time_names.log`, `TestOutput\agent_validate_physics_collision_time_names.log`, `TestOutput\agent_validate_fast_step_diagnostics_names.log`, `TestOutput\agent_validate_physics_step_diagnostics_names.log`, `TestOutput\agent_validate_fast_velocity_signatures.log`, `TestOutput\agent_validate_physics_velocity_signatures.log`, `TestOutput\agent_validate_fast_wake_apply_signatures.log`, `TestOutput\agent_validate_physics_wake_apply_signatures.log`, `TestOutput\agent_validate_fast_wake_command_mirror.log`, `TestOutput\agent_validate_physics_wake_command_mirror.log`, `TestOutput\agent_validate_fast_pending_impulse_overload.log`, `TestOutput\agent_validate_physics_pending_impulse_overload.log`, `TestOutput\agent_validate_fast_sleep_seed_mirror.log`, `TestOutput\agent_validate_physics_sleep_seed_mirror.log`, `TestOutput\agent_validate_fast_velocity_command_mirror.log`, `TestOutput\agent_validate_physics_velocity_command_mirror.log`, `TestOutput\agent_validate_fast_prediction_body_capture.log`, `TestOutput\agent_validate_physics_prediction_body_capture.log`, `TestOutput\agent_validate_fast_pending_impulse_mirror.log`, `TestOutput\agent_validate_physics_pending_impulse_mirror.log`; Phase 8/9 gates also passed, including SkullScope deep/query baselines. |
