...
| Last committed milestone | `42a14752` records replay v2 runtime-command reset coverage with real queued `ResetCurrentScene` capture, decoded query/checker coverage, checkpoint-precondition restore handling, and final replay v2/full validation. |
...
| Pending work | V2 now has smooth file-backed presentation scrub, branch provenance chunks/querying, bounded event chunks/querying, typed generated-scene-config/runtime-command/world-override/editor-place/editor-transform/launcher-config/launcher-fire event coverage, checkpoint event-cursor chunks/querying, per-tick solver hash chunks, sparse solver checkpoint chunks in saved artifacts, retained in-memory restore hash verification, saved checkpoint-frame restore verification, saved non-checkpoint target checkpoint-plus-event restore verification, generated-topology rebuild before mismatched saved checkpoint restore, reset-command checkpoint-precondition coverage, live branch-from-file wiring for loaded v2 artifacts, in-app v2 replay save/load controls, an explicit scrubber `BRANCH` action shared with `Enter`, and saved-file restore failure SkullScope rows/query validation. Editor-transform coverage includes committed translate/rotate/scale gizmo changes; external scene-load/create/advance runtime commands remain query-only/unsupported across checkpoint replay windows. |
| Blockers | None known. |
...
| Validation | Current runtime-command replay v2 slice is covered by `python -m py_compile tools\check_replay_v2_artifact.py tools\replay_query.py`, targeted `tools\validate_build.bat Debug`, focused `python tools\check_replay_v2_artifact.py`, `tools\validate_fast.bat`, `tools\validate_replay_v2_artifact.bat`, and `tools\validate_full.bat`. Final logs are in `TestOutput\agent_logs\2026-06-23_replay_v2_runtime_command_debug_build_iter1.log`, `TestOutput\agent_logs\2026-06-23_replay_v2_runtime_command_checker_iter1.log`, `TestOutput\agent_logs\2026-06-23_validate_fast_replay_v2_runtime_command_final1.log`, `TestOutput\agent_logs\2026-06-23_validate_replay_v2_runtime_command_final1.log`, and `TestOutput\agent_logs\2026-06-23_validate_full_replay_v2_runtime_command_final1.log`. |
