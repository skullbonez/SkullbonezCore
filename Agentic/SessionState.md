...
| Current objective | Complete validation-gate integrity V3 by obtaining one real `merge_group` proof after creating or joining an eligible organization and moving the repository there, or after GitHub expands merge-queue availability. The Fresh-Read Engine Review Campaign is complete and excluded from the live ledger. |
...
| Validation-gate integrity blocker | V0-V2 and V4-V5 are complete; V3 remains at 5/6 solely for a real `merge_group` proof. Hosted CPU run 30469139071 passes with pinned LLVM 21.1.8, and `main` strictly requires `Mandatory CPU lane (Windows hosted)`. Owner ruling: anything needing a graphics card is local-only validation. The experimental GitHub DX12 workflow is disabled and deleted; its repository variable and runner registration are deleted; its scheduled task is removed; and its dedicated runner and PowerShell directories were moved to the Recycle Bin. No GitHub GPU lane may replace it. GitHub rejected merge-queue activation with HTTP 422 because this public repository is user-owned, while the feature requires organization ownership. The authenticated owner has zero organization memberships, so no existing transfer destination is available. Owner: repository validation plus repository ownership administration. Create or join an eligible organization, transfer the repository, then explicitly authorize and enqueue a proof PR; alternatively wait for GitHub availability to change. Evidence: `Agentic/Plans/TODO/validation-gate-integrity.md` and `Agentic/Reports/validation_ci_v3_20260710.md`. |
...
stable hosted CPU check. Anything needing a graphics card is local-only; the
GitHub DX12 workflow and all runner infrastructure are removed and must not be
reintroduced. The sole remaining acceptance item is a real `merge_group`
proof. GitHub rejected merge-queue activation because `SkullbonezCore` is
user-owned and merge queues require an organization-owned repository. The
authenticated owner has no organization memberships, so first create or join
an eligible organization that can receive the repository, then transfer it and
explicitly authorize creation and enqueue of a proof PR; alternatively wait
for GitHub availability to change. Permanent Fresh-Read closure evidence is in
