...
| Last source/data milestone | Fixed-contact presentation highlights now gate on `PhysicsBodyRecord::isFixed` from `PhysicsBodyStore`, and `GameModel::NotifyFixedContact()` only updates its timer instead of rereading `m_isFixed`. |
