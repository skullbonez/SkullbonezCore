...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, replay restore/prediction, replay/editor transform restore wake, and ragdoll start-asleep commands now avoid model-index/model-state physics authority at their migrated boundaries. Replay prediction body restore uses `PhysicsEngine::RestoreReplayBodyState`, render-pose overlays no longer recapture presentation-only poses into `PhysicsBodyStore`, ragdoll construction seeds sleep through `PhysicsEngine::SeedBodyAsleep` with body handles, and RunFrame replay editor-transform restore wakes through `PhysicsEngine::WakeBody`. Runtime-boundary guardrails block those regressions from returning in the migrated files. |
| Pending work | Phase 7 next source row is `PHY-0709`: use CodeGraph callers plus compiler to prove the old `GameModelCollection` model-index physics wrappers are dead, then delete obsolete wrappers and lower/replace guardrails in the same diff. Later strict authority work still needs runtime stepping and final writeback pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 RunFrame replay/editor transform wake slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed in 5.0s with 0 warnings / 0 errors (`TestOutput\agent_build_debug_runframe_replay_wake.log`); `tools\validate_fast.bat` passed in 24.8s (`TestOutput\agent_validate_fast_runframe_replay_wake.log`); `tools\validate_physics.bat` passed in 14.5s with byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_runframe_replay_wake.log`); `tools\validate_full.bat` passed in 32.4s with DX12 InfoQueue 0 errors, screenshots matching committed baselines, and byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_full_runframe_replay_wake.log`). |
