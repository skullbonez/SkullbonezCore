...
| Last source/data milestone | Replay scrub render apply and prediction ghost requests now resolve `ReplayBodyId` through `PhysicsBodyStore` handles; `GameModelCollection::TryQueueReplayRenderPoseOverride()` validates against the body store instead of the `GameModel` replay-id mirror. |
| Pending work | Continue scene/entity metadata cleanup, clear the remaining non-render replay/restore identity and model-index fallbacks, then run a final completion audit for `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 replay render identity store-authority slice passed: focused Profile build at 0 warnings/errors, `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, `python tools\check_runtime_boundaries.py --repo .` with 0 errors, `tools\validate_full.bat` with DX12 InfoQueue errors 0, screenshots matching committed baselines, standalone/runtime physics smoke passing, and byte-exact 20,001-line `physics_regression_solver.csv`; `tools\validate_perf.bat` completed with DX12 frame avg 1.1006 ms and physics_bench frame avg 0.6653 ms. |
