...
| Last source/data milestone | Runtime attached-camera recovery, solver-sample replay restore preflight, and runtime handle smoke reorder checks now validate replay ids through `PhysicsBodyStore` records or authored constants instead of `GameModel::GetReplayBodyId()`. |
| Pending work | Continue scene/entity metadata cleanup, move creation/import replay-id ownership out of `GameModel`, clear remaining model-index fallbacks that are not pure UI/presentation hints, then run a final completion audit for `GameModel` body-state readers. |
