...
| Last documentation milestone | Fable-02 Profiler renderer-diagnostics bind evidence records the Core profiler Gfx removal, checker ratchet drop, blocker wording update, and validation gates. |
| Last source/data milestone | Fable-02 Profiler renderer-diagnostics bind removes Core `Profiler.cpp` direct `Gfx()`/`IsGfxReady()` and `IRenderBackend` dependency by binding `IRenderDiagnostics*` at runtime startup and clearing it before backend teardown. Current blocker ledger: 26 blocked rows remain. |
| Pending work | Continue fable-02 G3 / SVC-001/SVC-002 Gfx accessor endgame, then proceed through remaining fable plans up to fable-07. Fable-08 and fable-09 are intentionally left alone on this branch. |
...
| Validation | Latest source gates passed on 2026-07-07 for fable-02 Profiler renderer-diagnostics bind: `python tools\check_runtime_boundaries.py --self-test` (0.314s), `python tools\check_runtime_boundaries.py` (15.371s, 0 errors), `tools\validate_fast.bat` (52.424s; formatting/project filters/staged-size/runtime-boundaries/Profile+Debug builds passed with 0 warnings/errors), `tools\validate_full.bat` (41.890s; project filters/runtime boundaries passed; Profile and Debug builds 0 warnings/errors; DX12 InfoQueue 0 validation errors; screenshots matched baselines; `physics_regression_solver.csv` byte-exact), and `Profile\SKULLBONEZ_CORE.exe --platform-profiler-markers --renderer dx12 --vsync off --frames 2 --scene SkullbonezData\scenes\solver_smoke.scene.json` (1.879s; marker emission requested/enabled, clean exit). Logs: `Agentic\Reports\2026-07-07\logs\fable-02-profiler-render-diagnostics-boundary-self-test.log`, `Agentic\Reports\2026-07-07\logs\fable-02-profiler-render-diagnostics-boundary-scan.log`, `Agentic\Reports\2026-07-07\logs\fable-02-profiler-render-diagnostics-validate-fast.log`, `Agentic\Reports\2026-07-07\logs\fable-02-profiler-render-diagnostics-validate-full.log`, `Agentic\Reports\2026-07-07\logs\fable-02-profiler-render-diagnostics-platform-profiler-markers.log`. |
