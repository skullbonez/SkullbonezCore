...
| Last source/data milestone | Runtime scene setup, editor placement/gizmo wake-sleep, mouse-pickup impulse, launcher laser/projectile wake, replay velocity edit, replay restore/prediction, replay/editor transform restore wake, ragdoll start-asleep commands, and fixed-tree release wake now avoid `GameModelCollection` model-index physics wrappers at their migrated boundaries. The obsolete `GameModelCollection::WakeModel`, `SeedModelAsleep`, `ApplyBodyImpulse`, and `SetPendingBodyImpulse` wrappers were deleted. Contact-highlight ticking, post-step model stream invalidation, and bulk compatibility writeback moved from `PhysicsWorld::RunPhysics` to the `PhysicsScene` compatibility edge. Runtime-boundary guardrails block those regressions from returning. |
| Pending work | Commit/push the bulk-writeback strict-step slice, then continue reducing runtime stepping dependence on `PhysicsModelAccess` and `GameModelCollection`. Later strict authority work still needs Debug diagnostics and solver side effects pushed further toward direct store ownership. |
...
| Validation | 2026-07-04 bulk-writeback strict-step slice: `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; focused Debug build passed in 4.4s with 0 warnings/errors; `tools\validate_fast.bat` passed in 24.9s; `tools\validate_physics.bat` passed in 14.2s with byte-exact `physics_regression_solver.csv`; `tools\validate_perf.bat` passed in 22.6s with no DX12 or PHYSICS_BENCH regressions. |
