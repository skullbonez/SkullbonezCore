...
| Last documentation milestone | Updated the contact-audio plan for Sound tab reducer wording after adding `kindCounts`: `impact`, `heavy_landing`, `support`, `settle`, `roll_slide`, and `propagated_impulse`. |
| Last source milestone | Sound tab labels now match the contact-audio reducer model: `Reducer`, `Classifier`, `Material Recipe`, pair cooldown, spike rearm, patch distance, and voice cap. |
