...
| Last committed milestone | Runtime run decomposition is committed through Phase 3B on `nightrunner-24th-june-refactor`: generated/demo scene camera, mixed object, and solver object setup now live in `SceneGeneratedSetup`, with `Run` reduced to context-building wrappers. |
