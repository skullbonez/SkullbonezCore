...
| Last source/data milestone | Same-count editor/replay collider shape edits now pass explicit `PhysicsColliderCreateDesc` values into `CommitEditedModelColliderState()` instead of recapturing full collider descriptors from `GameModel`; runtime config already updates dense `ColliderStore` material scalars directly. |
| Pending work | Continue scene/entity metadata cleanup, move replay identity from the temporary collection sidecar into stable scene/entity metadata, replace the remaining topology-drift `CaptureAuthoredColliderDesc()` fallback, clear remaining model-index fallbacks that are not pure UI/presentation hints, and run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 editor collider descriptor command passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; CSV parse check; focused Profile build at 0 warnings/errors; touched-source comment audit; `tools\validate_fast.bat`; and `tools\validate_physics.bat` with standalone/runtime physics smoke passing, `collider_refresh=pass`, and byte-exact 20,001-line `physics_regression_solver.csv`. |
