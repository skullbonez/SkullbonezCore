...
| Active objective | Continue the master inventory in order. Plan 03 governance apparatus removal is the current top row: Step 0.2 inventory is complete and recorded in `engine-cleanup-plans/03-governance-apparatus-reduction.md`; next step is 1.1, re-express DX12-only runtime renderer exclusivity as a boolean check, preferably in `Agentic/Tests/Dx12ArchUnitTests`, gated by `tools\validate_tests.bat`. Plan 04 error-handling reconciliation remains in progress after the DX12 fence/readback recoverable batch; resume it only after the earlier master-plan rows/dependencies are handled or intentionally skipped. The overall engine cleanup goal remains active and incomplete. |
...
| Current Plan 03 validation | 2026-07-09 Step 0.2 inventory completed as documentation-only work. No repository validation required. Survivor list: only DX12-only runtime renderer exclusivity should be re-expressed as a real boolean check; allocation policy remains owned by Plan 07 and `tools/check_allocation_policy.py`; all frozen counts/tombstones in `tools/check_runtime_boundaries.py` should be deleted rather than preserved. |
