...
| Last source/data milestone | Replay prediction ghost drawing now reads collision shape/material data from `ColliderStore` and `RenderInstanceStore` snapshots instead of reopening `GameModel` collider/material mirrors. |
| Pending work | Move the remaining convex-hull render batches and editor/replay transform-scale shape reads off `GameModel::GetCollisionShape()`, continue scene/entity metadata cleanup, then run a final completion audit for `GameModel` body-state readers and presentation-only model-index fallbacks. |
...
| Validation | 2026-07-05 replay prediction ghost store-snapshot slice passed: `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; `tools\validate_full.bat` passed project filters, runtime boundaries, Profile/Debug builds at 0 warnings/errors, DX12 InfoQueue errors 0, DX12 screenshot comparisons, standalone/runtime physics smoke, and byte-exact 20,001-line `physics_regression_solver.csv`. |
