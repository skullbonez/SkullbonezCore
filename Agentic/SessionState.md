...
| Last source/data milestone | Moved replay cause-tree camera-focus body position/radius lookup off `GameModel` pose/shape mirrors. `ReplayRuntime::ResolveCauseTreeBodyPosition` now consumes `PhysicsBodyStore` and `ColliderStore`; live fallback resolves body pose by replay id and display radius through body-handle/collider pairing. |
...
| Validation | 2026-07-05 replay cause-tree store-authority slice: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py` passed with 0 errors; focused Debug build passed with 0 warnings/errors; touched-file comment audit passed for all touched source/tool files; intermittent `tools\validate_physics.bat` passed standalone smoke, runtime handle smoke, and byte-exact `physics_regression_solver.csv`; `tools\validate_fast.bat` passed formatting, project filters, runtime boundaries, and Profile/Debug builds with 0 warnings/errors. |
