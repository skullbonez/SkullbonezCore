...
| Active objective | Complete all fable plans. Fable-03 P3.0, fable-04 L1-L4 and C1-C5, and fable-05 P3 SpatialGrid/PhysicsWorld plus GameModelCollection classification/pure topology and legacy camera pose-read conversions are complete; fable-03 P3.1-P3.4, remaining fable-04, remaining fable-05, remaining fable-06, and fable-07 still have open work. |
| Last documentation milestone | `fable_plans/HANDOFF-2026-07-07.md`, `fable_plans/03-prediction-isolated-world-plan.md`, and `fable_plans/03-prediction-isolated-world-progress.md` now document fable-03 P3.0 build-prefix publication safety prep and updated counts. |
| Last source/data milestone | Fable-03 P3.0 is complete: `RunReplayPredictionState` owns published build-prefix helpers; replay render, automation, Director, scrubber, and cause-tree readers no longer touch `buildFrameCount` directly; cancellation/promotion paths document the future async-worker hazard. |
| Pending work | Not all fable plans are complete. Open work remains in fable-03 P3.1-P3.4 worker-job stepping, remaining fable-04 mega-file decomposition, remaining fable-05, remaining fable-06, and fable-07 per `fable_plans/HANDOFF-2026-07-07.md`; fable-01, fable-02, fable-08, and fable-09 are checklist complete. Next small slices: fable-05 GameModelCollection Lane R append cleanup at `GameModelCollection.cpp:288`, `:410`, and `:417`, or fable-06 `ModelRowHint`/body-store resolver. |
...
| Validation | Latest source gate passed on 2026-07-08 for fable-03 P3.0 build-prefix publication safety prep: touched-file comment audit inspected `ReplayRuntime.h`, `ReplayRuntime.cpp`, `RunReplayPredictionHelpers.inl`, `RunReplayPredictionVisualizer.inl`, `RunReplayScrubberTools.inl`, `RunDemoDirector.cpp`, and `RunInteractionAutomation.cpp` with 0 deferred; `tools\validate_format.bat` passed; `tools\validate_full.bat` rerun passed in 00:00:51.2659145 with runtime boundaries at 0 errors, Profile/Debug builds at 0 warnings/errors, DX12 validation errors 0, screenshots matching committed baselines, and `physics_regression_solver.csv` byte-exact. Focused `prediction_ragdoll_wall_200_predict` proof passed in 00:00:03.9054389 with `ok=true`, `predictionPathVisible=true`, and `liveSolverHashStableAcrossPrediction=true`. |
