...
| Last source/data milestone | Added store-owned standalone island rows: `PhysicsStandaloneWorld::Islands()` now returns deterministic body-handle spans generated from live body rows, contact rows, and point-joint endpoints after `Step()`. |
| Pending work | Physics standalone still needs constraint-only island smoke/constraint phase cleanup, later shape-pair contact coverage if required, full support propagation, remaining update-desc/state cleanup, compatibility assertion/guardrail shrink decisions, and runtime adapter migration. Contact-audio one-shot impact work remains complete; separate roll/slide loops are future work. |
...
| Validation | 2026-07-04 standalone island slice: Debug build passed with 0 warnings / 0 errors in 6.4s (`TestOutput\agent_build_debug_standalone_islands.log`); standalone smoke passed with `contacts=2`, `contact_hash=0x5DBDF5257E90EA9B`, runtime mirror pass, and deterministic hash `0xB3B6CEEFB6CCAFFA` (`TestOutput\agent_physics_standalone_smoke_islands_report.txt`); repeated smoke reports were byte-identical (`TestOutput\agent_physics_standalone_smoke_islands_repeat1.txt`, `TestOutput\agent_physics_standalone_smoke_islands_repeat2.txt`); `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors in 7.0s (`TestOutput\agent_runtime_boundaries_standalone_islands.log`); `tools\validate_physics.bat` passed in 22.4s with byte-exact `physics_regression_solver.csv` (`TestOutput\agent_validate_physics_standalone_islands.log`). |
