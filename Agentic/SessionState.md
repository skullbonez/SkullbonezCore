...
| Last source milestone | K003 store-boundary cleanup is in progress: `RenderInstanceStore`, `ColliderStore`, `SleepIslandSystem`, `PhysicsBodyStore`, `Ragdoll`, `PhysicsWorld` wake/seed island helpers, and top-level `PhysicsWorld::RunPhysics` contact-highlight ticking no longer refresh through `PhysicsModelAccess::Models()`; scene commands and persistent-contact wakes use store-backed `PhysicsWorld` overloads. |
| Pending work | K003 still has raw underwater sleep, tornado, persistent-contact, and solver model ranges. K004/K005 remain blocked on real force-authority and handle-allocation ownership moves. |
...
| Validation | K003 render-store slice passed `tools\validate_full.bat`, including runtime boundaries, DX12 validation with 0 InfoQueue errors, and byte-exact core physics baseline. K003 collider-store, sleep-island, body-store, ragdoll, wake-island, and contact-highlight slices each passed `python tools\check_runtime_boundaries.py --repo .` and `tools\validate_physics.bat`, including byte-exact core physics baseline. Latest guardrail slice passed `tools\validate_fast.bat`; final K003 diagnostics patch passed `tools\validate_physics.bat`. Deep physics known-issue signature mismatch is pre-existing on this branch. |
