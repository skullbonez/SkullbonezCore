...
| Last source/data milestone | Editor selection commands and editor state now carry `PhysicsBodyHandle`/`PhysicsColliderHandle` identity from picking, placement, and attached-camera inspect selection; stale handle/index pairings clear before gizmo code runs. |
...
| Validation | 2026-07-05 editor selection identity slice passed: `git diff --check`, `python -m py_compile tools\check_runtime_boundaries.py`, and `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_fast.bat` passed with formatting, project filters, runtime boundaries, and Profile/Debug builds at 0 warnings/errors; intermittent `tools\validate_physics.bat` passed with standalone/runtime handle smoke and byte-exact 20,001-line `physics_regression_solver.csv`. |
