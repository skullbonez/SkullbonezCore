...
| Active objective | Continue the master inventory in order. Plan 03 governance apparatus removal is the current top row: steps 0.2, 1.1, and 1.2 are complete. Next actionable work is the prerequisite for Plan 03 step 2.1: complete `Agentic/Plans/TODO/behavioral-test-depth.md` P1 and P4 so the AGENTS regex-gate rewrite can land honestly. Plan 04 error-handling reconciliation remains in progress after the DX12 fence/readback recoverable batch; resume it only after the earlier master-plan rows/dependencies are handled or intentionally skipped. The overall engine cleanup goal remains active and incomplete. |
...
| Current Plan 03 validation | 2026-07-09 Step 1.2 deleted `tools/check_runtime_boundaries.py`, `tools\validate_runtime_boundaries.bat`, and all live validation-script invocations. Search confirmed no `check_runtime_boundaries`, `validate_runtime_boundaries`, `runtime-boundaries`, or `runtime_boundaries` references remain in `tools/`, `.githooks`, `.pre-commit-config.yaml`, or `.github`. Comment audit inspected 3 edited source-bearing tool scripts with 0 deferred. `tools\validate_fast.bat` passed in 00:00:18.9508581 (`VALIDATE_FAST: ALL PASSED`, Profile/Debug ready, unit tests passed). `tools\validate_select.bat project-filters` passed in 00:00:05.3469357. `tools\validate_full.bat` passed in 00:00:28.5120019 with project filters clean, Profile/Debug builds 0 warnings/errors, DX12 validation errors 0, screenshots matching baselines, and `physics_regression_solver.csv` byte-exact. Logs: `Agentic/Reports/validate_fast_plan03_delete_runtime_boundaries_20260709_rerun.log`, `Agentic/Reports/validate_select_project_filters_plan03_delete_runtime_boundaries_20260709.log`, `Agentic/Reports/validate_full_plan03_delete_runtime_boundaries_20260709.log`. Next Plan 03 step: complete behavioral-test-depth P1+P4, then return to Plan 03 step 2.1. |
...
| Item | Status | Notes |
|------|--------|-------|
| Plan inventory | Authoritative reference | `Agentic/Plans/MASTER-PLAN.md` lists every remaining plan with percent-complete. Completed plans are deleted, not archived. Plan 03 is now the active first-row slice. |
| Behavioral test depth | Proposed plan (0%) — highest leverage | `Agentic/Plans/TODO/behavioral-test-depth.md`; solver/manifold/parser/replay unit tests. Prerequisite for engine-cleanup plan 03 step 2.1 (linter deletion's enforcement replacement). |
