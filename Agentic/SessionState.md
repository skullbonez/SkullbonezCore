...
| Current pushed baseline | `97d3b27c docs: inventory runtime UI control surfaces` |
| Current objective | Execute runtime UI/control U2: convert the replay scrubber to the shared fixed-capacity surface |
...
| Latest focused gates | U1 `validate_all_cpu_tests` passed in 17.3s: 131/131 doctests, 2,814 assertions, 22/22 interaction cases in Debug and Release, scene parser and DX12 architecture; U1 comment audit 2/2 |
...
Runtime UI/control is complete through U1 at 2/7. The owning plan reconciles all
60 tracked `SkullbonezSource/UI` source files plus 36 interactive replay,
editor, input, automation, shared-control, and render-boundary files. U1 adds an
inline, typed `RuntimeUiSurface` covering all required control kinds, ordered
hit resolution, bounded failure, and disposable hot/active state. Four new CPU
tests pass in both Debug and Release through the mandatory umbrella. U6
conversion/review remains independently unchecked. Next execute the U2 replay
scrubber vertical slice. Interaction gesture geometry and capture remain with
`RuntimeInteractionController` rather than UI-owned state.
