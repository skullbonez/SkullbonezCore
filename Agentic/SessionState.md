...
| Last source/data milestone | Editor-created releasable trees now pass explicit `SceneObjectGroupCreateDesc` root/part metadata from `PlaceEditorObjectAtTerrainPoint`; simple ragdolls already do the same from `Ragdoll::AddSimpleHumanoid`. `GameModelCollection` no longer recovers those new prefab groups from display names; legacy authored-scene name recovery remains scene/entity metadata load debt. |
| Pending work | Continue scene/entity metadata cleanup so legacy fixed-tree grouping and names/asset identity leave `GameModelCollection`, clear remaining model-index fallbacks that are not pure UI/presentation hints, and run a final completion audit for `GameModel` body/collider readers. |
...
| Validation | 2026-07-05 editor-tree grouping descriptor passed: `git diff --check`; `python -m py_compile tools\check_runtime_boundaries.py`; `python tools\check_runtime_boundaries.py --repo .` with 0 errors; workqueue CSV parse; focused Profile build at 0 warnings/errors after namespace fix; touched-source comment audit; `tools\validate_fast.bat` passed in about 28s; and `tools\validate_physics.bat` passed in about 14s with standalone/runtime handle smoke and byte-exact `physics_regression_solver.csv`. |
