...
| Last source/data milestone | Phase 10 `PHY-1020` is complete in this branch: `RenderInstanceStore` no longer exposes model-only refresh overloads, topology mismatches fail closed instead of rebuilding render transforms from `GameModel`, and scene perf-log open schedules a profiler reset so appended perf passes skip startup rows consistently. Guardrails block the deleted overloads and fallback call from returning. |
...
| Validation | 2026-07-05 `PHY-1020`: `git diff --check` passed; `python -m py_compile tools\check_runtime_boundaries.py` passed; `python tools\check_runtime_boundaries.py --repo .` passed with 0 errors; `tools\validate_perf.bat` passed with no DX12 or PHYSICS_BENCH regressions after profiler scheduled reset; `tools\validate_full.bat` passed with project filters, runtime boundaries, Profile/Debug builds, DX12 InfoQueue 0 errors, screenshots matching baselines, and byte-exact `physics_regression_solver.csv`. Logs: `TestOutput\agent_validate_perf_profiler_schedule_reset.log`, `TestOutput\agent_validate_full_render_instance_fallback_deletion.log`. |
