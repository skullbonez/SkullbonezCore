...
its header says which in an `Invariant:` block. A single-member aggregate, or one
whose sole consumer destructures every member at entry, owns nothing whatever it
is called. Reference-carrying view structs are judged as one surface — if any
