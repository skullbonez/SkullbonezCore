...
Left-click a world object outside editor/launcher/UI ownership to select it as the replay path target. Shift+Left Click adds another selected history target and promotes it as the prediction root; a normal click replaces the selected set. In launcher mode, use `Ctrl+Left Click` to select instead of firing, with Shift still additive. Selected root bodies get past/future path overlays built from retained solver samples. Red fades to white from the oldest retained root sample to the current solver scrub frame; white fades to green from the scrub frame to the latest retained frame. Future contacts involving a root add child bodies to that root's trace; child bodies now draw amber incoming paths and amber target rings from the current frame up to the first collision, then continue as grey post-contact paths with grey contact markers. A right-side `CAUSE TREE` panel lists the active root and child bodies as an indented hierarchy; clicking any row pauses into inspection mode when needed and points the camera at that object. At the live edge there is no retained future, so scrub the solver row backward to inspect the recorded future. Miss-clicking without Shift clears the selected traces.

The bottom scrubber also has a `PREDICT` checkbox with `-`, `+`, and a 1-10 second horizon slider. Prediction defaults to 10 seconds but remains off until enabled. When enabled with a selected replay path target, the runtime starts a sandboxed solver lookahead from the current live state and advances it as an amortized drawing job capped at 5 ms of speculative work per frame. Each slice restores the real live world before rendering, while the saved prediction cursor continues on the next frame. Partial samples draw as soon as they are captured, so long horizons fill in over several frames instead of blocking the demo scene. Changing the selected target, horizon, or edited velocity cancels the in-flight job, clears the partial paths, and starts a fresh prediction. The predicted root future draws white-to-green; bodies touched by that future draw amber incoming paths and target rings before their first predicted collision, then grey post-contact paths and contact markers after activation. The cause tree switches to the predicted chain when prediction rows are available. Prediction suppresses Debug physics diagnostics while the speculative ticks run, and exposes profiler/PIX markers under `Frame/Replay/Prediction/BeginJob`, `Frame/Replay/Prediction/Slice`, `Frame/Replay/Prediction/Steps`, `Frame/Replay/Prediction/StepPhysics`, `Frame/Replay/Prediction/ApplyJobState`, `Frame/Replay/Prediction/CaptureJobState`, `Frame/Replay/Prediction/RestoreLive`, `Frame/Replay/PathVisualizer/...`, `Frame/Replay/CauseTree/...`, `Frame/Replay/ScrubberInput`, `Frame/Replay/SimulationPause`, and `Frame/Replay/ScrubberOverlay`.
