...
The v2 artifact gate builds Debug, launches `replay_v2_solver_one.scene.json` with `--replay-save-probe TestOutput\validation\replay_v2\replay_save_probe.skreplay --physics-diag TestOutput\validation\replay_v2\replay_save_probe_runtime.physicsdiag.ndjson`, then reloads the saved file through the C++ v2 reader and proves an older loaded pose can be applied/restored. It also launches `--replay-load-probe` to prove the file-backed presentation row, `--replay-restore-file-probe` to hash-verify a saved checkpoint frame, `--replay-restore-target-file-probe` to restore a non-checkpoint target through checkpoint-plus-event replay, `--replay-restore-branch-file-probe` to drive the paused scrubber branch path from that saved artifact, and `--replay-restore-failure-file-probe` with `--physics-diag` to prove expected saved-file restore failures become queryable `replay_restore` rows. A second save probe writes `replay_generated_topology_probe.skreplay` from `replay_v2_generated_topology.scene.json`, then restores it from the mismatched one-body scene and requires `generated_topology_rebuilt=1`, proving the saved generated-scene config event can rebuild deterministic topology before the checkpoint is applied. Finally, it uses `tools\replay_query.bat` to query `summary`, `frame`, `body`, `branches`, `events`, `event-cursors`, `hashes`, `checkpoints`, and `export-skullscope`, and requires decoded generated-scene-config, world-override, editor-place, editor-transform, launcher-config, and launcher-fire payloads. The editor-transform proof covers committed translate, rotate, and scale flags; scale stores an axis plus factor against the checkpoint shape. The exported bounded NDJSON slice is imported with `tools\physics_query.bat ... summary`; the failure trace is imported with `tools\physics_query.bat ... restore --limit 4` so binary replay files and failed restore attempts stay inspectable without loading raw artifacts into the model.
