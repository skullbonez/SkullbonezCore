...
Press `Alt` outside editor mode, or click Modify Velocity in replay, to edit the selected dynamic body's linear or angular velocity. The mode holds live simulation and first completes an exact prediction of the current seed. Only then does it allocate a complete second Prediction owner under the existing Replay-phase `replay_prediction_working_set` registration and shared 960 MiB cap. Ordinary prediction never allocates that second owner. The original owner's frames, trajectories, committed solver evidence, and private engine remain intact; its unused evidence bank is reclaimed. The edited owner builds from the changed velocity after the drag ends. Each replacement releases its superseded diagnostic banks and reserves both path banks before optional evidence capture can consume the shared budget. A stationary handle click preserves a completed result and resumes an interrupted build on release.

During this comparison the original paths are faded blue and the edited paths are red. Scrubbing or replay playback uses one frame index for every original blue ghost and every real object on the edited future. Ghosts cover boxes, spheres, and convex hulls; historical time before the edit has no blue future ghosts. Repeated edits replace only red. Accept Red keeps the edited live seed and prediction; Accept Blue restores all original body states and the original solver snapshot before retaining the original prediction. Either choice destroys the discarded owner, removes comparison colors, and returns the cursor to the held present. Scene replacement also destroys the extra owner. Prediction disable, horizon changes, replay branch restoration, and replay save/load require a choice first. A denied red allocation preserves the original owner for Accept Blue.
