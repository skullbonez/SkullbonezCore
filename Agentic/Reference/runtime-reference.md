...
The bottom scrubber also has a top-row `PREDICT` checkbox with `-`, `+`, and a 1-10 second horizon slider below it. Prediction defaults to 10 seconds but remains off until enabled. When enabled with a selected replay path target, the runtime starts a sandboxed solver lookahead from the current live state and advances it as an amortized drawing job capped at 5 ms of speculative work per frame. Each slice restores the real live world before rendering, while the saved prediction cursor continues on the next frame. Partial samples draw as soon as they are captured, so long horizons fill in over several frames instead of blocking the demo scene. Changing the selected target, horizon, or edited velocity cancels the in-flight job, clears the partial paths, and starts a fresh prediction. Space-stepping while the scrubber pause button is active does not trigger the automatic source-refresh path, so existing prediction lines remain visible while the live simulation follows them. The predicted root future draws white-to-green; bodies touched by that future draw amber incoming paths and target rings before their first predicted collision, then grey post-contact paths and contact markers after activation. The cause tree switches to the predicted chain when prediction rows are available. Prediction suppresses Debug physics diagnostics while the speculative ticks run, and exposes profiler/PIX markers under `Frame/Replay/Prediction/BeginJob`, `Frame/Replay/Prediction/Slice`, `Frame/Replay/Prediction/Steps`, `Frame/Replay/Prediction/StepPhysics`, `Frame/Replay/Prediction/ApplyJobState`, `Frame/Replay/Prediction/CaptureJobState`, `Frame/Replay/Prediction/RestoreLive`, `Frame/Replay/PathVisualizer/...`, `Frame/Replay/CauseTree/...`, `Frame/Replay/FocusMask`, `Frame/Replay/InspectionCamera`, `Frame/Replay/ScrubberInput`, `Frame/Replay/SimulationPause`, and `Frame/Replay/ScrubberOverlay`. When VSync is off, the scrubber draws a red `WARNING: VSYNC OFF` label because free-running frames make scrub timing harder to reason about.
...
The v2 artifact gate builds Debug, launches `replay_v2_solver_one.scene.json` with `--replay-save-probe TestOutput\validation\replay_v2\replay_save_probe.skreplay --physics-diag TestOutput\validation\replay_v2\replay_save_probe_runtime.physicsdiag.ndjson`, then reloads the saved file through the C++ v2 reader and proves an older loaded pose can be applied/restored. The save probe performs a real queued `ResetCurrentScene`, saves the restarted timeline, and requires the decoded `runtimeCommand` event row to name `ResetCurrentScene`; saved restore treats that reset as a checkpoint precondition. It also launches `--replay-load-probe` to prove the file-backed presentation row, `--replay-restore-file-probe` to hash-verify a saved checkpoint frame, `--replay-restore-target-file-probe` to restore a non-checkpoint target through checkpoint-plus-event replay, `--replay-restore-branch-file-probe` to drive the paused scrubber branch path from that saved artifact, and `--replay-restore-failure-file-probe` with `--physics-diag` to prove expected saved-file restore failures become queryable `replay_restore` rows. The checker also mutates a copied artifact so a `CreateScene` runtime command lands inside the checkpoint-to-target replay window, then requires the runtime to reject it with `unsupported runtime timeline mutation event`; scene transitions are checkpoint barriers, not in-window fixed-tick events. A second save probe writes `replay_generated_topology_probe.skreplay` from `replay_v2_generated_topology.scene.json`, then restores it from the mismatched one-body scene and requires `generated_topology_rebuilt=1`, proving the saved generated-scene config event can rebuild deterministic topology before the checkpoint is applied. Finally, it uses `tools\replay_query.bat` to query `summary`, `frame`, `body`, `branches`, `events`, `event-cursors`, `hashes`, `checkpoints`, and `export-skullscope`, and requires decoded runtime-command, generated-scene-config, world-override, editor-place, editor-transform, launcher-config, and launcher-fire payloads. The editor-transform proof covers committed translate, rotate, and scale flags; scale stores an axis plus factor against the checkpoint shape. The exported bounded NDJSON slice is imported with `tools\physics_query.bat ... summary`; the failure trace is imported with `tools\physics_query.bat ... restore --limit 4` so binary replay files and failed restore attempts stay inspectable without loading raw artifacts into the model.
...
| Left Click | Outside UI/editor ownership, select or clear replay path targets. In launcher mode, fire the selected launcher action from the camera instead. Laser mode shows a short ribbon to the aimed hit; projectile mode shoots a small dynamic ball. |
...
| Enter | In launcher mode, write Debug-build repro data for the object under the crosshair to `Debug/launcher_repro_snapshots.txt`. While the replay scrubber is paused on the solver or `V2 FILE` row, trigger the same restore path as the scrubber `BRANCH` button. |
