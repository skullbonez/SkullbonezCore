...
| `--replay-seconds` | `1..600` | Request a replay retention window; default is 60 seconds at 120 Hz. Automatic visual history has a 20-second floor, while the larger solver-debug window may be reduced to fit the selected memory budget and configured body capacity. Alias: `--replay_seconds`. |
...
| `--predict` | scene object name | Arm the replay prediction workflow on the named body once the scene is simulating: show the scrubber, enable predict, set the path target, take the replay world-input owner, and pause. Implies `--replay on` so the solver track keeps re-predicting. Use it to reach worker prediction load for F5 profiling without the manual UI sequence. |
| `--predict-seconds` | `1.0..120.0` | Prediction horizon for `--predict`; omitted keeps the prediction owner's 20-second default. Alias: `--predict_seconds`. |
| `--predict-running` | flag | Leave the scene advancing instead of pausing when `--predict` arms. A running scene keeps changing the solver source, so the horizon rebuilds continuously and the prediction worker stays loaded. `--frames` also skips the pause, because cross-scene pause freezes the frame counter that `--frames` completes against. Alias: `--predict_running`. |
...
The bottom scrubber has a checked-by-default `HIGH DETAIL` checkbox beside `ALT VEL`, plus a top-row `PREDICT` checkbox with `-`, `+`, and a 1-120 second horizon slider below it. The detail preference remains usable while Predict is off and survives scene and generation resets; changing it joins and invalidates the current prediction generation before a fresh exact-source build. Prediction defaults to 20 seconds but remains off until enabled; the extended range is intended for small orbital scenes and remains bounded by the 8 GiB replay prediction working-set cap selected from the PSD2 evidence matrix. When enabled with a selected replay path target, the runtime seeds a replay-owned private `PhysicsEngine` from the current live topology, body rows, solver snapshot, runtime config, and world forces, then advances that private engine as an amortized drawing job capped at 5 ms of speculative work per frame. Live physics stores are not written while prediction builds; the private engine is retained under the `replay_prediction_working_set` reserve owner so rebuilds reuse capacity when the scene size has not grown. Partial samples draw as soon as they are captured, so long horizons fill in over several frames instead of blocking the demo scene. Completed build frames and their trajectory bank remain the coherent visible publication while the committed duplicate resumes between whole causal nodes under the presentation budget; readers switch only after the matching generation's topology and trajectories are complete. Child-marker discovery likewise retains per-node suffix cursors and scans only newly revealed frames at resumable budget boundaries. Turning `PREDICT` off immediately invalidates the reader-visible frame and trajectory prefixes after joining worker work, but retains both banks' outer and nested capacities for the next generation instead of reclaiming them in the input frame. Changing the selected target or horizon starts a fresh prediction. Modify Velocity freezes the original complete prediction and builds a separate edited future when the vector drag is released, as described below. Space-stepping while replay play-hold is active does not trigger the automatic source-refresh path, so existing prediction lines remain visible while the live simulation follows them. The predicted root future draws white-to-green; bodies touched by that future draw amber incoming paths and target rings before their first predicted collision, then grey post-contact paths and contact markers after activation. The cause tree switches to the predicted chain when prediction rows are available. Prediction exposes profiler/PIX markers under `Frame/Replay/Prediction/BeginJob`, `Frame/Replay/Prediction/SeedPrivateEngine`, `Frame/Replay/Prediction/Slice`, `Frame/Replay/Prediction/Steps`, `Frame/Replay/Prediction/StepPhysics`, `Frame/Replay/Prediction/CaptureJobState`, `Frame/Replay/Prediction/CaptureBodyState`, `Frame/Replay/Prediction/CaptureSample`, `Frame/Replay/Prediction/DrawRoot`, `Frame/Replay/Prediction/BuildTree`, `Frame/Replay/Prediction/DrawChildren`, `Frame/Replay/Prediction/DrawAffectedBodies`, `Frame/Replay/PathVisualizer/...`, `Frame/Replay/CauseTree/...`, `Frame/Replay/FocusMask`, `Frame/Replay/InspectionCamera`, `Frame/Replay/ScrubberInput`, `Frame/Replay/SimulationPause`, and `Frame/Replay/ScrubberOverlay`. When VSync is off, the scrubber draws a red `WARNING: VSYNC OFF` label because free-running frames make scrub timing harder to reason about.

Press `Alt` outside editor mode, or click Modify Velocity in replay, to edit the selected dynamic body's linear or angular velocity. Opening the widget preserves the current prediction, causal contents, camera and transport. The first changed vector holds live simulation and completes an exact original prediction if needed. That edit then allocates a complete second Prediction owner under the existing Replay-phase `replay_prediction_working_set` registration and shared 8 GiB cap. Ordinary prediction never allocates that second owner. The original owner's frames, trajectories, committed solver evidence, and private engine remain intact; its unused evidence bank is reclaimed. The edited owner builds from the changed velocity after the drag ends. Each replacement releases its superseded diagnostic banks and reserves both path banks before optional evidence capture can consume the shared budget. A stationary handle click preserves a completed result and resumes an interrupted build on release.

On the first changed vector, App copies the exact displayed path records, including interpolated heads, into one optional snapshot and changes only their colour and opacity to faded blue. Original geometry is never resampled or recomputed during the experiment. The snapshot shares the existing prediction reserve registration and 8 GiB cap, contains at most 40,000 ordinary and 3,000 priority compact records, and is released on accept, cancel or scene replacement. Its capacity is included in trajectory memory accounting.

Modified uses the normal retained prediction drawing path, detail setting, adaptive sampling and reveal cursor. Each changed-vector release clears only the superseded Modified publication and starts a fresh reveal, initially at one simulation second per real second (or the existing slower rate). Newly appended ribbons and interpolated heads are red. Original uses the renderer's existing per-lane record region while Modified uses its independent range region, so both retain the full normal drawing budget without another GPU allocation. `tools/validate_velocity_reveal.py` checks exact Original geometry, checkbox-only activation, progressive Modified reveal and repeated edits in the dense 200-box scene.

During this comparison the original paths are faded blue and the edited paths are red. Scrubbing or replay playback uses one frame index for every original blue ghost and every real object on the edited future. Ghosts cover boxes, spheres, and convex hulls; historical time before the edit has no blue future ghosts. Repeated edits replace only red. Accept Modified keeps the edited live seed and prediction; Accept Original restores all original body states and the original solver snapshot before retaining the original prediction. Either choice destroys the discarded owner, removes comparison colors, and returns the cursor to the held present. Scene replacement also destroys the extra owner. Prediction disable, horizon changes, replay branch restoration, and replay save/load require a choice first. A denied red allocation preserves the original owner for Accept Original.

The Editor tab also exposes Modify velocity and Angular velocity for placed dynamic bodies. Entering Edit cancels an unresolved replay comparison and restores authored frame zero, including unsaved placements. Velocity drags support undo/redo and save/reload; fixed bodies reject edits. Simulation stepping and replay scrubbing cannot advance time while editing.

Allocation telemetry remains with the existing prediction registration: Replay-only growth, 8 GiB shared hard cap, active/high-water bytes, replay growth count, and failed growth count. `ReplayPrediction::CreateAdditionalOwner` approves construction lazily; the owner-local policy is in `Runtime/Prediction/ReplayPredictionRetainedMemory.h`. The existing registration and Replay-only phase are unchanged; its aggregate budget is now 8 GiB. `tools/validate_velocity_divergence.py` verifies lazy creation, both choices, exact all-body ghost frames, repeated edits, playback, release, and strict allocation-guard shutdown.
Prediction allocates storage only when requested. The shared 8 GiB ceiling is
not a startup reserve; high-detail evidence has a 4 GiB limit per bank within
that total. Extending a horizon reuses the completed prefix, and shortening
clips presentation while retaining reusable simulation. Explicitly disabling
prediction joins its worker and releases both frame banks, trajectory buffers,
solver evidence and the private engine. Returning to live time can retain the
visible prediction snapshot until an explicit disable or replacement.
`tools/validate_prediction_memory.py` checks a 120-second, 300-body high-detail
future, complete evidence and selected/published/submitted identity, cached
shortening, release on disable and a small subsequent prediction.
...
| Ctrl+Left Click | Outside launcher mode, select or clear the closest-approach intercept target without replacing the replay path root. In launcher mode, select or clear the replay path target without firing; combine with Shift to add path targets. |
