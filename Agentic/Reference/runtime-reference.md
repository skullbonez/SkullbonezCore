...
The scrubber's inline save-icon button writes the presentation row to binary v2 artifacts named `replays\replay_v2_####.skreplay` and the solver row to legacy v1 JSON debug exports named `replays\solver_replay_####.skreplay`, incrementing like `Scenes\snapshot_####.scene.json` and `Screenshots\screenshot_####.bmp`. Solver v1 exports include compact authoritative snapshot, branch provenance, and launcher-visual summaries instead of dumping full persistent-contact rows.
...
The v2 artifact gate builds Debug, launches `physics_roll.scene.json` with `--replay-save-probe TestOutput\validation\replay_v2\replay_save_probe.skreplay --physics-diag TestOutput\validation\replay_v2\replay_save_probe_runtime.physicsdiag.ndjson`, then reloads the saved file through the C++ v2 reader and proves an older loaded pose can be applied/restored. It also launches `--replay-load-probe` against the saved artifact to prove the runtime scrub source can drive from the file-backed presentation row, then launches `--replay-restore-file-probe` to load one saved solver checkpoint and require the restored live solver hash to match. Finally, it uses `tools\replay_query.bat` to query `summary`, `frame`, `body`, `branches`, `hashes`, `checkpoints`, and `export-skullscope`. The exported bounded NDJSON slice is imported with `tools\physics_query.bat ... summary` so binary replay files stay inspectable without loading raw artifacts into the model. V2 branch/checkpoint chunks are queryable and checkpoint-frame restoreable saved solver payloads; event chunks, arbitrary target replay-forward, and branch-from-file are still separate remaining work.
