...
The bottom scrubber also has a `PREDICT` checkbox with `-` and `+` horizon controls, default off and defaulting to 3 seconds. The horizon clamps from 1 to 10 seconds. When enabled with a selected replay path target, the runtime runs a sandboxed solver lookahead for the selected horizon from the current live state, captures predicted positions and contact manifolds, then restores the live world before rendering the overlay. The predicted root future draws white-to-green; bodies touched by that future draw grey future-only paths and contact markers. Prediction suppresses Debug physics diagnostics while the speculative ticks run, and exposes profiler/PIX markers under `Frame/Replay/Prediction/...`, `Frame/Replay/PathVisualizer/...`, `Frame/Replay/ScrubberInput`, and `Frame/Replay/ScrubberOverlay`.
