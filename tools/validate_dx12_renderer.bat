...
findstr /X /C:"dx12_shader_manifest_warm attempted=16 complete=16 newly_published=16 cache_hits=0 stage_loads=32" "%REPO%\Profile\dx12_stdout.txt" >nul 2>&1
if errorlevel 1 (
...
findstr /X /C:"dx12_shader_manifest_cache_validation pass=1 first_attempted=16 first_complete=16 first_newly_published=16 first_cache_hits=0 first_stage_loads=32 second_newly_published=0 second_cache_hits=16 second_stage_loads=0 missing_first_stage_loads=1 missing_second_stage_loads=1 direct_stage_loads=2 reload_stage_loads=32" "%REPO%\Profile\dx12_stdout.txt" >nul 2>&1
if errorlevel 1 (
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $exe=$env:SKORE_RENDER_EXE; $repo=$env:SKORE_RENDER_REPO; $stdout=$env:SKORE_RENDER_STDOUT; $stderr=$env:SKORE_RENDER_STDERR; $timeout=[int]$env:SKORE_RENDER_TIMEOUT; $argText=$env:SKORE_RENDER_ARGS; $argv=@('--automation-hidden-window'); if(-not [string]::IsNullOrWhiteSpace($argText)){ $argv+=($argText -split ' ') }; try { $p=Start-Process -FilePath $exe -ArgumentList $argv -WorkingDirectory $repo -RedirectStandardOutput $stdout -RedirectStandardError $stderr -WindowStyle Hidden -PassThru; $waitMs=[Math]::Max(1, $timeout) * 1000; if(-not $p.WaitForExit($waitMs)){ Write-Host ('TIMEOUT: renderer process exceeded ' + $timeout + 's; killing PID ' + $p.Id); $live=Get-Process -Id $p.Id -ErrorAction SilentlyContinue; if($null -ne $live){ Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue; $p.WaitForExit(5000) | Out-Null }; exit 124 }; $p.Refresh(); exit $p.ExitCode } catch { Write-Host ('ERROR: renderer launch wrapper failed: ' + $_.Exception.Message); exit 125 }"
