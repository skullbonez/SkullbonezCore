...
| `validate_dependency_graph.bat` | Data-driven include direction, Runtime package matrix, Replay boundary, and single-project ownership fixtures/scan | ~2s |
...
| `validate_dependency_graph.bat` | Run data-only positive/negative package fixtures, scan tracked physical includes, and enforce dedicated project ownership such as `SkullbonezSource/UI` in `SKULLBONEZ_UI.vcxproj` |
