...
| `inventory_authority_free_aggregates.py [--repo .] [--self-test] [--format text\|json\|markdown] [--output path]` | Discover data-bearing structs/classes without suffix filtering; report members, behavior, stated invariants, lexical sites, and joined owner rulings; fail on an unruled, stale, malformed, or source-drifted structural/legacy review row |
