...
Summary:
  The baker compiles the complete shipping shader set with pinned inputs,
...
Glossary:
  ...
  DXIL (DirectX Intermediate Language): Compiled shader container consumed by
    the runtime and inspected here for binding/layout metadata.
  ABI (Application Binary Interface): The byte-level constant-buffer and
    resource-binding contract shared by generated metadata and runtime code.
...
def write_bytes_if_changed(path: Path, content: bytes) -> None:
    # Why: generated-header timestamps are C++ build inputs. Rewriting identical
...
def main() -> int:
    ...
    write_bytes_if_changed(manifest_path, manifest_bytes)
    write_bytes_if_changed(reflection_header_path, reflection_header_bytes)
    print(f"Shader bake PASS: {len(manifest['entries'])} stages with DXC {DXC_VERSION}.")
    return 0
