...
try:
    ...
    for label, value in [('past', 0.0), ('middle', 0.25), ('future', 1.0), ('past-again', 0.0), ('future-again', 1.0)]:
        ...
        ui = sample(label)
        seen[label] = (ui['gravityGridFirstSourceId'], ui['gravityGridFirstSourcePosition'], ui['gravityGridMinimumHeight'])
    ...
    settings = {k: ui[k] for k in ['gravityFieldHeight', 'gravityFieldOpacity', 'gravityFieldColor']}
    send('scene.save')
    sample('saved')
    saved = json.loads(fixture.read_text())['debug']['gravityField']
    assert saved == {'height': settings['gravityFieldHeight'], 'opacity': settings['gravityFieldOpacity'], 'color': 'orange'}, saved
    send('scene.reset')
    ui = sample('reloaded')
    ...
    (session / 'result.json').write_text(json.dumps({'passed': True, 'sources': [300, 200], 'checks': ['default-on', 'native-toggle-off-on', 'four-views', 'moving-wells', 'history-prediction-repeatability', 'saved-height-opacity-colour', 'ordinary-hidden', 'scene-retention']}))
    print('PASS gravity-grid native validation', flush=True)
finally:
    try:
        send('session.stop')
    finally:
