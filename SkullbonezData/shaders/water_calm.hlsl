...
float4 main_ps(VS_OUT input) : SV_TARGET
{
    ...
    if (uCinematicMode > 0.5f && uWaterMode == 4)
    {
        ...
        float3 deep = float3(0.055f, 0.30f, 0.44f);
        float3 mid = float3(0.14f, 0.58f, 0.68f);
        float3 shallow = float3(0.70f, 0.90f, 0.74f);
        waterColor = lerp(deep, mid, 0.30f + depthBand * 0.50f);
        waterColor = lerp(waterColor, shallow, shore * 0.58f);
        waterColor *= 0.94f + shard * 0.16f;
