...
float4 main_ps(VS_OUT input) : SV_TARGET
{
    ...
    if (uCinematicMode > 0.5f && uWaterMode == 4)
    {
        ...
        float3 deep = float3(0.014f, 0.095f, 0.14f);
        float3 mid = float3(0.045f, 0.23f, 0.29f);
        float3 shallow = float3(0.30f, 0.47f, 0.40f);
        waterColor = lerp(deep, mid, 0.18f + depthBand * 0.48f);
        ...
        waterColor = lerp(waterColor, waterColor * float3(0.64f, 0.78f, 0.78f), panelEdge * 0.18f);
        waterColor = lerp(waterColor, reflection.rgb, min(uReflectionStrength, 0.22f));
        ...
        waterColor += uSunColor * sunShard * 0.18f;
    }
