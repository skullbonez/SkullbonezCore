...
float4 main_ps(VS_OUT input) : SV_TARGET
{
    ...
    if (uCinematicMode > 0.5f && uWaterMode == 4)
    {
        ...
        float3 deep = float3(0.035f, 0.22f, 0.34f);
        float3 mid = float3(0.08f, 0.46f, 0.56f);
        float3 shallow = float3(0.42f, 0.76f, 0.66f);
        waterColor = lerp(deep, mid, 0.24f + depthBand * 0.54f);
        waterColor = lerp(waterColor, shallow, shore * 0.44f);
        waterColor *= 0.90f + shard * 0.15f;
        float wedge = frac(angle * 2.86479f + basinDistance * 0.30f + 0.5f);
        float panelEdge = 1.0f - smoothstep(0.0f, 0.040f, min(wedge, 1.0f - wedge));
        waterColor = lerp(waterColor, waterColor * float3(0.58f, 0.78f, 0.90f), panelEdge * 0.24f);
        waterColor = lerp(waterColor, reflection.rgb, min(uReflectionStrength, 0.16f));
        ...
        waterColor += float3(0.38f, 0.42f, 0.18f) * rimLine;
        ...
        waterColor += uSunColor * sunShard * 0.052f;
    }
