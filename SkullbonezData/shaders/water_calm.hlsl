...
float4 main_ps(VS_OUT input) : SV_TARGET
{
    ...
    if (uCinematicMode > 0.5f && uWaterMode == 4)
    {
        ...
        float3 deep = float3(0.014f, 0.15f, 0.23f);
        float3 mid = float3(0.038f, 0.34f, 0.42f);
        float3 shallow = float3(0.30f, 0.60f, 0.55f);
        waterColor = lerp(deep, mid, 0.15f + depthBand * 0.54f);
        waterColor = lerp(waterColor, shallow, shore * 0.30f);
        ...
        waterColor += uSunColor * sunShard * 0.135f;
    }
