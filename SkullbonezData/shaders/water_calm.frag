...
void main()
{
    ...
    if (uCinematicMode > 0.5 && uWaterMode == 4)
    {
        ...
        vec3 deep = vec3(0.030, 0.24, 0.31);
        vec3 mid = vec3(0.075, 0.48, 0.52);
        vec3 shallow = vec3(0.50, 0.76, 0.62);
        waterColor = mix(deep, mid, 0.24 + depthBand * 0.54);
        waterColor = mix(waterColor, shallow, shore * 0.36);
        ...
        waterColor = mix(waterColor, waterColor * vec3(0.64, 0.82, 0.86), panelEdge * 0.20);
        ...
        waterColor += uSunColor * sunShard * 0.105;
    }
