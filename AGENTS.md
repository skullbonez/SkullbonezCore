...
- **An automated physics-golden override is allowed if and only if the repository owner pre-approves that automation.** The pre-approval must be a user-authored instruction given before the agent invokes the override and must identify the exact candidate SHA-256 or explicitly authorize the exact candidate produced by the named Physics validation run. The override is single-use and content-bound: `--owner-approved-sha256` must exactly match the complete generated artifact, and any different or later candidate requires fresh pre-approval. Without that pre-approval, the interactive phrase remains mandatory. The same commit must retain the matching final Debug and Profile executables, plus the DLLs needed to launch them, under the owning Physics plan's artifact directory so the owner can reproduce or roll back behavior later; a manifest records hashes and the approved golden set.
