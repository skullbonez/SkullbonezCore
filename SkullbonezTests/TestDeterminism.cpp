...
{
  public:
    explicit DeterminismTerrainFixture( float terrainBaseY ) : m_config( MakeDeterministicConfig() ), m_terrain( terrainBaseY, 0.0f, 0.0f, m_config ), m_engine( std::make_unique<PhysicsEngine>() )
    {
...
void AddPromotionFixtureBody( PhysicsEngine& engine,
                              PhysicsTerrainView terrainView,
...
void AddMicroBody( PhysicsEngine& engine, PhysicsTerrainView terrainView, uint32_t sceneObjectIdValue, const Vector3& position, const Vector3& linearVelocity )
{
...
void AddSupportedSleepBody( PhysicsEngine& engine, PhysicsTerrainView terrainView, uint32_t sceneObjectIdValue, const Vector3& position, std::size_t pointJointCapacity = 0u )
{
...
void SeedSupportedSleepWorld( PhysicsEngine& engine, PhysicsTerrainView terrainView, const SkullbonezCore::Core::EngineConfig& config )
{
...
void SeedParallelContactSleepWorld( PhysicsEngine& engine, PhysicsTerrainView terrainView, const SkullbonezCore::Core::EngineConfig& config )
{
    engine.Clear();
    engine.ApplyRuntimeConfig( config );
    engine.SetSleepEnabled( true );
    ...
    for ( int bodyIndex = 0; bodyIndex < kParallelContactBodyCount; ++bodyIndex )
    {
        ...
        AddSupportedSleepBody( engine,
                               ...
                               Vector3( static_cast<float>( pairColumn * 8 - 76 ) + pairOffset, 1.0f, static_cast<float>( pairRow * 8 - 48 ) ),
                               2u );
    }
...
TEST_CASE( "Physics collision-time diagnostics cover every bounded fixed-step event" )
{
    ...
    CHECK( SkullbonezCore::Physics::PhysicsDiagnosticsSink::CollisionTimeEventCapacity() == SkullbonezCore::Scene::Capacity::MAX_SCENE_OBJECTS * 5 );
}
...
TEST_CASE( "Tornado force witness preserves exact one-step body state" )
{
    ...
    engine.Step( PHYSICS_FIXED_DT, NoGravityForces(), tornadoGameplay.BuildForceFrame( PHYSICS_FIXED_DT, 1 ), workers, SkullbonezCore::Physics::PhysicsDiagnosticsCsvWriter {} );
...
TEST_CASE( "Tornado lifecycle clock advances beyond the float fixed-step boundary" )
{
    ...
    const double expectedRestoreWobble = kExactFloatRestoreSeconds * static_cast<double>( driftConfig.vortices[0].driftSpeed ) * 0.67 + static_cast<double>( kTwoPi ) * 0.19;
    const float expectedRestoreX = driftConfig.vortices[0].field.center.x + static_cast<float>( std::cos( expectedRestorePhase ) * driftConfig.vortices[0].driftRadius + std::cos( expectedRestoreWobble ) * driftConfig.vortices[0].driftRadius * 0.34 );
    const float expectedRestoreZ = driftConfig.vortices[0].field.center.z + static_cast<float>( std::sin( expectedRestorePhase ) * driftConfig.vortices[0].driftRadius + std::sin( expectedRestoreWobble ) * driftConfig.vortices[0].driftRadius * 0.34 );
...
void SeedAuthoredSolarWorld( PhysicsEngine& engine, PhysicsTerrainView terrainView, const SkullbonezCore::Runtime::AuthoredScene& scene, bool earthGravityEnabled )
{
...
void CheckMutualGravityFieldExactAcrossWorkerCounts( int bodyCount, int ticks )
{
    ...
    auto seedField = [&config, bodyCount]( PhysicsEngine& engine, PhysicsTerrainView terrainView )
    {
        ...
        for ( int index = 0; index < bodyCount; ++index )
        {
            ...
            // holes in worker slices and exercise prefix reduction, while
            // larger cases cross bounded batch boundaries with the same order.
            const bool createReceiverGap = bodyCount == kParallelMutualGravityBodyCount && index % 9 < 2;
            // Large fixtures stay off cell boundaries with small spheres so
            // the unchanged 8,192-bucket grid can hold their separated bodies.
            const Vector3 cellOffset = bodyCount > kLargeMutualGravityBodyCount ? Vector3( 0.25f, 0.25f, 0.25f ) : Vector3( 0.0f, 0.0f, 0.0f );
            AddMutualGravityBody( engine,
                                  terrainView,
                                  static_cast<uint32_t>( 300u + index ),
                                  Vector3( static_cast<float>( column * 20 - 70 ), static_cast<float>( 100 + row * 17 ), static_cast<float>( ( index * 13 ) % 29 - 14 ) ) + cellOffset,
                                  Vector3( static_cast<float>( ( index % 5 ) - 2 ) * 0.03f, static_cast<float>( ( index % 3 ) - 1 ) * 0.02f, static_cast<float>( ( index % 7 ) - 3 ) * 0.01f ),
                                  1.0f + static_cast<float>( index % 11 ) * 0.25f,
                                  bodyCount > kLargeMutualGravityBodyCount ? 0.1f : 0.5f,
...
void SeedPredictiveBoundaryGroup( PhysicsEngine& engine, PhysicsTerrainView terrainView, const Vector3& origin, int group )
{
    ...
    for ( int part = 0; part < 4; ++part )
    {
        ...
        const Vector3 offset = grazing ? ( fixed ? Vector3( 0, 0, 0 ) : Vector3( -4, 1.99f, 0.3f ) ) : Vector3( fixed ? 2.25f : 0.0f, 0, 0 );
        const Vector3 velocity = fixed ? Vector3( 0, 0, 0 ) : Vector3( ( grazing ? 8.0f : 0.25f ) / PHYSICS_FIXED_DT, 0, 0 );
        AddPromotionFixtureBody( engine,
                                 terrainView,
                                 static_cast<uint32_t>( 22000 + group * 4 + part ),
                                 sphere,
                                 origin + offset + Vector3( 0, 0, part >= 2 ? 8.0f : 0.0f ),
                                 velocity,
...
void CheckPredictiveBoundaryContacts( const PhysicsEngine& engine, int tick )
{
    ...
    {
        const auto row = std::find_if( rows.begin(), rows.end(), [body]( const auto& contact ) { return contact.bodyA == body && contact.bodyB == body + 1; } );
        if ( tick == 0 || body >= 516 )
        {
            CHECK( row == rows.end() );
        }
...
void SeedPredictiveRotatingGroup( PhysicsEngine& engine, PhysicsTerrainView terrainView, const Vector3& origin, int group )
{
    for ( int part = 0; part < 4; ++part )
    {
        ...
        AddPromotionFixtureBody( engine,
                                 terrainView,
                                 static_cast<uint32_t>( 22000 + body ),
                                 shape,
                                 origin + Vector3( wall ? 1.3f : 0.0f, 0, part >= 2 ? 8.0f : 0.0f ),
                                 Vector3( 0, 0, 0 ),
                                 Vector3( 4.0f / 3.0f, 4.0f / 3.0f, 4.0f / 3.0f ),
                                 2.0f,
                                 0.0f,
                                 wall ? PhysicsBodyMotionKind::Fixed : PhysicsBodyMotionKind::Dynamic,
                                 "predictive-worker" );
        if ( !wall )
        {
            REQUIRE( engine.SetBodyVelocity( RequireBodyHandle( engine, body ), Vector3( 0, 0, 0 ), Vector3( 0, 0, -100 ), true ) );
        }
    }
}
...
bool DiagnosticsHasPipelineStageForPair( const PhysicsEngine& engine, SkullbonezCore::Physics::PhysicsPipelineStage stage, int bodyA, int bodyB )
{
    for ( const SkullbonezCore::Physics::PhysicsPipelineRecord& record : engine.GetDiagnosticsView().physicsPipelineTrace )
    {
        const bool samePair = ( record.bodyA == bodyA && record.bodyB == bodyB ) || ( record.bodyA == bodyB && record.bodyB == bodyA );
...
void CheckEngineKinematicsEqual( const PhysicsEngine& lhs, const PhysicsEngine& rhs )
{
    REQUIRE( SkullbonezCore::Physics::PhysicsEngine::ReadBodies( lhs ).Count() == SkullbonezCore::Physics::PhysicsEngine::ReadBodies( rhs ).Count() );
...
void CheckContactIdentityOrderEqual( const PhysicsEngine& lhs, const PhysicsEngine& rhs )
{
    ...
    for ( std::size_t index = 0; index < left.size(); ++index )
    {
        ...
        const std::array leftScalars { a.penetration, a.normalMass, a.tangentMass1, a.tangentMass2, a.bias, a.separationBias, a.accN, a.accT1, a.accT2, a.accRollingT1, a.accRollingT2, a.accSpin };
        const std::array rightScalars { b.penetration, b.normalMass, b.tangentMass1, b.tangentMass2, b.bias, b.separationBias, b.accN, b.accT1, b.accT2, b.accRollingT1, b.accRollingT2, b.accSpin };
...
void CheckTerrainManifoldOrderEqual( const PhysicsEngine& lhs, const PhysicsEngine& rhs )
{
    ...
    for ( std::size_t manifoldIndex = 0; manifoldIndex < left.size(); ++manifoldIndex )
    {
        ...
        for ( uint8_t pointIndex = 0; pointIndex < leftManifold.pointCount; ++pointIndex )
        {
            ...
            CHECK( std::memcmp( &leftPoint.penetration, &rightPoint.penetration, sizeof( leftPoint.penetration ) ) == 0 );
            CHECK( leftPoint.featureId == rightPoint.featureId );
        }

        CHECK( std::memcmp( &leftManifold.timeOfImpact, &rightManifold.timeOfImpact, sizeof( leftManifold.timeOfImpact ) ) == 0 );
...
void CheckPointJointOrderEqual( const PhysicsEngine& lhs, const PhysicsEngine& rhs )
{
    ...
    for ( std::size_t index = 0; index < left.size(); ++index )
    {
        ...
        CHECK( std::memcmp( &leftJoint.accumulatedImpulse, &rightJoint.accumulatedImpulse, sizeof( leftJoint.accumulatedImpulse ) ) == 0 );
        CHECK( leftJoint.groupId == rightJoint.groupId );
        CHECK( leftJoint.flags == rightJoint.flags );
    }
}

template <typename LeftRows, typename RightRows> void CheckDeterministicRowsEqual( const LeftRows& left, const RightRows& right )
{
    REQUIRE( left.size() == right.size() );
...
void CheckEngineWorkerDeterministicStateEqual( const PhysicsEngine& lhs, const PhysicsEngine& rhs )
{
    ...
    CheckDeterministicRowsEqual( SkullbonezCore::Physics::PhysicsEngine::ReadSleepStates( lhs ), SkullbonezCore::Physics::PhysicsEngine::ReadSleepStates( rhs ) );

    CheckDeterministicRowsEqual( SkullbonezCore::Physics::PhysicsEngine::ReadSleepSupportedStates( lhs ), SkullbonezCore::Physics::PhysicsEngine::ReadSleepSupportedStates( rhs ) );

    CheckDeterministicRowsEqual( SkullbonezCore::Physics::PhysicsEngine::ReadSleepInhibitedStates( lhs ), SkullbonezCore::Physics::PhysicsEngine::ReadSleepInhibitedStates( rhs ) );

    CheckDeterministicRowsEqual( SkullbonezCore::Physics::PhysicsEngine::ReadSleepIslandVisualIds( lhs ), SkullbonezCore::Physics::PhysicsEngine::ReadSleepIslandVisualIds( rhs ) );
    ...
    CHECK( leftDiagnostics.motionEligibilityStats.evaluatedBodies == rightDiagnostics.motionEligibilityStats.evaluatedBodies );
    CHECK( leftDiagnostics.motionEligibilityStats.discreteBodies == rightDiagnostics.motionEligibilityStats.discreteBodies );
    CHECK( leftDiagnostics.motionEligibilityStats.promotedBodies == rightDiagnostics.motionEligibilityStats.promotedBodies );
    CHECK( leftDiagnostics.motionEligibilityStats.angularExpandedBodies == rightDiagnostics.motionEligibilityStats.angularExpandedBodies );
    CHECK( leftDiagnostics.motionEligibilityStats.speculativeEnabled == rightDiagnostics.motionEligibilityStats.speculativeEnabled );
    CHECK( leftDiagnostics.motionEligibilityStats.articulatedBodies == rightDiagnostics.motionEligibilityStats.articulatedBodies );
    CHECK( leftDiagnostics.motionEligibilityStats.speculativeBodies == rightDiagnostics.motionEligibilityStats.speculativeBodies );
...
TEST_CASE( "Physics motion promotion: fixed-step force integration promotes in the same tick" )
{
    ...
    CHECK( ( diagnostics.motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
...
TEST_CASE( "Physics motion promotion: authoritative 200-box striker stays Discrete below its radius" )
{
    ...
    const SkullbonezCore::Core::SbResult loadResult = SkullbonezCore::Runtime::AuthoredScene::TryLoadFromFile( resultDiagnostics, "SkullbonezData/scenes/prediction_ragdoll_wall_200.scene.json", scene );
    INFO( std::string( loadResult.ErrorOwner() ) );
    INFO( std::string( loadResult.ErrorMessage() ) );
    ...
    AddPromotionFixtureBody( engine,
                             fixture.TerrainView(),
                             3201u,
                             MakeSphereShape( striker->radius ),
                             Vector3( striker->posX, striker->posY, striker->posZ ),
                             authoredVelocity,
                             Vector3( striker->inertiaX, striker->inertiaY, striker->inertiaZ ),
                             striker->mass,
                             striker->restitution,
                             PhysicsBodyMotionKind::Dynamic,
                             "generic-fast-sphere" );
...
TEST_CASE( "PhysicsEngine terrain fixtures reconstruct without cross-instance state" )
{
    const auto runColdFixture = []( float terrainBaseY )
    {
        ...
        engine.SetSleepEnabled( false );
        AddMicroBody( engine, fixture.TerrainView(), 501u, Vector3( 0.0f, 2.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ) );
        AddMicroBody( engine, fixture.TerrainView(), 502u, Vector3( 4.0f, 4.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ) );
        AddMicroBody( engine, fixture.TerrainView(), 503u, Vector3( 8.0f, 6.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ) );
    ...
    const std::array<uint64_t, 7> reconstructionHashes = { runColdFixture( kFlatTerrainBaseY ),
...
TEST_CASE( "PhysicsEngine multithreaded determinism: contact and sleep pipeline is exact across worker counts" )
{
    ...
    CHECK( ( serialDiagnostics.motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
...
TEST_CASE( "Tornado external-force lane is byte-exact across serial and parallel body partitions" )
{
    ...
    for ( int index = 0; index < bodyCount; ++index )
    {
        const Vector3 position( -95.0f + static_cast<float>( index % 20 ) * 10.0f, 20.0f + static_cast<float>( index / 20 ) * 5.0f, -40.0f + static_cast<float>( index % 9 ) * 10.0f );
        ...
        AddMicroBody( parallel, parallelFixture.TerrainView(), 5000u + static_cast<uint32_t>( index ), position, zeroVelocity );
    }
    ...
    serial.Step( PHYSICS_FIXED_DT, NoGravityForces(), serialGameplay.BuildForceFrame( PHYSICS_FIXED_DT, bodyCount ), serialWorkers, SkullbonezCore::Physics::PhysicsDiagnosticsCsvWriter {} );

    parallel.Step( PHYSICS_FIXED_DT, NoGravityForces(), parallelGameplay.BuildForceFrame( PHYSICS_FIXED_DT, bodyCount ), parallelWorkers, SkullbonezCore::Physics::PhysicsDiagnosticsCsvWriter {} );
...
TEST_CASE( "Physics motion promotion: sub-threshold opposing tiny balls retain geometry-selected Swept TOI" )
{
    ...
    AddPromotionFixtureBody( engine,
                             ...
                             3011u,
                             sphere,
                             Vector3( initialCenterSeparation * -0.5f, 0.0f, 0.0f ),
                             Vector3( speed, 0.0f, 0.0f ),
                             Vector3( inertia, inertia, inertia ),
                             mass,
                             1.0f,
    ...
    REQUIRE( diagnostics.motionEligibilityState.size() == 2u );
    CHECK( ( diagnostics.motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
    CHECK( ( diagnostics.motionEligibilityState[1] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
...
TEST_CASE( "Physics direction-valid promotion keeps sub-radius bodies out of the swept overlay" )
{
    ...
    SUBCASE( "sub-radius travel remains Discrete without a motion overlay" )
    {
        radius = 0.1f;
        speed = 9.0f;
        shouldPromote = false;
    ...
    SUBCASE( "small fast body promotes and retains a motion overlay" )
    {
        radius = 0.01f;
        speed = 4.8f;
        shouldPromote = true;
    ...
    AddPromotionFixtureBody( engine,
                             ...
                             0.0f,
                             PhysicsBodyMotionKind::Dynamic,
                             "directional-radius-overlay-probe" );

    StepMicroWorldWith( engine, 1, NoGravityForces() );
    const auto diagnostics = engine.GetDiagnosticsView();
    REQUIRE( diagnostics.motionEligibilityState.size() == 1u );
    CHECK( diagnostics.motionEligibilityStats.policyVersion == SkullbonezCore::Physics::PHYSICS_MOTION_ELIGIBILITY_POLICY_VERSION );
    CHECK( ( ( diagnostics.motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u ) == shouldPromote );

    const auto& grid = diagnostics.spatialGrid;
    const bool emittedSweptCoverage = grid.GetMaintenanceStats().sweptOverlayCellsAdded > 0 || grid.GetSweptFallbackBodyCount() > 0u;
...
TEST_CASE( "Physics sub-radius opposing balls resolve at the next discrete boundary" )
{
    ...
    AddPromotionFixtureBody( engine,
                             ...
                             1.0f,
                             PhysicsBodyMotionKind::Dynamic,
                             "radius-discrete-right" );

    StepMicroWorldWith( engine, 1, NoGravityForces() );
    const auto firstDiagnostics = engine.GetDiagnosticsView();
    ...
    const PhysicsBodyHotState firstLeft = RequireBodyHotState( engine, 0 );
    const PhysicsBodyHotState firstRight = RequireBodyHotState( engine, 1 );
    CHECK( firstLeft.position.x < firstRight.position.x );
    CHECK( firstRight.position.x - firstLeft.position.x < radius * 2.0f );
...
TEST_CASE( "Physics motion promotion: resting static-friction pair stays Discrete" )
{
    ...
    AddPromotionFixtureBody( engine,
                             ...
                             3021u,
                             dynamicBox,
                             Vector3( 0.0f, 0.99f, 0.0f ),
                             Vector3( initialTangentialSpeed, 0.0f, 0.0f ),
                             Vector3( 1.0f, 1.0f, 1.0f ),
                             1.0f,
                             0.0f,
                             PhysicsBodyMotionKind::Dynamic,
                             "resting-friction-box" );
    AddPromotionFixtureBody( engine,
                             fixture.TerrainView(),
                             3022u,
                             supportBox,
...
TEST_CASE( "Physics motion promotion: tilted terrain box keeps support long enough to sleep while Discrete" )
{
    ...
    const SkullbonezCore::Math::Orientation::Quaternion orientation( -0.010159f, -0.006545f, -0.000461f, 0.999927f );
    auto bodyDesc = MakePhysicsBodyCreateDesc( PhysicsSceneObjectId { 3031u },
                                               shape,
                                               Vector3( 592.0f, 1.5004f, 472.5f ),
                                               orientation,
                                               Vector3( 0.0f, -0.0001f, 0.0f ),
                                               Vector3( 0.0f, 0.0f, 0.0f ),
                                               Vector3( 32.42f, 29.3f, 13.7f ),
                                               12.0f,
                                               0.08f,
                                               PhysicsBodyMotionKind::Dynamic,
                                               "tilted-discrete-terrain-box" );
...
TEST_CASE( "Physics motion promotion: collision-promoted target uses Swept TOI against thin wall next tick" )
{
    ...
    AddPromotionFixtureBody( engine,
                             ...
                             Vector3( ballInertia, ballInertia, ballInertia ),
                             ballMass,
                             1.0f,
                             PhysicsBodyMotionKind::Dynamic,
                             "promotion-striker" );
    ...
    CHECK( ( firstTick.motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
    ...
    CHECK( ( secondTick.motionEligibilityState[1] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
...
void CheckPredictiveParallelWork( const PhysicsEngine& engine )
{
    ...
    CHECK( std::count_if( diagnostics.persistentContacts.begin(), diagnostics.persistentContacts.end(), []( const auto& row ) { return row.penetration < 0.0f && row.accN > 0.0f; } ) >= 256 );
}
} // namespace
...
TEST_CASE( "Predictive articulation is byte-exact across workers restore and joint topology changes" )
{
    ...
    for ( int tick = 0; tick < 12; ++tick )
    {
        ...
        if ( tick == 2 )
        {
            ...
            {
                restoredBodies.push_back( CaptureBodyReplayState( RequireBodyRecord( four, index ), RequireBodyHotState( four, index ) ) );
            }
        }
    }
    for ( const auto& state : restoredBodies )
    {
        REQUIRE( four.RestoreReplayBodyState( PhysicsBodyRestoreState { state.handle,
                                                                        state.sceneObjectId,
                                                                        state.fixed,
                                                                        state.position,
                                                                        state.orientation,
...
TEST_CASE( "PhysicsEngine mutual gravity: pair force is antisymmetric" )
{
    ...
    SeedTwoBodyGravityWorld( pairWorld, fixture.TerrainView(), Vector3( -12.0f, 80.0f, 0.0f ), Vector3( 12.0f, 80.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ), 5.0f, 0.5f );
...
TEST_CASE( "PhysicsEngine mutual gravity: softening keeps near pairs finite" )
{
    ...
    SeedTwoBodyGravityWorld( closeWorld, fixture.TerrainView(), Vector3( 0.0f, 80.0f, 0.0f ), Vector3( 0.03f, 80.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ), Vector3( 0.0f, 0.0f, 0.0f ), 3.0f, 0.01f );
...
TEST_CASE( "PhysicsEngine mutual gravity: equal-mass two-body orbit stays bounded" )
{
    ...
    const float acceleration = gravitationalConstant * mass * separation / ( softenedDistanceSq * sqrtf( softenedDistanceSq ) );
...
{
    SkullbonezCore::Runtime::AuthoredScene scene;
    REQUIRE( SkullbonezTests::ResultLoadFixtures::TryLoadAuthoredScene( resultDiagnostics, "SkullbonezData/scenes/solar_system_mars_slingshot.scene.json", scene ) );
    ...
    std::array<MoonOrbitProbe, 22> moonOrbits = { MoonOrbitProbe { "moon", "earth" },
                                                  MoonOrbitProbe { "phobos", "mars" },
                                                  MoonOrbitProbe { "deimos", "mars" },
                                                  MoonOrbitProbe { "io", "jupiter" },
                                                  MoonOrbitProbe { "europa", "jupiter" },
                                                  MoonOrbitProbe { "ganymede", "jupiter" },
                                                  MoonOrbitProbe { "callisto", "jupiter" },
                                                  MoonOrbitProbe { "mimas", "saturn" },
                                                  MoonOrbitProbe { "enceladus", "saturn" },
                                                  MoonOrbitProbe { "tethys", "saturn" },
                                                  MoonOrbitProbe { "dione", "saturn" },
                                                  MoonOrbitProbe { "rhea", "saturn" },
                                                  MoonOrbitProbe { "titan", "saturn" },
                                                  MoonOrbitProbe { "iapetus", "saturn" },
                                                  MoonOrbitProbe { "miranda", "uranus" },
                                                  MoonOrbitProbe { "ariel", "uranus" },
                                                  MoonOrbitProbe { "umbriel", "uranus" },
                                                  MoonOrbitProbe { "titania", "uranus" },
                                                  MoonOrbitProbe { "oberon", "uranus" },
                                                  MoonOrbitProbe { "proteus", "neptune" },
                                                  MoonOrbitProbe { "triton", "neptune" },
                                                  MoonOrbitProbe { "nereid", "neptune" }, };
    ...
    for ( MoonOrbitProbe& orbit : moonOrbits )
    {
        ...
        orbit.initialDistance = sqrtf( VectorMagnitudeSquared( RequireBodyHotState( live, orbit.moonIndex ).position - RequireBodyHotState( live, orbit.parentIndex ).position ) );
    ...
    for ( int tick = 0; tick < kPredictionTicks; ++tick )
    {
        ...
        closestMarsWithoutEarthGravity = (std::min)( closestMarsWithoutEarthGravity, sqrtf( VectorMagnitudeSquared( noEarthRocket.position - noEarthMars.position ) ) );
        ...
        for ( MoonOrbitProbe& orbit : moonOrbits )
        {
            const float parentRelativeDistance = sqrtf( VectorMagnitudeSquared( RequireBodyHotState( live, orbit.moonIndex ).position - RequireBodyHotState( live, orbit.parentIndex ).position ) );
...
TEST_CASE( "PhysicsEngine mutual gravity: chaotic triple is deterministic" )
{
    ...
    auto seedTriple = [&config]( PhysicsEngine& engine, PhysicsTerrainView terrainView )
    {
        ...
        AddMutualGravityBody( engine, terrainView, 301u, Vector3( -18.0f, 90.0f, 0.0f ), Vector3( 0.8f, 0.0f, -1.0f ), 12.0f, 0.45f );

        AddMutualGravityBody( engine, terrainView, 302u, Vector3( 16.0f, 90.0f, 4.0f ), Vector3( -0.4f, 0.0f, 1.1f ), 16.0f, 0.45f );

        AddMutualGravityBody( engine, terrainView, 303u, Vector3( 2.0f, 90.0f, 24.0f ), Vector3( -0.2f, 0.0f, -0.8f ), 10.0f, 0.45f );
...
TEST_CASE( "PhysicsEngine mutual gravity: large batches preserve exact multi-tick state across workers" )
{
    for ( int bodyCount : { 511, 512, 513, kLargeMutualGravityBodyCount, 1025 } )
    {
        CAPTURE( bodyCount );
        CheckMutualGravityFieldExactAcrossWorkerCounts( bodyCount, 8 );
    }
}
...
TEST_CASE( "PhysicsEngine replay restore preserves promoted impact and demotion paths byte-exactly" )
{
    ...
    const auto seedWorld = [&]( PhysicsEngine& engine, PhysicsTerrainView terrainView, uint32_t idBase )
    {
        ...
        AddPromotionFixtureBody( engine,
                                 ...
                                 sphere,
                                 Vector3( 0.0f, 0.0f, 0.0f ),
                                 Vector3( primeTravel / PHYSICS_FIXED_DT, 0.0f, 0.0f ),
                                 Vector3( inertia, inertia, inertia ),
                                 mass,
                                 0.0f,
                                 PhysicsBodyMotionKind::Dynamic,
                                 "replay-promoted-ball" );
        AddPromotionFixtureBody( engine,
                                 terrainView,
                                 idBase + 1u,
                                 wall,
    ...
    REQUIRE( ( reference.GetDiagnosticsView().motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );

    const Vector3 retainedVelocity( retainedTravel / PHYSICS_FIXED_DT, 0.0f, 0.0f );
    CHECK( retainedTravel == radius );
    REQUIRE( reference.SetBodyVelocity( RequireBodyHandle( reference, 0 ), retainedVelocity, Vector3( 0.0f, 0.0f, 0.0f ), true ) );
    ...
    for ( int bodyIndex = 0; bodyIndex < 2; ++bodyIndex )
    {
        ...
        REQUIRE( record != nullptr );
        bodyStates[static_cast<std::size_t>( bodyIndex )] = CaptureBodyReplayState( *record, RequireBodyHotState( restored, bodyIndex ) );
    }
    ...
    REQUIRE( ( solverSnapshot.motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );

    StepMicroWorldWith( restored, 1, forces );
    REQUIRE( DiagnosticsHasPipelineStageForPair( restored, SkullbonezCore::Physics::PhysicsPipelineStage::SweptObjectHit, 0, 1 ) );
    StepMicroWorldWith( restored, 1, forces );
    REQUIRE( restored.GetDiagnosticsView().motionEligibilityState.size() == 2u );
    REQUIRE( ( restored.GetDiagnosticsView().motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) == 0u );
    ...
    {
        REQUIRE( restored.RestoreReplayBodyState( PhysicsBodyRestoreState { state.handle,
                                                                            state.sceneObjectId,
                                                                            state.fixed,
                                                                            state.position,
                                                                            state.orientation,
    ...
    CheckEngineKinematicsEqual( reference, restored );
    REQUIRE( reference.GetDiagnosticsView().motionEligibilityState.size() == 2u );
    CHECK( ( reference.GetDiagnosticsView().motionEligibilityState[0] & SkullbonezCore::Physics::PhysicsMotionEligibilityLinearPromoted ) != 0u );
    REQUIRE( DiagnosticsHasPipelineStageForPair( reference, SkullbonezCore::Physics::PhysicsPipelineStage::SweptObjectHit, 0, 1 ) );
    REQUIRE( DiagnosticsHasPipelineStageForPair( restored, SkullbonezCore::Physics::PhysicsPipelineStage::SweptObjectHit, 0, 1 ) );
    ...
    CheckEngineKinematicsEqual( reference, restored );
    REQUIRE( reference.GetDiagnosticsView().motionEligibilityState.size() == 2u );
    CHECK( reference.GetDiagnosticsView().motionEligibilityState[0] == SkullbonezCore::Physics::PhysicsMotionEligibilityNone );
    CHECK_FALSE( DiagnosticsHasPipelineStageForPair( reference, SkullbonezCore::Physics::PhysicsPipelineStage::SweptObjectHit, 0, 1 ) );
    CHECK_FALSE( DiagnosticsHasPipelineStageForPair( reference, SkullbonezCore::Physics::PhysicsPipelineStage::SweptObjectMiss, 0, 1 ) );
}
