// Why: a missing clock or identical bounds has no meaningful interpolation
// interval, so publish the destination without arming animation state.// Invariant: maximize captures restore bounds before
// replacing them; restore then
// clamps that snapshot to the current screen rather than trusting stale dimensions.// Invariant: title fitting preserves the
// newest suffix and always terminates in the caller's bounded buffer, falling back to an ellipsis when nothing fits./*
            ...
            namespace SkullbonezCore
{
    ...
    {
        const bool sameBounds = std::fabs( from.x - to.x ) < 0.5f && std::fabs( from.y - to.y ) < 0.5f &&
                                std::fabs( from.w - to.w ) < 0.5f && std::fabs( from.h - to.h ) < 0.5f;
    ...
    {
        return { { windowBounds.x + windowBounds.w - 112.0f, windowBounds.y + 8.0f, 30.0f, 28.0f },
    ...
    void DrawWindowAnimationShell( const UIDrawContext& draw, const UIRect& bounds )
    {
        ...
        draw.RoundedRect( bounds.x + 4.0f, bounds.y + 6.0f, bounds.w, bounds.h, Style::Radii().window, 0.0f, 0.0f, 0.0f,
                          0.26f );
    ...
    void DrawMinimizedWindow( const UIDrawContext& draw, const UIRect& minimized, const char* titleText )
    {
        ...
        draw.RoundedRect( minimized.x + 4.0f, minimized.y + 5.0f, minimized.w, minimized.h, Style::Radii().window, 0.0f,
                          0.0f, 0.0f, 0.26f );
                          ...
                          const char* titleText )
    {
        ...
        draw.RoundedRect( bounds.x + 5.0f, bounds.y + 7.0f, bounds.w, bounds.h, Style::Radii().window, 0.0f, 0.0f, 0.0f,
                          0.28f );
        ...
        draw.Rect( bounds.x + 16.0f, bounds.y + titleH + tabH, bounds.w - 32.0f, 1.0f, palette.lineSoft.r,
                           ...
                           int mouseY )
    {
        ...
        Widgets::DrawTitleButton( draw, buttons.close, Widgets::TitleButtonIcon::Close,
                                  buttons.close.Contains( mouseX, mouseY ), false );
    }
