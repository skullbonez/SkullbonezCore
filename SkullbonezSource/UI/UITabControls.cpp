                         ...
                         int lastSolverBoxCount )
{
    ...
    const int displayBalls = std::clamp( state.previewSolverBallCount >= 0 ? state.previewSolverBallCount
                                                                           : lastSolverBallCount,
                                         UI_SOLVER_COUNT_MIN, modelCapacity );
    ...
    if ( state.seedSlider.HitTest( mouseX, mouseY ) )
    {
        ...
        result.commands.run.requestedSeed = static_cast<int>( state.seedSlider.ValueFromMouse( mouseX, static_cast<float>( UI_SEED_MIN ), static_cast<float>( UI_SEED_MAX ),
                                                                                               1.0f ) );
    ...
    if ( state.solverBallSlider.HitTest( mouseX, mouseY ) )
    {
        ...
        state.previewSolverBallCount = static_cast<int>( state.solverBallSlider.ValueFromMouse( mouseX, static_cast<float>( UI_SOLVER_COUNT_MIN ),
                                                                                                static_cast<float>( maxBalls ), 1.0f ) );
    ...
    if ( state.solverBoxSlider.HitTest( mouseX, mouseY ) )
    {
        ...
        state.previewSolverBoxCount = static_cast<int>( state.solverBoxSlider.ValueFromMouse( mouseX, static_cast<float>( UI_SOLVER_COUNT_MIN ),
                                                                                              static_cast<float>( maxBoxes ), 1.0f ) );
    ...
    if ( state.worldFluidHeightSlider.HitTest( mouseX, mouseY ) )
    {
        ...
        result.commands.water.requestedWorldFluidHeight = state.worldFluidHeightSlider
                                                              .ValueFromMouse( mouseX, UI_WORLD_FLUID_HEIGHT_MIN,
    ...
    if ( state.worldFluidDensitySlider.HitTest( mouseX, mouseY ) )
    {
        ...
        result.commands.water.requestedWorldFluidDensity = state.worldFluidDensitySlider
                                                               .ValueFromMouse( mouseX, UI_WORLD_FLUID_DENSITY_MIN,
           ...
           float contentY, float contentW, float contentH, float scrolledY )
{
    ...
    const int displaySolverBalls = std::clamp( state.previewSolverBallCount >= 0 ? state.previewSolverBallCount
                                                                                 : data.solverBallCount,
                                               UI_SOLVER_COUNT_MIN, modelCapacity );

    const int displaySolverBoxes = std::clamp( state.previewSolverBoxCount >= 0 ? state.previewSolverBoxCount
                                                                                : data.solverBoxCount,
                                               UI_SOLVER_COUNT_MIN, modelCapacity );
    ...
    {
        state.seedSlider.Draw( draw, "Seed", buf, static_cast<float>( displaySeed ), static_cast<float>( UI_SEED_MIN ),
                               static_cast<float>( UI_SEED_MAX ) );
    }

    if ( IsRowVisible( contentY, contentH, scrolledY + 104.0f, 18.0f ) )
    {
        DrawSectionTitle( draw, contentX, contentY, contentH, scrolledY + 104.0f, 12.0f, "Scene Objects" );
    }
    ...
    {
        state.solverBallSlider.Draw( draw, "Balls", buf, static_cast<float>( displaySolverBalls ),
                                     static_cast<float>( UI_SOLVER_COUNT_MIN ), static_cast<float>( displayBallMax ) );
    }
    ...
    {
        state.solverBoxSlider.Draw( draw, "Boxes", buf, static_cast<float>( displaySolverBoxes ),
                                    static_cast<float>( UI_SOLVER_COUNT_MIN ), static_cast<float>( displayBoxMax ) );
    }
    ...
    {
        state.worldFluidHeightSlider.Draw( draw, "Fluid height", buf, data.worldFluidHeight, UI_WORLD_FLUID_HEIGHT_MIN,
                                           UI_WORLD_FLUID_HEIGHT_MAX );
    }
    ...
    {
        state.worldFluidDensitySlider.Draw( draw, "Fluid density", buf, data.worldFluidDensity, UI_WORLD_FLUID_DENSITY_MIN,
                                            UI_WORLD_FLUID_DENSITY_MAX );
    }
}
