...
void DrawDescription( const UIDrawContext& draw, const UIRect& bounds, const char* text )
{
    ...
    for ( int row = 0; row < 3 && *cursor; ++row )
    {
        ...
        draw.Text( bounds.x + 10.0f, bounds.y + 10.0f + static_cast<float>( row ) * 15.0f, 11.0f, palette.textPrimary.r,
                   palette.textPrimary.g, palette.textPrimary.b, line );
        cursor += count;
        while ( *cursor == ' ' )
        {
            ++cursor;
...
{
    return m_target.id != 0 && std::isfinite( now ) && now >= m_hoverStarted &&
           ( m_target.focused || now - m_hoverStarted >= 0.45 );
}

UIRect UITooltip::Bounds( const UIRect& window ) const
{
    const float width = (std::min)( 360.0f, (std::max)( 0.0f, window.w ) );
    const char* description = m_target.enabled || !HasText( m_target.text.disabledReason ) ? m_target.text.action
                                                                                           : m_target.text.disabledReason;
    const bool singleLine = !HasText( description ) ||
                            UIFontMetrics::MeasureText( 11.0f, description ) <= (std::max)( 0.0f, width - 20.0f );
    const float descriptionHeight = singleLine ? 15.0f : 45.0f;
    const float detailHeight = ( HasText( m_target.text.units ) ? 15.0f : 0.0f ) +
                               ( HasText( m_target.text.shortcut ) ? 15.0f : 0.0f );
...
void UITooltip::Draw( const UIDrawContext& draw, const UIRect& window, double now ) const
{
    ...
    DrawDescription( draw, bounds,
                     m_target.enabled || !HasText( m_target.text.disabledReason ) ? m_target.text.action
                                                                                  : m_target.text.disabledReason );
    float detailY = bounds.y + bounds.h - 10.0f;
    if ( HasText( m_target.text.shortcut ) )
    {
        detailY -= 15.0f;
        draw.Text( bounds.x + 10.0f, detailY, 10.0f, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b,
                   m_target.text.shortcut );
    }
    if ( HasText( m_target.text.units ) )
    {
        detailY -= 15.0f;
        draw.Text( bounds.x + 10.0f, detailY, 10.0f, palette.textSecondary.r, palette.textSecondary.g,
                   palette.textSecondary.b, m_target.text.units );
    }
    draw.PopClip();
    draw.EndForeground();
}
