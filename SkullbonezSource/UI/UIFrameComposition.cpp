...
uint32_t BuildUIContentSignature( const InGameUIFrameData& data )
{
    ...
    hash = HashFloat( hash, data.contactAudioMinImpactEnergy, 1000.0f );
    hash = HashInt( hash, static_cast<int>( data.contactAudioEventsSeen ) );
    hash = HashInt( hash, static_cast<int>( data.contactAudioPairCandidates ) );
    hash = HashInt( hash, static_cast<int>( data.contactAudioRejectedByMotion ) );
    hash = HashInt( hash, static_cast<int>( data.contactAudioRejectedByEnergy ) );
    hash = HashInt( hash, static_cast<int>( data.contactAudioRejectedByCooldown ) );
    hash = HashInt( hash, static_cast<int>( data.contactAudioRejectedByDistance ) );
