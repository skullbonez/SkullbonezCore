...
void UIDrawContext::
    RoundedRect( float x, float y, float w, float h, float radius, float r, float g, float b, float a ) const
{
    if ( m_drawList )
    {
...
void UIDrawContext::RoundedPanel(
    const UIRect& bounds,
    float radius,
    const Style::UIColor& fill,
    const Style::UIColor& border
) const
{
...
void UIDrawContext::FlushText() const
{
    ...
    assert(
        m_renderTextures && m_renderCommands && "UIDrawContext immediate FlushText requires texture and geometry owners"
    );
