/*
File: SkullbonezSource/SkullbonezShaderContracts.h
Purpose:
...
inline bool ShaderContractMatchesBaseName( const char* baseName, const char* pathOrBaseName )
{
    ...
    return baseName &&
           std::strlen( baseName ) == candidateLength &&
...
inline const ShaderProgramDesc* HighRiskShaderContracts()
{
    ...
    static constexpr ShaderProgramDesc contracts[] = {
        ...
        { "sky_atmosphere", "sky", "FullscreenP2_UV2", skyAtmosphereUniforms, sizeof( skyAtmosphereUniforms ) / sizeof( skyAtmosphereUniforms[0] ), nullptr, 0 },
        { "post_tonemap", "post", "FullscreenP2_UV2", tonemapUniforms, sizeof( tonemapUniforms ) / sizeof( tonemapUniforms[0] ), tonemapResources, sizeof( tonemapResources ) / sizeof( tonemapResources[0] ) },
        { "post_volumetric_light", "post", "FullscreenP2_UV2", volumetricUniforms, sizeof( volumetricUniforms ) / sizeof( volumetricUniforms[0] ), volumetricResources, sizeof( volumetricResources ) / sizeof( volumetricResources[0] ) },
    };
    return contracts;
}
