...
Physics::PhysicsConstraintType ParseConstraintType( const Json& value, const std::string& path )
{
    const std::string type = Lowercase( ReadString( value, path, "physicsConstraint.type" ) );
    if ( type == "ballsocket" || type == "ball_socket" || type == "ball-socket" )
    {
        return Physics::PhysicsConstraintType::BallSocket;
    }
    if ( type == "hinge" )
    {
        return Physics::PhysicsConstraintType::Hinge;
    }
    if ( type == "conetwist" || type == "cone_twist" || type == "cone-twist" )
    {
        return Physics::PhysicsConstraintType::ConeTwist;
    }
    if ( type == "slider" )
    {
        return Physics::PhysicsConstraintType::Slider;
    }
    if ( type == "sixdof" || type == "six_dof" || type == "six-dof" || type == "6dof" )
    {
        return Physics::PhysicsConstraintType::SixDof;
    }
    Fail( path, "physicsConstraint.type has an unknown constraint type: " + type );
}

Physics::ConstraintAxisMode ParseConstraintAxisMode( const Json& value, const std::string& path, const char* context )
{
    const std::string mode = Lowercase( ReadString( value, path, context ) );
    if ( mode == "free" )
    {
        return Physics::ConstraintAxisMode::Free;
    }
    if ( mode == "locked" || mode == "lock" )
    {
        return Physics::ConstraintAxisMode::Locked;
    }
    if ( mode == "limited" || mode == "limit" )
    {
        return Physics::ConstraintAxisMode::Limited;
    }
    Fail( path, std::string( context ) + " must be free, locked, or limited" );
}

void ReadConstraintAxisModes( const Json& value,
                              const std::string& path,
                              const char* context,
                              std::array<Physics::ConstraintAxisMode, 3>& modes )
{
    RequireArray( value, path, context );
    if ( value.size() != 3 )
    {
        Fail( path, std::string( context ) + " must contain exactly 3 entries" );
    }
    for ( size_t i = 0; i < 3; ++i )
    {
        modes[i] = ParseConstraintAxisMode( value[i], path, context );
    }
}

Physics::PhysicsConstraintLimit
ReadConstraintLimit( const Json& value, const std::string& path, const char* context, bool degrees )
{
    static constexpr float DEG_TO_RAD = 0.01745329251994329577f;
    Physics::PhysicsConstraintLimit limit;
    if ( value.is_array() )
    {
        if ( value.size() < 2 || value.size() > 5 )
        {
            Fail( path, std::string( context ) + " arrays must contain min, max, and optional softness/damping/rest" );
        }
        limit.minValue = ReadFloat( value[0], path, context );
        limit.maxValue = ReadFloat( value[1], path, context );
        if ( value.size() > 2 )
        {
            limit.softness = ReadFloat( value[2], path, context );
        }
        if ( value.size() > 3 )
        {
            limit.damping = ReadFloat( value[3], path, context );
        }
        if ( value.size() > 4 )
        {
            limit.restitution = ReadFloat( value[4], path, context );
        }
    }
    else
    {
        RequireObject( value, path, context );
        limit.minValue = ReadFloat( RequireMember( value, path, context, "min" ), path, context );
        limit.maxValue = ReadFloat( RequireMember( value, path, context, "max" ), path, context );
        if ( const Json* softness = FindMember( value, "softness" ) )
        {
            limit.softness = ReadFloat( *softness, path, context );
        }
        if ( const Json* damping = FindMember( value, "damping" ) )
        {
            limit.damping = ReadFloat( *damping, path, context );
        }
        if ( const Json* restitution = FindMember( value, "restitution" ) )
        {
            limit.restitution = ReadFloat( *restitution, path, context );
        }
    }

    if ( degrees )
    {
        limit.minValue *= DEG_TO_RAD;
        limit.maxValue *= DEG_TO_RAD;
    }
    return limit;
}

void ReadConstraintLimits( const Json& value,
                           const std::string& path,
                           const char* context,
                           bool degrees,
                           std::array<Physics::PhysicsConstraintLimit, 3>& limits )
{
    RequireArray( value, path, context );
    if ( value.size() != 3 )
    {
    ...
    for ( size_t i = 0; i < 3; ++i )
    {
        limits[i] = ReadConstraintLimit( value[i], path, context, degrees );
    }
}
...
class TestSceneParser
{
  private:
    ...
    void ApplyPointJointConstraint( const Json& jointJson, const std::string& path )
    {
        ...
        if ( const Json* solverEnabled = FindMember( jointJson, "solverEnabled" ) )
        {
            joint.solverEnabled = ReadBool( *solverEnabled, path, "ragdollJoint.solverEnabled" );
        }
        m_scene.m_pointJointConstraints.push_back( joint );
    }

    void ApplyPhysicsConstraint( const Json& constraintJson, const std::string& path )
    {
        RequireObject( constraintJson, path, "physicsConstraint" );
        ScenePhysicsConstraint sceneConstraint = {};
        ReadRequiredStringField( sceneConstraint.bodyA, constraintJson, path, "physicsConstraint", "bodyA" );
        ReadRequiredStringField( sceneConstraint.bodyB, constraintJson, path, "physicsConstraint", "bodyB" );

        Physics::PhysicsConstraintDescriptor& descriptor = sceneConstraint.descriptor;
        descriptor.type =
            ParseConstraintType( RequireMember( constraintJson, path, "physicsConstraint", "type" ), path );
        if ( const Json* localAnchorA = FindMember( constraintJson, "localAnchorA" ) )
        {
            ReadVec3( *localAnchorA,
                      path,
                      "physicsConstraint.localAnchorA",
                      descriptor.localAnchorA.x,
                      descriptor.localAnchorA.y,
                      descriptor.localAnchorA.z );
        }
        if ( const Json* localAnchorB = FindMember( constraintJson, "localAnchorB" ) )
        {
            ReadVec3( *localAnchorB,
                      path,
                      "physicsConstraint.localAnchorB",
                      descriptor.localAnchorB.x,
                      descriptor.localAnchorB.y,
                      descriptor.localAnchorB.z );
        }
        if ( const Json* localAxisA = FindMember( constraintJson, "localAxisA" ) )
        {
            ReadVec3( *localAxisA,
                      path,
                      "physicsConstraint.localAxisA",
                      descriptor.localAxisA.x,
                      descriptor.localAxisA.y,
                      descriptor.localAxisA.z );
        }
        if ( const Json* localAxisB = FindMember( constraintJson, "localAxisB" ) )
        {
            ReadVec3( *localAxisB,
                      path,
                      "physicsConstraint.localAxisB",
                      descriptor.localAxisB.x,
                      descriptor.localAxisB.y,
                      descriptor.localAxisB.z );
        }
        if ( const Json* localSecondaryAxisA = FindMember( constraintJson, "localSecondaryAxisA" ) )
        {
            ReadVec3( *localSecondaryAxisA,
                      path,
                      "physicsConstraint.localSecondaryAxisA",
                      descriptor.localSecondaryAxisA.x,
                      descriptor.localSecondaryAxisA.y,
                      descriptor.localSecondaryAxisA.z );
        }
        if ( const Json* localSecondaryAxisB = FindMember( constraintJson, "localSecondaryAxisB" ) )
        {
            ReadVec3( *localSecondaryAxisB,
                      path,
                      "physicsConstraint.localSecondaryAxisB",
                      descriptor.localSecondaryAxisB.x,
                      descriptor.localSecondaryAxisB.y,
                      descriptor.localSecondaryAxisB.z );
        }
        if ( const Json* linearModes = FindMember( constraintJson, "linearAxisModes" ) )
        {
            ReadConstraintAxisModes( *linearModes,
                                     path,
                                     "physicsConstraint.linearAxisModes",
                                     descriptor.linearAxisModes );
        }
        if ( const Json* angularModes = FindMember( constraintJson, "angularAxisModes" ) )
        {
            ReadConstraintAxisModes( *angularModes,
                                     path,
                                     "physicsConstraint.angularAxisModes",
                                     descriptor.angularAxisModes );
        }
        if ( const Json* linearLimits = FindMember( constraintJson, "linearLimits" ) )
        {
            ReadConstraintLimits( *linearLimits,
                                  path,
                                  "physicsConstraint.linearLimits",
                                  false,
                                  descriptor.linearLimits );
        }
        if ( const Json* angularLimits = FindMember( constraintJson, "angularLimits" ) )
        {
            ReadConstraintLimits( *angularLimits,
                                  path,
                                  "physicsConstraint.angularLimits",
                                  false,
                                  descriptor.angularLimits );
        }
        if ( const Json* angularLimitsDegrees = FindMember( constraintJson, "angularLimitsDegrees" ) )
        {
            ReadConstraintLimits( *angularLimitsDegrees,
                                  path,
                                  "physicsConstraint.angularLimitsDegrees",
                                  true,
                                  descriptor.angularLimits );
        }
        if ( const Json* slack = FindMember( constraintJson, "slack" ) )
        {
            descriptor.slack = (std::max)( 0.0f, ReadFloat( *slack, path, "physicsConstraint.slack" ) );
        }
        if ( const Json* stiffness = FindMember( constraintJson, "stiffness" ) )
        {
            descriptor.stiffness =
                std::clamp( ReadFloat( *stiffness, path, "physicsConstraint.stiffness" ), 0.0f, 1.0f );
        }
        if ( const Json* damping = FindMember( constraintJson, "damping" ) )
        {
            descriptor.damping = std::clamp( ReadFloat( *damping, path, "physicsConstraint.damping" ), 0.0f, 4.0f );
        }
        if ( const Json* angularDamping = FindMember( constraintJson, "angularDamping" ) )
        {
            descriptor.angularDamping =
                std::clamp( ReadFloat( *angularDamping, path, "physicsConstraint.angularDamping" ), 0.0f, 4.0f );
        }
        if ( const Json* group = FindMember( constraintJson, "groupId" ) )
        {
            descriptor.groupId =
                static_cast<uint32_t>( (std::max)( 0, ReadInt( *group, path, "physicsConstraint.groupId" ) ) );
        }
        if ( const Json* stableId = FindMember( constraintJson, "stableId" ) )
        {
            descriptor.stableId = ReadUInt( *stableId, path, "physicsConstraint.stableId" );
        }
        if ( const Json* debugName = FindMember( constraintJson, "debugName" ) )
        {
            CopyStringField( descriptor.debugName, ReadString( *debugName, path, "physicsConstraint.debugName" ) );
        }
        if ( const Json* motorEnabled = FindMember( constraintJson, "motorEnabled" ) )
        {
            descriptor.motorEnabled = ReadBool( *motorEnabled, path, "physicsConstraint.motorEnabled" );
        }
        if ( const Json* breakEnabled = FindMember( constraintJson, "breakEnabled" ) )
        {
            descriptor.breakEnabled = ReadBool( *breakEnabled, path, "physicsConstraint.breakEnabled" );
        }
        if ( const Json* breakImpulse = FindMember( constraintJson, "breakImpulseThreshold" ) )
        {
            descriptor.breakImpulseThreshold =
                (std::max)( 0.0f, ReadFloat( *breakImpulse, path, "physicsConstraint.breakImpulseThreshold" ) );
        }

        m_scene.m_physicsConstraints.push_back( sceneConstraint );
    }
    ...
    void ApplySceneBody( const Json& root, const std::string& path )
    {
        ...
        if ( const Json* physicsConstraints = FindMember( root, "physicsConstraints" ) )
        {
            RequireArray( *physicsConstraints, path, "physicsConstraints" );
            for ( const Json& constraint : *physicsConstraints )
            {
                ApplyPhysicsConstraint( constraint, path );
            }
        }
