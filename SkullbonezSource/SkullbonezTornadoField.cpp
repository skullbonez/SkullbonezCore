...
Vector3 TornadoField::SampleAcceleration( const Vector3& position ) const
{
    ...
    Vector3 inward = horizontal > TOLERANCE
                         ? Vector3( -dx / horizontal, 0.0f, -dz / horizontal )
                         : Vector3( 1.0f, 0.0f, 0.0f );
    Vector3 tangent( -inward.z, 0.0f, inward.x );

    const float radialMask = SmoothStep01( 1.0f, 0.0f, radial01 );
    const float columnMask = SmoothStep01( 0.0f, 0.12f, height01 ) *
                             SmoothStep01( 1.0f, 0.78f, height01 );
...
void TornadoField::RenderVectors( const Matrix4& viewProj )
{
    ...
    for ( int h = 0; h < HEIGHT_STEPS; ++h )
    {
        ...
        {
            const float radial01 = 0.22f + static_cast<float>( rIndex ) * ( 0.72f / static_cast<float>( RADIUS_STEPS - 1 ) );
