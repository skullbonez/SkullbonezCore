...
class Terrain
{
  ...
  public:
    ...
    );                                                          // Lane R factory for external RAW height-map input.
    ...
    Terrain(
        ...
        Assets::AssetSystem& assets,
        Rendering::Dx12ResourceBuilder& resources
    );                                                          // Construction shell used by TryCreateFromHeightMap; step
    ...
    Terrain(
        ...
        int iTextureWrap,
        const SkullbonezCore::Core::EngineConfig& config
    );                                                          // CPU-only shell used by the value-producing load path.
    Terrain(
        float slopeBaseY,
        float slopeX,
        float slopeZ,
    ...
    Terrain(
        ...
        float slopeZ,
        const SkullbonezCore::Core::EngineConfig& config
    );                                                          // Physics-only analytic slope; owns no GPU resources.
    ~Terrain();
    ...
    void BindRenderContexts(
        ...
        Assets::AssetSystem& assets,
        Rendering::Dx12ResourceBuilder& resources
    );                                                          // Borrow rebuild-only services for terrain resources.
    ...
    float GetTerrainHeightAt(
        float xPosition,
        float zPosition,
        bool isFluidMin = false
    );                                                          // Height sample; isFluidMin asks water tests for the lowest terrain support.
    Math::Vector::Vector3
    ...
    );                                                          // Combined lookup: one cached-cell query instead of two.
    void GetTerrainHeightAndPlaneAt(
        float xPosition,
        float zPosition,
        float& outHeight,
        Plane& outPlane
    );                                                          // Physics fast path: direct cached plane plus height lookup.
  ...
  private:
    ...
    Terrain(
        ...
        Assets::AssetSystem* assets,
        Rendering::Dx12ResourceBuilder* resources
    );                                                          // Shared CPU/GPU height-map construction shell.
    Terrain(
        float slopeBaseY,
        float slopeX,
        float slopeZ,
    ...
    int GetQuadCacheIndex(
        float xPosition,
        float zPosition,
        bool& isTriangleA
    );                                                          // Maps world X/Z to cached terrain quad and triangle half.
    void QueryCollisionData(
        float xPosition,
        float zPosition,
        float& outHeight,
        Math::Vector::Vector3* outNormal,
    ...
    void QueryCollisionDataUnchecked(
        float xPosition,
        float zPosition,
        float& outHeight,
        Math::Vector::Vector3* outNormal,
