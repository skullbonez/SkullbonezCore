// Hazard: a UAV ordering point without the exact native resource or command list
// cannot be emitted; the result preserves that missing evidence for fatal invariant review.// Hazard: barrier emission
// requires a real resource, two concrete graph states, and a command list. Dry-run callers receive flags rather than a
// hidden DX12 call.// Invariant: Unknown writes a harmless diagnostic placeholder but returns false; callers must never
// treat COMMON as a fabricated source-state proof./*
                  ...
                  namespace SkullbonezCore
{
    ...
    void AppendDx12StateFlag( std::ostringstream& out, bool& wroteAny, D3D12_RESOURCE_STATES state,
                              D3D12_RESOURCE_STATES flag, const char* name )
    {
        if ( ( state & flag ) != 0 )
        {
            if ( wroteAny )
