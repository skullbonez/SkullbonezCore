...
class Sha256Writer
{
  public:
    ...
    {
        const bool ok = m_hash &&
                        BCryptFinishHash( m_hash, digest.data(), static_cast<ULONG>( digest.size() ), 0 ) >= 0;
                      ...
                      std::array<std::uint8_t, Dx12CachedPsoStore::DIGEST_BYTES>& digest )
{
    ...
    const std::uint8_t* bytes = mapping ? static_cast<const std::uint8_t*>(
...
bool HashGraphicsDesc( Sha256Writer& writer, const D3D12_GRAPHICS_PIPELINE_STATE_DESC& desc )
{
    ...
    if ( !HashValue( writer, depth.DepthEnable ) || !HashValue( writer, depth.DepthWriteMask ) ||
         ...
         !HashValue( writer, depth.FrontFace.StencilFailOp ) ||
         !HashValue( writer, depth.FrontFace.StencilDepthFailOp ) ||
         !HashValue( writer, depth.FrontFace.StencilPassOp ) || !HashValue( writer, depth.FrontFace.StencilFunc ) ||
         !HashValue( writer, depth.BackFace.StencilFailOp ) ||
         !HashValue( writer, depth.BackFace.StencilDepthFailOp ) ||
...
{
    const DWORD overrideLength = GetEnvironmentVariableW( L"SKULLBONEZ_PSO_CACHE_DIR",
                                                          directory,
...
bool Dx12CachedPsoStore::BuildPersistentEntryNameForTest(
...
bool Dx12CachedPsoStore::Initialize( SkullbonezCore::Core::ByteView rootSignatureBytes )
{
    ...
    if ( !GetFileSizeEx( m_file, &fileSize ) ||
         fileSize.QuadPart < static_cast<LONGLONG>( sizeof( CacheFileHeader ) ) ||
