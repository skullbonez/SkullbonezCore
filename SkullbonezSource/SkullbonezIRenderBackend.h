...
#include "SkullbonezDrawCallTrace.h"
...
{
  public:
    DrawCallTraceScope( const char* fullPathOrLeaf, uint32_t hash )
        : m_hash( hash ), m_active( IsGfxReady() )
    {
        if ( m_active )
        {
...
#define DRAW_CALL_TRACE_SCOPE( name )                                                          \
    ...
    ::SkullbonezCore::Rendering::DrawCallTraceScope DRAW_CALL_TRACE_PASTE( _drawTraceScope_, __LINE__ )( name, DRAW_CALL_TRACE_PASTE( _drawTraceHash_, __LINE__ ) )
