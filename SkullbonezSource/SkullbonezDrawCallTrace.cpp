/*
File: SkullbonezSource/SkullbonezDrawCallTrace.cpp
Purpose:
...
void DrawCallTrace::PopScope( uint32_t hash )
{
    if ( m_currentNodeIndex >= 0 )
    {
        ...
        const uint32_t leafHash = node.leafName ? HashStringRange( node.leafName, static_cast<int>( strlen( node.leafName ) ) ) : 0u;
