...
{
  m_pLineShader = new Shader("SkullbonezData/shaders/line_2d.vert", "SkullbonezData/shaders/line_2d.frag");
...
void Renderer2D::DrawLine(float x1, float y1, float x2, float y2, float r, float g, float b, float a)
{
  ...
  glBufferSubData(GL_COPY_WRITE_BUFFER, m_currentLineVertices * sizeof(LineVertex), sizeof(LineVertex),
                  &v1);
  glBufferSubData(GL_COPY_WRITE_BUFFER, (m_currentLineVertices + 1) * sizeof(LineVertex),
                  sizeof(LineVertex), &v2);
  glBindBuffer(GL_COPY_WRITE_BUFFER, 0);

  m_currentLineVertices += 2;
}

void Renderer2D::DrawCircle(float cx, float cy, float radius, int segments, float r, float g, float b,
                            float a)
{
...
void Renderer2D::EndFrame()
{
  ...
  m_pLineShader->Use();
  m_pLineShader->SetMatrix4("projection", orthoProj);
  m_pLineShader->SetMatrix4("view", identity);
  m_pLineShader->SetMatrix4("model", identity);
