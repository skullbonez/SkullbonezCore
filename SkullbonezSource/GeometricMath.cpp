/*
File: SkullbonezSource/GeometricMath.cpp
Purpose:
...
Related:
  - SkullbonezSource/GeometricMath.h
...
float GeometricMath::CalculateIntersectionTime( const Triangle& triangle, const Ray& ray )
{
    return GeometricMath::CalculateIntersectionTime( GeometricMath::ComputePlane( triangle ), ray );
}


Vector3 GeometricMath::ComputeIntersectionPoint( const Plane& plane, const Ray& ray )
{
...
Vector3 GeometricMath::ComputeIntersectionPoint( const Ray& ray, float fCollisionTime )
{
    return ray.origin + ( ray.vector3 * fCollisionTime );
}


bool GeometricMath::IsPointInsideTriangle( const Triangle& triangle, const Vector3& point )
{
    Vector3 barycentricCoords = GeometricMath::ComputeBarycentricCoordinates( triangle, point );
