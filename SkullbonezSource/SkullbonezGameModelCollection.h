...
class GameModelCollection
{
  ...
  private:
    ...
    Math::CollisionDetection::SpatialGrid m_spatialGrid;                           // Broadphase spatial grid for collision culling
    ...
    std::array<Math::Vector::Vector3, MAX_GAME_MODELS> m_soaPositions;             // Hot SoA stream: current model centers
    ...
    std::array<Math::Transformation::Matrix4, MAX_GAME_MODELS> m_soaModelMatrices; // Per-frame render matrix stream reused by reflection/main passes
    ...
    struct PersistentContact
    {
        ...
        Math::Vector::Vector3 normal = Math::Vector::ZERO_VECTOR;        // Push direction.  Normal impulses separate bodies; they never pull.
        Math::Vector::Vector3 tangent1 = Math::Vector::ZERO_VECTOR;      // First sideways direction at the contact point, used for friction.
        Math::Vector::Vector3 tangent2 = Math::Vector::ZERO_VECTOR;      // Second sideways direction.  3D contacts need two friction axes.
        Math::Vector::Vector3 rA = Math::Vector::ZERO_VECTOR;            // Vector from body A center to the contact point, for spin/torque.
        Math::Vector::Vector3 rB = Math::Vector::ZERO_VECTOR;            // Vector from body B center to the contact point, for spin/torque.
        ...
        Math::Vector::Vector3 terrainNormal = Math::Vector::ZERO_VECTOR; // Surface normal; row normal is opposite because it points A -> terrain.
  ...
  public:
    ...
    void RenderModels( const Math::Transformation::Matrix4& view, const Math::Transformation::Matrix4& proj, const float lightPos[4], const Basics::CinematicRenderConfig* cinematic = nullptr, const Rendering::ShadowFrameData* shadow = nullptr, float materialAlpha = 1.0f ); // Renders the game models
