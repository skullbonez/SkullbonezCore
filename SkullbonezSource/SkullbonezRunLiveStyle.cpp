...
void SkullbonezRun::SetLiveStyleControlDirectory( const char* path )
{
    if ( !path || path[0] == '\0' )
    {
        return;
...
void SkullbonezRun::TickLiveStyleControl()
{
    ...
    if ( captureStamp != 0 && captureStamp != m_liveStyle.captureStamp )
    {
        ...
        if ( ReadCaptureRequest( m_liveStyle.capturePath, requestedPath, sizeof( requestedPath ) ) )
        {
            ...
            {
                strcpy_s( m_liveStyle.pendingScreenshotPath, sizeof( m_liveStyle.pendingScreenshotPath ), requestedPath );
            }
            else
            {
                JoinControlPath( m_liveStyle.directory, requestedPath, m_liveStyle.pendingScreenshotPath, sizeof( m_liveStyle.pendingScreenshotPath ) );
            }
...
void SkullbonezRun::TickLiveStyleControlCapture()
{
