// Invariant: every user-visible string is bounded and single-line before the
// flattened settings block is serialized into the receipt transaction.    // Hazard: exclusive creation is the collision
// proof. Treat an existing path as failure instead of merging files from two generated looks.// Invariant: bundle names
// accept exactly YYYY-MM-DD_HH-MM-SS with bounded
// calendar/time fields so metadata cannot escape the one-directory transaction./*
                             ...
                             namespace SkullbonezCore::Runtime
{
    ...
    Core::SbResult LookLabBundleWriter::CreateBundleDirectory( Core::SbDiagnosticStore & diagnostics,
    ...
    Core::SbResult LookLabBundleWriter::BuildReceipt( Core::SbDiagnosticStore & diagnostics,
                                                      ...
                                                      const LookLabBundlePaths& paths, std::string& output )
    {
        ...
        std::string settings;
        Core::SbResult listingResult = Scene::StandaloneStyleWriter::BuildFlattenedListing( diagnostics, snapshot,
                                                                                            settings );
        ...
        receipt << "seed=" << std::hex << std::nouppercase << std::setw( 16 ) << std::setfill( '0' ) << facts.seed
                << std::dec << '\n';
