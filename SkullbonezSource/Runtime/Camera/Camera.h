...
class Camera
{
  ...
  private:
    ...
    );                                                                  // Commits or clamps the staged translation while preserving view distance.
    ...
    float UpVectorViewVectorRotationCap(
        float requestRadians,
        const CameraMovementSettings& settings
    );                                                                  // Caps pitch so view and up vectors cannot
       // collapse into the same direction.
    void RecoverViewMagnitude(
        bool isOnBoundX,
        bool isOnBoundZ,
        const CameraMovementSettings& settings
    );                                                                  // One-axis bound clamps may need a guarded
    ...
    void MoveCamera(
        const TravelDirection enumDir,
        float fQuantity,
        const CameraMovementSettings& settings
    );                                                                  // fQuantity is world-space travel along camera-local axes.
    void ApplyDelta(
        const Camera& delta,
        const CameraMovementSettings& settings
    );                                                                  // Relative cameras add the primary-camera pose delta through this helper.
    void RotateCamera(
        float xMove,
        float yMove,
        const CameraMovementSettings& settings
    );                                                                  // Mouse-look delta path; clamps pitch before mutating the view target.
