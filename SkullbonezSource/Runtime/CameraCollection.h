...
Mental model:
  Run owns the camera collection and passes a borrowed pointer through older
  runtime seams while authored scene data, input, simulation, render backends,
  and validation-oriented launch modes are being split into narrower owners.

Glossary:
  EngineConfig: Live process settings borrowed from Run and forwarded into
    camera slots for movement/clamp tuning.
...
class CameraCollection
{
  ...
  private:
    ...
    const SkullbonezCore::Basics::EngineConfig* m_config;          // Borrowed live config; Run owns the pointed-to process singleton.
  ...
  public:
    ...
    void AddCamera( const Math::Vector::Vector3& vPosition,
                    ...
                    uint32_t hash );
    void Reset();                                                  // Scene reload and shutdown path; keeps Run-owned storage reusable.
};
} // namespace Environment
} // namespace SkullbonezCore
