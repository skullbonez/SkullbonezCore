...
class RetainedUIDrawStatsScope
{
  public:
    ...
    ~RetainedUIDrawStatsScope()
    {
        ...
        const bool drawStatsRequested = getenv_s( &drawStatsFlagLength, drawStatsFlag, sizeof( drawStatsFlag ), "SKORE_UI_DRAW_STATS" ) == 0 && drawStatsFlag[0] != '\0';
        ...
        const auto overflowed = []( const SkullbonezCore::UI::UIDrawList::Stats& stats ) { return stats.commandOverflow || stats.textOverflow || stats.clipOverflow; };

        const bool overflow = overflowed( testPatternStats ) || overflowed( badgeStats ) || overflowed( detachedStats ) || overflowed( profilerStats );

        std::fprintf( stderr,
                      "[ui-retained-draw-stats] test=%d/%d badge=%d/%d detached=%d/%d profiler=%d/%d clip=%d/%d/%d/%d " "overflow=%d\n",
                      testPatternStats.commandCount,
                      testPatternStats.textBytes,
                      badgeStats.commandCount,
                      badgeStats.textBytes,
                      detachedStats.commandCount,
                      detachedStats.textBytes,
                      profilerStats.commandCount,
                      profilerStats.textBytes,
                      testPatternStats.maxClipDepth,
                      badgeStats.maxClipDepth,
                      detachedStats.maxClipDepth,
...
SkullbonezCore::Core::SbResult UiTextPass::EnsureGpuResources( Rendering::Dx12TextureOwner& renderTextures,
                                                               ...
                                                               int screenH )
{
    const SkullbonezCore::Core::SbResult fontResult = Text2d::BuildFont( m_resultDiagnostics,
                                                                         m_textBatch,
                                                                         renderTextures,
                                                                         renderGeometry,
                                                                         std::move( textShader ),
                                                                         std::move( solidShader ),
                                                                         std::move( solidBatchShader ),
                                                                         screenW,
                                                                         screenH,
                                                                         "Verdana" );
...
{
    return visibility.textOnly || !visibility.sceneMode || visibility.sceneText || visibility.overlayVisible || visibility.uiTextNeeded ||
           ( visibility.crossScenePauseLocked && !visibility.topTextHidden ) || ( visibility.sceneTestComplete && !visibility.topTextHidden ) || visibility.replayScrubberVisible ||
...
void UiTextPass::RenderChromeStatus( const UiTextViewport& viewport,
                                     const UiChromeStatusValues& values,
                                     ...
                                     Rendering::Dx12Diagnostics& renderDiagnostics )
{
    ...
    if ( values.textOnly )
    {
        Text2d::Render2dTextColor( textBatch, -0.95f, 0.90f, 0.04f, 1.0f, 1.0f, 1.0f, "The quick brown fox jumps over the lazy dog." );
        return;
    }
    ...
    const UI::UIRect content = values.contentBounds.w >= 0.0f ? values.contentBounds : UI::UIRect { 0, 0, static_cast<float>( viewport.screenW ), static_cast<float>( viewport.screenH ) };
    float topRightBadgeY = content.y + TOP_RIGHT_BADGE_MARGIN;
    m_badgeDrawList.Clear();
    const SkullbonezCore::UI::UIDrawContext badgeDraw( (std::max)( 1, viewport.screenW ), (std::max)( 1, viewport.screenH ), m_badgeDrawList );
    ...
    const auto renderScenePauseBadge = [&]()
    {
        ...
        {
            const int sceneFrame = values.sceneTestComplete && values.currentFrame > values.targetFrameCount ? values.targetFrameCount : values.currentFrame;

            sprintf_s( sceneLine, sizeof( sceneLine ), "Scene %d/%d  Frame %d/%d", values.currentSceneIndex + 1, values.sceneQueueSize, sceneFrame, values.targetFrameCount );
        }
        else
        {
        ...
        const float contentW = (std::max)( Text2d::MeasureText( titlePx, sceneLine ), Text2d::MeasureText( valuePx, stateLine ) );
        ...
        draw.RoundedRect( x + 1.0f,
                          ...
                          values.crossScenePauseLocked ? palette.warningAccent.b : palette.accent.b,
                          0.90f );

        draw.Text( x + padX, y + padY, titlePx, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b, sceneLine );
    ...
    const auto renderRuntimeModeBadge = [&]()
    {
        ...
        draw.Text( x + padX, y + padY + lineGap, detailPx, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, detail );
    };
    ...
    const auto renderInteractionIndicator = [&]()
    {
        ...
        draw.Text( dotX + dotSize + 6.0f, y + 5.0f, 12.0f, statusR, statusG, statusB, failed ? "RECORDING FAILED" : ( playing ? "PLAYBACK" : "REC F8 TO STOP" ) );

        if ( failed )
        {
            char statusLine[160] = {};
            sprintf_s( statusLine, sizeof( statusLine ), "%s", values.interactionFailure );
            draw.Text( x + 8.0f, y + 23.0f, 9.5f, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, statusLine );
        }
        else if ( playing )
        {
            char statusLine[160] = {};
            const std::size_t visibleTurn = values.interactionPlaybackTurnCount == 0u ? 0u : (std::min)( values.interactionPlaybackTurn + 1u, values.interactionPlaybackTurnCount );
            sprintf_s( statusLine, sizeof( statusLine ), "%zu / %zu turns", visibleTurn, values.interactionPlaybackTurnCount );
            draw.Text( x + 8.0f, y + 23.0f, 9.5f, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, statusLine );
        }
        else
        {
            char statusLine[160] = {};
            sprintf_s( statusLine,
                       sizeof( statusLine ),
                       "%.1fs / %dm   %zu / %zu turns",
                       values.interactionRecordingElapsedSeconds,
                       values.interactionRecordingMaximumMinutes,
                       values.interactionRecordingFrameCount,
                       values.interactionRecordingFrameCapacity );
            draw.Text( x + 8.0f, y + 23.0f, 9.5f, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, statusLine );
        }
    };
    ...
    {
        m_uiDrawSubmission.Submit( m_badgeDrawList, textBatch, m_gpuTiming, renderTextures, renderCommands, renderDiagnostics, (std::max)( 1, viewport.screenW ), (std::max)( 1, viewport.screenH ) );
    }
}
...
void UiTextPass::RenderChromeTail( const UiChromeTailValues& values, Rendering::Dx12GeometryOwner& renderCommands )
{
    ...
    if ( values.launcherCameraMode )
    {
        ...
        if ( values.reproSnapshotMessage[0] != '\0' && values.reproMessageAgeSeconds <= values.reproSnapshotMessageUntil )
        {
            ...
            Text2d::Render2dTextColor( textBatch, -msgW * 0.5f, -0.065f, msgSz, 0.65f, 0.92f, 1.0f, "%s", values.reproSnapshotMessage );
        }
#endif
    }
}
...
void UiTextPass::PrepareOperatorSubmission( const UiTextViewport& viewport,
                                            ...
                                            Rendering::Dx12Diagnostics& renderDiagnostics )
{
    ...
    if ( drawTestPattern )
    {
        DrawUiTestPattern( m_uiDrawSubmission, m_testPatternDrawList, textBatch, renderTextures, renderCommands, renderDiagnostics, viewport.screenW, viewport.screenH );
    }
}
                                         ...
                                         const UiTextViewport& viewport )
{
    ...
    m_uiDrawSubmission
        .SubmitWithPreviews( drawList, renderTargetPreviews, textBatch, m_gpuTiming, assets, renderResources, renderTextures, renderCommands, renderDiagnostics, viewport.screenW, viewport.screenH );
                                       ...
                                       Rendering::Dx12Diagnostics& renderDiagnostics )
{
    ...
    if ( mode == UiOverlayMode::SceneStats )
    {
        ...
        Text2d::Render2dTextColor( textBatch, panX0 + panPad, panY1 - panPad - titleSz, titleSz, 1.0f, 0.85f, 0.35f, "SCENE TELEMETRY" );

        Text2d::Render2dTextColor( textBatch, panX0 + panPad, panY1 - panPad - titleSz - lineH, entrySz, 0.85f, 0.85f, 0.85f, "Model Count: %d", modelCount );

        Text2d::Render2dTextColor( textBatch, panX0 + panPad, panY1 - panPad - titleSz - lineH * 2.0f, entrySz, 0.85f, 0.85f, 0.85f, "Scene Energy: %.6f", sceneEnergyForDisplay );
    ...
    if ( mode == UiOverlayMode::BarsNormalized || mode == UiOverlayMode::BarsAbsolute )
    {
        ...
        profilerOverlay.RecordBarOverlay( profiler.FrameView(), profilerDraw, panX, panY, panW, panH, absolute );
        {
            ...
            m_uiDrawSubmission.Submit( m_profilerDrawList, textBatch, m_gpuTiming, renderTextures, renderCommands, renderDiagnostics, viewport.screenW, viewport.screenH );
        }
        return;
    }
#endif
    ...
    if ( mode == UiOverlayMode::Keys )
    {
        ...
        static const KeyEntry kLeft[nRows] = { { "Tab", "Camera mode" },
                                               { "N", "Launcher mode" },
...
#if defined( SKULLBONEZ_PROFILE_ENABLED )
    {
        ...
        m_profilerDrawList.Clear();
        const UI::UIDrawContext profilerDraw( viewport.screenW, viewport.screenH, m_profilerDrawList );
        profilerOverlay.RecordOverlay( profiler.FrameView(), profilerDraw, -( hw - mX ), -( hh - mY ) - padY, lineH, profFSz, rollingFpsTime );
                                 ...
                                 Rendering::Dx12Diagnostics& renderDiagnostics )
{
    ...
    m_uiDrawSubmission.Submit( drawList, m_textBatch, nullptr, renderTextures, renderCommands, renderDiagnostics, viewport.screenW, viewport.screenH );
}


void UiTextPass::FinalizeOverlay( UiOverlayMode mode, Rendering::Dx12TextureOwner& renderTextures, Rendering::Dx12GeometryOwner& renderCommands, Rendering::Dx12Diagnostics& renderDiagnostics )
{
    if ( mode == UiOverlayMode::BarsNormalized || mode == UiOverlayMode::BarsAbsolute )
    {
        return;
...
void UiTextPass::ReportRetainedDrawStats()
{
    ...
    const RetainedUIDrawStatsScope retainedDrawStats( m_testPatternDrawList, m_badgeDrawList, m_detachedDrawStats, m_profilerDrawList );
}
