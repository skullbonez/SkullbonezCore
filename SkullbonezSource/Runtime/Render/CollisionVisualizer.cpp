/*
File: SkullbonezSource/Runtime/Render/CollisionVisualizer.cpp
Purpose:
...
Related:
  - SkullbonezSource/Runtime/Render/CollisionVisualizer.h
...
constexpr PassRasterStateBucket
    COLLISION_TRANSLUCENT_RASTER = MakePassRasterStateBucket( 1, { true, false, true, BlendFactor::SrcAlpha,
...
void CollisionVisualizer::BuildSphereMesh( Dx12GeometryOwner& renderGeometry )
{
    ...
    m_sphereInstMesh = renderGeometry.CreateInstancedMesh( verts.data(), m_sphereVertexCount, 6, INSTANCE_FLOATS, 3,
                                                           instanceAttribSizes, staticAttribSizes );
}
...
void CollisionVisualizer::BuildBoxMesh( Dx12GeometryOwner& renderGeometry )
{
    ...
    m_boxInstMesh = renderGeometry.CreateInstancedMesh( verts.data(), m_boxVertexCount, 6, INSTANCE_FLOATS, 3,
                                                        instanceAttribSizes, staticAttribSizes );
}
...
void CollisionVisualizer::DrawInstances( Dx12GeometryOwner& renderCommands, uint32_t mesh, int vertexCount,
                                         const std::vector<float>& instanceData, const PassRasterStateBucket& rasterState )
{
...
void CollisionVisualizer::DrawHullInstance( Dx12GeometryOwner& renderCommands, const ConvexHullShape& hull,
                                            const Matrix4& model, const Color& color,
                                            const PassRasterStateBucket& rasterState )
{
    static_assert( HULL_MAX_TRIANGLE_VERTICES == ConvexHullShape::MAX_FACES * ( ConvexHullShape::MAX_FACE_VERTICES - 2 ) * 3,
    ...
    if ( vertexCount > 0 )
    {
        renderCommands.UploadAndDrawDynamicVB( m_hullDynamicVB,
                                  ...
                                  const Matrix4& cameraView, const Matrix4& proj, const float lightPos[4] )
{
    ...
    }
    {
        ...
        for ( int i = 0; i < modelCount; ++i )
        {
            ...
            DrawHullInstance( renderGeometry, *hull, instances[static_cast<std::size_t>( i )].modelMatrix, color,
                              rasterState );
        }
    }
}
