...
Invariants:
  ...
  - UI-text scheduling samples frame metrics once, then executes focused chrome,
    operator submission, HUD overlay, Replay, and final-flush graphs in visual
    order. Each graph completes before its callback borrows expire; font,
    timing, and ray-tracing capabilities remain inside UiTextPass.
...
//
// Lifetime: payloads are stack objects consumed during the same world or overlay
// phase call. Never cache these pointers across graph execution or use them as a
...
// Lifetime: these graph callback records never cross RuntimeRenderer's public
// boundary. Each focused submission method executes its graph before returning.
...
void RenderReplayPredictionGhosts( std::span<const ReplayPredictionGhostDrawRequest> ghostRequests,
...
void ExecuteReplayGhostGraphCallback( const SkullbonezCore::Rendering::RenderGraphPassContext& /*context*/, ReplayGhostGraphInvocation& data )
{
    ...
    RenderReplayPredictionGhosts( data.replayVisualPacket->ghostRequests,
                                  *data.camera,
                                  *data.models,
...
void RuntimeRenderer::ExecuteSceneTargetBeginThroughRenderGraph( const RenderCameraLighting& camera,
                                                                 const SkullbonezCore::Core::CinematicRenderConfig& cinematic,
                                                                 Rendering::Dx12GeometryOwner& renderGeometry,
                                                                 Rendering::Dx12TextureOwner& renderTextures,
    ...
    Rendering::RenderGraphTransientMaterializationStats transientMaterialization;
    {
        ...
        if ( volumetricDeclared )
        {
            ...
            if ( !postState.volumetricLight.IsValid() )
            {
                ...
                SkullbonezCore::Core::Log().WriteEventf( "render_graph_volumetric_transient_unavailable materialization_failed=%d " "hresult=0x%08X resource=%s",
...
void RuntimeRenderer::SubmitUiChrome( const UiTextViewport& viewport, const UiChromeStatusValues& status, const UiChromeTailValues& tail )
{
    ...
    const uint32_t pass = graph.AddPass( "UiChrome", Rendering::RenderGraphQueueType::Graphics );
    graph.AddWrite( pass, backbuffer, Rendering::RenderGraphResourceAccess::RenderTarget );
    graph.SetPassCallback<ExecuteUiChromeGraphCallback>( pass, invocation, true, "Frame/UI/Chrome" );
    const Rendering::RenderGraphCompileResult& compiled = CompileRenderPassGraph( graph );
    invocation.compiled = &compiled;
    invocation.expectedTransitionCount = CountCompiledTransitionsForPass( compiled, pass );
    ExecuteGraphCallbacksOrFatal( graph, 1u, "UiChrome" );
}
...
void RuntimeRenderer::PrepareOperatorUiSubmission( const UiTextViewport& viewport, bool drawTestPattern )
{
    ...
    Rendering::RenderGraph& graph = BeginRenderPassGraph();
    const Rendering::RenderGraphResourceHandle backbuffer = AddBackbufferResource( graph, m_resources.RenderGraph() );
    const uint32_t pass = graph.AddPass( "UiOperatorPrepare", Rendering::RenderGraphQueueType::Graphics );
    graph.AddWrite( pass, backbuffer, Rendering::RenderGraphResourceAccess::RenderTarget );
    graph.SetPassCallback<ExecuteUiOperatorPrepareGraphCallback>( pass, invocation, true, "Frame/UI/Operator/Prepare" );
    const Rendering::RenderGraphCompileResult& compiled = CompileRenderPassGraph( graph );
    invocation.compiled = &compiled;
    invocation.expectedTransitionCount = CountCompiledTransitionsForPass( compiled, pass );
    ExecuteGraphCallbacksOrFatal( graph, 1u, "UiOperatorPrepare" );
}
...
void RuntimeRenderer::SubmitOperatorUiDrawList( const UI::UIDrawList& drawList, const RuntimeRenderTargetPreviewSnapshot& previews, Assets::AssetSystem& assets, const UiTextViewport& viewport )
{
    ...
    invocation.compiled = &compiled;
    invocation.expectedTransitionCount = CountCompiledTransitionsForPass( compiled, pass );
    ExecuteGraphCallbacksOrFatal( graph, 1u, "UiOperatorSubmission" );
}
...
void RuntimeRenderer::SubmitUiOverlay( const UiTextViewport& viewport, UiOverlayMode mode, int modelCount, float rollingFpsTime, float sceneEnergyForDisplay )
{
    ...
    invocation.modelCount = modelCount;
    invocation.rollingFpsTime = rollingFpsTime;
    invocation.sceneEnergyForDisplay = sceneEnergyForDisplay;
    invocation.renderTextures = &m_resources.RenderTextures();
    invocation.renderGeometry = &m_resources.RenderGeometry();
    invocation.renderDiagnostics = &m_resources.RenderDiagnostics();
    ...
    Rendering::RenderGraph& graph = BeginRenderPassGraph();
    const Rendering::RenderGraphResourceHandle backbuffer = AddBackbufferResource( graph, m_resources.RenderGraph() );
    const uint32_t pass = graph.AddPass( "UiOverlay", Rendering::RenderGraphQueueType::Graphics );
    graph.AddWrite( pass, backbuffer, Rendering::RenderGraphResourceAccess::RenderTarget );
    graph.SetPassCallback<ExecuteUiOverlayGraphCallback>( pass, invocation, true, "Frame/UI/Overlay" );
    const Rendering::RenderGraphCompileResult& compiled = CompileRenderPassGraph( graph );
    invocation.compiled = &compiled;
    invocation.expectedTransitionCount = CountCompiledTransitionsForPass( compiled, pass );
    ExecuteGraphCallbacksOrFatal( graph, 1u, "UiOverlay" );
}
...
void RuntimeRenderer::FinalizeUiOverlay( UiOverlayMode mode )
{
    ...
    Rendering::RenderGraph& graph = BeginRenderPassGraph();
    const Rendering::RenderGraphResourceHandle backbuffer = AddBackbufferResource( graph, m_resources.RenderGraph() );
    const uint32_t pass = graph.AddPass( "UiFinalize", Rendering::RenderGraphQueueType::Graphics );
    graph.AddWrite( pass, backbuffer, Rendering::RenderGraphResourceAccess::RenderTarget );
    graph.SetPassCallback<ExecuteUiFinalizeGraphCallback>( pass, invocation, true, "Frame/UI/Finalize" );
    const Rendering::RenderGraphCompileResult& compiled = CompileRenderPassGraph( graph );
    invocation.compiled = &compiled;
    invocation.expectedTransitionCount = CountCompiledTransitionsForPass( compiled, pass );
    ExecuteGraphCallbacksOrFatal( graph, 1u, "UiFinalize" );
}


int RuntimeRenderer::EndUiTextFrame( int drawCallStart )
{
...
void RuntimeRenderer::UpdateDebugVisualizers( float secondsPerFrame, const RuntimeRenderDebugViews& debug, const RuntimeRenderFramePolicy& policy )
{
    ...
    m_collisionVisualizer.Update( secondsPerFrame, CollisionVisualizerFrameView { debug.collision.colliders,
...
RuntimeRenderer::WorldOverlayTransaction RuntimeRenderer::RenderWorldFrame( const WorldFrameSubmission& world )
{
    // Invariant: Run opens graph ownership before choosing a world or text-only
    // path. RenderWorldFrame may only append to that active frame and may never
    ...
    if ( world.cinematicRequested )
    {
        const Vector3 sunDirection = ResolveCinematicSunDirection( world.cinematic );
    ...
    const char* shadowShaderBaseName = m_resources.Assets().ResolveShaderBaseName( "shader.shadow_depth_instanced" );
    ...
    Rendering::RenderInstanceRenderer instanceRenderer( primitiveRenderer,
                                                        renderDiagnostics,
                                                        ordinaryLighting,
    ...
    // Invariant: BeginFrameGraph reset this owner snapshot before the world phase.
    // World and overlay phases write disjoint executed-pass facts into that one
    // published value, so no continuation object relays diagnostics booleans.
    m_frameGraphSnapshot.cinematicRender = world.cinematicRequested;
    m_frameGraphSnapshot.useCinematicTarget = useCinematicTarget;
    m_frameGraphSnapshot.terrainShadowValid = terrainShadowFrame && terrainShadowFrame->valid;
    m_frameGraphSnapshot.objectShadowValid = objectShadowFrame && objectShadowFrame->valid;
...
bool RuntimeRenderer::RenderFrameOverlays( const WorldOverlayTransaction& world, const OverlayFrameSubmission& overlays )
{
    if ( !world.m_replayVisual )
    {
        SB_FATAL( "RunRender", "Overlay phase requires the replay visual selected by the world phase." );
    }
    ...
    const DebugOverlayPassInputs debugInputs { world.m_camera,
                                               world.m_terrain,
    ...
    {
        cinematicPostOutput = ExecuteCinematicPostThroughRenderGraph( { world.m_camera,
                                                                        world.m_cinematic,
                                                                        renderGeometry,
                                                                        renderTextures,
                                                                        renderFrame,
    ...
    m_frameGraphSnapshot.volumetricReady = cinematicPostOutput.volumetricReady;
    m_frameGraphSnapshot.volumetricTextureHandle = cinematicPostOutput.volumetricTextureHandle;
    m_frameGraphSnapshot.volumetricWidth = cinematicPostOutput.volumetricWidth;
    m_frameGraphSnapshot.volumetricHeight = cinematicPostOutput.volumetricHeight;
...
SkullbonezCore::Core::SbResult RuntimeRenderer::ReleaseBackendOwnedRuntimeResources( const BackendResourceReleaseContext& context )
{
    ...
    const BackendResourcePhase releaseSteps[] = { { "world_environment", BackendResourceStep::WorldEnvironment },
...
bool RuntimeRenderer::WorldOverlayTransaction::SubmitOverlays( const OverlayFrameSubmission& overlays )
{
    BeginOverlaySubmissionOrFatal();

    if ( m_policy.textOnly )
    {
        return false;
    }

    return m_renderer->RenderFrameOverlays( *this, overlays );
}


RuntimeRenderer::WorldOverlayTransaction RuntimeRenderer::BeginWorldFrame( const WorldFrameSubmission& world )
{
    m_resources.UiText().SetDxrReflectionPreviewTexture( 0 );

    if ( world.framePolicy.textOnly )
    {
        return WorldOverlayTransaction( *this, world.camera, world, 1, 1, false );
    }
