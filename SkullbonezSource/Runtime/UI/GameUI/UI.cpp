...
{
    return { scene.modelCapacity, scene.rngSeed, scene.solverBallCount, scene.solverBoxCount, world.worldFluidHeight, world.worldFluidDensity };
}
...
void DrawDockFold( const UIDrawContext& draw, const UIRect& bounds, bool pointsRight, bool hovered, bool vertical = false )
{
    ...
    if ( hovered )
    {
        draw.RoundedRect( bounds.x + 2.0f, bounds.y + 2.0f, bounds.w - 4.0f, bounds.h - 4.0f, 4.0f, palette.controlHover.r, palette.controlHover.g, palette.controlHover.b, 1.0f );
    }
    ...
    if ( vertical )
    {
        draw.Triangle( cx, cy + direction * 3, cx + direction * 4, cy - direction * 2, cx - direction * 4, cy - direction * 2, ink.r, ink.g, ink.b, 1 );
        return;
    }
    draw.Triangle( cx + direction * 3.0f, cy, cx - direction * 2.0f, cy - direction * 4.0f, cx - direction * 2.0f, cy + direction * 4.0f, ink.r, ink.g, ink.b, 1.0f );
}

void PublishDrawStats( InGameUITab activeTab, const UIDrawList& frame, const UIDrawList& histogram, const UIDrawList& memory )
{
    char drawStatsFlag[2] = {};
    size_t drawStatsFlagLength = 0;
    const bool requested = getenv_s( &drawStatsFlagLength, drawStatsFlag, sizeof( drawStatsFlag ), "SKORE_UI_DRAW_STATS" ) == 0 && drawStatsFlag[0] != '\0';
                               ...
                               bool memoryEnabled )
{
    if ( histogramEnabled )
    {
        ...
        draw.PushClip( profiler.histogramViewport );
    ...
    if ( memoryEnabled )
    {
        ...
        draw.PushClip( memoryState.floatingViewport );
...
void UIWindowInteractionOwner::DrawMinimizedContent( const InGameUIFrameData& data, UIDrawList& drawList, int screenW, int screenH )
{
    drawList.Clear();
    ...
    m_window.minimizedWidth = data.editor.editorModeEnabled ? EditorMinimizedWidth( data.EditorTabFrame(), screenW )
    ...
    else
    {
        ...
        const uint32_t disabledMask = ( ( 1u << CAMERA_MODE_OPTION_COUNT ) - 1u ) & ~( data.surface.cameraModeEnabledMask & ( ( 1u << CAMERA_MODE_OPTION_COUNT ) - 1u ) );
        m_cameraModeCombo.Draw( draw, "", { std::span<const char* const>( kCameraModeOptions ), cameraModeIndex, disabledMask }, { m_mouseX, m_mouseY } );
    }

    DrawEditorObjectCounter( draw, data, screenW, screenH );
}

void UIWindowInteractionOwner::DrawRenderTabContent( const InGameUIFrameData& data, const UIDrawContext& draw, const UIRect& content, float scrolledY )
{
    char buffer[128];
    ...
    DrawContentToggle( draw, content.y, content.h, m_renderShadowToggle, content.x, scrolledY + UI_RENDER_FEATURE_START_Y, colW, "Shadows", data.rendering.ordinaryRender.shadow.enabled );
    m_saveRenderDefaultsButton.SetBounds( content.x + content.w - UI_RENDER_SAVE_BUTTON_W, scrolledY + UI_RENDER_FEATURE_START_Y, UI_RENDER_SAVE_BUTTON_W, 24.0f );
    ...
    for ( int index = 0; index < static_cast<int>( UIRenderParam::Count ); ++index )
    {
        ...
        if ( RenderSliderStartsSection( index ) && IsRowVisible( content.y, content.h, sliderY - UI_RENDER_SECTION_H + 4.0f, 18.0f ) )
        {
            DrawSectionTitle( draw, content.x, content.y, content.h, sliderY - UI_RENDER_SECTION_H + 4.0f, 12.0f, UIRenderAuthoringSectionName( spec.section ) );

            if ( spec.section == UIRenderAuthoringSection::PredictionPaths )
            {
                m_saveTrajectoryStyleButton.SetBounds( content.x + content.w - UI_TRAJECTORY_SAVE_BUTTON_W, sliderY - UI_RENDER_SECTION_H + 1.0f, UI_TRAJECTORY_SAVE_BUTTON_W, 20.0f );
                m_saveTrajectoryStyleButton.Draw( draw, "Save Paths", m_mouseX, m_mouseY );
            }
        }

        const float value = std::clamp( RenderValueForParam( data.rendering.ordinaryRender, spec.param ), spec.minValue, spec.maxValue );
...
void UIWindowInteractionOwner::DrawTargetsTabContent( const InGameUIFrameData& data, const UIDrawContext& draw, UIDrawList& drawList, const UIRect& content, float scrolledY )
{
    ...
    {
        DrawLabelValueAt( draw, content.y, content.h, content.x, scrolledY + UI_TARGETS_META_Y - 24.0f, "Resources", countText, palette.accent.r, palette.accent.g, palette.accent.b );
    }
    ...
    if ( selected )
    {
        ...
        if ( available )
        {
            snprintf( detailText, sizeof( detailText ), "%s, %d x %d, #%d", RenderTargetPreviewTypeText( *selected ), selected->width, selected->height, selectedIndex );
        }
        else
        {
    ...
    if ( IsBlockVisible( content.y, content.h, previewPanel.y, previewPanel.h ) )
    {
        ...
        const UIRect inset = { previewPanel.x + 10.0f, previewPanel.y + 10.0f, (std::max)( 1.0f, previewPanel.w - 20.0f ), (std::max)( 1.0f, previewPanel.h - 20.0f ) };
    ...
    if ( available && IsBlockVisible( content.y, content.h, previewImage.y, previewImage.h ) )
    {
        ...
        drawList.AddPreviewImage( { static_cast<uint16_t>( selectedIndex ), true }, previewImage, palette.windowSubtle, "Preview unavailable" );
        drawList.PopClip();
    }
    else if ( IsRowVisible( content.y, content.h, scrolledY + UI_TARGETS_PREVIEW_Y + 116.0f, 18.0f ) )
    {
        draw.Text( previewPanel.x + 18.0f, previewPanel.y + 116.0f, 12.0f, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b, "Not available this frame" );
    }

    if ( IsBlockVisible( content.y, content.h, previewPanel.y, previewPanel.h ) )
    {
        draw.Outline( previewImage.x, previewImage.y, previewImage.w, previewImage.h, palette.border.r, palette.border.g, palette.border.b, 0.72f );
    }
...
void DrawWideFooterStats( const InGameUIFrameData& data, const UIFooterDrawContext& context, const UIFooterGeometry& geometry, float statsX, float statsWidth )
{
    const Style::UIPalette& palette = Style::Palette();
    char status[128];
    const float frameDisplayMs = data.surface.fps > 0.0f ? 1000.0f / data.surface.fps : 0.0f;
    const int cpuPercent = static_cast<int>( std::clamp( ( data.surface.renderMs + data.surface.physicsMs ) / 16.67f * 100.0f, 0.0f, 99.0f ) );
    ...
    DrawFooterStatCell( context.draw, statsX + 18.0f, geometry.footerY, "FPS", status, palette.accent.r, palette.accent.g, palette.accent.b );
    DrawFooterStatDivider( context.draw, statsX + cellWidth, geometry.footerY );
    snprintf( status, sizeof( status ), "%.2f ms", frameDisplayMs );
    DrawFooterStatCell( context.draw, statsX + cellWidth + 18.0f, geometry.footerY, "Frame Time", status, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b );
    DrawFooterStatDivider( context.draw, statsX + cellWidth * 2.0f, geometry.footerY );
    snprintf( status, sizeof( status ), "%d%%", cpuPercent );
    DrawFooterStatCell( context.draw, statsX + cellWidth * 2.0f + 18.0f, geometry.footerY, "CPU", status, palette.accent.r, palette.accent.g, palette.accent.b );
    DrawFooterStatDivider( context.draw, statsX + cellWidth * 3.0f, geometry.footerY );
    snprintf( status, sizeof( status ), "%d%%", gpuPercent );
    DrawFooterStatCell( context.draw, statsX + cellWidth * 3.0f + 18.0f, geometry.footerY, "GPU", status, palette.accent.r, palette.accent.g, palette.accent.b );
    ...
    DrawFooterStatCell( context.draw, statsX + cellWidth * 4.0f + 18.0f, geometry.footerY, "Draws / UI", status, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b );
}
...
void DrawFooterStats( const InGameUIFrameData& data, const UIFooterDrawContext& context, const UIFooterGeometry& geometry )
{
    ...
    {
        if ( context.titleStatWidth > 0.0f && context.titleStatX + context.titleStatWidth < context.x + context.width - 116.0f )
        {
            context.draw.Text( context.titleStatX, context.y + 17.0f, 10.5f, palette.accent.r, palette.accent.g, palette.accent.b, context.titleStat );
        }
        return;
    }
    ...
    context.draw.RoundedPanel( { statsX, geometry.footerY + 16.0f, statsWidth, 56.0f }, Style::Radii().control, palette.windowSubtle, palette.innerBorder );
    if ( statsWidth < 350.0f )
    {
        ...
        snprintf( drawText, sizeof( drawText ), "%d/%d", data.surface.drawCallsBeforeUI + data.surface.UIDrawCalls, data.surface.UIDrawCalls );
        DrawCompactFooterStat( context.draw, statsX, geometry.footerY + 23.0f, "FPS", fpsText, palette.accent.r, palette.accent.g, palette.accent.b );
        DrawCompactFooterStat( context.draw, statsX, geometry.footerY + 41.0f, "Frame", frameText, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b );
        DrawCompactFooterStat( context.draw, statsX, geometry.footerY + 59.0f, "Draw/UI", drawText, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b );
        return;
    }
...
UIRect UIWindowInteractionOwner::DrawCompactToolsFooter( const InGameUIFrameData& data, const UIDrawContext& draw, const ToolsChromeRects& chrome )
{
    ...
    if ( m_reflectionCombo.IsOpen() )
    {
        ...
        m_reflectionCombo.Draw( draw, "", { std::span<const char* const>( reflections ), WaterReflectionModeFromData( data ), ReflectionDisabledMask() }, { m_mouseX, m_mouseY } );
    }
    else
    {
        m_toolsDisplayCombo.Draw( draw, "", { std::span<const char* const>( options ), -1, 1u, "Display settings" }, { m_mouseX, m_mouseY } );
    }
                                                    ...
                                                    const char* titleStat )
{
    ...
    context.draw.Rect( context.x + 16.0f, geometry.footerY, context.width - 32.0f, 1.0f, palette.lineSoft.r, palette.lineSoft.g, palette.lineSoft.b, 0.14f );
    ...
    m_rendererCombo.Draw( context.draw, "Renderer", { std::span<const char* const>( rendererOptions ), 0 }, { m_mouseX, m_mouseY } );
    ...
    m_reflectionCombo.Draw( context.draw, "Water", { std::span<const char* const>( reflectionOptions ), WaterReflectionModeFromData( data ), ReflectionDisabledMask() }, { m_mouseX, m_mouseY } );
    DrawFooterStats( data, context, geometry );
    context.draw.Rect( context.x + context.width - 24.0f, context.y + context.height - 9.0f, 14.0f, 2.0f, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b, 0.58f );
    context.draw.Rect( context.x + context.width - 18.0f, context.y + context.height - 15.0f, 8.0f, 2.0f, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b, 0.46f );
    context.draw.Rect( context.x + context.width - 12.0f, context.y + context.height - 21.0f, 2.0f, 2.0f, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b, 0.38f );
    return controlsBounds;
}


void UIWindowInteractionOwner::DrawActiveTabContent( const InGameUIFrameData& data, const UIDrawContext& draw, UIDrawList& drawList, const UIRect& content, float scrolledY )
{
    ...
    case InGameUITab::Profiler:
        ProfilerTab::Draw( m_profilerTab, draw, data.ProfilerTabFrame(), content.x, content.y, content.w, content.h, m_scrollY, m_activeSlider );
        break;
    case InGameUITab::Memory:
        MemoryTab::Draw( draw, m_memoryOverlay, data.MemoryTabFrame(), content.x, content.y, content.w, content.h, scrolledY, m_activeSlider, m_mouseX, m_mouseY );
        break;
    ...
    case InGameUITab::Options:
        OptionsTab::Draw( m_optionsTab, draw, data.OptionsTabFrame(), content.x, content.y, content.w, content.h, scrolledY, m_activeSlider );
        break;
    ...
    case InGameUITab::Sky:
        SkyTab::Draw( m_skyTab, draw, data.rendering.cinematic, content.x, content.y, content.w, content.h, scrolledY, m_mouseX, m_mouseY );
        break;
    case InGameUITab::Cinematic:
        CinematicTab::Draw( m_cinematicTab, draw, data.CinematicTabFrame(), content.x, content.y, content.w, content.h, scrolledY, m_mouseX, m_mouseY );
        break;
    case InGameUITab::Keys:
        ControlsTab::Draw( m_controlsTab, draw, data.ControlsTabFrame(), content.x, content.y, content.w, content.h, scrolledY );
        break;
    default:
        break;
    }
...
{
    return m_windowInteraction.BlocksCameraMouse() || m_panelTransitions->BlocksPointer( { static_cast<float>( m_windowInteraction.m_mouseX ), static_cast<float>( m_windowInteraction.m_mouseY ) } );
}
...
{
    return m_windowInteraction.BlocksCauseMouse() || m_panelTransitions->BlocksPointer( { static_cast<float>( m_windowInteraction.m_mouseX ), static_cast<float>( m_windowInteraction.m_mouseY ) } );
}
...
GameLayout::ComboPopupPresentation InGameUI::RecordingPopup() const
{
    ...
    return { popup.DropdownBounds( visible ), owner.m_sceneTab.recordingComboScroll, popup.VisibleOptionCount( visible ), count, popup.IsOpen(), m_sceneNavigation.recordings.selectedIndex };
}

GameLayout::ComboPopupPresentation InGameUI::CameraPopup() const
{
    const UIComboBox& popup = m_windowInteraction.m_cameraModeCombo;
    return { popup.DropdownBounds( CAMERA_MODE_OPTION_COUNT ), 0, CAMERA_MODE_OPTION_COUNT, CAMERA_MODE_OPTION_COUNT, popup.IsOpen() };
}
...
void UIWindowInteractionOwner::DrawHitboxOverlay( const UIDrawContext& draw, const InGameUIFrameData& data, const UIRect& windowBounds, const UIRect& contentBounds, const UIRect& footerBounds )
{
...
void UIWindowInteractionOwner::DrawWindowHitboxes( const UIDrawContext& draw, const UIRect& windowBounds, const UIRect& contentBounds )
{
...
void UIWindowInteractionOwner::DrawActiveTabHitboxes( const UIDrawContext& draw, const InGameUIFrameData& data )
{
    ...
    {
    case InGameUITab::Scene:
        DrawComboHitboxes( draw, m_sceneTab.combo, SceneDropdownHitboxOptionCount( m_sceneTab, data.SceneTabFrame() ), contentR, contentG, contentB );
                                           ...
                                           uint32_t cameraModeEnabledMask )
{
    ...
    if ( !m_windowInteraction.m_interaction.isResizing && m_panelTransitions->BlocksPointer( { static_cast<float>( input.mouseX ), static_cast<float>( input.mouseY ) } ) )
    {
    ...
    return m_windowInteraction
        .UpdateInput( pointerInput, m_sceneNavigation, screenWidth, screenHeight, now, editorModeEnabled, placementModeEnabled, placeStaticObject, autoTerrainAlign, cameraModeEnabledMask );
}
...
void InGameUI::SetPresentationWorkspace( Workspace workspace )
{
    ...
    m_windowInteraction.m_presentationRects = ComputePresentationRects( m_windowInteraction.m_presentation, m_windowInteraction.m_lastScreenW, m_windowInteraction.m_lastScreenH );
}
...
{
    return m_windowInteraction.m_presentationEnabled &&
           ( PresentationLayout() == LayoutMode::Editor || PresentationWorkspace() == Workspace::SolverLab || m_windowInteraction.m_presentation.detailsOpen || ( IsVisible() && !IsMinimized() ) );
}
...
GameLayout::DiagnosticPresentation InGameUI::DiagnosticPresentation() const
{
    ...
    view.markerBounds = view.markerHistoryVisible
                            ? ( profiler.histogramDockedBounds.h > 0 ? profiler.histogramDockedBounds
                                                                     : UIRect { profiler.histogramPanelX, profiler.histogramPanelY, profiler.histogramPanelW, profiler.histogramPanelH } )
                            : UIRect {};
    ...
    view.drawExpanderBounds = ProfilerTab::FirstDrawExpanderBounds( m_windowInteraction.m_profilerTab, ToolsContentBounds(), m_windowInteraction.m_scrollY );
    for ( int index = 0; index < m_windowInteraction.m_profilerTab.expandedHashCount; ++index )
    {
        view.profilerExpansionHash = HashCombine( view.profilerExpansionHash, m_windowInteraction.m_profilerTab.expandedHashes[index] );
    }
    for ( int index = 0; index < m_windowInteraction.m_profilerTab.drawExpandedHashCount; ++index )
    {
        view.drawExpansionHash = HashCombine( view.drawExpansionHash, m_windowInteraction.m_profilerTab.drawExpandedHashes[index] );
    }
    for ( int index = 0; index < m_windowInteraction.m_profilerTab.histogramOptionCount; ++index )
    {
        if ( m_windowInteraction.m_profilerTab.histogramOptionSelected[index] )
        {
            ...
            view.markerSelectionHash = HashCombine( view.markerSelectionHash, m_windowInteraction.m_profilerTab.histogramOptionFrameTotals[index] ? 1u : 2u );
            view.markerSelectionHash = HashCombine( view.markerSelectionHash, m_windowInteraction.m_profilerTab.histogramOptionHashes[index] );
        }
    }
    return view;
}
...
void UIWindowInteractionOwner::DrawPresentationDocks( const InGameUIFrameData& data )
{
...

    {
        ...
        const bool folded[] = { m_presentation.preferences.leftFolded, m_presentation.preferences.rightFolded, m_presentation.preferences.replayFolded };
    ...
    const float transportAlpha = m_presentation.workspace == Workspace::SolverLab || m_presentation.preferences.layout == LayoutMode::Editor ? 1.0f
    ...
    if ( transportAlpha > 0.0f )
    {
        ...
        {
            draw.Rect( 0.0f, m_presentationRects.transport.y, m_presentationRects.window.w, m_presentationRects.transport.h, palette.window.r, palette.window.g, palette.window.b, transportAlpha );
        }
        if ( !m_presentation.toolsOpen )
        {
            ...
            draw.Text( details.x + 10.0f, details.y + 8.0f, 11.0f, palette.textPrimary.r * transportAlpha, palette.textPrimary.g * transportAlpha, palette.textPrimary.b * transportAlpha, "Tools" );
        }
    }
    ...
    for ( int index = 0; index < 2; ++index )
    {
        if ( tabs[index].w > 0.0f )
        {
            ...
            draw.RoundedRect( tabs[index].x, tabs[index].y, tabs[index].w, tabs[index].h, 4.0f, fill.r, fill.g, fill.b, 1.0f );
            if ( selected )
            {
...
void UIWindowInteractionOwner::DrawEditorDock( const InGameUIFrameData& data )
{
    ...
    const auto verticalTab = [&]( const UIRect& bounds, const char* label, const Style::UIColor& color )
    {
        ...
        draw.VerticalText( { bounds.x + ( bounds.w - font ) * 0.5f, bounds.y + ( bounds.h - advance * font ) * 0.5f }, font, color, label );
        draw.PopClip();
    };
    ...
    for ( int index = 0; index < 2; ++index )
    {
        ...
        const char* label = index == 1 ? "Replay" : ( m_presentation.workspace == Workspace::SolverLab ? "Controls" : "Editor" );
        if ( panes[index].w <= 24 )
        {
        ...
        else
        {
            ...
            draw.Text( tabs[index].x + 8, tabs[index].y + 6, 12, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b, label );
        }
    ...
    for ( int index = 0; index < 3; ++index )
    {
        ...
        const bool active = grip.Contains( m_mouseX, m_mouseY ) || ( m_interaction.isResizing && m_interaction.resizeRegion == ( index == 2 ? 3 : 2 ) );
    ...
    if ( bounds.w > 0.0f )
    {
        ...
        EditorTab::Draw( m_editorTab, draw, data.EditorTabFrame(), bounds.x, bounds.y, bounds.w, bounds.h, bounds.y - m_presentationRects.editorScroll, m_mouseX, m_mouseY );
...
void UIWindowInteractionOwner::DrawPresentationHeader( const InGameUIFrameData& data )
{
    ...
    if ( !m_presentationEnabled ||
         ( m_presentation.workspace == Workspace::Scene && m_presentation.preferences.layout == LayoutMode::Canvas && !m_presentationHeaderHovered && !m_cameraModeCombo.IsOpen() ) )
    {
        return;
    }
    const UIDrawContext draw( data.surface.screenW, data.surface.screenH, m_frameDrawList );
    ...
    draw.Rect( 0.0f, 0.0f, m_presentationRects.header.w, m_presentationRects.header.h, palette.window.r, palette.window.g, palette.window.b, 1.0f );
    draw.Rect( 0.0f, m_presentationRects.header.h - 1.0f, m_presentationRects.header.w, 1.0f, palette.border.r, palette.border.g, palette.border.b, 0.65f );
    if ( m_presentation.toolsOpen || bounds.skull.Contains( m_mouseX, m_mouseY ) )
    {
        ...
        draw.RoundedRect( bounds.skull.x - 2.0f, bounds.skull.y - 2.0f, bounds.skull.w + 4.0f, bounds.skull.h + 4.0f, 5.0f, fill.r, fill.g, fill.b, 1.0f );
    }
    ...
    const auto button = [&]( const UIRect& rect, const char* label )
    {
        ...
        draw.PushClip( rect );
        draw.Text( rect.x + ( rect.w < 50.0f ? 4.0f : 10.0f ), rect.y + 7.0f, 12.0f, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b, label );
        draw.PopClip();
    };
    ...
    if ( bounds.scene.w > 16.0f )
    {
        ...
        draw.Text( bounds.scene.x + 8.0f, bounds.scene.y + 6.0f, 14.0f, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b, scene.data() );
        draw.PopClip();
    }
    draw.PopClip();
    if ( m_presentation.workspace == Workspace::SolverLab )
    {
        ...
        draw.Text( bounds.camera.x + 4.0f, bounds.camera.y + 8.0f, 11.0f, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, bounds.camera.w < 80.0f ? "Pair" : "Paired view" );
        draw.PopClip();
        return;
    }
    m_cameraModeCombo.SetLabelVisible( false );
...
void UIWindowInteractionOwner::DrawToolsDrawerChrome( const UIDrawContext& draw, const UIRect& bounds )
{
    ...
    draw.Text( bounds.x + 46.0f, logoY + 4.0f, 13.0f, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b, "Tools" );
    const bool gripActive = m_presentationRects.drawerResize.Contains( m_mouseX, m_mouseY ) || ( m_interaction.isResizing && m_interaction.resizeRegion == 1 );
...
{
    const bool sceneAvailable = m_window.isVisible && !m_window.isMinimized && m_activeTab == InGameUITab::Scene && !m_sceneTab.combo.IsOpen() && !m_sceneTab.recordingCombo.IsOpen() &&
    ...
    for ( const auto& candidate : candidates )
    {
        const bool editorAvailable = m_presentationRects.editorControls.Contains( m_mouseX, m_mouseY ) ||
                                     ( m_window.isVisible && !m_window.isMinimized && m_activeTab == InGameUITab::Editor && content.Contains( m_mouseX, m_mouseY ) );
    ...
    const UIDrawContext draw( data.surface.screenW, data.surface.screenH, m_frameDrawList );
    m_tooltip.Draw( draw, { 0.0f, 0.0f, static_cast<float>( data.surface.screenW ), static_cast<float>( data.surface.screenH ) }, hoverNow );
}
...
void UIWindowInteractionOwner::DrawDiagnosticLinks( const InGameUIFrameData& data )
{
    ...
    for ( int index = 0; index < 2; ++index )
    {
        ...
        {
            draw.Outline( panels[index].x, panels[index].y, panels[index].w, panels[index].h, palette.accent.r, palette.accent.g, palette.accent.b, 1.0f );
        }
        const auto& fill = details.Contains( m_mouseX, m_mouseY ) ? palette.controlHover : palette.control;
        draw.RoundedRect( details.x, details.y, details.w, details.h, 3.0f, fill.r, fill.g, fill.b, 1.0f );
        draw.Text( details.x + 10.0f, details.y + 5.0f, 11.0f, palette.textPrimary.r, palette.textPrimary.g, palette.textPrimary.b, "Details" );
    }
}
...
const UIDrawList& UIWindowInteractionOwner::Draw( const InGameUIFrameData& data )
{
    ...
    {
        AppendStandaloneOverlays( m_profilerTab, m_memoryOverlay, data, m_frameDrawList, m_histogramDrawList, m_memoryOverlayDrawList, screenW, screenH, histogramEnabled, memoryOverlayEnabled );
        return finishDraw();
    }
    ...
    cacheKey.styleSignature = HashBool( HashBool( 2166136261u + static_cast<uint32_t>( Style::CurrentTheme() ), m_blurPreviewEnabled ), m_hitboxOverlayEnabled );
    ...
    cacheKey.interactionSignature = BuildUIInteractionSignature( { { m_mouseX, m_mouseY }, windowBounds, openControls, m_selectedRenderTargetPreview, m_activeSlider } );
    ...
    m_backdropBlur.Draw( draw, blurBounds, screenW, screenH, data.scene.currentFrame, data.surface.now, m_blurPreviewEnabled );
    ...
    draw.RoundedPanel( { contentX - inset, contentY - inset, contentW + inset * 2.0f, contentH + inset + 2.0f }, Style::Radii().window, palette.windowSubtle, palette.innerBorder );
    ...
    m_scrollBar.Draw( draw, static_cast<float>( ContentHeight() ), contentH, m_scrollY, m_scrollbarVisibleUntil, data.surface.now );
