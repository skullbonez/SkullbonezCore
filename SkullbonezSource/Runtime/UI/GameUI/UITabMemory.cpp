...
{
    const int value = state.previewRetentionSeconds >= 0 ? state.previewRetentionSeconds : state.lastRequestedRetentionSeconds;
...
void SetReplayMemoryPolicyCommand( SkullbonezCore::UI::InGameUIInputResult& result, int presetIndex, int retentionSeconds, int budgetMiB )
{
...
void SetReplayPolicyControlBounds( SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, float contentX, float panelY, float contentW )
{
    const float buttonGap = 6.0f;
    const float buttonW = ( contentW - 28.0f - buttonGap * static_cast<float>( SkullbonezCore::UI::MemoryTab::MEMORY_REPLAY_PRESET_COUNT - 1 ) ) /
    ...
    {
        state.replayPresetButtons[i].SetBounds( contentX + 14.0f + static_cast<float>( i ) * ( buttonW + buttonGap ), buttonY, buttonW, 24.0f );
    }
...
void RefreshReplayPolicySnapshot( SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, const SkullbonezCore::UI::UIMemoryTabFrameView& data )
{
    state.lastReplayPreset = std::clamp( data.replayMemoryPreset, 0, SkullbonezCore::UI::MemoryTab::MEMORY_REPLAY_PRESET_COUNT - 1 );

    state.lastRequestedRetentionSeconds = data.replayMemoryRequestedRetentionSeconds > 0 ? data.replayMemoryRequestedRetentionSeconds : MEMORY_REPLAY_DEFAULT_RETENTION_SECONDS;

    state.lastRequestedBudgetMiB = data.replayMemoryRequestedBudgetMiB > 0 ? data.replayMemoryRequestedBudgetMiB : MEMORY_REPLAY_DEFAULT_BUDGET_MIB;

    state.lastPresentationRetentionSeconds = data.replayMemoryPresentationRetentionSeconds > 0 ? data.replayMemoryPresentationRetentionSeconds : state.lastRequestedRetentionSeconds;

    state.lastSolverRetentionSeconds = data.replayMemorySolverRetentionSeconds > 0 ? data.replayMemorySolverRetentionSeconds : state.lastRequestedRetentionSeconds;
...
float MemoryOverlayYForBytes( const SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, const SkullbonezCore::UI::UIRect& plot, uint64_t bytes )
{
    const uint64_t axisSpan = state.axisMaxBytes > state.axisMinBytes ? state.axisMaxBytes - state.axisMinBytes : 1u;
    const double normalized = ( static_cast<double>( bytes ) - static_cast<double>( state.axisMinBytes ) ) / static_cast<double>( axisSpan );

    return plot.y + plot.h - std::clamp( static_cast<float>( normalized ), 0.0f, 1.0f ) * plot.h;
}

void DrawMemoryOverlayLineSegment( const SkullbonezCore::UI::UIDrawContext& draw, float x0, float y0, float x1, float y1, float thickness, float r, float g, float b, float a )
{
...
void RetainOverlayEvent( SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, const SkullbonezCore::Core::Allocation::RuntimeReserveGrowthEventView& event, uint64_t levelBytes, double now )
{
...
void RefreshOverlayEvents( SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, const SkullbonezCore::UI::UIMemoryTabFrameView& data, uint64_t levelBytes )
{
    ...
    const int eventCount = std::clamp( data.reserveGrowthEventCount, 0, SkullbonezCore::UI::UI_RUNTIME_RESERVE_GROWTH_EVENT_MAX );
...
void PushOverlaySample( SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, const SkullbonezCore::UI::UIMemoryTabFrameView& data )
{
...
void RefreshOverlayAxis( SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state, const SkullbonezCore::UI::UIMemoryTabFrameView& data )
{
    ...
    {
        const int sampleIndex = ( state.sampleHead - state.sampleCount + i + SkullbonezCore::UI::MemoryTab::MEMORY_OVERLAY_SAMPLE_COUNT ) % SkullbonezCore::UI::MemoryTab::MEMORY_OVERLAY_SAMPLE_COUNT;
    ...
    const uint64_t paddedSpan = (std::max)( OverlayMinimumAxisSpan( currentTotal ), visibleSpan + visibleSpan / 2u + 1u * MEMORY_BYTES_PER_MIB );
...
void MemoryEventColor( const SkullbonezCore::Core::Allocation::RuntimeReserveGrowthEventView& event, float& r, float& g, float& b )
{
    if ( !event.granted )
    {
        r = 0.96f;
...
const SkullbonezCore::UI::MemoryTab::MemoryOverlayPinnedEvent* NewestPinnedEvent( const SkullbonezCore::UI::MemoryTab::UIMemoryOverlayState& state )
{
...
void DrawMemoryStackSegment( const SkullbonezCore::UI::UIDrawContext& draw, float& x, float y, float h, float stackW, float stackEndX, uint64_t bytes, uint64_t totalBytes, float r, float g, float b )
{
    ...
    const float w = std::clamp( static_cast<float>( static_cast<double>( bytes ) / static_cast<double>( totalBytes ) ) * stackW, 1.0f, remainingW );
...
void DrawMemoryRow( const SkullbonezCore::UI::UIDrawContext& draw, float x, float y, float labelW, const char* label, uint64_t bytes )
{
    char value[32] = {};
    ...
    draw.Text( x + labelW, y, 9.6f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, value );
}
...
uint64_t ReplayMemoryCategoryCounter( const SkullbonezCore::Core::MainMemoryReplayStats& replay, SkullbonezCore::Core::MainMemoryReplayByteCategory category )
{
...
)
{
    ...
    const SkullbonezCore::UI::Style::UIPalette& palette = SkullbonezCore::UI::Style::Palette();
    draw.Rect( contentX, panelY, contentW, MEMORY_REPLAY_POLICY_BLOCK_H, tablePalette.windowSubtle.r, tablePalette.windowSubtle.g, tablePalette.windowSubtle.b, 1.0f );
    draw.Outline( contentX, panelY, contentW, MEMORY_REPLAY_POLICY_BLOCK_H, tablePalette.border.r, tablePalette.border.g, tablePalette.border.b, 0.62f );
    draw.Rect( contentX, panelY + 27.0f, contentW, 1.0f, tablePalette.lineSoft.r, tablePalette.lineSoft.g, tablePalette.lineSoft.b, 0.45f );
    draw.Text( contentX + 14.0f, panelY + 9.0f, 10.4f, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, "Replay Policy" );
    ...
    for ( int i = 0; i < SkullbonezCore::UI::MemoryTab::MEMORY_REPLAY_PRESET_COUNT; ++i )
    {
        DrawReplayPresetButton( draw, state.replayPresetButtons[i], ReplayMemoryPresetLabel( i ), state.lastReplayPreset == i, state.replayPresetButtons[i].HitTest( mouseX, mouseY ) );
    }
    ...
    {
        state.replayRetentionSlider
            .Draw( draw, "Retention", text, static_cast<float>( displayRetention ), static_cast<float>( MEMORY_REPLAY_RETENTION_MIN ), static_cast<float>( MEMORY_REPLAY_RETENTION_MAX ) );
    }
    ...
    {
        state.replayBudgetSlider
            .Draw( draw, "Budget", text, static_cast<float>( displayBudget ), static_cast<float>( MEMORY_REPLAY_BUDGET_MIN_MIB ), static_cast<float>( MEMORY_REPLAY_BUDGET_MAX_MIB ) );
    }

    if ( state.lastBudgetClamped && activeSlider == 0 )
    {
        draw.Text( contentX + 14.0f, panelY + 136.0f, 8.0f, tablePalette.warningAccent.r, tablePalette.warningAccent.g, tablePalette.warningAccent.b, "budget clamp active" );
    }
}
...
)
{
    ...
    draw.Rect( panelX, panelY, panelW, panelH, tablePalette.windowSubtle.r, tablePalette.windowSubtle.g, tablePalette.windowSubtle.b, 1.0f );
    draw.Outline( panelX, panelY, panelW, panelH, tablePalette.border.r, tablePalette.border.g, tablePalette.border.b, 0.62f );
    draw.Rect( panelX, panelY + 27.0f, panelW, 1.0f, tablePalette.lineSoft.r, tablePalette.lineSoft.g, tablePalette.lineSoft.b, 0.45f );
    draw.Text( x, panelY + 9.0f, 10.4f, palette.textSecondary.r, palette.textSecondary.g, palette.textSecondary.b, "Main Memory" );

    snprintf( text, sizeof( text ), "%s", memory.process.taskManagerMetricName );
    draw.Text( panelX + panelW - 118.0f, panelY + 9.0f, 9.2f, tablePalette.textMuted.r, tablePalette.textMuted.g, tablePalette.textMuted.b, text );
    ...
    draw.Text( subX, row0 + 18.0f, 8.4f, tablePalette.textMuted.r, tablePalette.textMuted.g, tablePalette.textMuted.b, text );
    ...
    draw.Text( subX, row0 + 32.0f, 8.1f, tablePalette.textMuted.r, tablePalette.textMuted.g, tablePalette.textMuted.b, text );

    DrawMemoryRow( draw, x, row0 + 50.0f, labelW, "Objects", memory.gameObjects.totalBytes );
    const uint64_t gameObjectStoreBytes = memory.gameObjects.physicsStoreBytes + memory.gameObjects.colliderStoreBytes + memory.gameObjects.renderStoreBytes;
    ...
    FormatReplayTrajectoryPair( pastPair, sizeof( pastPair ), "past", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::PastRoot );

    FormatReplayTrajectoryPair( futurePair, sizeof( futurePair ), "future", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::FutureRoot );

    FormatReplayTrajectoryPair( childInPair, sizeof( childInPair ), "child-in", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::FutureChildIncoming );

    FormatReplayTrajectoryPair( childOutPair, sizeof( childOutPair ), "child-out", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::FutureChildOutgoing );
    ...
    FormatReplayTrajectoryPair( retainedPair, sizeof( retainedPair ), "retained", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::RetainedTrail );

    FormatReplayTrajectoryPair( baselinePair, sizeof( baselinePair ), "base", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::BaselineRoot );

    FormatReplayTrajectoryPair( markerPair, sizeof( markerPair ), "mark", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::CausalMarker );

    FormatReplayTrajectoryPair( auxiliaryPair, sizeof( auxiliaryPair ), "aux", memory.replay.trajectory, SkullbonezCore::Core::MainMemoryReplayTrajectoryLane::AuxiliaryTrail );
    ...
    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PresentationBodies ), a, sizeof( a ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::SolverBodies ), b, sizeof( b ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PredictionFrameBodies ), c, sizeof( c ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::LoadedBodies ), d, sizeof( d ) );
    ...
    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::SolverWorldState ), a, sizeof( a ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PredictionWorldState ), b, sizeof( b ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PredictionEngine ), c, sizeof( c ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PredictionFutureTree ), d, sizeof( d ) );
    ...
    const uint64_t recordBytes = ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PresentationSampleRecords ) +
                                 ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::SolverSampleRecords ) +
                                 ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::LoadedSampleRecords ) +
                                 ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PredictionFrameRecords );

    const uint64_t checkpointBytes = ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PresentationCheckpoints ) +
                                     ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::SolverCheckpoints );

    const uint64_t scratchBytes = ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PresentationScratch ) +
    ...
    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::Events ), d, sizeof( d ) );
    ...
    const uint64_t visualPathBytes = ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PathTargets ) +
                                     ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PathFutureNodes );

    const uint64_t launcherVisualBytes = ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::SolverLauncherVisuals ) +
                                         ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::RenderLauncherBackup );

    FormatMemoryMiB( visualPathBytes, a, sizeof( a ) );
    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::PathCauseRows ), b, sizeof( b ) );

    FormatMemoryMiB( ReplayMemoryCategoryCounter( memory.replay, SkullbonezCore::Core::MainMemoryReplayByteCategory::RenderGhostRequests ), c, sizeof( c ) );
    ...
    const uint64_t buildEvidenceBytes = memory.replay.predictionEvidence.buildContactCapacityBytes + memory.replay.predictionEvidence.buildPipelineCapacityBytes +
                                        memory.replay.predictionEvidence.buildFrameCapacityBytes;
    const uint64_t committedEvidenceBytes = memory.replay.predictionEvidence.committedContactCapacityBytes + memory.replay.predictionEvidence.committedPipelineCapacityBytes +
    ...
    draw.Text( x, row0 + 224.0f, 8.0f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, text );
    ...
    FormatMemoryMiB( render.uploadCategoryPeakBytes[static_cast<std::size_t>( SkullbonezCore::UI::UIRenderUploadCategory::Constants )], a, sizeof( a ) );

    FormatMemoryMiB( render.uploadCategoryPeakBytes[static_cast<std::size_t>( SkullbonezCore::UI::UIRenderUploadCategory::DynamicVertex )], b, sizeof( b ) );

    FormatMemoryMiB( render.uploadCategoryPeakBytes[static_cast<std::size_t>( SkullbonezCore::UI::UIRenderUploadCategory::InstanceData )], c, sizeof( c ) );
    ...
    FormatMemoryMiB( render.uploadCategoryPeakBytes[static_cast<std::size_t>( SkullbonezCore::UI::UIRenderUploadCategory::TextureRows )], a, sizeof( a ) );

    FormatMemoryMiB( render.uploadCategoryPeakBytes[static_cast<std::size_t>( SkullbonezCore::UI::UIRenderUploadCategory::RetainedGeometry )], b, sizeof( b ) );
...
{
    const int rowCount = data.reserveCapacityRows ? std::clamp( data.reserveCapacityRowCount, 0, SkullbonezCore::UI::UI_RUNTIME_RESERVE_CAPACITY_ROW_MAX ) : 0;
...
)
{
    ...
    const SkullbonezCore::UI::UIRuntimeReserveCapacityRow* sortedRows[SkullbonezCore::UI::UI_RUNTIME_RESERVE_CAPACITY_ROW_MAX] = {};
    ...
    draw.Rect( tableX, visibleTableTop, tableW, visibleTableBottom - visibleTableTop, tablePalette.windowSubtle.r, tablePalette.windowSubtle.g, tablePalette.windowSubtle.b, 1.0f );
    ...
    {
        draw.Rect( tableX, headerY + 20.0f, tableW, 1.0f, tablePalette.lineSoft.r, tablePalette.lineSoft.g, tablePalette.lineSoft.b, 0.45f );
        draw.Text( ownerX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Owner" );
        draw.Text( subsystemX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "System" );
        draw.Text( elementX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Elem" );
        draw.Text( capacityX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Cap" );
    ...
    for ( int index = 0; index < rowCount; ++index )
    {
        ...
        const double peakUtilisation = row.currentCapacity > 0 ? static_cast<double>( row.sessionHighWater ) * 100.0 / static_cast<double>( row.currentCapacity ) : 0.0;
        ...
        draw.Rect( tableX, rowY + MEMORY_CAPACITY_ROW_H - 1.0f, tableW, 1.0f, tablePalette.lineSoft.r, tablePalette.lineSoft.g, tablePalette.lineSoft.b, 0.30f );
        draw.Text( ownerX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, owner );
        draw.Text( subsystemX, rowY + 5.0f, 8.2f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, row.subsystemName );

        draw.Text( elementX, rowY + 5.0f, 8.2f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, element );
        draw.Text( capacityX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, capacity );
        draw.Text( liveX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, live );
        draw.Text( peakX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, peak );
        draw.Text( utilisationX, rowY + 5.0f, 8.2f, tablePalette.accentStrong.r, tablePalette.accentStrong.g, tablePalette.accentStrong.b, utilisation );
        draw.Text( residentX, rowY + 5.0f, 8.2f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, resident );
    }
}
...
)
{
    ...
    draw.Rect( tableX, tableY, tableW, tableH, tablePalette.windowSubtle.r, tablePalette.windowSubtle.g, tablePalette.windowSubtle.b, 1.0f );
    ...
    draw.Rect( tableX, headerY + 20.0f, tableW, 1.0f, tablePalette.lineSoft.r, tablePalette.lineSoft.g, tablePalette.lineSoft.b, 0.45f );
    draw.Text( frameX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Frame" );
    draw.Text( targetX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Struct" );
    draw.Text( allocX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Alloc" );
    draw.Text( capX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Capacity" );
    draw.Text( statusX, headerY, 8.8f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Status" );
    ...
    for ( int i = 0; i < eventCount; ++i )
    {
        ...
        draw.Rect( tableX, rowY + MEMORY_EVENT_ROW_H - 1.0f, tableW, 1.0f, tablePalette.lineSoft.r, tablePalette.lineSoft.g, tablePalette.lineSoft.b, 0.30f );
        draw.Text( frameX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, frame );
        draw.Text( targetX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, target );
        draw.Text( allocX, rowY + 5.0f, 8.2f, tablePalette.textPrimary.r, tablePalette.textPrimary.g, tablePalette.textPrimary.b, bytes );
        draw.Text( capX, rowY + 5.0f, 8.2f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, capacity );
...
int ContentHeight()
{
    return static_cast<int>( MEMORY_REPLAY_POLICY_BLOCK_H + MEMORY_PANEL_GAP + MEMORY_SUMMARY_BLOCK_H + 18.0f + MEMORY_CAPACITY_HEADER_H + static_cast<float>( UI_RUNTIME_RESERVE_CAPACITY_ROW_MAX ) * MEMORY_CAPACITY_ROW_H + MEMORY_CAPACITY_BOTTOM_PAD + MEMORY_EVENT_HEADER_H + static_cast<float>( UI_RUNTIME_RESERVE_GROWTH_EVENT_MAX ) * MEMORY_EVENT_ROW_H + MEMORY_EVENT_BOTTOM_PAD );
}
...
void DrawOverlay( UIMemoryOverlayState& state, const UIDrawContext& draw, const UIMemoryTabFrameView& data, float preferredX, float preferredY )
{
    ...
    const float panelX = std::clamp( preferredX, MEMORY_OVERLAY_MARGIN, (std::max)( MEMORY_OVERLAY_MARGIN, screenW - MEMORY_OVERLAY_PANEL_W - MEMORY_OVERLAY_MARGIN ) );

    const float panelY = std::clamp( preferredY, MEMORY_OVERLAY_MARGIN, (std::max)( MEMORY_OVERLAY_MARGIN, screenH - MEMORY_OVERLAY_PANEL_H - MEMORY_OVERLAY_MARGIN ) );
    ...
    const UIRect viewport = state.floatingViewport.w > 0 ? state.floatingViewport : UIRect { 0, 0, screenW, screenH };
    floating = Layout::ClampFloatingRect( floating, viewport, 80, 80 );
    ...
    const UIRect plot = { panel.x + gutter, panel.y + 38.0f, (std::max)( 1.0f, panel.w - gutter - railWidth - 14.0f ), (std::max)( 1.0f, panel.h - ( shortPanel ? 46.0f : 100.0f ) ) };
    ...
    draw.Rect( plot.x, plot.y + plot.h * 0.5f, plot.w, 1.0f, palette.lineSoft.r, palette.lineSoft.g, palette.lineSoft.b, 0.12f );
    ...
    if ( !shortPanel )
    {
        ...
        draw.Text( panel.x + 9.0f, plot.y - 1.0f, 8.2f, tablePalette.textMuted.r, tablePalette.textMuted.g, tablePalette.textMuted.b, axisText );
        FormatMemoryMiB( state.axisMinBytes, axisText, sizeof( axisText ) );
        draw.Text( panel.x + 9.0f, plot.y + plot.h - 9.0f, 8.2f, tablePalette.textMuted.r, tablePalette.textMuted.g, tablePalette.textMuted.b, axisText );
    }
    ...
    for ( int i = 0; i < state.sampleCount; ++i )
    {
        const int sampleIndex = ( state.sampleHead - state.sampleCount + i + MEMORY_OVERLAY_SAMPLE_COUNT ) % MEMORY_OVERLAY_SAMPLE_COUNT;
        ...
        if ( previousValid )
        {
            DrawMemoryOverlayLineSegment( draw, previousX, previousY, x, y, 2.0f, tablePalette.accentStrong.r, tablePalette.accentStrong.g, tablePalette.accentStrong.b, 0.88f );
        }

        draw.Rect( x - 1.0f, y - 1.0f, 2.0f, 2.0f, tablePalette.accentStrong.r, tablePalette.accentStrong.g, tablePalette.accentStrong.b, 0.82f );
        previousX = x;
        previousY = y;
        previousValid = true;
    }

    draw.RoundedRect( eventRail.x, eventRail.y, eventRail.w, eventRail.h, Style::Radii().control, tablePalette.windowRaised.r, tablePalette.windowRaised.g, tablePalette.windowRaised.b, 0.70f );

    draw.Outline( eventRail.x, eventRail.y, eventRail.w, eventRail.h, palette.innerBorder.r, palette.innerBorder.g, palette.innerBorder.b, 0.52f );

    if ( !shortPanel )
    {
        draw.Text( eventRail.x + 6.0f, eventRail.y + 5.0f, 8.2f, tablePalette.textSecondary.r, tablePalette.textSecondary.g, tablePalette.textSecondary.b, "Events" );
    }
    int retainedCount = 0;
    ...
    for ( int i = 0; i < state.pinnedEventCount; ++i )
    {
        ...
        const float alpha = pinned.event.granted ? std::clamp( 0.92f - static_cast<float>( ageSeconds ) * 0.012f, 0.46f, 0.92f ) : 0.96f;
    ...
    {
        draw.Text( panel.x + 10.0f, panel.y + panel.h - 24.0f, 9.2f, tablePalette.textMuted.r, tablePalette.textMuted.g, tablePalette.textMuted.b, "no allocator growth events" );
    }

    if ( state.retainedOverflowEventCount > 0u )
    {
        snprintf( text, sizeof( text ), "+%llu coalesced", static_cast<unsigned long long>( state.retainedOverflowEventCount ) );

        draw.Text( panel.x + panel.w - 92.0f, panel.y + panel.h - 24.0f, 8.2f, tablePalette.warningAccent.r, tablePalette.warningAccent.g, tablePalette.warningAccent.b, text );
    }
    if ( docked )
    {
        draw.PopClip();
    ...
    {
        draw.Rect( panel.x + panel.w - 13, panel.y + panel.h - 4, 10, 1, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b, 0.54f );
        draw.Rect( panel.x + panel.w - 8, panel.y + panel.h - 9, 5, 1, palette.textMuted.r, palette.textMuted.g, palette.textMuted.b, 0.44f );
    }
}
...
bool HandleContentClick( UIMemoryOverlayState& state, InGameUIInputResult& result, int& activeSlider, int mouseX, int mouseY, float contentX, float scrolledY, float contentW )
{
    ...
    if ( state.replayRetentionSlider.HitTest( mouseX, mouseY ) )
    {
        ...
        state.previewRetentionSeconds = static_cast<int>( state.replayRetentionSlider.ValueFromMouse( mouseX, static_cast<float>( MEMORY_REPLAY_RETENTION_MIN ), static_cast<float>( MEMORY_REPLAY_RETENTION_MAX ), 1.0f ) );
