...
UITooltipTarget PhysicsTooltip( const PhysicsTab::UIPhysicsTabState& state, int mouseX, int mouseY )
{
    const UITooltipTarget candidates[] = {
        { 2000, state.toggles[0].Bounds(), { "Show collision geometry and physics diagnostics." } },
        ...
        { 2018,
          state.launcherProjectileSpeedSlider.Bounds(),
          { "Set the launcher's initial projectile speed.", "Metres per second" } },
        ...
        { 2022,
          state.rollingFrictionSlider.Bounds(),
          { "Set rolling resistance for contacts.", "Rolling friction coefficient" } },
        ...
        { 2025,
          state.tornadoInwardSlider.Bounds(),
          { "Set acceleration toward the tornado's centre.", "Metres per second squared" } },
        { 2026,
          state.tornadoSwirlSlider.Bounds(),
          { "Set acceleration around the tornado's centre.", "Metres per second squared" } },
        { 2027,
          state.tornadoLiftSlider.Bounds(),
          { "Set upward acceleration inside the tornado.", "Metres per second squared" } },
    };
...
{
    const UITooltipTarget candidates[] = {
        { 2100, state.toggles[0].Bounds(), { "Use capture lockstep for fixed simulation advancement." } },
        ...
        { 2106,
          state.timeScaleSlider.Bounds(),
          { "Control how quickly the simulation advances.", "Multiplier of real time" } },
        { 2107,
          state.modelCountSlider.Bounds(),
          { "Change the generated model count. Release to rebuild the scene.", "Objects" } },
    };
    return HitTooltip( candidates, mouseX, mouseY );
}
...
{
    const UITooltipTarget candidates[] = {
        { 2200, state.replayPresetButtons[0].Bounds(), { "Request the Lossless replay memory preset." } },
        ...
        { 2203,
          state.replayRetentionSlider.Bounds(),
          { "Request replay history duration. Actual retained windows are shown below.", "Seconds" } },
        { 2204,
          state.replayBudgetSlider.Bounds(),
          { "Request a replay memory budget. The replay owner applies capacity limits.", "MiB (1,048,576 bytes)" } },
    };
    return HitTooltip( candidates, mouseX, mouseY );
}
} // namespace
...
{
    const UITooltipTarget actions[] = {
        { 2800, m_renderShadowToggle.Bounds(), { "Enable or disable ordinary rendering shadows." } },
        { 2801, m_saveRenderDefaultsButton.Bounds(), { "Save ordinary rendering settings as defaults." } },
        { 2802,
          m_saveTrajectoryStyleButton.Bounds(),
          { "Save edited path appearance through the render defaults profile." } },
    };
    ...
    for ( int index = 0; index < static_cast<int>( UIRenderParam::Count ); ++index )
    {
        ...
        {
            return { static_cast<uint32_t>( 2820 + index ), m_renderSliders[index].Bounds(), kRenderTooltipText[index],
                     true };
        }
    }
    return {};
...
UITooltipTarget UIWindowInteractionOwner::FindToolsTooltip( const UIRect& content ) const
{
    ...
    if ( content.Contains( m_mouseX, m_mouseY ) )
    {
        ...
        {
            const UITooltipTarget playback[] = {
                { 2520, m_sceneTab.pauseLockToggle.Bounds(), { "Keep scene simulation paused across scene changes." } },
                { 2521,
                  m_sceneTab.singleStepButton.Bounds(),
                  { "Advance one scene turn while Pause lock is enabled.", "One turn", "Space holds stepping",
                    "Enable Pause lock to step once." },
                  false,
                  false,
        ...
        case InGameUITab::Keys:
        {
            const UITooltipTarget candidates[] = {
                { 2700,
                  m_controlsTab.seedSlider.Bounds(),
                  { "Change the generated scene seed. Release to rebuild.", "Seed" } },
                { 2701,
                  m_controlsTab.solverBallSlider.Bounds(),
                  { "Change the generated sphere count. Release to rebuild.", "Objects" } },
                { 2702,
                  m_controlsTab.solverBoxSlider.Bounds(),
                  { "Change the generated box count. Release to rebuild.", "Objects" } },
                { 2703, m_controlsTab.worldFluidHeightSlider.Bounds(), { "Set the fluid surface height.", "World units" } },
                { 2704,
                  m_controlsTab.worldFluidDensitySlider.Bounds(),
                  { "Set fluid density used for buoyancy.", "Density" } },
            };
        ...
        case InGameUITab::Targets:
            return HitTooltip( std::array { UITooltipTarget { 2400,
                                                              m_renderTargetCombo.Bounds(),
                                                              { "Inspect an available render target. Unavailable targets "
                                                                "are disabled." } } },
                               m_mouseX, m_mouseY );
        default:
            return {};
    ...
    const UIRect bounds { static_cast<float>( m_window.x ), static_cast<float>( m_window.y ),
                          static_cast<float>( m_window.width ), static_cast<float>( m_window.height ) };
    if ( GameLayout::ComputeToolsChromeRects( bounds, m_presentationEnabled ).compact )
    {
        const UITooltipTarget compact[] = {
            { 2510, m_toolsTabCombo.Bounds(), { "Open any Tools tab. Scroll the list to see all tools." } },
            { 2511,
              m_toolsDisplayCombo.Bounds(),
              { "Open renderer, reflection, blur, VSync and diagnostic display settings." } },
        };
        return HitTooltip( compact, m_mouseX, m_mouseY );
    }
    const UITooltipTarget footer[] = {
        { 2500, m_rendererCombo.Bounds(), { "The active rendering backend is DirectX 12." } },
        ...
        { 2505,
          m_histogramToggle.Bounds(),
          { "Show or hide the Canvas marker-history chart. Editor keeps it pinned.", "Milliseconds", "F5" } },
        { 2506,
          m_timelineToggle.Bounds(),
          { "Switch the detailed Profiler between its table and timeline views.", "Milliseconds" } },
    };
