/*
File: SkullbonezSource/Runtime/Camera.h
Purpose:
...
class Camera
{
  ...
  private:
    ...
    void FinishTranslation( const CameraMovementSettings&
                                settings );                                  // Commits or clamps the staged translation while preserving view distance.
    ...
    void SetAll( const Math::Vector::Vector3& vPosition,
                 const Math::Vector::Vector3& vView,
                 const Math::Vector::Vector3& vUpVector );
    void
    MoveCamera( const TravelDirection enumDir,
                float fQuantity,
                const CameraMovementSettings& settings );                    // fQuantity is world-space travel along camera-local axes.
    void ApplyDelta( const Camera& delta,
                     const CameraMovementSettings&
                         settings );                                         // Relative cameras add the primary-camera pose delta through this helper.
    void RotateCamera( float xMove,
                       float yMove,
                       const CameraMovementSettings&
                           settings );                                       // Mouse-look delta path; clamps pitch before mutating the view target.
