...
{
    return {
        { "enabled", state.predictionEnabled },
...
{
    return {
        { "stage", Physics::PhysicsPipelineStageName( row.stage ) },
        { "bodyA", row.bodyA },
        { "bodyB", row.bodyB },
...
Json BuildTrajectories( const ReplayAutomationView& replay, SkarnessStateDetail detail )
{
    ...
    return {
        { "topologyVersion", replay.visualPacket.header.topologyVersion },
...
Json BuildRenderGeometryEvidence( const ReplayVisualPacket& packet )
{
    ...
    const uint64_t
        ribbonHash = ReplayVisualPacketOperations::CombineReplayVisualSubmissionHashes( facts.ordinaryRibbonHash, facts.ordinaryRibbonBytes, facts.priorityRibbonHash, facts.priorityRibbonBytes );
...
Json BuildDivergence( const ReplayAutomationView& replay )
{
    ...
    const auto found = std::lower_bound(
        replay.divergenceBlueFrames.begin(),
    ...
    return {
        { "allocatedOwnerBytes", replay.divergenceAllocatedBytes },
...
Json BuildLegacyReplay( const SkarnessFrameState& state, const ReplayAutomationView& replay )
{
    const auto loading = BuildCauseLoading( replay );
    return {
        { "divergence", BuildDivergence( replay ) },
        { "predictionEnabled", state.predictionEnabled },
