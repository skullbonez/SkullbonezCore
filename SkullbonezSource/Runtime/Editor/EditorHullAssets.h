// Invariant: only foliage/trunk pieces opt into impact release by default;
// structural rocks and generic primitives remain fixed unless explicitly authored.    // Compatibility: pre-token-table
// scenes used these two short aliases.// Caller contract: accepts canonical tokens, tracked paths, and filenames so old
// scene spellings resolve to one current built-in asset identity.// Compatibility: rooted-tree offsets preserve existing
// scene placement and snapshot reconstruction; these are authored world-space Y distances./*
File : SkullbonezSource / Runtime / Editor / EditorHullAssets.h Purpose : Names built -
        in convex hull assets used by the editor and scene snapshots.

        Summary : Scene files should prefer stable logical hull ids such as diamond or
    hex_prism.The asset boundary resolves those ids to the current hull files
            .

        Glossary : Hull asset : Authored convex collision mesh used by editor primitives.Hull token : Stable scene -
        file string that names a built -
        in hull asset.

        Invariants : -Scene -
        file tokens are compatibility contracts;
...
do
{
    not rename them casually.-
        Defaults here must match editor placement and scene snapshot serialization.
            ...
            Runtime / Editor / EditorTools.cpp -
        SkullbonezSource / Runtime / Editor / EditorInteractionTools.cpp -
        SkullbonezSource / Runtime / Scene / SceneController.Load.cpp - SkullbonezSource / Scene / SceneSnapshotWriter.cpp -
...
#include <cstring>
            ...
            namespace SkullbonezCore
    {
        ...
        inline constexpr EditorHullAssetInfo EDITOR_HULL_ASSETS[] = {
            ...
            { EditorHullAsset::TAPERED_BLOCK, "tapered_block", "tapered_block.hull",
            ...
            { EditorHullAsset::ROCK_SLAB_FLAT, "rock_slab_flat", "rock_slab_flat.hull",
            ...
            { EditorHullAsset::PINE_TIER_MID, "pine_tier_mid", "pine_tier_mid.hull",
              "SkullbonezData/hulls/pine_tier_mid.hull" },
            { EditorHullAsset::PINE_TIER_TOP, "pine_tier_top", "pine_tier_top.hull",
              "SkullbonezData/hulls/pine_tier_top.hull" },
            { EditorHullAsset::CEDAR_TIER_LOW, "cedar_tier_low", "cedar_tier_low.hull",
              "SkullbonezData/hulls/cedar_tier_low.hull" },
            { EditorHullAsset::CEDAR_TIER_MID, "cedar_tier_mid", "cedar_tier_mid.hull",
            ...
            { EditorHullAsset::CEDAR_TIER_TOP, "cedar_tier_top", "cedar_tier_top.hull",
        ...
        inline constexpr std::size_t EDITOR_HULL_ASSET_COUNT = sizeof( EDITOR_HULL_ASSETS ) /
                                                               sizeof( EDITOR_HULL_ASSETS[0] );
        ...
        inline EditorHullAsset EditorHullAssetFromToken( const char* token )
        {
            ...
            for ( const EditorHullAssetInfo& info : EDITOR_HULL_ASSETS )
            {
                if ( strcmp( token, info.token ) == 0 || strcmp( token, info.path ) == 0 ||
                     strcmp( base, info.token ) == 0 || strcmp( base, info.fileName ) == 0 )
                {
                    return info.asset;
                }
            }
