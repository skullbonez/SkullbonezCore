                                                               ...
                                                               const EditorInteractionPreviewInput& input )
{
    ...
    if ( context.editor.editorModeEnabled && context.editor.placementModeEnabled )
    {
        ...
        context.editor.placementPreviewVisible = TryUpdateEditorPlacementPreview( { context.editor, terrain, context.assets,
                                                                                    placementScaleActive },
    ...
    {
        UpdateEditorGizmoHotAxes( { context.editor, context.world, context.interaction }, input.mouseRayOrigin,
                                  input.mouseRayDirection, input.scaleMode );
    }
...
void BuildEditorToolOverlayTrace( EditorToolOverlayTraceContext context, const EditorToolOverlayTraceInput& input )
{
    ...
    if ( context.editor.editorModeEnabled && context.editor.placementModeEnabled && context.editor.placementPreviewVisible )
    {
        context.tracer.AddPlacementRay( context.editor.placementRayOrigin, context.editor.placementRayHit );
        context.tracer.AddPlacementGhost( context.editor.objectType, context.editor.placementCenter,
                                          context.editor.placementTerrainPoint, context.editor.placementScale,
                                          context.editor.placementOrientation, context.assets );
    }
         ...
         context.editor.selectedBody.IsValid() )
    {
        ...
        if ( selectedModelIndex >= 0 && selectedModelIndex < context.world.SceneEntityCount() &&
             TryTraceEditorSelectionOverlayFromStores( context.world, context.editor.selectedBody,
                                                       context.editor.selectedCollider, selectedModelIndex, context.tracer,
                                                       gizmoOrigin, radius ) )
        {
            context.tracer.AddGizmo( gizmoOrigin, radius, context.editor.hotGizmoAxis, context.editor.hotRotationAxis,
    ...
    if ( input.gesture.kind == RuntimeInteractionGestureKind::MousePickupDrag && context.mousePickup.body.IsValid() )
    {
        ...
        else
        {
            ...
            context.tracer.AddSelectionOutline( bodyPosition, PhysicsBodyOrientation( hotFields, bodyIndex ),
                                                collider->shape );

            context.tracer.AddReplayPathSegment( grabPoint, context.mousePickup.targetPoint, 0.1f, 0.95f, 1.0f );
            context.tracer.AddReplayContactMarker( context.mousePickup.targetPoint, context.mousePickup.planeNormal, 0.1f,
                                                   0.95f, 1.0f );
