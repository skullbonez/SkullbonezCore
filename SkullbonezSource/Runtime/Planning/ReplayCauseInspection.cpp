...
bool ContactPairMatches( const Physics::PhysicsSolverPersistentContactSample& contact, int bodyA, int bodyB, bool terrain ) noexcept
{
...
bool PipelineRecordMatches( const ReplayCauseSolverDetailResult& result, const Physics::PhysicsPipelineRecord& record ) noexcept
{
    ...
    for ( std::size_t contactIndex = 0; contactIndex < result.SourceContactCount(); ++contactIndex )
    {
        ...
        if ( contact && ContactPairMatches( *contact, result.bodyA, result.bodyB, result.terrain ) && contact->featureId == record.featureId )
        {
            return true;
        }
    }
...
void FormatFloat( float value, char* destination, std::size_t capacity ) noexcept
{
    ...
    const std::to_chars_result result = std::to_chars( destination, destination + capacity - 1u, value, std::chars_format::general, std::numeric_limits<float>::max_digits10 );
    destination[result.ec == std::errc {} ? static_cast<std::size_t>( result.ptr - destination ) : 0u] = '\0';
}
...
void AddRawValue( ReplayCauseRawRecordProjection& projection, const char* label, const char* value, const char* unit = "" ) noexcept
{
    if ( projection.rowCount >= projection.rows.size() )
    {
        return;
...
template <typename Integer> void AddRawInteger( ReplayCauseRawRecordProjection& projection, const char* label, Integer value, const char* unit = "" ) noexcept
{
    char text[48] = {};
...
void AddRawVector( ReplayCauseRawRecordProjection& projection, const char* label, const Math::Vector::Vector3& value, const char* unit ) noexcept
{
    char text[128] = {};
...
ReplayCauseSolverPanelRowText BuildReplayCauseSolverPanelRowText( const ReplayCauseSolverDetailView& solverDetail, int rowIndex ) noexcept
{
...
ReplayCauseRawRecordProjection BuildReplayCauseRawRecordProjection( const ReplayCauseSolverDetailView& solverDetail, const ReplayCauseTransportView& transport, int rowIndex ) noexcept
{
...
bool SerializeReplayCauseRawRecord( const ReplayCauseRawRecordProjection& projection, char* destination, std::size_t destinationCapacity ) noexcept
{
...
ReplayCauseIterationsProjection BuildReplayCauseIterationsProjection( const ReplayCauseSolverDetailView& solverDetail, int rowIndex ) noexcept
{
    ReplayCauseIterationsProjection projection;

    if ( solverDetail.solverDetailContacts.empty() || rowIndex < 0 || static_cast<std::size_t>( rowIndex ) >= solverDetail.solverDetailContacts.size() )
    {
...
bool ShouldBeginReplayCauseReturn( const ReplayCauseTransportView& transport, bool nonSelectionClick, bool scrubExit ) noexcept
{
    return transport.mode != ReplayCauseInspectionMode::Inactive && ( nonSelectionClick || scrubExit || transport.mode == ReplayCauseInspectionMode::Returning );
}
...
void PlaceCauseInspectorInShell( ReplayCauseInspectorLayout& layout, const UI::UIRect& bounds, const UI::UIRect& viewport )
{
    ...
    layout.compound = { expanded ? layout.drawer.x : bounds.x, expanded ? top : bounds.y, bounds.w + ( expanded ? width : 0 ), expanded ? layout.drawer.h : bounds.h };
    layout.targetCompound = { layout.drawer.x, top, bounds.w + width, layout.drawer.h };
    layout.resize = {};
    layout.sharedSeam = {};
    layout.hierarchyScrollbar = { bounds.x + bounds.w - 9.0f, bounds.y + 48.0f, 5.0f, (std::max)( 0.0f, layout.hierarchy.h - 60.0f ) };
}
} // namespace
                                                            ...
                                                            const UI::UIRect& shellViewport ) noexcept
{
    ...
    const int iterationLines = ( maximumIterations + REPLAY_CAUSE_SOLVER_PANEL_ITERATIONS_PER_LINE - 1 ) / REPLAY_CAUSE_SOLVER_PANEL_ITERATIONS_PER_LINE;
    ...
    layout.targetCompound = { layout.targetDrawer.x, layout.hierarchy.y, layout.hierarchy.w + targetDrawerWidth, layout.hierarchy.h };
    ...
    layout.rawCopy = { layout.content.x, layout.content.y + layout.content.h - REPLAY_CAUSE_RAW_RECORD_COPY_HEIGHT, layout.content.w, REPLAY_CAUSE_RAW_RECORD_COPY_HEIGHT };
    ...
    if ( hasRows )
    {
        ...
        layout.visibleRows = std::clamp( static_cast<int>( layout.content.h / layout.rowHeight ), 0, REPLAY_CAUSE_SOLVER_PANEL_VISIBLE_ROWS );
    }

    return layout;
}

UI::UIRect ReplayCauseSummarySectionRect( const ReplayCauseInspectorLayout& layout, const ReplayCauseDisplayView& display, int section ) noexcept
{
    float preceding = 0;
    ...
    return { layout.content.x, layout.content.y + 254.0f + section * 34.0f + preceding - display.summaryScrollOffset, layout.content.w - 10.0f, 34.0f };
}
...
int ReplayCauseSummaryMaxScroll( const ReplayCauseInspectorLayout& layout, const ReplayCauseDisplayView& display ) noexcept
{
    ...
    return static_cast<int>( (std::max)( 0.0f, last.y + display.summaryScrollOffset - layout.content.y - layout.content.h ) );
}
...
ReplayFrameIndex EvaluateReplayCauseTransitionFrame( ReplayFrameIndex sourceFrame, ReplayFrameIndex targetFrame, float easedProgress ) noexcept
{
...
const Physics::PhysicsSolverPersistentContactSample* ReplayCauseSolverDetailResult::SourceContactAt( std::size_t index ) const noexcept
{
...
const Physics::PhysicsSolverPersistentContactSample* ReplayCauseSolverDetailResult::ContactRowAt( std::size_t detailRow ) const noexcept
{
    if ( !HasDetail() )
    {
        return nullptr;
...
const Physics::PhysicsPipelineRecord* ReplayCauseSolverDetailResult::PipelineRecordAt( std::size_t detailRecord ) const noexcept
{
    if ( !HasDetail() )
    {
        return nullptr;
...
ReplayCauseSolverDetailResult EvaluateReplayCauseSolverDetail( const RunReplayCauseTreeRow& row, const ReplayCauseSeekResult& seek, const ReplayCauseSolverDetailSource& source ) noexcept
{
    ...
    const bool exactRowKind = row.kind == RunReplayCauseTreeRowKind::Manifold || row.kind == RunReplayCauseTreeRowKind::SolverRow;
    ...
    if ( predictionSource )
    {
        ...
        if ( !source.prediction || !source.prediction->available || source.prediction->identity != expected || query.identity != expected || query.contactIndex != row.contactIndex ||
             query.pipelineIndex != row.pipelineIndex || query.focusedBody != row.modelRow.value || query.counterpartBody != row.counterpartModelRow.value || query.featureId != row.featureId ||
    ...
    if ( !focusedBodyMatches || row.featureId < 0 || static_cast<uint32_t>( row.featureId ) != anchor.featureId || row.terrain != anchorTerrain ||
    ...
    if ( predictionSource )
    {
        ...
        if ( !sequenceAnchor || !IsSolverDetailPipelineStage( sequenceAnchor->stage ) || sequenceAnchor->featureId != anchor.featureId ||
    ...
    for ( std::size_t contactIndex = 0; contactIndex < result.SourceContactCount(); ++contactIndex )
    {
        ...
        {
            if ( contactIndex == static_cast<std::size_t>( row.contactIndex ) || ( row.featureId >= 0 && static_cast<uint32_t>( row.featureId ) == contact->featureId ) )
            {
...
Rendering::ContactManifoldPresentation BuildReplayCauseContactPresentation( const ReplayCauseSolverDetailResult& detail, const ReplaySolverFrameSample& sample ) noexcept
{
    ...
    {
        const auto found = std::find_if( sample.bodies.begin(), sample.bodies.end(), [&]( const ReplaySolverBodySample& body ) { return body.modelRow.value == modelRow; } );
    ...
    auto publishBody = [&]( int modelRow, std::size_t presentationIndex ) -> bool
    {
        ...
        pose.orientation = Math::Orientation::Quaternion( body->orientation[0], body->orientation[1], body->orientation[2], body->orientation[3] );
        pose.valid = true;
    ...
    const std::size_t presentedContactCount = (std::min)( detail.contactRowCount, Rendering::CONTACT_MANIFOLD_PRESENTATION_POINT_CAPACITY );
    ...
    for ( std::size_t contactIndex = 0; contactIndex < presentedContactCount; ++contactIndex )
    {
        ...
        for ( std::size_t pipelineIndex = 0; pipelineIndex < detail.pipelineRecordCount; ++pipelineIndex )
        {
            ...
            if ( candidate && candidate->stage == Physics::PhysicsPipelineStage::ManifoldRow && candidate->featureId == contact->featureId )
            {
        ...
        point.normal = manifoldRecord ? manifoldRecord->normal : ( contact->isTerrain ? contact->terrainNormal : contact->normal );
...
Rendering::ContactManifoldPresentation BuildReplayCauseContactPresentation( const ReplayCauseSolverDetailResult& detail, const RunReplayPredictionFrame& frame ) noexcept
{
    ...
    {
        const auto found = std::find_if( frame.bodies.begin(), frame.bodies.end(), [&]( const RunReplayPredictionBodySample& body ) { return body.modelRow.value == modelRow; } );
...
ReplayCauseSeekResult EvaluateReplayCauseSeek( const RunReplayCauseTreeRow& row, const ReplayRecorderStats& solverStats, std::span<const RunReplayPredictionFrame> predictionFrames ) noexcept
{
    ...
    if ( row.prediction )
    {
        const auto match = std::find_if( predictionFrames.begin(), predictionFrames.end(), [&]( const auto& frame ) { return frame.frameIndex == row.firstFrame; } );
    ...
    const ReplayFrameIndex oldestFrame = solverStats.nextFrameIndex > retainedCount ? solverStats.nextFrameIndex - retainedCount : 0;
...
bool ReplayCauseInspection::Select( int rowIndex, const ReplayCauseSeekResult& seek, ReplayFrameIndex presentedFrame, bool simulationAlreadyPaused, double nowSeconds ) noexcept
{
...
bool ReplayCauseInspection::SeekPredictionPlayback( std::span<const RunReplayPredictionFrame> frames, float normalized, double nowSeconds ) noexcept
{
    ...
    const std::size_t index = static_cast<std::size_t>( std::round( std::clamp( normalized, 0.0f, 1.0f ) * static_cast<float>( frames.size() - 1 ) ) );
    const ReplayFrameIndex previousFrame = m_state.presentedFrame;
    m_state.presentedFrame = frames[index].frameIndex;
    m_playbackSeconds = frames[index].simulationSeconds;
    m_playbackDirection = 0;
    // Seeking shares arrow playback's immutable selection, evidence and camera.
...
void ReplayCauseInspection::AdvancePredictionPlayback( std::span<const RunReplayPredictionFrame> frames, int direction, double nowSeconds ) noexcept
{
    ...
    const auto current = std::find_if( frames.begin(), frames.end(), [&]( const auto& frame ) { return frame.frameIndex == m_state.presentedFrame; } );
    ...
    if ( direction != m_playbackDirection )
    {
        ...
        const auto next = direction < 0 ? ( current == frames.begin() ? current : current - 1 ) : ( current + 1 == frames.end() ? current : current + 1 );
    ...
    m_playbackSeconds = std::clamp( m_playbackSeconds + elapsed * ( direction < 0 ? -0.3 : 0.3 ), frames.front().simulationSeconds, frames.back().simulationSeconds );
    auto frame = std::lower_bound( frames.begin(), frames.end(), m_playbackSeconds, []( const auto& sample, double seconds ) { return sample.simulationSeconds < seconds; } );
...
void ReplayCauseInspection::Advance( double nowSeconds ) noexcept
{
    ...
    m_state.contactFlashAlpha = m_contactFlashStartedAtSeconds >= 0.0 ? static_cast<float>( std::clamp( 1.0 - ( nowSeconds - m_contactFlashStartedAtSeconds ) / 0.4, 0.0, 1.0 ) ) : 0.0f;
    ...
    const ReplayFrameIndex requested = EvaluateReplayCauseTransitionFrame( m_state.sourceFrame, m_state.targetFrame, m_state.easedProgress );
    ...
    if ( m_state.easedProgress >= 1.0f && m_state.presentedFrame == m_state.targetFrame && !m_state.transportInFlight && !m_state.transportPending )
    {
                                                 ...
                                                 const std::array<ReplayCauseObjectDetails, 2>& objects ) noexcept
{
    ...
    if ( !detail.HasDetail() || detail.contactRowCount > m_solverDetailContacts.size() || detail.pipelineRecordCount > m_solverDetailPipelineRecords.size() )
    {
        return;
    }
    ...
    m_state.solverDetailContacts = std::span<const Physics::PhysicsSolverPersistentContactSample>( m_solverDetailContacts.data(), detail.contactRowCount );
    m_state.solverDetailPipelineRecords = std::span<const Physics::PhysicsPipelineRecord>( m_solverDetailPipelineRecords.data(), detail.pipelineRecordCount );
...
void ReplayCauseInspection::RestoreInteractionRecordingBaseline( const ReplayCauseInspectionRecordingState& baseline, double nowSeconds ) noexcept
{
    ...
    m_state.transportInFlight = false;
    m_state.transportPending = baseline.mode == ReplayCauseInspectionMode::Transporting && ( baseline.transportPending || baseline.transportInFlight );
                                                        ...
                                                        ReplayCauseInspectorCommand* outCommand ) noexcept
{
    ...
    const ReplayCauseInspectorLayout layout = BuildReplayCauseInspectorLayout( m_state, causeTree, screenWidth, screenHeight, m_state.drawerProgress );
    ...
    const bool innerContent = m_state.detailVisible ? PointInside( layout.content, mouseX, mouseY ) : ReplayOverlay::ReplayCauseWindowContentRect( causeTree ).Contains( mouseX, mouseY );
    const bool paneEdge = mouseX < m_state.shellBounds.x + 8.0f || mouseX >= m_state.shellBounds.x + m_state.shellBounds.w - 8.0f;
    ...
    if ( leftPressed && ( PointInside( layout.drawerToggle, mouseX, mouseY ) || ( m_state.drawerOpen && PointInside( layout.drawerClose, mouseX, mouseY ) ) ) )
    {
    ...
    const int contactRow = ( m_state.selectedDetailContactRow >= 0 && static_cast<std::size_t>( m_state.selectedDetailContactRow ) < m_state.solverDetailContacts.size() )
    ...
    if ( leftPressed )
    {
        ...
        if ( m_state.activeTab == ReplayCauseInspectorTab::Summary && PointInside( layout.content, mouseX, mouseY ) )
        {
            for ( int section = 0; section < 3; ++section )
            {
                if ( PointInside( ReplayCauseSummarySectionRect( layout, m_state.Display(), section ), mouseX, mouseY ) )
                {
                    ...
                    m_state.summaryScrollOffset = (std::min)( m_state.summaryScrollOffset, ReplayCauseSummaryMaxScroll( layout, m_state.Display() ) );
                    return true;
                }
            }
        }
        ...
        if ( m_state.activeTab == ReplayCauseInspectorTab::RawRecord && PointInside( layout.rawCopy, mouseX, mouseY ) )
        {
            if ( outCommand )
            {
                ...
                const ReplayCauseRawRecordProjection projection = BuildReplayCauseRawRecordProjection( m_state.SolverDetail(), m_state.Transport(), contactRow );
    ...
    if ( wheelDelta != 0 )
    {
        if ( m_state.activeTab == ReplayCauseInspectorTab::Summary )
        {
            ...
            m_state.summaryScrollOffset = std::clamp( m_state.summaryScrollOffset + delta, 0, ReplayCauseSummaryMaxScroll( layout, m_state.Display() ) );
        }
        else if ( m_state.activeTab == ReplayCauseInspectorTab::Iterations && layout.iterationsVisibleRows > 0 )
        {
            const ReplayCauseIterationsProjection projection = BuildReplayCauseIterationsProjection( m_state.SolverDetail(), contactRow );
        ...
        else if ( layout.visibleRows > 0 )
        {
            ...
            const int maximumFirstRow = (std::max)( 0, static_cast<int>( m_state.solverDetailContacts.size() ) - layout.visibleRows );
            m_state.solverDetailFirstRow = std::clamp( m_state.solverDetailFirstRow + direction * wheelSteps, 0, maximumFirstRow );
        }
    }
...
void ReplayCauseInspection::SetShellPresentation( bool enabled, const UI::UIRect& bounds, const UI::UIRect& viewport ) noexcept
{
    m_state.sharedShell = enabled;
    m_state.shellBounds = enabled ? bounds : UI::UIRect {};
    m_state.shellViewport = enabled ? viewport : UI::UIRect {};
    m_state.shellScroll = enabled ? std::clamp( m_state.shellScroll, 0.0f, (std::max)( 0.0f, REPLAY_CAUSE_SHELL_MIN_CONTENT_HEIGHT - bounds.h ) ) : 0.0f;
}
...
void ReplayCauseInspection::AdvanceDrawer( double nowSeconds ) noexcept
{
    ...
    const float eased = EvaluateReplayCauseTransitionProgress( elapsed * REPLAY_CAUSE_TRANSITION_SECONDS / REPLAY_CAUSE_INSPECTOR_DRAWER_SECONDS );
