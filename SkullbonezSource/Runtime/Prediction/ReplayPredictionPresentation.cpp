...
bool QueuePose( Rendering::RenderInstanceStore& renderInstances,
                          ...
                          int modelCount )
{
    ...
    for ( int modelIndex = 0; modelIndex < modelCount; ++modelIndex )
    {
        ...
        queuedAny = renderInstances.OverridePose( modelIndex,
    ...
    const Physics::PhysicsBodyStore& bodyStore )
{
    ...
    const std::size_t stride = (std::max)( std::size_t { 1u },
                                           ( lastIndex + REPLAY_PREDICTION_GHOST_MAX_FRAMES - 1u ) /
    ...
    const auto appendGhostFrame = [&]( std::size_t index )
    {
        ...
        const float t = lastFrame > 0
                            ? std::clamp( static_cast<float>( frame.frameIndex ) / static_cast<float>( lastFrame ),
                                          0.0f,
                                          1.0f )
                            : 1.0f;
        ...
        {
            int resolvedModelIndex = -1;
            if ( !ResolveReplayBody( bodyStore,
                                     body.id,
                                     body.modelRow.value,
