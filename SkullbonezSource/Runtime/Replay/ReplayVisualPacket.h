...
inline uint64_t HashReplayVisualFloatBuffers( std::span<const float> first, std::span<const float> second, std::span<const float> third, std::span<const float> fourth ) noexcept
{
    uint64_t hash = REPLAY_VISUAL_BUFFER_FNV_OFFSET;
    const auto appendBytes = [&hash]( SkullbonezCore::Core::ByteView bytes )
    {
...
inline uint64_t CombineReplayVisualSubmissionHashes( uint64_t retainedHash, uint64_t retainedBytes, uint64_t frameHash, uint64_t frameBytes ) noexcept
{
    uint64_t hash = REPLAY_VISUAL_BUFFER_FNV_OFFSET;
    const auto appendBytes = [&hash]( SkullbonezCore::Core::ByteView bytes )
    {
...
inline uint64_t BuildCanonicalReplayVisualArchiveSemanticHash( uint64_t visualStateHash, uint64_t exactPacketHash, uint32_t topologyVersion, uint64_t replayReserveGrowthEvents ) noexcept
{
...
struct ReplayVisualPacket
{
    ...
    // Borrowed immutable records in the renderer's independent per-lane arena.
    std::span<const float> retainedSecondaryOrdinaryRecords;
    std::span<const float> retainedSecondaryPriorityRecords;
    uint64_t retainedSecondaryStreamId = 0;
    ...
    {
        return !retainedSecondaryOrdinaryRecords.empty() || !retainedSecondaryPriorityRecords.empty() || !combinedLines.empty() || !retainedPredictionOrdinaryLines.empty() ||
               !retainedPredictionPriorityLines.empty() || !expandedRibbonVertices.empty() || !priorityExpandedRibbonVertices.empty() || !retainedPredictionRibbonVertices.empty() ||
...
inline void AttachRetainedPredictionGeometry( ReplayVisualPacket& packet, const ReplayVisualPacket& retainedPacket, uint64_t streamId, uint64_t revision ) noexcept
{
    {
    ...
    if ( packet.retainedPredictionRibbonVertices.empty() && packet.retainedPredictionPriorityRibbonVertices.empty() && packet.retainedPredictionOrdinaryLines.empty() &&
         packet.retainedPredictionPriorityLines.empty() && packet.retainedPredictionRibbonRanges.empty() )
    {
        return;
    }

    const auto& retainedSubmission = retainedPacket.submission;
    const bool hasFrameLocalGeometry = !packet.ordinaryLines.empty() || !packet.priorityLines.empty() || !packet.ordinaryRibbonSegments.empty() || !packet.priorityRibbonSegments.empty() ||
                                       !packet.expandedRibbonVertices.empty() || !packet.priorityExpandedRibbonVertices.empty();
    ...
    packet.submission.ordinaryLineBytes = packet.retainedPredictionOrdinaryLines.size_bytes() + packet.ordinaryLines.size_bytes();
    packet.submission.ordinaryLineVertexCount = retainedSubmission.ordinaryLineVertexCount + packet.submission.ordinaryLineVertexCount;
    ...
    packet.submission.priorityLineBytes = packet.retainedPredictionPriorityLines.size_bytes() + packet.priorityLines.size_bytes();
    packet.submission.priorityLineVertexCount = retainedSubmission.priorityLineVertexCount + packet.submission.priorityLineVertexCount;
    ...
    packet.submission.ordinaryRibbonBytes = packet.retainedPredictionOrdinaryRibbonSegments.size_bytes() + packet.ordinaryRibbonSegments.size_bytes();
    packet.submission.ordinaryRibbonSegmentCount = retainedSubmission.ordinaryRibbonSegmentCount + packet.submission.ordinaryRibbonSegmentCount;
    ...
    packet.submission.priorityRibbonBytes = packet.retainedPredictionPriorityRibbonSegments.size_bytes() + packet.priorityRibbonSegments.size_bytes();
    packet.submission.priorityRibbonSegmentCount = retainedSubmission.priorityRibbonSegmentCount + packet.submission.priorityRibbonSegmentCount;
    packet.submission.vertexHash = CombineReplayVisualSubmissionHashes( retainedSubmission.vertexHash, retainedSubmission.vertexBytes, packet.submission.vertexHash, packet.submission.vertexBytes );
    ...
    packet.submission.ordinaryVertexBytes = packet.retainedPredictionRibbonVertices.size_bytes() + packet.expandedRibbonVertices.size_bytes();
    packet.submission.ordinaryVertexCount = retainedSubmission.ordinaryVertexCount + packet.submission.ordinaryVertexCount;
    packet.submission.vertexBytes = packet.retainedPredictionRibbonVertices.size_bytes() + packet.expandedRibbonVertices.size_bytes() + packet.retainedPredictionPriorityRibbonVertices.size_bytes() +
...
namespace ReplayVisualPacketOperations
{
inline bool FindReplayVisualBufferDifference( std::span<const float> expected, std::span<const float> actual, ReplayVisualPacketBuffer buffer, ReplayVisualPacketDifference& outDifference ) noexcept
{
...
inline bool FindReplayVisualFloatDifference( float expected,
                                             ...
                                             ReplayVisualPacketDifference& outDifference ) noexcept
{
    return FindReplayVisualValueDifference( std::bit_cast<uint32_t>( expected ), std::bit_cast<uint32_t>( actual ), field, recordIndex, componentIndex, outDifference );
}
                                              ...
                                              ReplayVisualPacketDifference& outDifference ) noexcept
{
    return FindReplayVisualFloatDifference( expected.x, actual.x, field, recordIndex, firstComponentIndex, outDifference ) ||
           FindReplayVisualFloatDifference( expected.y, actual.y, field, recordIndex, firstComponentIndex + 1u, outDifference ) ||
           FindReplayVisualFloatDifference( expected.z, actual.z, field, recordIndex, firstComponentIndex + 2u, outDifference );
}
                                                  ...
                                                  ReplayVisualPacketDifference& outDifference ) noexcept
{
    ...
    return FindReplayVisualFloatDifference( expectedX, actualX, field, recordIndex, firstComponentIndex, outDifference ) ||
           FindReplayVisualFloatDifference( expectedY, actualY, field, recordIndex, firstComponentIndex + 1u, outDifference ) ||
           FindReplayVisualFloatDifference( expectedZ, actualZ, field, recordIndex, firstComponentIndex + 2u, outDifference ) ||
           FindReplayVisualFloatDifference( expectedW, actualW, field, recordIndex, firstComponentIndex + 3u, outDifference );
}

inline bool FindReplayTrajectoryDifference( std::span<const ReplayTrajectoryRecord> expected, std::span<const ReplayTrajectoryRecord> actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
    if ( FindReplayVisualValueDifference( expected.size(), actual.size(), ReplayVisualPacketField::TrajectoryRecordCount, 0u, 0u, outDifference ) )
    {
        outDifference.countMismatch = true;
        return true;
    }
    ...
    for ( std::size_t recordIndex = 0; recordIndex < expected.size(); ++recordIndex )
    {
        ...
        if ( FindReplayVisualValueDifference( expectedRecord.key.bodyId.value, actualRecord.key.bodyId.value, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 0u, outDifference ) ||
             FindReplayVisualValueDifference( static_cast<uint8_t>( expectedRecord.key.lane ),
                                              ...
                                              1u,
                                              outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.key.branchOrdinal, actualRecord.key.branchOrdinal, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 2u, outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.version, actualRecord.version, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 3u, outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.publishedPointCount, actualRecord.publishedPointCount, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 4u, outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.styleId, actualRecord.styleId, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 5u, outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.parentId.value, actualRecord.parentId.value, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 6u, outDifference ) ||
             FindReplayVisualValueDifference( static_cast<uint64_t>( static_cast<int64_t>( expectedRecord.depth ) ),
                                              ...
                                              7u,
                                              outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.firstFrame, actualRecord.firstFrame, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 8u, outDifference ) ||
             FindReplayVisualValueDifference( expectedRecord.contactDerived, actualRecord.contactDerived, ReplayVisualPacketField::TrajectoryRecord, recordIndex, 9u, outDifference ) )
        {
            return true;
        }

        const std::size_t expectedPointCount = (std::min)( expectedRecord.publishedPointCount, expectedRecord.points.size() );
        const std::size_t actualPointCount = (std::min)( actualRecord.publishedPointCount, actualRecord.points.size() );

        if ( FindReplayVisualValueDifference( expectedPointCount, actualPointCount, ReplayVisualPacketField::TrajectoryPointCount, recordIndex, 0u, outDifference ) )
        {
            outDifference.countMismatch = true;
            return true;
        }
        ...
        for ( std::size_t pointIndex = 0; pointIndex < expectedPointCount; ++pointIndex )
        {
            ...
            if ( FindReplayVisualValueDifference( expectedPoint.frameIndex, actualPoint.frameIndex, ReplayVisualPacketField::TrajectoryPoint, recordIndex, pointIndex * 4u, outDifference ) ||
                 FindReplayVisualVectorDifference( expectedPoint.position, actualPoint.position, ReplayVisualPacketField::TrajectoryPoint, recordIndex, pointIndex * 4u + 1u, outDifference ) )
            {
                return true;
            }
        }
...
inline bool FindReplayFutureNodeDifference( std::span<const RunReplayPathTraceNode> expected, std::span<const RunReplayPathTraceNode> actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
    if ( FindReplayVisualValueDifference( expected.size(), actual.size(), ReplayVisualPacketField::FutureNodeCount, 0u, 0u, outDifference ) )
    {
        outDifference.countMismatch = true;
        return true;
    }
    ...
    for ( std::size_t index = 0; index < expected.size(); ++index )
    {
        ...
        if ( FindReplayVisualValueDifference( expectedNode.id.value, actualNode.id.value, ReplayVisualPacketField::FutureNode, index, 0u, outDifference ) ||
             FindReplayVisualValueDifference( expectedNode.parentId.value, actualNode.parentId.value, ReplayVisualPacketField::FutureNode, index, 1u, outDifference ) ||
             ...
             FindReplayVisualValueDifference( static_cast<uint64_t>( static_cast<int64_t>( expectedNode.parentModelRow.value ) ),
                                              ...
                                              3u,
                                              outDifference ) ||
             FindReplayVisualValueDifference( expectedNode.firstFrame, actualNode.firstFrame, ReplayVisualPacketField::FutureNode, index, 4u, outDifference ) ||
             FindReplayVisualVectorDifference( expectedNode.contactPoint, actualNode.contactPoint, ReplayVisualPacketField::FutureNode, index, 5u, outDifference ) ||
             FindReplayVisualVectorDifference( expectedNode.contactNormal, actualNode.contactNormal, ReplayVisualPacketField::FutureNode, index, 8u, outDifference ) ||
             FindReplayVisualValueDifference( static_cast<uint64_t>( static_cast<int64_t>( expectedNode.depth ) ),
                                              ...
                                              11u,
                                              outDifference ) ||
             FindReplayVisualValueDifference( expectedNode.contactDerived, actualNode.contactDerived, ReplayVisualPacketField::FutureNode, index, 12u, outDifference ) )
        {
            return true;
        }
    }
...
inline bool FindReplayRetainedMarkerDifference( std::span<const ReplayPredictionRetainedMarker> expected, std::span<const ReplayPredictionRetainedMarker> actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
    if ( FindReplayVisualValueDifference( expected.size(), actual.size(), ReplayVisualPacketField::RetainedMarkerCount, 0u, 0u, outDifference ) )
    {
        outDifference.countMismatch = true;
        return true;
    }
    ...
    for ( std::size_t index = 0; index < expected.size(); ++index )
    {
        ...
        if ( FindReplayVisualValueDifference( expectedMarker.id.value, actualMarker.id.value, ReplayVisualPacketField::RetainedMarker, index, 0u, outDifference ) ||
             FindReplayVisualValueDifference( static_cast<uint64_t>( static_cast<int64_t>( expectedMarker.modelRow.value ) ),
                                              ...
                                              1u,
                                              outDifference ) ||
             FindReplayVisualValueDifference( expectedMarker.hasEntryPose, actualMarker.hasEntryPose, ReplayVisualPacketField::RetainedMarker, index, 2u, outDifference ) ||
             FindReplayVisualValueDifference( expectedMarker.hasRestPose, actualMarker.hasRestPose, ReplayVisualPacketField::RetainedMarker, index, 3u, outDifference ) ||
             FindReplayVisualValueDifference( expectedMarker.hasHorizonPose, actualMarker.hasHorizonPose, ReplayVisualPacketField::RetainedMarker, index, 4u, outDifference ) ||
             FindReplayVisualVectorDifference( expectedMarker.entryPosition, actualMarker.entryPosition, ReplayVisualPacketField::RetainedMarker, index, 5u, outDifference ) ||
             FindReplayVisualQuaternionDifference( expectedMarker.entryOrientation, actualMarker.entryOrientation, ReplayVisualPacketField::RetainedMarker, index, 8u, outDifference ) ||
             FindReplayVisualVectorDifference( expectedMarker.restPosition, actualMarker.restPosition, ReplayVisualPacketField::RetainedMarker, index, 12u, outDifference ) ||
             FindReplayVisualQuaternionDifference( expectedMarker.restOrientation, actualMarker.restOrientation, ReplayVisualPacketField::RetainedMarker, index, 15u, outDifference ) ||
             FindReplayVisualVectorDifference( expectedMarker.horizonPosition, actualMarker.horizonPosition, ReplayVisualPacketField::RetainedMarker, index, 19u, outDifference ) ||
             FindReplayVisualQuaternionDifference( expectedMarker.horizonOrientation, actualMarker.horizonOrientation, ReplayVisualPacketField::RetainedMarker, index, 22u, outDifference ) )
        {
            return true;
        }
    }
...
inline bool FindReplayGhostRequestDifference( std::span<const ReplayPredictionGhostDrawRequest> expected, std::span<const ReplayPredictionGhostDrawRequest> actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
    if ( FindReplayVisualValueDifference( expected.size(), actual.size(), ReplayVisualPacketField::GhostRequestCount, 0u, 0u, outDifference ) )
    {
        outDifference.countMismatch = true;
        return true;
    }
    ...
    for ( std::size_t index = 0; index < expected.size(); ++index )
    {
        ...
        if ( FindReplayVisualValueDifference( static_cast<uint64_t>( static_cast<int64_t>( expectedGhost.modelRow.value ) ),
                                              ...
                                              0u,
                                              outDifference ) ||
             FindReplayVisualVectorDifference( expectedGhost.position, actualGhost.position, ReplayVisualPacketField::GhostRequest, index, 1u, outDifference ) ||
             FindReplayVisualQuaternionDifference( expectedGhost.orientation, actualGhost.orientation, ReplayVisualPacketField::GhostRequest, index, 4u, outDifference ) ||
             FindReplayVisualFloatDifference( expectedGhost.alpha, actualGhost.alpha, ReplayVisualPacketField::GhostRequest, index, 8u, outDifference ) ||
             FindReplayVisualFloatDifference( expectedGhost.tintR, actualGhost.tintR, ReplayVisualPacketField::GhostRequest, index, 9u, outDifference ) ||
             FindReplayVisualFloatDifference( expectedGhost.tintG, actualGhost.tintG, ReplayVisualPacketField::GhostRequest, index, 10u, outDifference ) ||
             FindReplayVisualFloatDifference( expectedGhost.tintB, actualGhost.tintB, ReplayVisualPacketField::GhostRequest, index, 11u, outDifference ) ||
             FindReplayVisualFloatDifference( expectedGhost.tintStrength, actualGhost.tintStrength, ReplayVisualPacketField::GhostRequest, index, 12u, outDifference ) )
        {
            return true;
        }
    }
...
inline bool FindReplayTrajectoryDiagnosticDifference( const SkullbonezCore::Core::MainMemoryReplayTrajectoryStats& expected, const SkullbonezCore::Core::MainMemoryReplayTrajectoryStats& actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
    std::size_t fieldIndex = 0u;
#define SB_REPLAY_VISUAL_COMPARE_DIAGNOSTIC( member )                                                                                                                                                  \
    if ( FindReplayVisualValueDifference( expected.member, actual.member, ReplayVisualPacketField::TrajectoryDiagnostic, 0u, fieldIndex++, outDifference ) )                                           \
    {                                                                                                                                                                                                  \
        return true;                                                                                                                                                                                   \
    }
...
#undef SB_REPLAY_VISUAL_COMPARE_DIAGNOSTIC
    ...
    {
        if ( FindReplayVisualValueDifference( expected.emittedSegments[index], actual.emittedSegments[index], ReplayVisualPacketField::TrajectoryDiagnostic, index, fieldIndex, outDifference ) ||
             FindReplayVisualValueDifference( expected.droppedSegments[index], actual.droppedSegments[index], ReplayVisualPacketField::TrajectoryDiagnostic, index, fieldIndex + 1u, outDifference ) )
        {
            return true;
        }
    }
    ...
    {
        if ( FindReplayVisualValueDifference( expected.budgetExpiries[index], actual.budgetExpiries[index], ReplayVisualPacketField::TrajectoryDiagnostic, index, fieldIndex, outDifference ) )
        {
            return true;
        }
    }
    ...
    {
        if ( FindReplayVisualValueDifference( expected.rebuildCauses[index], actual.rebuildCauses[index], ReplayVisualPacketField::TrajectoryDiagnostic, index, fieldIndex, outDifference ) )
        {
            return true;
        }
    }
...
inline bool FindReplaySubmissionDiagnosticDifference( const SkullbonezCore::Core::MainMemoryReplayTrajectorySubmissionStats& expected, const SkullbonezCore::Core::MainMemoryReplayTrajectorySubmissionStats& actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
    std::size_t fieldIndex = 0u;
#define SB_REPLAY_VISUAL_COMPARE_SUBMISSION( member )                                                                                                                                                  \
    if ( FindReplayVisualValueDifference( expected.member, actual.member, ReplayVisualPacketField::SubmissionDiagnostic, 0u, fieldIndex++, outDifference ) )                                           \
    {                                                                                                                                                                                                  \
        return true;                                                                                                                                                                                   \
    }
...
inline bool FindReplayVisualPacketDifference( const ReplayVisualPacket& expected, const ReplayVisualPacket& actual, ReplayVisualPacketDifference& outDifference ) noexcept
{
...
#undef SB_REPLAY_VISUAL_COMPARE_HEADER
    ...
    if ( FindReplayVisualVectorDifference( expected.header.cameraEye, actual.header.cameraEye, ReplayVisualPacketField::CameraEye, 0u, 0u, outDifference ) ||
         FindReplayVisualVectorDifference( expected.header.cameraUp, actual.header.cameraUp, ReplayVisualPacketField::CameraUp, 0u, 0u, outDifference ) ||
         ...
         FindReplayTrajectoryDiagnosticDifference( expected.trajectoryDiagnostics, actual.trajectoryDiagnostics, outDifference ) ||
         FindReplaySubmissionDiagnosticDifference( expected.submission, actual.submission, outDifference ) )
    {
        return true;
    }

    return FindReplayVisualBufferDifference( expected.combinedLines, actual.combinedLines, ReplayVisualPacketBuffer::CombinedLines, outDifference ) ||
           FindReplayVisualBufferDifference( expected.ordinaryLines, actual.ordinaryLines, ReplayVisualPacketBuffer::OrdinaryLines, outDifference ) ||
           FindReplayVisualBufferDifference( expected.priorityLines, actual.priorityLines, ReplayVisualPacketBuffer::PriorityLines, outDifference ) ||
           FindReplayVisualBufferDifference( expected.retainedPredictionOrdinaryLines, actual.retainedPredictionOrdinaryLines, ReplayVisualPacketBuffer::OrdinaryLines, outDifference ) ||
           FindReplayVisualBufferDifference( expected.retainedPredictionPriorityLines, actual.retainedPredictionPriorityLines, ReplayVisualPacketBuffer::PriorityLines, outDifference ) ||
           FindReplayVisualBufferDifference( expected.ordinaryRibbonSegments, actual.ordinaryRibbonSegments, ReplayVisualPacketBuffer::OrdinaryRibbonSegments, outDifference ) ||
           FindReplayVisualBufferDifference( expected.priorityRibbonSegments, actual.priorityRibbonSegments, ReplayVisualPacketBuffer::PriorityRibbonSegments, outDifference ) ||
           ...
           FindReplayVisualBufferDifference( expected.retainedPredictionPriorityRibbonSegments,
                                             ...
                                             outDifference ) ||
           FindReplayVisualBufferDifference( expected.expandedRibbonVertices, actual.expandedRibbonVertices, ReplayVisualPacketBuffer::ExpandedRibbonVertices, outDifference ) ||
           FindReplayVisualBufferDifference( expected.priorityExpandedRibbonVertices, actual.priorityExpandedRibbonVertices, ReplayVisualPacketBuffer::ExpandedRibbonVertices, outDifference ) ||
           FindReplayVisualBufferDifference( expected.retainedPredictionRibbonVertices, actual.retainedPredictionRibbonVertices, ReplayVisualPacketBuffer::ExpandedRibbonVertices, outDifference ) ||
