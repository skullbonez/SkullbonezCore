...
bool ReadBody( ArchiveReader& reader, RunReplayPredictionBodySample& body )
{
    return reader.Scalar( body.id.value ) && reader.Scalar( body.modelRow.value ) && reader.Vector( body.position ) &&
           reader.Quaternion( body.orientation ) && reader.Vector( body.linearVelocity ) &&
...
{
    return reader.Scalar( node.id.value ) && reader.Scalar( node.parentId.value ) &&
           reader.Scalar( node.modelRow.value ) && reader.Scalar( node.parentModelRow.value ) &&
           reader.Scalar( node.firstFrame ) && reader.Vector( node.contactPoint ) &&
           reader.Vector( node.contactNormal ) && reader.Scalar( node.depth ) && reader.Boolean( node.contactDerived );
}
...
{
    return reader.Scalar( pose.id.value ) && reader.Scalar( pose.modelRow.value ) &&
           reader.Boolean( pose.hasEntryPose ) && reader.Boolean( pose.hasRestPose ) &&
           reader.Vector( pose.entryPosition ) && reader.Quaternion( pose.entryOrientation ) &&
           reader.Vector( pose.restPosition ) && reader.Quaternion( pose.restOrientation );
}
                                   ...
                                   std::vector<uint8_t>& outBytes )
{
    ...
    for ( const ReplayTrajectoryRecord& record : prediction.trajectoryStore.records )
    {
        ...
        if ( totalPointCount > REPLAY_PREDICTION_ARCHIVE_MAX_POINTS ||
                                  ...
                                  std::size_t reasonSize )
{
    ...
    if ( !reader.Float( state.simulation.horizonSeconds ) || !reader.Scalar( state.simulation.targetModelRow.value ) ||
         ...
         !reader.Scalar( state.simulation.sourceSolverHash ) ||
         !reader.Double( state.simulation.sourceSimulationSeconds ) ||
    ...
    if ( !reader.Scalar( state.trajectoryBuild.rootId.value ) ||
         !reader.Boolean( state.trajectoryBuild.usingBuildFrames ) || !reader.Scalar( rootFrameCount ) ||
