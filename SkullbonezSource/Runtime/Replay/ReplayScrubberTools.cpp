...
bool ReplayScrubber::BuildRestoreRequest(
...
void PositionReplayCauseTreeCamera(
...
)
{
    ...
    if ( cameras )
    {
        ...
        if ( restoreCameraAvailable )
        {
            ...
            {
                cameras
                    ->TweenPrimaryToPose( replayCamera.restoreEye, replayCamera.restoreView, replayCamera.restoreUp );
            }
            if ( terrain )
            {
...
bool ReplayRuntime::BeginLoadedPresentationActivationScrubber(
    bool hasLoadedPresentation,
    InputRouter& inputRouter,
    RuntimeInteractionController& interaction
)
{
    return ReplayPresentationOperations::BeginLoadedPresentationActivation(
...
void ReplayRuntime::ArmLoadedPresentationScrubber(
    float normalized,
    double now,
    RuntimeInteractionController& interaction
)
...
)
{
    ...
    if ( planner.visible && planner.available && plannerPointer.hasClientPosition )
    {
        ...
        plannerSurface
            .ResolvePointer( plannerPointer.clientX, plannerPointer.clientY, input.uiBlocksMouse || porkchopOwnsMouse );
    ...
    const ReplayInspectionCameraAction scrubberHostAction = TickScrubberInput(
        input.uiBlocksMouse || porkchopOwnsMouse || tripPlannerOwnsMouse,
    ...
    {
        causeTreeRowsReady = m_authoring.BuildCauseTreeRows(
    ...
    const bool causeTreeOwnsMouse = m_authoring.TickCauseTreeInput(
        m_visualPresentation,
        m_scrubberOwner,
        inputRouter,
        interaction,
    ...
    if ( requestedCauseTreeFocusRow >= 0 && m_authoring.ActivateCauseTreeRow(
                                                requestedCauseTreeFocusRow,
                                                m_visualPresentation,
                                                m_scrubberOwner,
                                                m_predictionOwner.State(),
         ...
         ) )
    {
        ...
        {
            (void)m_authoring.TryPickVelocityEditTarget(
                m_visualPresentation,
                m_scrubberOwner,
                CurrentSolverScrubSample(),
                entities,
...
ReplayLiveRestoreOutcome ReplayRuntime::ApplyLiveRestoreRequest(
...
)
{
    ...
    if ( request.kind == ReplayLiveRestoreKind::V2ArtifactTarget )
    {
        outcome.restored = RestoreV2ArtifactTargetState(
            transaction,
            topologyOwners,
            request.path,
...
)
{
    ...
    if ( authoring.SetVelocityEditEnabled( enableVelocityEdit ) )
    {
        ...
        predictionOwner.ApplyAuthoringRequest(
...
bool BeginReplayScrubberGesture(
    InputRouter& inputRouter,
    RuntimeInteractionController& interaction,
    RuntimeInteractionGestureKind kind,
...
ReplayScrubberPointerDecision ReplayScrubber::ResolvePointerAction( const ReplayScrubberPointerFrame& frame )
{
    ...
    ReplayScrubberSurfaceInput surfaceInput = DescribeReplayScrubberAvailability(
        View(),
        frame.solverStats,
    ...
    else if ( inputFrame.leftPressed && canTakeMouse )
    {
        ...
        if ( hotControl )
        {
            ...
            if ( decision.action == ReplayScrubberAction::RestoreBranch && !branchControlVisible )
            {
                ...
                decision.action != ReplayScrubberAction::None && decision.action != ReplayScrubberAction::Scrub &&
...
)
{
    ...
    case ReplayScrubberAction::SetPredictionHorizon:
        if ( !HandleReplayPredictionHorizonPressed(
                 m_predictionOwner,
                 m_scrubberOwner,
                 inputRouter,
                 interaction,
    ...
    const bool scrubberGestureHandled = TickReplayScrubDrag(
                                            m_scrubberOwner,
                                            inputRouter,
                                            interaction,
                                            solverPresentTrackPosition,
    ...
    m_scrubberOwner.UpdateVisibilityFade(
        scrubberTargetVisible,
        now,
