...
void PhysicsDebugVisualizer::EmitContacts( GameModelCollection& models )
{
    ...
    for ( const PhysicsDebugContact& contact : contacts )
    {
        ...
        EmitCross( contact.point, size, 1.0f, 0.95f, 0.15f );
        EmitArrow( contact.point, contact.point + contact.normal * normalLen, 0.0f, 0.9f, 1.0f );
        EmitLine( contact.point, contact.point + contact.tangent1 * 1.25f, 1.0f, 0.45f, 0.05f );
        EmitLine( contact.point, contact.point + contact.tangent2 * 1.25f, 1.0f, 0.45f, 0.05f );
        if ( contact.bodyA >= 0 && contact.bodyB >= 0 && contact.bodyA < models.GetModelCount() && contact.bodyB < models.GetModelCount() )
        {
            ...
            EmitLine( a, b, 0.45f, 0.45f, 0.45f );
        }
    }
}
