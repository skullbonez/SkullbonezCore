...
bool SkullbonezCore::Physics::PersistentContactCacheHasImpulse( std::span<const PersistentContactCacheEntry> cache, int bodyA, int bodyB, uint32_t featureId )
{
    const int64_t key = MakePersistentContactCacheKey( bodyA, bodyB, featureId );
    const auto cachedIt = std::lower_bound( cache.begin(), cache.end(), key, []( const PersistentContactCacheEntry& entry, int64_t lookupKey ) { return entry.key < lookupKey; } );
    return cachedIt != cache.end() && cachedIt->key == key && ( cachedIt->accN > 0.0f || fabsf( cachedIt->accT1 ) > TOLERANCE || fabsf( cachedIt->accT2 ) > TOLERANCE );
}
...
float HullMaterialFriction( const PersistentContact& contact, std::span<const ColliderRecord> colliders, const PersistentContactSolverStepPolicy& policy )
{
    const ColliderRecord& a = colliders[contact.bodyA];
    const bool hullPair = a.shapeKind == ColliderShapeKind::ConvexHull || ( !contact.isTerrain && colliders[contact.bodyB].shapeKind == ColliderShapeKind::ConvexHull );
    if ( !hullPair )
    {
        return -1.0f;
    }
    ...
    {
        return ContactSolver::MixMaterialFriction( a.friction, policy.terrainFrictionCoefficient );
    }
    // ColliderStore stamps the default material from terrainFrictionCoefficient.
    // Normalize the geometric mean by that default so the independent object
    // coefficient keeps its meaning when both colliders use the scene material.
    // Explicit per-collider zero friction remains zero in either order.
    const float mixed = ContactSolver::MixMaterialFriction( a.friction, colliders[contact.bodyB].friction );
    return policy.terrainFrictionCoefficient > 0.0f ? policy.objectFrictionCoefficient * ( mixed / policy.terrainFrictionCoefficient ) : 0.0f;
}

// Refresh both local anchors before accepting any stored impulse. Feature IDs
// break ties, but cannot authorize a contact that separated or changed normal.
const PersistentContactCacheEntry*
MatchHullContact( std::span<const PersistentContactCacheEntry> cache, std::span<uint8_t> used, const PhysicsBodyHotFieldsConstView& hot, const PersistentContact& contact )
{
    const uint64_t pair = static_cast<uint64_t>( contact.key ) & 0xffffffff00000000ull;
    ...
    const auto rotationA = PhysicsBodyOrientation( hot, contact.bodyA ).GetOrientationMatrix();
    const auto rotationB = contact.isTerrain ? Math::Transformation::RotationMatrix() : PhysicsBodyOrientation( hot, contact.bodyB ).GetOrientationMatrix();
    const Vector3 positionA = PhysicsBodyPosition( hot, contact.bodyA );
    const Vector3 positionB = contact.isTerrain ? ZERO_VECTOR : PhysicsBodyPosition( hot, contact.bodyB );
    const Vector3 currentA = positionA + contact.rA + contact.normal * ( 0.5f * contact.penetration );
    const Vector3 currentB = contact.isTerrain ? positionA + contact.rA - contact.normal * ( 0.5f * contact.penetration ) : positionB + contact.rB - contact.normal * ( 0.5f * contact.penetration );
    const PersistentContactCacheEntry* best = nullptr;
    float bestDistance = FLT_MAX;
    for ( auto it = first; it != cache.end() && ( static_cast<uint64_t>( it->key ) & 0xffffffff00000000ull ) == pair; ++it )
    {
        const auto& geometry = it->geometry;
        if ( used[static_cast<std::size_t>( it - cache.begin() )] != 0u || geometry.lifetime == 0u )
        {
            continue;
        }
        const Vector3 normalA = rotationA * geometry.localNormalA;
        const Vector3 normalB = contact.isTerrain ? geometry.localNormalB : rotationB * geometry.localNormalB;
        if ( Dot( normalA, contact.normal ) < 0.98f || Dot( normalB, contact.normal ) < 0.98f )
        {
            continue;
        }
        const Vector3 anchorA = positionA + rotationA * geometry.localAnchorA;
        const Vector3 anchorB = contact.isTerrain ? geometry.localAnchorB : positionB + rotationB * geometry.localAnchorB;
        const Vector3 delta = anchorB - anchorA;
        const float separation = Dot( delta, contact.normal );
        const Vector3 drift = delta - contact.normal * separation;
        const float distanceLimitSq = geometry.breakingDistance * geometry.breakingDistance;
        const float distance = Vector::VectorMagSquared( currentA - anchorA ) + Vector::VectorMagSquared( currentB - anchorB );
        if ( separation > geometry.breakingDistance || Vector::VectorMagSquared( drift ) > distanceLimitSq || distance > 2.0f * distanceLimitSq )
        {
            continue;
        }
        if ( distance < bestDistance || ( distance == bestDistance && ( !best || it->key < best->key ) ) )
        {
            best = &*it;
    ...
    if ( best )
    {
        used[static_cast<std::size_t>( best - cache.data() )] = 1u;
    }
    return best;
}
...
{
    auto cachedIt = std::lower_bound( cache.begin(), cache.end(), exactKey, []( const PersistentContactCacheEntry& entry, int64_t lookupKey ) { return entry.key < lookupKey; } );
    ...
    if ( cachedIt != cache.end() && cachedIt->key == exactKey )
    {
        ...
        result.carriedLoad = cachedIt->accN > 0.0f || fabsf( cachedIt->accT1 ) > TOLERANCE || fabsf( cachedIt->accT2 ) > TOLERANCE;
    }
...
float ResolveContactInterval( const PhysicsBodyHotFieldsConstView& hotFields, std::span<const uint8_t> sleepState, std::span<const float> timeRemaining, int bodyA, int bodyB, float dt )
{
    ...
    {
        if ( bodyIndex < 0 || hotFields.fixed[static_cast<std::size_t>( bodyIndex )] != 0u || sleepState[static_cast<std::size_t>( bodyIndex )] != 0u )
        {
            return;
        }

        const float bodyInterval = std::clamp( timeRemaining[static_cast<std::size_t>( bodyIndex )], 0.0f, fixedStepInterval );
...
bool ContactPatchHasArea( const ObjectContactManifold& manifold )
{
    if ( manifold.pointCount < 3u )
    {
        return false;
    }

    const Vector3 origin = manifold.points[0].point;

    for ( uint8_t first = 1u; first + 1u < manifold.pointCount; ++first )
    {
        const Vector3 firstArm = manifold.points[first].point - origin;

        for ( uint8_t second = static_cast<uint8_t>( first + 1u ); second < manifold.pointCount; ++second )
        {
            const Vector3 secondArm = manifold.points[second].point - origin;
            const Vector3 patchNormal = Vector::CrossProduct( firstArm, secondArm );

            if ( Vector::VectorMagSquared( patchNormal ) > TOLERANCE * TOLERANCE )
            {
                return true;
            }
        }
...
bool IsNarrowBoxSupportPatch( const ObjectContactManifold& manifold, bool shapeAIsBox, bool shapeBIsBox )
{
    ...
    // Invariant: the manifold normal points from A to B. Positive world-Y
    // therefore supports B; negative world-Y supports A. Two solver rows from
    // one physical edge remain one narrow patch and never masquerade as two
    // stabilizing contacts.
    const bool supportedBodyIsBox = manifold.normal.y > 0.0f ? shapeBIsBox : shapeAIsBox;
    return supportedBodyIsBox && !ContactPatchHasArea( manifold );
}

uint8_t SelectReducedObjectContactRows( const PersistentContactCacheList& cache, int bodyA, int bodyB, const ObjectContactManifold& manifold, uint8_t* selectedPointIndices )
{
    const auto betterPenetrationTie = [&]( int lhs, int rhs )
    {
        if ( rhs < 0 )
        {
            return true;
    ...
    for ( uint8_t pointIndex = 0; pointIndex < manifold.pointCount; ++pointIndex )
    {
        cachedPointCount += PersistentContactCacheHasImpulse( std::span<const PersistentContactCacheEntry>( cache.data(), cache.size() ), bodyA, bodyB, manifold.points[pointIndex].featureId );
    }
    ...
    for ( uint8_t pointIndex = 0; pointIndex < manifold.pointCount; ++pointIndex )
    {
        ...
        const bool usesCache = PersistentContactCacheHasImpulse( std::span<const PersistentContactCacheEntry>( cache.data(), cache.size() ), bodyA, bodyB, candidate.featureId );
...
float ResolveContactMassShare( std::span<const PhysicsBodyRecord> bodyRecords, const PhysicsBodyRowList<uint16_t>& contactCounts, const PersistentContact& contact )
{
    const uint16_t countA = contactCounts[contact.bodyA] > 0u ? contactCounts[contact.bodyA] : 1u;
    float contactMass = bodyRecords[static_cast<std::size_t>( contact.bodyA )].mass / static_cast<float>( countA );

    if ( !contact.isTerrain )
    {
        const uint16_t countB = contactCounts[contact.bodyB] > 0u ? contactCounts[contact.bodyB] : 1u;
        const float bodyBContactMass = bodyRecords[static_cast<std::size_t>( contact.bodyB )].mass / static_cast<float>( countB );
        contactMass = (std::min)( contactMass, bodyBContactMass );
    }
    return contactMass;
}

void PrecomputeContactBias( PersistentContact& contact,
                            std::span<const ColliderRecord> colliderRecords,
                            const PersistentContactSolverStepPolicy& stepPolicy,
                            const PreviousObjectContactLookup& previousObjectContact,
                            ...
                            float inverseContactInterval )
{
    ...
    if ( contact.isTerrain )
    {
        ...
        {
            // Invariant: position correction owns quiet support overlap decay;
            // the velocity row must not manufacture separation.
            return;
        }
        if ( fabsf( normalVelocity ) < stepPolicy.contactRestitutionThreshold )
        {
            const float penetrationError = contact.penetration - stepPolicy.terrainSlop;
            if ( penetrationError > 0.0f )
            {
                contact.bias = stepPolicy.terrainBaumgarteBeta * penetrationError * inverseContactInterval;
                contact.bias = (std::min)( contact.bias, stepPolicy.maxBaumgarteBias );
                contact.separationBias = contact.bias;
            }
            return;
        }
        if ( normalVelocity < -stepPolicy.contactRestitutionThreshold )
        {
            const float restitution = stepPolicy.elasticCollisions ? 1.0f : colliderRecords[static_cast<std::size_t>( contact.bodyA )].restitution;
            contact.bias = -restitution * normalVelocity;
        }
        return;
    }

    const bool continuesLoadedContact = !stepPolicy.elasticCollisions && previousObjectContact.carriedLoad;
    if ( normalVelocity < -stepPolicy.contactRestitutionThreshold && !continuesLoadedContact )
    {
        float restitution = 1.0f;
        if ( !stepPolicy.elasticCollisions )
        {
            const float restitutionA = colliderRecords[static_cast<std::size_t>( contact.bodyA )].restitution;
            const float restitutionB = colliderRecords[static_cast<std::size_t>( contact.bodyB )].restitution;
    ...
    const float penetrationError = contact.penetration - stepPolicy.objectSlop;
    if ( penetrationError > 0.0f )
    {
        contact.bias = stepPolicy.objectBaumgarteBeta * penetrationError * inverseContactInterval;
        if ( stepPolicy.maxBaumgarteBias > 0.0f )
        {
            contact.bias = (std::min)( contact.bias, stepPolicy.maxBaumgarteBias );
        }
        contact.separationBias = contact.bias;
...
PersistentContactSolverStepPolicy PhysicsContactSolverStage::ResolveStepPolicy( const PhysicsRuntimeSettings& settings, const PhysicsWorldForces& worldForces, float stepDurationSeconds ) noexcept
{
    ...
    policy.stepDurationSeconds = (std::max)( 0.0f, stepDurationSeconds );
    policy.contactEpsilon = settings.body.contactEpsilon;
    policy.iterations = (std::max)( 1, settings.solver.iterations );
    return policy;
}

void ConstraintSolveTransaction::AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase next, const char* operation )
{
    const ConstraintSolvePhaseCursor::Phase current = m_phase.Current();
    ...
    if ( !m_phase.TryAdvance( next ) )
    {
        ...
        SB_FATAL( "Physics/ConstraintSolveTransaction",
...
void ConstraintSolveTransaction::BeginEntryPolicySetup()
{
    if ( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::Complete && !m_phase.ResetAfterComplete() )
    {
        SB_FATAL( "Physics/ConstraintSolveTransaction", "Completed solve cursor could not reset." );
    }

    AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase::EntryPolicySetup, "BeginEntryPolicySetup" );
...
void ConstraintSolveTransaction::Complete()
{
    AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase::Complete, "Complete" );
}
...
int64_t ConstraintSolveTransaction::MakeKey( int bodyA, int bodyB, uint32_t featureId )
{
    return MakePersistentContactCacheKey( bodyA, bodyB, featureId );
}

float ConstraintSolveTransaction::ConservativeContactRadius( const ColliderRecord& collider )
{
...
Vector3 ConstraintSolveTransaction::ApplyInverseInertia( int bodyIndex, const Vector3& value ) const
{
    ...
    return Body( static_cast<std::size_t>( bodyIndex ) ).ApplyInverseInertia( value );
}

Vector3 SolverBodyState::ApplyInverseInertia( const Vector3& value ) const
{
    Vector3 result;
    const auto multiplyByBodyInverseInertia = [&]( const Vector3& bodyValue, Vector3& outBodyResult )
    {
        outBodyResult = Math::Transformation::SymmetricMatrix3( invInertia, invInertiaProducts ) * bodyValue;
        return true;
    };
    TryApplyWorldInertiaResponse( orientation, useWorldInertia, value, multiplyByBodyInverseInertia, result );
    return result;
}
...
void ConstraintSolveTransaction::ApplyImpulse( const PersistentContact& contact, const Vector3& impulse )
{
...
void ConstraintSolveTransaction::SetupBodies( const PhysicsBodyStore& bodyStore, std::span<const uint8_t> sleepState, int modelCount, Core::Profiler* )
{
    PROFILE_SCOPED( "Frame/Physics/Narrowphase/PersistentContacts/BodySetup" );
    using Phase = ConstraintSolvePhaseCursor::Phase;
    const bool continuation = m_phase.Current() == Phase::FixedContactRelease;
    AdvanceOrFatal( continuation ? Phase::ReleasedBodySetup : Phase::BodySetup, "SetupBodies" );
    if ( continuation )
    {
        const auto hot = bodyStore.HotFields();
        for ( int i = 0; i < modelCount; ++i )
        {
            if ( m_bodies[i].invMass == 0.0f && hot.fixed[i] == 0u && sleepState[i] == 0u )
            {
                m_reactivatedBodies.push_back( i );
            }
        }
    }
    ...
    for ( int i = 0; i < modelCount; ++i )
    {
        ...
        if ( sleepState[bodyIndex] || hotRead.fixed[bodyIndex] != 0u )
        {
            ...
            body.invInertiaProducts = ZERO_VECTOR;
        ...
        else
        {
            ...
            body.invInertiaProducts = PhysicsBodyInverseInertiaProducts( hotRead, bodyIndex );
...
void ConstraintSolveTransaction::BuildManifolds( PhysicsContactSolverStage& stage,
                                                 ...
                                                 Core::Profiler* profiler )
{
    ...
    AdvanceOrFatal( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedBodySetup ? ConstraintSolvePhaseCursor::Phase::ReleasedManifolds : ConstraintSolvePhaseCursor::Phase::BuildManifolds, "BuildManifolds" );
    ...
    auto appendSleepSupportEdge = [&]( int aIndex, int bIndex, const Vector3& normal, bool canSeedSupport )
    {
        ...
        if ( normal.y > supportNormalY )
        {
            ...
            if ( observePipelineEvent() )
            {
                ...
                record.point = ( PhysicsBodyPosition( hotRead, static_cast<std::size_t>( aIndex ) ) + PhysicsBodyPosition( hotRead, static_cast<std::size_t>( bIndex ) ) ) * 0.5f;
    ...
    auto objectContactRowsAreQuiet = [&]( int bodyA, int bodyB, const ObjectContactManifold& manifold ) -> bool
    {
        ...
        const float linearLimit = (std::max)( stepPolicy.sleepLinearSpeed * 2.0f, stepPolicy.rawContactRestitutionThreshold * 0.25f );
        ...
        return Vector::VectorMagSquared( solverA.angularVelocity ) <= angularLimitSq && Vector::VectorMagSquared( solverB.angularVelocity ) <= angularLimitSq;
    };
    // CATTO REF:
    ...
    for ( const auto& cp : candidatePairs )
    {
        ...
        if ( aIndex == bIndex || aIndex < 0 || bIndex < 0 || aIndex >= modelCount || bIndex >= modelCount || ( sleepState[aIndex] && sleepState[bIndex] ) ||
        ...
        if ( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedManifolds )
        {
            const auto previousEnd = stage.m_persistentContacts.begin() + m_continuationContactCount;
            if ( !IsReactivatedPair( aIndex, bIndex ) ||
                 std::any_of( stage.m_persistentContacts.begin(), previousEnd, [&]( const PersistentContact& row ) { return !row.isTerrain && row.bodyA == aIndex && row.bodyB == bIndex; } ) )
            {
                continue;
            }
        }
        ...
        Vector3 centerDelta = PhysicsBodyPosition( hotRead, static_cast<std::size_t>( bIndex ) ) - PhysicsBodyPosition( hotRead, static_cast<std::size_t>( aIndex ) );

        float contactDistance = ConservativeContactRadius( colliderA ) + ConservativeContactRadius( colliderB ) + stepPolicy.contactEpsilon;
        const bool articulated = UsesSpeculativeContacts( stepPolicy.collisionPathState, aIndex ) || UsesSpeculativeContacts( stepPolicy.collisionPathState, bIndex );
        if ( articulated )
        {
            ...
            contactDistance += stepPolicy.stepDurationSeconds *
                               ( Vector::VectorMag( solverB.linearVelocity - solverA.linearVelocity ) + Vector::VectorMag( solverA.angularVelocity ) * colliderA.maximumCenterOfMassRadius +
        ...
        bool manifoldBuilt = false;
        {
            ...
            manifoldBuilt = articulated ? BuildArticulatedContactManifold( bodyStore, colliderStore, stepPolicy.stepDurationSeconds, stepPolicy.contactEpsilon, aIndex, bIndex, manifold )
                                        : BuildObjectContactManifold( profiler, bodyA, colliderA.shape, bodyB, colliderB.shape, aIndex, bIndex, stepPolicy.contactEpsilon, manifold );
        }
        ...
        if ( manifoldBuilt )
        {
            ...
            if ( hasConvexHull )
            {
                RefreshHullContactPatch( std::span<const PersistentContactCacheEntry>( stage.m_persistentContactCache.data(), stage.m_persistentContactCache.size() ), bodyStore.HotFields(), manifold );
            }
            // Hull patches keep their area through all four rows. Two rows form
            // a line and cannot retain the full three-dimensional support patch.
            ...
            // Sleep geometry is deliberately separate from friction and warm
            // starting. A narrow box patch still solves with ordinary resting
            ...
            ContactSupportGeometry hullSupport;
            const bool supportedHull = manifold.normal.y > 0.0f ? shapeBIsConvexHull : shapeAIsConvexHull;
            if ( supportedHull )
            {
                for ( uint8_t point = 0; point < manifold.pointCount; ++point )
                {
                    hullSupport.Add( manifold.normal.y > 0.0f ? manifold.points[point].rB : manifold.points[point].rA );
                }
            }
            const bool hullSupportArea = !supportedHull || hullSupport.SupportsCenter();
            hasRestingFootprint = !speculative && ( !hasConvexHull || hullSupportArea );
            uint8_t selectedPointIndices[4] = { 0, 1, 2, 3 };
            uint8_t selectedPointCount = manifold.pointCount;

            if ( manifold.pointCount > 2 && !hasSphere && sameShapeFaceFootprint && hasRestingFootprint && objectContactRowsAreQuiet( aIndex, bIndex, manifold ) )
            {
                // Why: primitive box/box face manifolds often
                ...
                // hull pairs, fresh-impact, and sphere contacts keep full rows
                ...
                selectedPointCount = SelectReducedObjectContactRows( stage.m_persistentContactCache, aIndex, bIndex, manifold, selectedPointIndices );
            }

            // One fixed-tick budget covers both the initial and release passes.
            // Check the entire manifold before publishing any of its rows.
            if ( !stage.CanAppendObjectManifold( selectedPointCount ) )
            {
                SB_FATAL( "Physics/PhysicsContactSolverStage", "Combined contact manifold capacity exhausted." );
            }
            for ( uint8_t selectedIndex = 0; selectedIndex < selectedPointCount; ++selectedIndex )
            {
                ...
                c.inhibitsSleep = !speculative && ( narrowBoxSupportPatch || !hullSupportArea );
...
template <bool RetainPipelineRecords>
void ConstraintSolveTransaction::BuildTerrainRows( PhysicsContactSolverStage& stage,
                                                   const PhysicsBodyStore& bodyStore,
                                                   const PersistentContactSolverStepPolicy& stepPolicy,
                                                   ...
                                                   Core::Profiler* )
{
    ...
    AdvanceOrFatal( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedManifolds ? ConstraintSolvePhaseCursor::Phase::ReleasedTerrainRows : ConstraintSolvePhaseCursor::Phase::TerrainRows, "BuildTerrainRows" );
    ...
    {
        if ( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedTerrainRows )
        {
            const auto previousEnd = stage.m_persistentContacts.begin() + m_continuationContactCount;
            if ( !IsReactivatedPair( manifold.bodyA, -1 ) ||
                 std::any_of( stage.m_persistentContacts.begin(), previousEnd, [&]( const PersistentContact& row ) { return row.isTerrain && row.bodyA == manifold.bodyA; } ) )
            {
                continue;
            }
        }
        ...
        if ( manifold.bodyA < 0 || manifold.bodyA >= modelCount || manifold.pointCount == 0 || ( manifold.bodyA < static_cast<int>( sleepState.size() ) && sleepState[manifold.bodyA] ) )
        {
            continue;
        }
        ...
        // sleep authority.
        const float supportSeedScale = manifold.supportsRestingPolicy ? TERRAIN_RESTING_SUPPORT_SEED_SCALE : ( manifold.inhibitsSleep ? TERRAIN_SHORELINE_SUPPORT_SEED_SCALE : 0.0f );
...
void ConstraintSolveTransaction::PrecomputeTerrainAngularResistance( PersistentContact& contact,
                                                                     const ColliderRecord& collider,
                                                                     const PersistentContactSolverStepPolicy& stepPolicy,
                                                                     float normalVelocity )
{
    if ( !contact.isTerrain || !contact.supportsRestingPolicy || !contact.allowsTangentFriction || stepPolicy.elasticCollisions || normalVelocity <= -stepPolicy.contactRestitutionThreshold ||
         ( stepPolicy.rollingFrictionCoefficient <= 0.0f && stepPolicy.spinFrictionCoefficient <= 0.0f ) )
    {
        return;
    }

    // Concept: rolling resistance is a bounded angular row in the same PGS
    // owner as normal and tangent friction. The coupled tangent inverse stays
    // separate from scalar spin about the contact normal.
    const Vector3 inverseInertiaT1 = ApplyInverseInertia( contact.bodyA, contact.tangent1 );
    const Vector3 inverseInertiaT2 = ApplyInverseInertia( contact.bodyA, contact.tangent2 );
    const float k11 = Dot( contact.tangent1, inverseInertiaT1 );
    const float k12 = Dot( contact.tangent1, inverseInertiaT2 );
    const float k22 = Dot( contact.tangent2, inverseInertiaT2 );
    const float determinant = k11 * k22 - k12 * k12;
    const float spinInverseMass = Dot( contact.normal, ApplyInverseInertia( contact.bodyA, contact.normal ) );

    if ( determinant > TOLERANCE * TOLERANCE )
    {
        const float inverseDeterminant = 1.0f / determinant;
        contact.rollingMass11 = k22 * inverseDeterminant;
        contact.rollingMass12 = -k12 * inverseDeterminant;
        contact.rollingMass22 = k11 * inverseDeterminant;
    }
    if ( spinInverseMass > TOLERANCE )
    {
        contact.spinMass = 1.0f / spinInverseMass;
    }

    contact.rollingRadius = VisitCollisionShape( collider.shape, []( const auto& shape )
                                                 {
...
bool ConstraintSolveTransaction::IsReactivatedPair( int a, int b ) const
{
    return std::find( m_reactivatedBodies.begin(), m_reactivatedBodies.end(), a ) != m_reactivatedBodies.end() ||
           ( b >= 0 && std::find( m_reactivatedBodies.begin(), m_reactivatedBodies.end(), b ) != m_reactivatedBodies.end() );
}
void ConstraintSolveTransaction::RecomputeContactMass( PersistentContact& c )
{
    const SolverBodyState terrain;
    const auto& bodyA = Body( static_cast<std::size_t>( c.bodyA ) );
    const auto& bodyB = c.isTerrain ? terrain : Body( static_cast<std::size_t>( c.bodyB ) );
    auto applyInvInertiaA = [&]( const Vector3& v ) -> Vector3 { return ApplyInverseInertia( c.bodyA, v ); };

    auto applyInvInertiaB = [&]( const Vector3& v ) -> Vector3 { return c.isTerrain ? ZERO_VECTOR : ApplyInverseInertia( c.bodyB, v ); };

    c.normalMass = Physics::ContactSolver::ComputeTwoBodyEffectiveMass( bodyA.invMass, bodyB.invMass, c.normal, c.rA, c.rB, applyInvInertiaA, applyInvInertiaB );

    c.tangentMass1 = Physics::ContactSolver::ComputeTwoBodyEffectiveMass( bodyA.invMass, bodyB.invMass, c.tangent1, c.rA, c.rB, applyInvInertiaA, applyInvInertiaB );

    c.tangentMass2 = Physics::ContactSolver::ComputeTwoBodyEffectiveMass( bodyA.invMass, bodyB.invMass, c.tangent2, c.rA, c.rB, applyInvInertiaA, applyInvInertiaB );
...
template <bool RetainPipelineRecords>
void ConstraintSolveTransaction::PrecomputeRows( PhysicsContactSolverStage& stage,
                                                 const PhysicsBodyStore& bodyStore,
                                                 const ColliderStore& colliderStore,
                                                 const PersistentContactSolverStepPolicy& stepPolicy,
                                                 std::span<const uint8_t> sleepState,
                                                 ...
                                                 Core::Profiler* )
{
    ...
    AdvanceOrFatal( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedJoints ? ConstraintSolvePhaseCursor::Phase::ReleasedPrecompute : ConstraintSolvePhaseCursor::Phase::Precompute, "PrecomputeRows" );
    ...
    const bool continuation = m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedPrecompute;
    const std::size_t firstNewRow = continuation ? m_continuationContactCount : 0u;
    m_materialFriction.resize( stage.m_persistentContacts.size(), -1.0f );
    m_contactLifetime.resize( stage.m_persistentContacts.size(), 0u );
    if ( !continuation )
    {
        m_cacheUsed.assign( stage.m_persistentContactCache.size(), 0u );
    }
    for ( std::size_t rowIndex = firstNewRow; rowIndex < stage.m_persistentContacts.size(); ++rowIndex )
    {
        PersistentContact& c = stage.m_persistentContacts[rowIndex];
        ...
        const float materialFriction = HullMaterialFriction( c, colliderRecords, stepPolicy );
        const PersistentContactCacheEntry* geometricContact = materialFriction >= 0.0f && !continuation ? MatchHullContact( stage.m_persistentContactCache, m_cacheUsed, hotRead, c ) : nullptr;
        const PreviousObjectContactLookup previousObjectContact = materialFriction >= 0.0f ? PreviousObjectContactLookup { geometricContact, geometricContact && geometricContact->accN > 0.0f }
                                                                  : c.isTerrain            ? PreviousObjectContactLookup()
                                                                                           : InspectPreviousObjectContact( stage.m_persistentContactCache, c.key );
        m_contactLifetime[rowIndex] = materialFriction < 0.0f ? 0u : geometricContact ? (std::min)( geometricContact->geometry.lifetime, UINT32_MAX - 1u ) + 1u : 1u;
        ...
        RecomputeContactMass( c );

        const float contactMass = ResolveContactMassShare( bodyRecords, stage.m_persistentContactCounts, c );
        m_materialFriction[rowIndex] = materialFriction;
        if ( materialFriction >= 0.0f )
        {
            c.normalCoupledFriction = true;
        }


        if ( !c.allowsTangentFriction || materialFriction == 0.0f )
        {
            c.tangentMass1 = 0.0f;
            c.tangentMass2 = 0.0f;
        }
        ...
        PrecomputeTerrainAngularResistance( c, colliderRecords[static_cast<size_t>( c.bodyA )], stepPolicy, vn );
        ...
        // Restitution belongs to a new impact; exact feature cache identity
        // proves whether an object contact already carried load.
        PrecomputeContactBias( c, colliderRecords, stepPolicy, previousObjectContact, vn, inverseContactInterval );
        ...
        c.frictionLimit = ( stepPolicy.elasticCollisions || !c.allowsTangentFriction )
                              ? 0.0f
                              : ( c.isTerrain ? stepPolicy.terrainFrictionCoefficient * c.terrainWarmStart
                                              : ( c.normalCoupledFriction ? 0.0f : stepPolicy.objectFrictionCoefficient * contactMass * stepPolicy.gravityMagnitude * contactInterval ) );
        ...
        if ( canUseCachedWarmStart )
        {
            ...
            else if ( c.isTerrain )
            {
                auto cachedIt = std::lower_bound( stage.m_persistentContactCache.begin(),
                                                  ...
                                                  c.key,
                                                  []( const PersistentContactCacheEntry& entry, int64_t key ) { return entry.key < key; } );
        ...
        if ( cachedEntry )
        {
            ...
            if ( cachedEntry->geometry.lifetime != 0u )
            {
                const Vector3 tangentImpulse = bodyA.orientation * cachedEntry->geometry.localTangentImpulseA;
                c.accT1 = Dot( tangentImpulse, c.tangent1 );
                c.accT2 = Dot( tangentImpulse, c.tangent2 );
            }
            const float cachedFrictionLimit = ( stepPolicy.elasticCollisions || !c.allowsTangentFriction )
                                                  ? 0.0f
                                                  : ( c.isTerrain ? stepPolicy.terrainFrictionCoefficient * ( ( c.accN > c.terrainWarmStart ) ? c.accN : c.terrainWarmStart )
                                                                  : ( c.normalCoupledFriction ? stepPolicy.objectFrictionCoefficient * c.accN : c.frictionLimit ) );

            const float materialLimit = materialFriction >= 0.0f ? materialFriction * c.accN : cachedFrictionLimit;
            Physics::ContactSolver::ClampFrictionVector( c.accT1, c.accT2, stepPolicy.elasticCollisions || !c.allowsTangentFriction ? 0.0f : materialLimit );
...

        {
            ...
            const Vector3 contactVelB = c.isTerrain ? ZERO_VECTOR : b.linearVelocity + Vector::CrossProduct( b.angularVelocity, c.rB );
...
void ConstraintSolveTransaction::OrderCandidatePairs( const PhysicsBodyStore& bodyStore, std::span<const std::pair<int, int>> candidates )
{
    ...
    for ( auto pair : candidates )
    {
        ...
        if ( pair.first >= 0 && pair.second < bodyStore.Count() && pair.first != pair.second )
        {
            m_candidatePairs.push_back( pair );
        }
    }
    const auto bodyKey = [&]( int row )
    {
        const auto& body = records[static_cast<std::size_t>( row )];
        return std::tuple( body.sceneObjectId.value, body.handle.index, body.handle.generation );
    };
    const auto less = [&]( const auto& a, const auto& b ) { return std::tuple( bodyKey( a.first ), bodyKey( a.second ) ) < std::tuple( bodyKey( b.first ), bodyKey( b.second ) ); };
    // Preserve the manifold builder's deterministic geometric feature order.
    // Sorting individual points changes the support footprint's PGS response.
    if ( !std::is_sorted( m_candidatePairs.begin(), m_candidatePairs.end(), less ) )
    {
        std::sort( m_candidatePairs.begin(), m_candidatePairs.end(), less );
    }
}

void ConstraintSolveTransaction::PrepareJoints( PhysicsContactSolverStage& stage,
                                                const PhysicsBodyStore& bodyStore,
                                                std::span<const PointJointConstraint> constraints,
                                                const PersistentContactSolverStepPolicy& policy )
{
    using Phase = ConstraintSolvePhaseCursor::Phase;
    const bool continuation = m_phase.Current() == Phase::ReleasedTerrainRows;
    AdvanceOrFatal( continuation ? Phase::ReleasedJoints : Phase::PrepareJoints, "PrepareJoints" );
    if ( !continuation )
    {
        m_jointBlocks.ResetDefault( constraints.size() );
        m_jointSamples.clear();
        if ( policy.collectConvergenceDiagnostics )
        {
            m_jointSamples.ResetDefault( constraints.size() );
        }
    }
    for ( std::size_t i = 0; i < constraints.size(); ++i )
    {
        auto& block = m_jointBlocks[i];
        const bool wasActive = block.Active();
        const std::size_t source = continuation ? block.SourceIndex() : i;
        PointJointConstraint descriptor = constraints[source];
        if ( continuation )
        {
    ...
    std::sort( m_jointBlocks.begin(), m_jointBlocks.end(), []( const PointJointBlock& a, const PointJointBlock& b ) { return std::tuple( a.Handle().generation, a.Handle().index, a.BodyA(), a.BodyB() ) < std::tuple( b.Handle().generation, b.Handle().index, b.BodyA(), b.BodyB() ); } );
    m_islands.Prepare( bodyStore, m_bodies, stage.m_persistentContacts, m_jointBlocks, policy.iterations );
}

void ConstraintSolveTransaction::WarmStartJoints()
{
    AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase::WarmStartJoints, "WarmStartJoints" );
    for ( const auto& block : m_jointBlocks )
    {
        block.WarmStart( m_bodies );
    }
}
void ConstraintSolveTransaction::StoreJointImpulses( std::span<PointJointConstraint> constraints ) const
{
    if ( m_phase.Current() != ConstraintSolvePhaseCursor::Phase::CacheStore )
    {
        SB_FATAL( "Physics/ConstraintSolveTransaction", "Joint cache publication requires the cache-store phase." );
    }
    for ( const auto& block : m_jointBlocks )
    {
...
template <bool CollectDiagnostics> void ConstraintSolveTransaction::SolveJointBlocks( const PhysicsBodyStore& bodyStore, int iteration )
{
    for ( auto& block : m_jointBlocks )
    {
        if ( !block.Active() || !m_islands.Active( block.BodyA(), block.BodyB() ) )
        {
            continue;
        }
        PointJointIterationSample* sample = nullptr;
        if constexpr ( CollectDiagnostics )
        {
            sample = &m_jointSamples[block.SourceIndex()];
            sample->iteration = iteration;
            const int root = m_islands.RootForPair( block.BodyA(), block.BodyB() );
            sample->islandRoot = bodyStore.Records()[static_cast<std::size_t>( root )].handle;
        }
        const float deltaSq = block.Solve( m_bodies, sample );
        m_islands.Observe( block.BodyA(), block.BodyB(), deltaSq );
    }
}

template <bool CollectConvergenceDiagnostics, bool RetainPipelineRecords>
void ConstraintSolveTransaction::SolveRowsIterations( PhysicsContactSolverStage& stage,
                                                      const PhysicsBodyStore& bodyStore,
                                                      const PersistentContactSolverStepPolicy& stepPolicy,
                                                      std::size_t pipelineRecordCapacity )
{
    ...
    for ( int iter = 0; iter < maximumSweeps; ++iter )
    {
        ...
        for ( std::size_t contactIndex = 0; contactIndex < stage.m_persistentContacts.size(); ++contactIndex )
        {
            PersistentContact& c = stage.m_persistentContacts[contactIndex];
            if ( !m_islands.Active( c.bodyA, c.bodyB ) )
            {
                continue;
            }
            ...
            if ( c.rollingRadius > TOLERANCE )
            {
                if ( c.spinMass > 0.0f && stepPolicy.spinFrictionCoefficient > 0.0f )
                {
                    ...
                    float spinLimit = supportingNormalImpulse;
                    spinLimit *= stepPolicy.spinFrictionCoefficient;
                ...
                const float supportingNormalImpulse = ( c.accN > c.terrainWarmStart ) ? c.accN : c.terrainWarmStart;
                float rollingLimit = supportingNormalImpulse * c.rollingRadius;
                rollingLimit *= stepPolicy.rollingFrictionCoefficient;
            ...
            const float frictionLimit = ( stepPolicy.elasticCollisions || !c.allowsTangentFriction )
                                            ? 0.0f
                                            : ( c.isTerrain ? stepPolicy.terrainFrictionCoefficient * ( ( c.accN > c.terrainWarmStart ) ? c.accN : c.terrainWarmStart )
                                                            : ( c.normalCoupledFriction ? stepPolicy.objectFrictionCoefficient * c.accN : c.frictionLimit ) );

            float materialLimit = frictionLimit;
            if ( m_materialFriction[contactIndex] >= 0.0f )
            {
                // Each row receives mu times its solved normal impulse. Splitting
                // the same normal load across more rows cannot increase the sum.
            ...
            const float rollingDeltaSq = rollingLinearDeltaT1 * rollingLinearDeltaT1 + rollingLinearDeltaT2 * rollingLinearDeltaT2 + spinLinearDelta * spinLinearDelta;
            const float tangentFamilyDeltaSq = tangentDeltaSq + rollingDeltaSq;
            const float rowDeltaSq = normalDeltaSq + tangentFamilyDeltaSq;
            m_islands.Observe( c.bodyA, c.bodyB, rowDeltaSq );
            ...
            if ( rowDeltaSq > maxRowImpulseDeltaSq )
            {
                ...
                if constexpr ( CollectConvergenceDiagnostics )
                {
                    ...
                    const int root = m_islands.RootForPair( c.bodyA, c.bodyB );
                    iterationDiagnostics.maxRowIslandBody = bodyStore.Records()[static_cast<std::size_t>( root )].handle;
                }
            }
            ...
            if constexpr ( CollectConvergenceDiagnostics )
            {
                ...
                if ( deltaT1 != 0.0f || deltaT2 != 0.0f || deltaRollingT1 != 0.0f || deltaRollingT2 != 0.0f || deltaSpin != 0.0f )
                {
            ...
            if ( observePipelineEvent() )
            {
                ...
                record.featureId = c.featureId;
                record.point = PhysicsBodyPosition( bodyStore.HotFields(), static_cast<size_t>( c.bodyA ) ) + c.rA;
                record.normal = c.normal;
        ...
        SolveJointBlocks<CollectConvergenceDiagnostics>( bodyStore, iter );
...
void ConstraintSolveTransaction::SolveRows( PhysicsContactSolverStage& stage,
                                            const PhysicsBodyStore& bodyStore,
                                            const PersistentContactSolverStepPolicy& stepPolicy,
                                            ...
                                            Core::Profiler* )
{
    ...
    AdvanceOrFatal( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedPrecompute ? ConstraintSolvePhaseCursor::Phase::ReleasedSolveRows : ConstraintSolvePhaseCursor::Phase::SolveRows, "SolveRows" );
...
template <bool RetainPipelineRecords>
void ConstraintSolveTransaction::WriteBack( PhysicsContactSolverStage& stage,
                                            ...
                                            Core::Profiler* )
{
    ...
    AdvanceOrFatal( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedSolveRows ? ConstraintSolvePhaseCursor::Phase::ReleasedWriteBack : ConstraintSolvePhaseCursor::Phase::WriteBack, "WriteBack" );
    ...
    for ( int i = 0; i < modelCount; ++i )
    {
        if ( sleepState[i] || isFixedBody( i ) || ( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedWriteBack && !m_islands.Included( i ) ) )
        {
            continue;
        }
...
void ConstraintSolveTransaction::PublishDebugContacts( PhysicsContactSolverStage& stage, const PhysicsBodyStore& bodyStore, PhysicsStepDiagnostics& stepDiagnostics, Core::Profiler* )
{
    PROFILE_SCOPED( "Frame/Physics/Narrowphase/PersistentContacts/DebugContacts" );
    AdvanceOrFatal( m_phase.Current() == ConstraintSolvePhaseCursor::Phase::ReleasedWriteBack ? ConstraintSolvePhaseCursor::Phase::ReleasedDebugContacts : ConstraintSolvePhaseCursor::Phase::DebugContacts, "PublishDebugContacts" );
    ...
    stage.m_sideEffects.fixedContactBodies.clear();
...
template <bool RetainPipelineRecords>
void ConstraintSolveTransaction::CorrectPositions( PhysicsContactSolverStage& stage,
                                                   PhysicsBodyStore& bodyStore,
                                                   const PersistentContactSolverStepPolicy& stepPolicy,
                                                   std::span<const uint8_t> sleepState,
                                                   std::size_t pipelineRecordCapacity,
                                                   Core::Profiler* )
{
    ...
    AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase::PositionCorrection, "CorrectPositions" );
    ...
    while ( manifoldBegin < stage.m_persistentContacts.size() )
    {
        ...
        float invMassA = ( sleepState[c.bodyA] || hotFields.fixed[bodyAIndex] != 0u ) ? 0.0f : hotFields.inverseMass[bodyAIndex];
        ...
        if ( c.bodyB != TERRAIN_BODY_INDEX )
        {
            ...
            hasBodyB = true;
            invMassB = ( sleepState[c.bodyB] || hotFields.fixed[bodyBIndex] != 0u ) ? 0.0f : hotFields.inverseMass[bodyBIndex];
        }
...
template <bool RetainPipelineRecords>
void ConstraintSolveTransaction::StoreCache( PhysicsContactSolverStage& stage,
                                             const PhysicsBodyStore& bodyStore,
                                             const PersistentContactSolverStepPolicy& stepPolicy,
                                             std::size_t pipelineRecordCapacity,
                                             Core::Profiler* )
{
    ...
    AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase::CacheStore, "StoreCache" );
    ...
    {
        std::sort( stage.m_persistentContactCache.begin(), stage.m_persistentContactCache.end(), []( const auto& a, const auto& b ) { return a.key < b.key; } );
        stage.m_persistentContactCache.erase( std::unique( stage.m_persistentContactCache.begin(), stage.m_persistentContactCache.end(), []( const auto& a, const auto& b ) { return a.key == b.key; } ), stage.m_persistentContactCache.end() );
    };
    for ( int family = 0; family < 2; ++family )
    {
        for ( std::size_t row = 0; row < stage.m_persistentContacts.size(); ++row )
        {
            const PersistentContact& c = stage.m_persistentContacts[row];
            const bool hull = m_contactLifetime[row] != 0u;
            if ( hull != ( family == 1 ) || ( !hull && ( !c.supportsRestingPolicy || ( c.accN <= 0.0f && fabsf( c.accT1 ) <= TOLERANCE && fabsf( c.accT2 ) <= TOLERANCE ) ) ) )
            {
                continue;
            }
            ...
            if ( hull )
            {
                const auto rotationA = PhysicsBodyOrientation( hotRead, c.bodyA ).GetOrientationMatrix();
                cached.geometry.localAnchorA = rotationA.TransposeMultiply( c.rA + c.normal * ( 0.5f * c.penetration ) );
                cached.geometry.localNormalA = rotationA.TransposeMultiply( c.normal );
                cached.geometry.localTangentImpulseA = rotationA.TransposeMultiply( c.tangent1 * c.accT1 + c.tangent2 * c.accT2 );
                if ( c.isTerrain )
                {
                    cached.geometry.localAnchorB = PhysicsBodyPosition( hotRead, c.bodyA ) + c.rA - c.normal * ( 0.5f * c.penetration );
                    cached.geometry.localNormalB = c.normal;
                }
                else
                {
                    const auto rotationB = PhysicsBodyOrientation( hotRead, c.bodyB ).GetOrientationMatrix();
                    cached.geometry.localAnchorB = rotationB.TransposeMultiply( c.rB - c.normal * ( 0.5f * c.penetration ) );
                    cached.geometry.localNormalB = rotationB.TransposeMultiply( c.normal );
                }
                // Units: one percent of the contact arm, with twice the contact
                // epsilon as the distance floor. This bounds refresh work and
                // rejects sliding anchors independently of their feature IDs.
                cached.geometry.breakingDistance = (std::max)( stepPolicy.contactEpsilon * 2.0f, 0.01f * sqrtf( Vector::VectorMagSquared( c.rA ) ) );
                cached.geometry.lifetime = m_contactLifetime[row];
            }
            stage.m_persistentContactCache.push_back( cached );
        }
        // Hazard: legacy primitive clipping can contain duplicate keys. Resolve
        // those using exactly its own rows before hull contacts join the sort.
        // The new hull provenance supplies unique source features per patch.
        if ( stage.m_persistentContactCache.size() > 1 )
        {
            sortUnique();
        }
    }
    // Preserve diagnostic event order independently of cache packing order.
    for ( std::size_t row = 0; row < stage.m_persistentContacts.size(); ++row )
    {
        const PersistentContact& c = stage.m_persistentContacts[row];
        if ( ( !c.supportsRestingPolicy && m_contactLifetime[row] == 0u ) || ( c.accN <= 0.0f && fabsf( c.accT1 ) <= TOLERANCE && fabsf( c.accT2 ) <= TOLERANCE ) )
        {
            continue;
        }
        if ( observePipelineEvent() )
...
void ConstraintSolveTransaction::ReleaseFixedContacts( PhysicsContactSolverStage& stage, PhysicsBodyStore& bodyStore, int modelCount, Core::Profiler* )
{
    PROFILE_SCOPED( "Frame/Physics/Narrowphase/PersistentContacts/FixedContactRelease" );
    AdvanceOrFatal( ConstraintSolvePhaseCursor::Phase::FixedContactRelease, "ReleaseFixedContacts" );
    ...
    auto queueFixedTreeRelease = [&]( const PhysicsFixedTreeReleaseEvent& event ) { stage.m_sideEffects.fixedTreeReleases.push_back( event ); };
    ...
    auto releaseFixedContactBody = [&]( int fixedIndex, int otherIndex, const PersistentContact& c, bool fixedIsBodyA )
    {
        ...
        if ( !isFixedBody( fixedIndex ) || !fixedRecord.releasesFromFixedOnContact || isFixedBody( otherIndex ) || otherRecord.releasesFromFixedOnContact ||
             c.accN < fixedRecord.contactReleaseImpulseThreshold )
        {
            return;
        }
                                       ...
                                       std::span<PointJointConstraint> joints )
{
    ...
    const int modelCount = (std::min)( { bodyStoreCount, static_cast<int>( bodyRecords.size() ), static_cast<int>( colliderRecords.size() ) } );
    ...
    if ( modelCount <= 0 || ( candidatePairs.empty() && terrainContactManifolds.empty() && joints.empty() ) )
    {
    ...
    m_solveTransaction.SetupBodies( bodyStore, sleepState, modelCount, nullptr );
    m_solveTransaction.OrderCandidatePairs( bodyStore, candidatePairs );

    if ( m_persistentContactCache.size() > 1 )
    {
        std::sort( m_persistentContactCache.begin(), m_persistentContactCache.end(), []( const PersistentContactCacheEntry& lhs, const PersistentContactCacheEntry& rhs ) { return lhs.key < rhs.key; } );

#ifdef _DEBUG
        assert( std::is_sorted( m_persistentContactCache.begin(), m_persistentContactCache.end(), []( const PersistentContactCacheEntry& lhs, const PersistentContactCacheEntry& rhs ) { return lhs.key < rhs.key; } ) && "persistent contact cache must be sorted before lower_bound lookup" );
#endif
    }
    ...
    {
        m_solveTransaction
            .BuildManifolds<true>( *this, bodyStore, colliderStore, stepPolicy, m_solveTransaction.m_candidatePairs, sleepState, sleepSupportEdges, modelCount, pipelineRecordCapacity, nullptr );
        m_solveTransaction
            .BuildTerrainRows<true>( *this, bodyStore, stepPolicy, terrainContactManifolds, sleepState, timeRemaining, modelCount, pipelineRecordCapacity, stepPolicy.stepDurationSeconds, nullptr );
    }
    else
    {
        m_solveTransaction
            .BuildManifolds<false>( *this, bodyStore, colliderStore, stepPolicy, m_solveTransaction.m_candidatePairs, sleepState, sleepSupportEdges, modelCount, pipelineRecordCapacity, nullptr );
        m_solveTransaction
            .BuildTerrainRows<false>( *this, bodyStore, stepPolicy, terrainContactManifolds, sleepState, timeRemaining, modelCount, pipelineRecordCapacity, stepPolicy.stepDurationSeconds, nullptr );
    }

    m_solveTransaction.PrepareJoints( *this, bodyStore, joints, stepPolicy );

    if ( m_persistentContacts.empty() && joints.empty() )
    {
        m_persistentContactCache.clear();
        physicsDebugContacts.clear();
        m_solveTransaction.Complete();
    ...
    if ( retainPipelineRecords )
    {
        m_solveTransaction.PrecomputeRows<true>( *this, bodyStore, colliderStore, stepPolicy, sleepState, timeRemaining, pipelineRecordCapacity, stepPolicy.stepDurationSeconds, nullptr );
    }
    else
    {
        m_solveTransaction.PrecomputeRows<false>( *this, bodyStore, colliderStore, stepPolicy, sleepState, timeRemaining, pipelineRecordCapacity, stepPolicy.stepDurationSeconds, nullptr );
    }

    // Invariant: both families have applied every warm impulse before sweep zero.
    m_solveTransaction.WarmStartJoints();
    m_solveTransaction.SolveRows( *this, bodyStore, stepPolicy, retainPipelineRecords, pipelineRecordCapacity, nullptr );

    if ( retainPipelineRecords )
    {
        m_solveTransaction.WriteBack<true>( *this, bodyStore, sleepState, modelCount, pipelineRecordCapacity, nullptr );
    }
    else
    {
        m_solveTransaction.WriteBack<false>( *this, bodyStore, sleepState, modelCount, pipelineRecordCapacity, nullptr );
    }

    m_solveTransaction.PublishDebugContacts( *this, bodyStore, stepDiagnostics, nullptr );

    if ( retainPipelineRecords )
    {
        m_solveTransaction.CorrectPositions<true>( *this, bodyStore, stepPolicy, sleepState, pipelineRecordCapacity, nullptr );
    }
    else
    {
        m_solveTransaction.CorrectPositions<false>( *this, bodyStore, stepPolicy, sleepState, pipelineRecordCapacity, nullptr );
    }
    m_solveTransaction.ReleaseFixedContacts( *this, bodyStore, modelCount, nullptr );
    if ( !m_sideEffects.releaseWakeBodies.empty() )
    {
        // Lifetime: PhysicsWorld wakes the new articulation and refreshes current
        // contacts before resuming this same transaction. No cache is published yet.
        m_solveTransaction.m_continuationContactCount = m_persistentContacts.size();
        return;
    }
    if ( retainPipelineRecords )
    {
        m_solveTransaction.StoreCache<true>( *this, bodyStore, stepPolicy, pipelineRecordCapacity, nullptr );
    }
    else
    {
        m_solveTransaction.StoreCache<false>( *this, bodyStore, stepPolicy, pipelineRecordCapacity, nullptr );
    }
    m_solveTransaction.StoreJointImpulses( joints );
    m_solveTransaction.Complete();
}

void PhysicsContactSolverStage::PrepareReleasedBodies( const PhysicsBodyStore& bodies, std::span<const uint8_t> sleepState )
{
    if ( !HasPendingReleasedConstraints() )
    {
        SB_FATAL( "Physics/ConstraintSolveTransaction", "No pending contact release." );
    }
    m_solveTransaction.SetupBodies( bodies, sleepState, bodies.Count(), nullptr );
    for ( auto& contact : m_persistentContacts )
    {
        m_solveTransaction.RecomputeContactMass( contact );
    }
}

void PhysicsContactSolverStage::ContinueReleasedConstraints( PhysicsBodyStore& bodies,
                                                             ...
                                                             std::span<PointJointConstraint> joints )
{
    const bool full = diagnostics.RetainsFullPipelineRecords();
    const std::size_t capacity = static_cast<std::size_t>( diagnostics.RemainingPipelineRecordCapacity() );
    const int count = bodies.Count();
    m_solveTransaction.OrderCandidatePairs( bodies, candidates );
    if ( full )
    {
        m_solveTransaction.BuildManifolds<true>( *this, bodies, colliders, policy, m_solveTransaction.m_candidatePairs, sleep, supportEdges, count, capacity, nullptr );
        m_solveTransaction.BuildTerrainRows<true>( *this, bodies, policy, terrain, sleep, timeRemaining, count, capacity, policy.stepDurationSeconds, nullptr );
    }
    else
    {
        m_solveTransaction.BuildManifolds<false>( *this, bodies, colliders, policy, m_solveTransaction.m_candidatePairs, sleep, supportEdges, count, capacity, nullptr );
        m_solveTransaction.BuildTerrainRows<false>( *this, bodies, policy, terrain, sleep, timeRemaining, count, capacity, policy.stepDurationSeconds, nullptr );
    }
    m_solveTransaction.PrepareJoints( *this, bodies, joints, policy );
    m_solveTransaction.m_islands.RestrictToReleasedComponents( m_sideEffects.releaseWakeBodies );
    if ( full )
    {
        m_solveTransaction.PrecomputeRows<true>( *this, bodies, colliders, policy, sleep, timeRemaining, capacity, policy.stepDurationSeconds, nullptr );
    }
    else
    {
        m_solveTransaction.PrecomputeRows<false>( *this, bodies, colliders, policy, sleep, timeRemaining, capacity, policy.stepDurationSeconds, nullptr );
    }
    // Applied impulses and original bounce targets survive. Only newly active
    // contacts prepare cold; there is no second joint warm start, positional
    // correction, or release decision at this boundary.
    m_solveTransaction.SolveRows( *this, bodies, policy, full, capacity, nullptr );
    m_persistentContactSolverStats.rowCount = static_cast<int>( m_persistentContacts.size() );
    if ( full )
    {
        m_solveTransaction.WriteBack<true>( *this, bodies, sleep, count, capacity, nullptr );
    }
    else
    {
        m_solveTransaction.WriteBack<false>( *this, bodies, sleep, count, capacity, nullptr );
    }
    m_solveTransaction.PublishDebugContacts( *this, bodies, diagnostics, nullptr );
    if ( full )
    {
        m_solveTransaction.StoreCache<true>( *this, bodies, policy, capacity, nullptr );
    }
    else
    {
        m_solveTransaction.StoreCache<false>( *this, bodies, policy, capacity, nullptr );
    }
    m_solveTransaction.StoreJointImpulses( joints );
    m_solveTransaction.Complete();
}
