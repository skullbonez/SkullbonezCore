...
int SelectContactCandidateIndices(
    const ContactCandidate ( &candidates )[CandidateCapacity],
    int candidateCount,
...
bool BuildSphereBoxOrdered(
    const ObjectContactBodyView& sphereBody,
...
)
{
    ...
    Vector3 closestLocal(
        ClampFloat( local.x, -bw.halfExtents.x, bw.halfExtents.x ),
...
int ClipIncidentFaceToReference(
    const BoxWorld& refBox,
    int refAxis,
    float refSign,
...
bool BuildBoxFaceContact(
    const ObjectContactBodyView& aBody,
...
)
{
    ...
    Vector3 refFaceCenter =
        refBox.center + refBox.axes[referenceAxis] * ( refSign * Component( refBox.halfExtents, referenceAxis ) );
...
bool IsUsefulPolyEdgeAxis(
    const PolytopeWorld& a,
...
int ClipPolyAgainstPlaneLimited(
    const ClipVertex* input,
    int inputCount,
    const Vector3& planePoint,
    const Vector3& inwardNormal,
...
void ConsiderSphereHullCandidate(
...
ClosestSphereHullBoundaryFeature( const PolytopeWorld& hullWorld, const Vector3& sphereCenter, float contactSkin )
{
    ...
    for ( uint16_t e = 0; e < hullWorld.edgeCount; ++e )
    {
        ...
        float t = 0.0f;
        const Vector3 point = ClosestPointOnSegment(
            hullWorld.vertices[edge.vertexA],
...
)
{
    ...
    for ( int i = 0; i < count; ++i )
    {
        ...
        if ( candidateCount < MAX_POLY_CLIP_VERTS )
        {
            ...
            candidates[candidateCount].featureId =
                EncodeHullFaceFeature( referenceIsA, refFace.sourceId, incFace.sourceId, workA[i].id );
            ++candidateCount;
        }
    }
...
)
{
    ...
    constexpr float tieEpsilon = 1.0e-4f;
    const bool tryAFirst =
        alignA > alignB + tieEpsilon || ( fabsf( alignA - alignB ) <= tieEpsilon && preferredReference == 0 );
    ...
    auto tryBuild = [&]( bool referenceIsA, ObjectContactManifold& candidate ) -> bool
    {
        ...
        return referenceIsA ? BuildPolyFaceContact(
                                  aBody,
                                  bBody,
                                  polyA,
                                  polyB,
...
)
{
    ...
    if ( sat.axisType == 2 && sat.hasFaceAxis )
    {
        ...
        if ( sat.faceOverlap <= sat.overlap + faceAxisTolerance )
        {
            ...
            const bool builtFace = BuildBestPolyFaceContact(
                aBody,
                bBody,
                polyA,
                polyB,
...
)
{
    ...
    {
        const SphereHullClosestFeature closest =
            ClosestSphereHullBoundaryFeature( hullWorld, sphereCenter, contactSkin );
...
bool SkullbonezCore::Physics::BuildObjectContactManifold(
